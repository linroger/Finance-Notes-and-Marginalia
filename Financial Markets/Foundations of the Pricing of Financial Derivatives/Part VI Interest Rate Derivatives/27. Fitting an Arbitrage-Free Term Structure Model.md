...
- 27. Fitting an Arbitrage-Free Term Structure Model and financial analysis
- 27. Fitting an Arbitrage-Free Term Structure Model in modern finance
- Applications of 27. Fitting an Arbitrage-Free Term Structure Model
- 'Case study: 27. Fitting an Arbitrage-Free Term Structure Model'
...
To convert a Wiener process%20Process.md) to a binomial, we simply express the random variable as a binary variable with a value of $+1$ or $^{-1}$ at each time step and assume martingale probabilities of $[^1]h$ , as we did in Chapter 16. We assume each time step has a defined length of one unit. Hence, the stochastic process%20Process.md) becomes  
$$
...
5 Within the discretized version of the forward rate stochastic process%20Process.md) of the HJM model, compute the appropriate forward rates at time 1 based on the following inputs.  
$$
