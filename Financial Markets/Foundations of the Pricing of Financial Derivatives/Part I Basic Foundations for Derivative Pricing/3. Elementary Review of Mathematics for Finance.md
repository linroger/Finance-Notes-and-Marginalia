...
For example, most [[An Asset Allocation Primer|portfolio]] managers will frequently need to know the values of their portfolios. The manager may own [[A Practical Guide for Actuaries and other Business Professionals.|financial instruments]] in numerous categories, such as domestic bonds, domestic stocks, international bonds, international stocks, [[Financial Instruments PSET Solutions|financial derivatives]], [[Private Equity|private equity]], and so forth. Within each category, the manager may own varying amounts of numerous individual instruments. Based on the principle of value additivity, the [[An Asset Allocation Primer|portfolio]] value of each category is found by summing the number of units held for each instrument $(N_{t,j,i})$ times the estimated price of the instrument $(P_{t,j,i})$ , where $t$ denotes the point in time, $j$ denotes the category, and $i$ denotes the specific instrument. Thus, each category’s value $(\mathbf{}C_{t,j})$ can be represented as  
$$
C_{t,j}=\sum_{i=1}^{N_{j}}N_{t,j,i}P_{t,j,i}.
$$  

The overall [[An Asset Allocation Primer|portfolio]] value ( $V_{t}$ , uppercase Greek $p i$ ) can be expressed as  
$$
...
The rate of return is an important metric in finance and can be measured in many different ways. For this illustration, we assume monthly discrete [[Asset Price Dynamics|compounded rates of return]] based on the [[An Asset Allocation Primer|portfolio]]’s estimated value at the start $(V_{t-1})$ and end $(V_{t})$ of the month. Thus, the monthly discrete compounded rate of return $(R_{t}^{d})$ is defined as  
$$
R_{t}^{d}={\frac{V_{t}-V_{t-1}}{V_{t-1}}}={\frac{V_{t}}{V_{t-1}}}-1.
$$  

Note that we assume the initial value is positive. Many [[Financial Instruments PSET Solutions|financial derivatives]] have initial values of zero presenting some challenges addressed later. The monthly total return $(T R_{t})$ is based on rearranging the monthly rate of return expression or  
$$
T R_{t}=\frac{V_{t}}{V_{t-1}}=1+R_{t}^{d}.
$$  

For example, consider a [[An Asset Allocation Primer|portfolio]]’s return performance over the past five years measured monthly. Hence, we have 60 observations of the rate of return and the five-year total return is  
$$
...
Using the properties of products above, one could easily manipulate total [[Assets|returns]] based on calendar months (January, February, and so forth) or further decompose monthly total [[Assets|returns]] into daily [[Assets|returns]].  

# 3.3 LOGARITHMS AND EXPONENTIALS  

The logarithm of a number is an exponent to which a base raised to that power equals the number. There are two primary types of logarithms, the base $e$ , called the natural or Naperian log, and the base 10, called the common log. We write the former as $\log_{\mathrm{e}}$ or sometimes ln, and the latter as $\log_{10}$ . In financial applications we nearly always use natural logs, which is primarily because [[PSET 7 Solutions-Financial Instruments|continuously compounded interest]] is consistent with the natural log function.  
...
Rather than compute discrete rates of return, it is often much more useful to compute continuously [[Asset Price Dynamics|compounded rates of return]] $(R_{t}^{c})$ . Following the previous illustration, we assume monthly [[Interest Rate Quotations|continuous compounding]]. The monthly continuous rate of return can be expressed as  
$$
...
Thus, the monthly total return with [[Interest Rate Quotations|continuous compounding]] is  
$$
T R_{t}=\frac{V_{t}}{V_{t-1}}=e^{R_{t}^{c}}.
$$  

Again, consider a [[An Asset Allocation Primer|portfolio]]’s performance over the past five years measured monthly with [[Interest Rate Quotations|continuous compounding]]. Hence, we have 60 observations of the rate of return and the five-year total return is  
$$
T R_{T}={\frac{V_{T}}{V_{0}}}=\prod_{t=1}^{60}T R_{t}=\prod_{t=1}^{60}e^{R_{t}^{c}}=e^{\sum_{t=1}^{60}R_{t}^{c}}.
$$  

Using the previous product and summation properties, one could again easily manipulate total [[Assets|returns]] based on calendar months (January, February, and so forth) or further decompose monthly total [[Assets|returns]] into daily [[Assets|returns]].  
...
Sensitivities such as these have proven very useful in various [[Financial Mathematics Course|risk management]] tasks, particularly related to managing bond portfolios. We leave one specific application related to bond [[An Analytical Decomposition of Forward Rates|duration and convexity]] to an end-of-chapter problem.  
...
Although trigonometric functions rarely make their way into finance applications, it does happen occasionally, particularly when estimating functions, such as the yield curve, or when deploying econometric models. We show here selected [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] of trigonometric functions:  
$$
...
where $m$ denotes the number of compounding periods per year, and $N$ denotes [[Hedging Strategies with Forwards|time to maturity]] expressed in compounding periods. Alternatively, we can express the annualized yield to maturity as a function of the future value in the following form,  
$$
...
or in other words, as the product of two [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. This is called the chain rule.  

For example, manipulating the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] option [[Arbitrage Pricing of Derivatives|pricing]] model often requires use of the chain rule. For example, the cumulative [[Verification of Central Limit Theorem|standard normal distribution]] can be expressed as  
$$
...
where $s$ denotes the current [[Chapter 16 - Black–Scholes Model|stock price]], $X$ denotes the [[Call and Put Payoffs at Expiry|strike price]], $r_{c}$ denotes the annualized, continuously compounded, [[Exercises|risk-free interest rate]], $\sigma$ denotes the volatility, and $\tau$ denotes the [[Chapter 16 - Black–Scholes Model|time to expiration]]. Therefore, the first derivative of $N\left(d_{1}\right)$ , the normal probability based on $d_{1}$ , with respect to the [[Chapter 16 - Black–Scholes Model|stock price]] can be worked  out based on the chain rule or  
$$
...
and sometimes as $f^{\prime\prime}(x)$ . The second derivative provides further information, beyond that of the first derivative, about the characteristics of the function. For example, a positive first derivative and positive second derivative describes a function that is upward sloping with the slope increasing at an increasing rate. A positive first derivative and negative second derivative describes a function that is increasing at a decreasing rate. A negative first derivative and positive second derivative is a function that is decreasing at a decreasing rate. A negative first derivative and negative second derivative is a function that is decreasing at an increasing rate. A zero first derivative and positive second derivative is a function that is at a minimum, whereas a zero first derivative and negative second derivative is a function that is at a maximum. There are also third- and higher-order [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], but we rarely need them in finance.  

Again, manipulating the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] option [[Arbitrage Pricing of Derivatives|pricing]] model often requires use of the higher-order [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. For example, the second derivative of the call option price with respect to the underlying instrument is known as gamma and, though rarely used, the third derivative of the call option price with respect to the underlying instrument is known as speed.  

# 3.5.8 Partial Derivatives  

A function containing more than one variable that can be differentiated with respect to one of the variables by treating the other variables as constants is a derivative called a partial derivative. Consider $y=f(x,z)$ where both $x$ and $z$ are variables. Then the partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are written as $\partial y/\partial x$ and $\partial y/\partial z$ , and they represent the change in $y$ for a change in either $x$ or $z$ , while treating the other variable as a constant.  

The rules for taking partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are the same as the rules for taking ordinary [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. For example, consider the function $y=2x^{3}+4z^{2}+c$ where $c$ is a constant. Then  
$$
\begin{array}{c}{{\displaystyle{\frac{\partial y}{\partial x}}=6x^{2}}}\\ {{\displaystyle{\frac{\partial y}{\partial z}}=8z.}}\end{array}
$$  

Likewise, there are second partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. Hence,  
$$
...
Consider the following function: $y=4x^{3}z^{2}$ . The partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are $\partial y/\partial x=$ $12x^{2}z^{2}$ and $\partial y/\partial z=8x^{3}z$ . You can also take the partial derivative of each of these [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] with respect to the other variable. In other words,  
$$
...
In mathematical notation these [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], sometimes called cross-partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], often are indicated with such symbols as $y_{12}$ and $y_{21}$ , but as noted as shown previously we do not use this notation much in finance.  
...
This operation is also called a [[Verification of Central Limit Theorem|Taylor series expansion]], which is useful when we need to study the properties of the rate of change of a function, rather than its specific value at a point. Although [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] do the same thing, they represent the change in a function when there is an infinitesimal change in a variable. Taylor series expansions are useful when the variable change is not infinitesimal, as is often the case in finance.  

If the function has more than one variable, we can apply Taylor’s Theorem to a given variable at a time, holding the other variables constant. The [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] expressed are then partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. Alternatively, we could do a [[Verification of Central Limit Theorem|Taylor series expansion]] for more than one variable. For example, let $y=f(x,z)$ . Then a [[Verification of Central Limit Theorem|Taylor series expansion]] for both $x$ and $z$ would be written as  
$$
...
where the additional terms would represent higher-order partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] and all possible cross-partial combinations.  

A [[Verification of Central Limit Theorem|Taylor series expansion]] is exact only in the limit, that is, where the number of terms on the RHS is infinite. In applications we often do a [[Verification of Central Limit Theorem|Taylor series expansion]] to the second order, that is, where terms after the second derivative are dropped, and then change the equals sign to the approximation sign, which is $\approx$ .  

The Taylor series approximation is a widely used tool in [[BMW Valuation|financial analysis]]. For example, various risk measures related to bonds, such as [[An Analytical Decomposition of Forward Rates|duration and convexity]], are derived based on this approximation.  
...
Note that the total differential looks somewhat like a first-order [[Verification of Central Limit Theorem|Taylor series expansion]]. A first-order [[Verification of Central Limit Theorem|Taylor series expansion]], however, is just an approximation. The total differential is an exact expression. The distinction lies in the fact that the [[Verification of Central Limit Theorem|Taylor series expansion]] applies to discrete changes in $x$ and $z$ ( $\Delta x$ and $\Delta z$ ) and uses partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] to approximate the discrete change in $y\left(\Delta y\right)$ . If we let $\Delta x$ and $\Delta z$ be infinitesimal changes, we denote them as $d x$ and $d z$ . Then we restate the [[Verification of Central Limit Theorem|Taylor series expansion]] for the two variables that we presented previously as  
$$
...
A differential equation is an equation that contains a derivative. The objective of solving a differential equation is to determine the original function whose derivative is given by the differential equation. Differential equations that contain only ordinary [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are called ordinary differential equations. Differential equations that contain partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are called partial differential equations.  
...
Note, however, that without knowing the value of $a$ we cannot be very specific about the solution, which can vary widely depending on the value of $a$ . To determine a more precise solution, we often impose one or more conditions on the value of $q$ , which are called initial conditions or [[Appendix 21.C Solutions for Black-Scholes PDE|boundary conditions]] and represent values we know. For example, if we know that at $x=0$ , the function value is 50, then we can substitute zero for $x$ and obtain $50=(3q/5)0^{2}+a$ so $a=50$ . Or if we know that at $x=100$ , $y=5{,}200$ , then we know that $5,200=(3q/5)100^{2}+a$ , giving us a value for $a$ in terms of $q$ .  

Differential equations that contain partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are naturally called partial differential equations or PDEs. They are usually much harder to solve than are ordinary differential equations. Much of the process of solving differential equations involves classifying the equation into a given category of differential equations and then following known rules, hints, and suggestions for solutions of equations in that category. We will also sometimes exploit the fact that we have an idea of what the solution might look like. One approach to [[Arbitrage Pricing of Derivatives|pricing]] options relies on solving a PDE.  

# 3.8 RECAP AND PREVIEW  

In this chapter we reviewed the rules of summations, products, logarithms, exponentials, and series formulas. In addition, we covered calculus [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] and integrations, as well as differential equations. This material is strictly for the purposes of refreshing our knowledge from math courses that would have been previously taken.  

In Chapter 4, we move on to another review, which covers concepts in probability.  

# QUESTIONS AND PROBLEMS  

The following table provides a summary of a particular [[An Asset Allocation Primer|portfolio]] at year end. Demonstrate that the value of the overall [[An Asset Allocation Primer|portfolio]] can be found two ways: (1) finding the sector’s value and then aggregating to the overall [[An Asset Allocation Primer|portfolio]] and (2) aggregating each position in the overall [[An Asset Allocation Primer|portfolio]].  

<html><body><table><tr><td>Instrument ID</td><td> Sector</td><td>Number of Shares</td><td>Price per Share</td></tr><tr><td>ABC147MZO</td><td>Technology</td><td>100</td><td>56.00</td></tr><tr><td>GIM256AEF</td><td>Technology</td><td>300</td><td>27.00</td></tr><tr><td>BRC829GFP</td><td>Technology</td><td>200</td><td>33.00</td></tr><tr><td>FMC061DEC</td><td>Technology</td><td>500</td><td>17.00</td></tr><tr><td>MRF122NAJ</td><td>[[HBR Case Study- Oaktree|Financial services]]</td><td>1,000</td><td>24.00</td></tr><tr><td>BRO010CHN</td><td>[[HBR Case Study- Oaktree|Financial services]]</td><td>600</td><td>54.00</td></tr><tr><td>NOD332BER</td><td>[[HBR Case Study- Oaktree|Financial services]]</td><td>300</td><td>61.00</td></tr><tr><td>CNC522OKS</td><td>[[Macroeconomic Models of Business Cycles|Consumer durables]]</td><td>1,400</td><td>77.00</td></tr><tr><td>ZAF501ZTY</td><td>[[Macroeconomic Models of Business Cycles|Consumer durables]]</td><td>800</td><td>27.00</td></tr></table></body></html>  

2 Because the [[Week 1 Ratio Analysis + Valuation Review|dividend discount model]] assumes the value of a common stock $(V_{S})$ is simply the present value at a common discretely compounded [[PSET 7- Kohler|discount rate]] $(k_{d c})$ and expected annual [[Exercises|dividend payments]] $(D_{i})$ , we have an infinite series or mathematically,  
$$
...
Let us assume a single growth rate $(g)$ , annual [[Exercises|dividend payments]], and apply the growth rate solely on a dividend payment date. Assuming annual discrete compounding, we can express the stock value, based on this model, as  
$$
V_{S}=\sum_{i=1}^{\infty}{\frac{D_{0}{\left(1+g_{d c}\right)}^{i}}{\left(1+k_{d c}\right)^{i}}}.
$$  

Assuming a growth rate less than the [[PSET 7- Kohler|discount rate]] and based on the infinite series results presented in this chapter, express the stock value in closed form. Specifically, prove that the stock value can also be expressed as  
$$
V_{S}=\frac{D_{0}\left(1+g_{d c}\right)}{\left(k_{d c}-g_{d c}\right)}.
$$  

3 Derive the first derivative of the stock value with respect to the [[PSET 7- Kohler|discount rate]] as well as the derivative of the stock value with respect to the growth rate when the stock value can be expressed as  
$$
V_{S}=\frac{D_{0}\left(1+g_{d c}\right)}{\left(k_{d c}-g_{d c}\right)}.
$$  

Assuming $D_{0}=1.0$ , $g_{d c}=5.0\%$ , and $k_{d c}=10\%$ , compute the stock value as well as both [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. Justify your derivative solutions by recalculating the derivative assuming a one basis point increase in the growth rate and then a one basis point increase in the [[PSET 7- Kohler|discount rate]]. (Note that a basis point is 0.0001, or $1/100\%$ . For example, if a rate goes from $6\%$ to $6.03\%$ , it has gone up three basis points.)  
...
where Coupon denotes the stated annual coupon percentage rate, m denotes the payment frequency (for US treasury bonds, $m=2$ ), Par denotes the bond dollar par value, and $N$ denotes the number of remaining [[Realized Returns|coupon payments]]. Derive the closed-form expression for the bond value.  

5 [[A Guide to Duration DV01 and Yield Curve|Modified duration]] and standard [[PSET II Fixed Income Asset Pricing 1|convexity]] are common bond-related risk measures. Mathematically, they are defined as follows (using the notation in the previous question):  
$$
...
Based on the properties of the bond valuation equation in the previous problem, derive the [[A Guide to Duration DV01 and Yield Curve|modified duration]] and standard [[PSET II Fixed Income Asset Pricing 1|convexity]] expressions. Illustrate the solution in both series format and closed form.  

6 One approach to establishing the theoretical price of a [[Futures Not Subject to Cash-And-Carry|futures]] contract is based on the carry model. The carry model asserts the [[Futures Price and the Quality Option Before E|futures price]] that expires at time $T\left(F_{T}\right)$ is simply the spot price at time $t\left(S_{t}\right)$ grossed up by the [[Exercises|risk-free interest rate]] $(r_{c})$ and other carry costs $(c c)$ over the [[Chapter 16 - Black–Scholes Model|time to expiration]] $\tau=T-t$ or $F_{T}=S_{t}e^{(r_{c}+c c)\tau}$ . For many [[Financial Mathematics Course|risk management]] applications, we are interested in the first derivative, known as delta, and second derivative, known as gamma, with respect to the spot price. Solve for these values as well as interpret them.  

# NOTES  

1. Recall LHS and RHS denotes the left-hand side and right-hand side of equations.  

2. These [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are referred to as calculus [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], which is not typical terminology in math classes. In finance, however, options, [[Forwards and Futures|forwards]], [[Futures Not Subject to Cash-And-Carry|futures]], and swaps are referred to as derivative instruments inasmuch as they derive their values from an [[Risk Neutral Pricing of Options|underlying asset]] or rate. Hence, we are compelled to make a distinction.   
