...
ne of the most popular option underlyings is foreign [[Forwards and Futures Notes|currency]]. The right to buy or sell investors, and speculators. Yet, options on foreign currencies can be a bit more difficult to understand than options on stocks, bonds, and [[Futures Not Subject to Cash-And-Carry|commodities]]. The reason primarily lies in the fact that every [[Forwards and Futures Notes|currency]] contract involves not one but two currencies. Options on stocks, bonds, and [[Futures Not Subject to Cash-And-Carry|commodities]], by contrast, do not involve more than one [[Forwards and Futures Notes|currency]]. They do, however, implicitly involve more than one asset. For example, an option to buy a stock is an option to exchange a [[Forwards and Futures Notes|currency]] for the stock. From the opposite perspective, however, it is clearly an option to exchange stock for the [[Forwards and Futures Notes|currency]]. We may speak of a share of stock as costing 50 but we could just as easily say that the cost of 1.0 is 0.02 shares of stock. That is, one could theoretically exchange 0.02 shares of stock and receive 1.0. Although the notion of exchanging stock for money is a valid economic concept and explains precisely the same idea as if we described the transaction as exchanging money for stock, it is not the form we typically use in commerce. But in the [[Forwards and Futures Notes|currency]] world, things are just a bit different.  

Hence, exchanging dollars for euros is the mirror image of exchanging euros for dollars. Likewise, exchanging dollars for stock is the mirror image of exchanging stock for dollars. Because we never describe the stock transaction as the latter, we may have trouble understanding the [[Forwards and Futures Notes|currency]] transaction from that point of view.  

In the world of [[Forward Points in Currency|currency trading]], however, it is necessary to understand the transaction from either point of view. In particular, foreign [[Forwards and Futures Notes|currency]] option transactions have an important characteristic that sheds light on some interesting issues in option valuation. That characteristic is that a foreign [[Forwards and Futures Notes|currency]] call can be viewed as a foreign [[Forwards and Futures Notes|currency]] put from the mirror image perspective. We describe this idea as the concept of isomorphism, the notion that two things have essentially the same form. This point would also hold for calls on stocks because they can be viewed as puts on cash, but because we virtually never view a stock sale as the purchase of money paid for by tendering stock, we almost never use this concept elsewhere in [[Financial Markets and Institutions Lecture Notes|financial markets]].  

In foreign [[Forwards and Futures Notes|currency]] transactions, we shall refer to the two currencies as the base [[Forwards and Futures Notes|currency]] and the quoted [[Forwards and Futures Notes|currency]]. The base [[Forwards and Futures Notes|currency]] is just the underlying [[Forwards and Futures Notes|currency]]. For example, a call option quoted in dollars in which the underlying is euros has the dollar as the quoted [[Forwards and Futures Notes|currency]] and the euro as the base [[Forwards and Futures Notes|currency]].1 In addition, the instruments we examine are sometimes called foreign [[Forwards and Futures Notes|currency]] options, [[Forwards and Futures Notes|currency]] options, and foreign exchange options, so be prepared for alternative nomenclature.  

# 32.1 DEFINITION OF TERMS  

Let us start by giving a clear definition of the terms we shall use:  
$S_{0}$ , ${{S}_{T}}=$ [[Arbitrage Opportunity Accounting|spot exchange rate]] at times 0 or $T_{\mathbf{\delta}}$ , quoted in the domestic [[Forwards and Futures Notes|currency]], that is,   
units of the domestic [[Forwards and Futures Notes|currency]] or quote [[Forwards and Futures Notes|currency]] per unit of the foreign [[Forwards and Futures Notes|currency]], or   
base [[Forwards and Futures Notes|currency]]   $R_{0}=1/S_{0}$ and $R_{T}=1/S_{T}$ , the [[Arbitrage Opportunity Accounting|spot exchange rate]] at times 0 or $T$ , quoted in the foreign   
[[Forwards and Futures Notes|currency]], that is, units of the foreign [[Forwards and Futures Notes|currency]] per unit of domestic [[Forwards and Futures Notes|currency]], or the   
reciprocal of the [[Arbitrage Opportunity Accounting|spot exchange rate]]   $X=$ [[Options|exercise price]], sometimes called the exercise rate, expressed in the quote [[Forwards and Futures Notes|currency]]   
of a call option to buy one unit of the foreign [[Forwards and Futures Notes|currency]]   $T=$ [[Risk Neutral Pricing of Options|expiration date]]   $\sigma=$ volatility of the log return on the quote [[Forwards and Futures Notes|currency]]   $r_{D}=$ domestic [[Exercises|risk-free interest rate]]; note that we have often used the symbol $r_{f},$ but   
we do not wish to suggest that the $f$ represents the foreign [[Forwards and Futures Notes|currency]]   $r_{F}=$ foreign [[Exercises|risk-free interest rate]]; note that the subscript is capitalized  
...
Of course, we frequently wrote this compactly as $\operatorname*{max}\bigl(0,S_{T}-X\bigr)$ . We will now show that this option is isomorphic to a put option denominated in the foreign [[Forwards and Futures Notes|currency]] to sell $X$ units of the domestic [[Forwards and Futures Notes|currency]] at an [[Options|exercise price]] of $1/X$ . Let $X_{R}=1/X$ , the reciprocal strike. We shall call this put the reciprocal put. Because the reciprocal put has an [[Options|exercise price]] of $1/X$ and the underlying value at expiration is $1/S_{T}$ , which we designate as $R_{T},$ the put will exercise when $1/S_{T}<1/X$ . Rearranging, we have  
$$
...
Having established that the moneyness conditions of a standard call and reciprocal put are the same, we now proceed to show that the payoffs are the same, provided we adjust the quantity of the reciprocal put and translate the payoff value into the domestic [[Forwards and Futures Notes|currency]]. The payoff of the reciprocal put option is  
$$
...
This payoff is in units of the foreign [[Forwards and Futures Notes|currency]]. To convert to domestic [[Forwards and Futures Notes|currency]], we multiply by $S_{T}$ :  
$$
...
So, to repeat, a spot call on a foreign [[Forwards and Futures Notes|currency]] with an [[Options|exercise price]] of $X$ quoted in terms of the domestic [[Forwards and Futures Notes|currency]] is equivalent to $X$ reciprocal puts on the domestic [[Forwards and Futures Notes|currency]] with an [[Options|exercise price]] of $1/X$ in terms of the foreign [[Forwards and Futures Notes|currency]]. As such, in the world of foreign [[Forwards and Futures Notes|currency]] options, every call is also a put. Now, just to remind you, this would be true for any option if we were comfortable talking in terms of calls say on a stock as being puts on money in terms of stock. But we typically do not characterize other types of options in this manner.  

# 32.3 VALUATION OF THE OPTIONS  

Now we demonstrate how the premiums of these two options are equivalent in the BlackScholes-[[Credit Markets Session 5|Merton model]]. The BSM value of the call option is  
$$
...
Note that we use the $D$ superscript on the normal [[Verification of Central Limit Theorem|distribution function]] to indicate that this is the normal probability for the call written on the foreign [[Forwards and Futures Notes|currency]] and denominated in the domestic [[Forwards and Futures Notes|currency]]. That is, it is being viewed from the domestic standpoint.  

We now provide a numerical example: Suppose the euro exchange rate is $\$1.15$ per euro, the exercise price is $\$1.14$ per euro, the [[Hedging Strategies with Forwards|time to maturity]] is 0.25, the domestic US interest rate is $0.8815\%$ , the foreign euro interest rate is $0.4\%$ , and the volatility is $15\%$ . First, solving for the values of $d_{1}^{D}$ and $d_{2}^{D}$ , we have  
$$
...
As we stated, this call is equivalent to $X$ puts written on the domestic [[Forwards and Futures Notes|currency]] denominated in the foreign [[Forwards and Futures Notes|currency]] with a strike of $X_{R}=1/X$ . We now validate that this result is upheld in the [[Fixed Income Versus Equity Derivatives|BSM model]].  
...
where $p_{R}$ is denominated in the foreign [[Forwards and Futures Notes|currency]] and2  
$$
...
[[Options|exercise price]] is $\$1.14$ per euro, or $\phantom{-}\mathrm{\}60.877193\$ per dollar, the time to maturity is 0.25, the domestic euro interest rate is $0.4\%$ , the foreign US interest rate is $0.8815\%$ , and the volatility is $15\%$ . Again, solving for the value of $d_{1}^{F}$ and $d_{2}^{F}$ , we have  
$$
...
Now let us examine why this is still the correct put formula at the core. In words, the BSM formula for the value of a put is the strike times the present value factor for the investor’s [[Forwards and Futures Notes|currency]] times $1-N\big(d_{2}^{F}\big)$ minus the underlying price in the investor’s [[Forwards and Futures Notes|currency]] times the present value factor for the yield on the [[Forwards and Futures Notes|currency]], times $1-N\big(d_{2}^{F}\big)$ . Thus, in the previous formula for $p$ , the strike is properly designated as $X_{R}=1/X$ . The strike is then multiplied by the present value factor for the investor’s [[Forwards and Futures Notes|currency]]. Because this put is being valued from the foreign investor’s perspective, it is appropriate to discount the strike at the foreign rate. Then we use the normal probability based on $-d_{2}^{F}$ , with the $F$ superscript to distinguish it from $d_{2}^{D}$ in the call. From the product of these three terms, we subtract the exchange rate in foreign [[Forwards and Futures Notes|currency]], $R_{0}=1/S_{0}$ , times the present value factor for the opposite [[Forwards and Futures Notes|currency]]. From the foreign investor’s perspective, this rate is $\boldsymbol{r}_{D}$ . Then this product is multiplied by one minus the normal probability for $d_{1}^{F}$ .  
...
Recall that the isomorphic put is actually $X$ puts, so the aggregate [[Chapter 17 - Option Strategies|put premium]] is  
$$
...
This value is in units of the foreign [[Forwards and Futures Notes|currency]]. To convert to the domestic [[Forwards and Futures Notes|currency]], we simply multiply by $S_{0}$ .  
$$
...
Assuming we purchase $X=1.14$ puts and then convert to USD, we note that $\quad S_{0}X p=$ \$1.15∕€ (1.14) $\mathsf{\d{E0.0306}=\mathbb{S}0.0402}$ . This numerical example highlights the two different ways to express [[Forwards and Futures Notes|currency]] options.  
...
Let the true [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] for $S$ be given as  
$$
\frac{d S_{t}}{S_{t}}=\alpha d t+\sigma d\mathrm{W}_{t}.
$$  

Here $\alpha$ and $\sigma$ are the [[Lecture 1- Probability Distributions of Returns|expected return]] and volatility of $d S_{t}/S_{t}$ . It is important to remember that the [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] for the log return is  
$$
d\ln S_{t}=\mu d t+\sigma d\mathrm{W}_{t},
$$  

with the log [[Lecture 1- Probability Distributions of Returns|expected return]] given as  
$$
\mu=\alpha-\sigma^{2}/2,
$$  
...
Now we need the [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] of the reciprocal exchange rate. We will apply Itô’s lemma for the reciprocal exchange rate, $R_{t}=1/S_{t}$ . Thus, we will need certain partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]:  
$$
...
Thus, the [[Lecture 1- Probability Distributions of Returns|expected return]] on the reciprocal exchange rate is $-\alpha+\sigma^{2}$ . Note, however, the minus term on the volatility, which contributes to the reason why the probability of exercise of the put is not given by the $N\left(-d_{2}\right)$ term in its formula, as it usually is in the [[Fixed Income Versus Equity Derivatives|BSM model]].4  
...
Some foreign [[Forwards and Futures Notes|currency]] option traders do not like the terminology put and call. They see every option as being both a put and a call. Hence, it is common in foreign [[Forwards and Futures Notes|currency]] option transactions to use slightly different terminology than in other types of options. Foreign [[Forwards and Futures Notes|currency]] options, thus, are often written with two values referred to as the call [[Forwards and Futures Notes|currency]] amount and the put [[Forwards and Futures Notes|currency]] amount. As with any option, the holder of the option makes the exercise decision. If they choose to exercise, they tender the put [[Forwards and Futures Notes|currency]] amount and receive the call [[Forwards and Futures Notes|currency]] amount.6 As an example, suppose an option written on the pound designates the call [[Forwards and Futures Notes|currency]] amount as $\yen10,000,000$ and the put [[Forwards and Futures Notes|currency]] amount as $\$15,000,000$ At expiration, if the exchange rate is higher than $\$1.50$ , the holder of the option tenders (puts) $\$15,000,000$ to the writer and claims (calls) £10,000,000. With the exchange rate higher than $\$1.50$ , the option holder can exchange the $\yen10,000,000$ and receive more than $\$15,000,000$ .  
...
In the [[Forwards and Futures Notes|currency]] world, we can do a similar operation. We can indicate the ratio USD/ EUR. In conformance with the notion that this is a ratio, / is interpreted as per and we read that ratio as “the number of dollars per euro.” Thus, if $\mathrm{USD/EUR}=\S1.30$ , we say that one euro costs $\$1.30$ .  

As it turns out, however, the [[Forwards and Futures Notes|currency]] world does just the opposite. The notation USD/ EUR means “the number of euros per US dollar.” The first term (before the slash) is the base [[Forwards and Futures Notes|currency]] and the second term (after the slash) is the quote [[Forwards and Futures Notes|currency]]. The multiplication works just the same and is just as valid. For example, using the dollar (USD), euro (EUR), and Japanese yen (JPY), we have $(U S D/E U R)*(E U R/J P Y)=U S D/J P Y.$ . This is read as euros per dollar times yen per euro equals yen per dollar, which is $U S D/J P Y$ in the industry’s notation. This notational form is standard practice but does appear reversed. If it were not, we could write the price of milk as gal. $/\$3$ , which we would never do. Thus, you should be aware of this format.  

So, in short, if the euro costs $\$1.30$ , it would be written as $E U R/U S D=1.30$ .  

# 32.6 RECAP  

In this final chapter of the book, we look at foreign [[Forwards and Futures Notes|currency]] puts and calls. They are viewed somewhat differently from standard underlyings in that a call on one [[Forwards and Futures Notes|currency]] can be viewed from the perspective of a put on the other [[Forwards and Futures Notes|currency]]. We showed that [[Arbitrage Pricing of Derivatives|pricing]] options on foreign [[Forwards and Futures Notes|currency]] is, however, fairly easy to implement using the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]].  

This chapter completes the book. We have laid the foundations of the theory of derivative [[Arbitrage Pricing of Derivatives|pricing]] within the framework of the much broader discipline of [[Quantitative Trading Strategies Lecture Notes|quantitative finance]]. With the caveat that no theory ever works perfectly in practice, you can now proceed to study further and begin applying what you know to gain a better understanding of how options are used directly and how they apply indirectly to a variety of problems in finance. In addition, you are well positioned for further study of other [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], such as [[Forwards and Futures|forwards]], [[Futures Not Subject to Cash-And-Carry|futures]], and swaps.  

# QUESTIONS AND PROBLEMS  

1 In your own words, define what is meant by isomorphism within the context of foreign exchange options.  

2 Prove that a call option on a foreign [[Forwards and Futures Notes|currency]] with an [[Options|exercise price]] of $X$ quoted in terms of the domestic [[Forwards and Futures Notes|currency]] is equivalent to $X$ puts on the local [[Forwards and Futures Notes|currency]] with an [[Options|exercise price]] of $1/X$ in terms of the foreign [[Forwards and Futures Notes|currency]].  
...
6 Suppose the Australian dollar exchange rate is $\$0.72$ per AUD, the exercise price is $\$0.75$ per AUD, the [[Hedging Strategies with Forwards|time to maturity]] is 1.0, the domestic U.S. interest rate is $1.05\%$ , the foreign AUD interest rate is $2.97\%$ , and the volatility is $20\%$ .  

a. Calculate the value of a foreign exchange call option (US $\$5$ per AUD) based on the Black-Scholes-Merton model from a US-based investor perspective. b. Calculate the value of a foreign exchange put option (AUD per US $\$5$ ) based on the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]] from an Australian-based investor perspective. c. Demonstrate how to reconcile these two results.  

# NOTES  

1. The primary works that uncovered the unique characteristics of [[Forwards and Futures Notes|currency]] options and the application of many of the standard principles of option [[Arbitrage Pricing of Derivatives|pricing]] to [[Forwards and Futures Notes|currency]] options are Grabbe (1983) and Garman and Kohlhagen (1983).   
...
4. The probability in the BSM formula is based on a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] in which the noise term, $\sigma d\mathbb{V}_{t}$ , is added, not subtracted.   
5. This result is easy to see in the risk-neutral context. The [[Black Scholes Derivation|risk-free rate]] term in the numerator of $d_{1}^{D}$ is $r_{D}-r_{F}$ where under [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|risk neutrality]], $\boldsymbol{r}_{D}$ is the risk-neutralized [[Lecture 1- Probability Distributions of Returns|expected return]] on the domestic [[Forwards and Futures Notes|currency]] and $r_{F}$ is the risk-neutralized [[Lecture 1- Probability Distributions of Returns|expected return]] on the foreign [[Forwards and Futures Notes|currency]]. From the foreign investor’s point of view, this corresponding term would be $r_{F}-r_{D}$ , which is $-\big(r_{D}-r_{F}\big).$ That is, the [[Lecture 1- Probability Distributions of Returns|expected return]] is converted to its negative value.   
6. In this context, it is easy to see why [[Forwards and Futures Notes|currency]] options are referred to as both puts and calls. The holder of the option puts one [[Forwards and Futures Notes|currency]] to the writer of the option and calls the other [[Forwards and Futures Notes|currency]]. In the context of assets other than currencies, the holder of the option puts the asset and calls the agreed-on cash, the [[Options|exercise price]], or the other way around.  