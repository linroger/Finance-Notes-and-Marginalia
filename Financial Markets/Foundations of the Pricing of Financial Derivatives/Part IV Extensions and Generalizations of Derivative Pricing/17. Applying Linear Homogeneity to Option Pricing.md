...
n this chapter, we shall illustrate how the mathematical principle of linear homogeneity and its associated Euler rule can be used in [[Arbitrage Pricing of Derivatives|pricing]] options. Our objective is to illustrate the [[Arbitrage Pricing of Derivatives|pricing]] of exchange options and forward start options. We shall, however, illustrate how the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]] can be priced using this procedure. We then show how the exchange option model is a more general model than [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]], which is but a special case of an exchange option. The analysis provided here relies on the concepts of homogeneous functions, linear homogeneity, and Euler’s rule. But first let us introduce exchange options.  

# 17.1 INTRODUCTION TO EXCHANGE OPTIONS  

We will use the concept of linear homogeneity to price exchange options but we first have to define an exchange option. The exchange option, first covered by Margrabe (1978), has proven to be an extremely powerful generalization of the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]]. In addition, the intuitive insights in the derivation of the exchange option model are very useful in other applications in option [[Arbitrage Pricing of Derivatives|pricing]].  
...
With this option, asset 2 plays the role of the [[Options|exercise price]], but the difference from the standard case is that the value of asset 2 is stochastic. Thus, an exchange option is an option with a stochastic [[Options|exercise price]]. Alternatively, one can view this option as a put in which asset 2 can be exchanged for asset 1. In the context of a put, asset 1 plays the role of the [[Options|exercise price]]. Let this option price be denoted as $p_{t}\big(S_{2},\bar{S}_{1}\big)$ and the payoff at expiration be  
$$
...
TABLE 17.1 [[7. Black Scholes Model|Put-Call Parity]] of Exchange Options   


<html><body><table><tr><td colspan="3"></td><td colspan="2">Value at Expiration</td></tr><tr><td></td><td>Instrument</td><td>Current Value</td><td>S1T ≤ S2T</td><td>S1T > S2T</td></tr><tr><td rowspan="3">[[An Asset Allocation Primer|Portfolio]] A</td><td>Long exch. call to exchange asset 2 for asset 1</td><td>ct(S,S2)</td><td>0</td><td>S1T-S2T</td></tr><tr><td>Short exch. put to exchange asset 1 for asset 2</td><td>-pt(S1,S)</td><td>-(S2T -S1T)</td><td>0</td></tr><tr><td>Total</td><td>c(S,S2)</td><td>S1T- S2T</td><td>S1T -S2T</td></tr><tr><td rowspan="3">[[An Asset Allocation Primer|Portfolio]] B</td><td>Long asset 1</td><td>-pt(S1,S) S1t</td><td>+S1T</td><td>+S1T</td></tr><tr><td>Short asset 2</td><td>-S2t</td><td>-S2T</td><td>-S2T</td></tr><tr><td>Total</td><td>S1t-S2t</td><td>S1T -S2T</td><td>S1T-S2T</td></tr></table></body></html>  

First let us establish [[7. Black Scholes Model|put-call parity]] between exchange options. In the previous example, we argued that $p_{t}\big(S_{2},\bar{S_{1}}\big)=c_{t}\big(\bar{S}_{1},\bar{S_{2}}\big)$ . But what about the relationship of $c_{t}{\left(S_{2},S_{1}\right)}$ to $p_{t}\big(S_{2},S_{1}\big)$ ? Keep in mind that in options notation, the first term in parentheses is the underlying, and the second is the strike. Table 17.1 illustrates equivalent portfolios A and B, with the former being long the call and short the put and the latter being long asset 1 and short asset 2. We see that the payoffs of the two portfolios are equivalent.  
...
With this [[Squam Lake Group Introduction|introduction]] to exchange options, we turn to explaining homogeneous functions, linear homogeneity, and Euler’s rule, all useful concepts related to valuing exchange options.  
...
In [[Credit Markets Session 5|Merton]]’s (1973b) classic article on option [[Arbitrage Pricing of Derivatives|pricing]] theory, he demonstrated that the price of a European call option is linearly homogeneous with respect to the [[A Preview of Alternative Formulations|asset price]] and [[Options|exercise price]]. [[Credit Markets Session 5|Merton]] derived this result by defining the problem in terms of the rate of return on the option. In this chapter, we shall take a slightly different but economically equivalent approach.  

As we previously covered, the price of an option can be obtained by appealing to the risk-neutral or equivalent martingale approach, which changes the probabilities of the expiration value of the [[Risk Neutral Pricing of Options|underlying asset]] such that the current [[A Preview of Alternative Formulations|asset price]] is the expected future [[A Preview of Alternative Formulations|asset price]] discounted at the [[Black Scholes Derivation|risk-free rate]]. As such, the expected expiration value of the option is taken using the adjusted probabilities and discounted at the [[Black Scholes Derivation|risk-free rate]] to obtain the well-known [[Option Model Handbook, Part III European Option Pricing With QuantLib Python|Black-Scholes-Merton formula]].  

An option is an asset, so by definition the price at time $t$ of an option is the expected future value of that option discounted at a rate appropriate for the time value of money and the risk. Consequently, without changing the probability distribution, we can obtain the option [[Arbitrage Pricing of Derivatives|pricing]] formula by taking the expected value of the option at expiration using the actual probabilities and discounting at a rate suitable for the option’s risk. In other words,  
$$
c_{t}=e^{-k\tau}E{\left(c_{T}\right)},
$$  

where $k$ is the risk-adjusted [[PSET 7- Kohler|discount rate]] appropriate for the option and $\tau=T-t$ is the [[Chapter 16 - Black–Scholes Model|time to expiration]]. Again, this statement holds by definition for any asset, whether an option or not.  
...
where $S_{T}$ is the [[Black Scholes Derivation|underlying asset price]] at expiration and $X$ is the [[Options|exercise price]]. Now observe that this function is linearly homogeneous with respect to the [[A Preview of Alternative Formulations|asset price]] and [[Options|exercise price]], because  
$$
\operatorname*{max}(0,\lambda S_{T}-\lambda X)=\lambda\operatorname*{max}\bigl(0,S_{T}-X\bigr).
$$  

Thus, we know that the expiration value of the option is linearly homogeneous, but we are interested in whether the current value of the option is linearly homogeneous. Here we appeal to the aforementioned result that the current value of any asset is the expected future value discounted back to the current time. Determining an expected value is a linear mathematical operation. That is, taking an expectation is a linear operation and discounting merely involves multiplying an expected payoff by a [[Discount Factors|discount factor]]. Note that a linear operation performed on a linearly homogeneous function preserves its property of linear homogeneity. Discounting an expected payoff, such as by the factor $e^{-\hat{k\tau}}$ , involves simply multiplying it by a constant, so we preserve the linear homogeneity of the function. Thus, the current price of the option is linearly homogeneous with respect to the [[A Preview of Alternative Formulations|asset price]] and the [[Options|exercise price]]. Hence, we can employ Euler’s rule, and it will be quite useful.  

Applying Euler’s rule to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]] call price, $c_{t}$ , focused on $S_{t}$ and $X$ , we have  
$$
...
By substituting these [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], we attain a statement of the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]] that is consistent with linear homogeneity. We introduce the current price of a zerocoupon bond with par value of $X$ as $B_{t}\equiv e^{-r_{c}\tau}X$ . Hence,  
$$
...
Alternatively, let us simply start by assuming the option price is linearly homogeneous with respect to $S_{t}$ and $B_{t}$ , two assets that can be easily traded. Note that we are not starting with any specified [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]. Using Euler’s rule, we know that the current option price, which is a function of $S_{t}$ and $t$ , can be expressed as  
$$
...
Several important insights can be gained from linear homogeneity. First, no assumptions were made regarding risk-free [[Key Rates O1s Durations and Hedging|hedging]]. Second, the only assumption made regarding the [[Risk Neutral Pricing of Options|underlying asset]] [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] is that it follows an Itô process. Third, the drift term of the Itô process has no influence on the resulting partial differential equation. Fourth, if we assume a generic [[Black Scholes Derivation|geometric Brownian motion]] of the form $d S_{t}=\alpha(S_{t},\bar{t})d t+\sigma S_{t}d\mathbb{W}_{t}$ , we obtain the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]]. In Appendix 17A, we explore linear homogeneity applied to the arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] model.  
...
Although this statement, viewed from the context of Euler’s theorem, is purely mathematical, it also has a natural interpretation in finance. It says that a [[An Asset Allocation Primer|portfolio]] consisting of the purchase of one unit of the exchange call option and short positions in $\partial c/\partial S_{1}$ units of asset 1 and $\partial c/\partial S_{2}$ units of asset 2 would require no initial [[An Asset Allocation Primer|investment]]. Therefore, to avoid profitable [[Arbitrage Pricing of Derivatives|arbitrage]], such a [[An Asset Allocation Primer|portfolio]] must generate an instantaneous return of zero.  
...
We can check the solution by taking the partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] of Equation (17.27) and inserting them into Equation (17.26).  

Because we know that $c$ is linearly homogeneous with respect to the prices of assets 1 and 2, we can say that $a c\big(S_{1},S_{2}\big)=\dot{c}\big(a S_{1},a\bar{S_{2}}\big)$ , where $a$ is a constant. Define $a$ as $1/S_{2}$ , which gives us $\left(1/S_{2}\right)c\left(S_{1},S_{2}\right)=c(S,1)$ or $c\big(S_{1},S_{2}\big)=S_{2}c(S,1)$ where $S=S_{1}/S_{2}$ . In effect we have created a new artificial asset, the ratio of the value of asset 1 to the value of asset 2. Because it is not possible to deliver such an asset, this instrument can perhaps be more easily expressed as a cash-settled call option that enables one to exchange one unit of [[Forwards and Futures Notes|currency]] and receive the value $S=S_{1}/S_{2}$ in cash. Thus, the exchange call is an ordinary European call on $s$ with an [[Options|exercise price]] of 1. We can, therefore, differentiate the exchange option price by differentiating its equivalent value, $S_{2}c(S,1)$ . We will also need second partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] with respect to $s_{1},s_{2}$ , the cross partial of $S_{1}$ and $S_{2}$ , and the first derivative with respect to $t$ .  
...
Equation (17.29) is an ordinary [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] option on the artificial asset $S$ with [[Options|exercise price]] of 1. The $d_{1}$ and $d_{2}$ variables in this variation are the same as shown in Equation (17.27).  

By converting $S_{1}$ and $S_{2}$ to $s$ and the strike to 1, we have normalized these assets. In asset management and [[Lecture 2- Asset Allocation with Multiple Risky Assets|asset allocation]] in particular, we are often interested in the relative performance of asset 1 in terms of asset 2. In other words, we want to know which asset class outperforms the other and by how much. In practice, the actual prices of asset 1 and 2 are often significantly different and hence some form of normalization is necessary. Investors decide the dollar amount to invest in each asset class independent of the [[Accrued Interest|quoted price]] of each asset. We see here clearly that an exchange option can capture in option value in terms of the relative behavior of the two assets.  

Now, for completeness, we need to verify the solution to the PDE in the traditional manner. That is, we must show that Equation (17.27) solves Equation (17.26). Our approach is to transform the PDE into one that is isomorphic to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] and infer the solution via the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]].  

Let us initially examine the [[Vasicek Short Rate Model|first derivatives]] with respect to $S_{1}$ and $S_{2}$ , commonly referred to as the option delta. We will make use of the artificial asset $S$ to simplify the math, thereby enabling us to use many of the results we already know from the BlackScholes-[[Credit Markets Session 5|Merton model]]. We shall need certain partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. The first partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] with respect to the underlying prices are  
$$
...
The second partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] with respect to the asset prices are  
$$
...
Expressing Equation (17.26) based on the [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] above in terms of $s$ and $\sigma$ , we find  
$$
...
We can also see that the first two lines in the equation show that this partial differential equation is the same as the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] partial differential equation when the interest rate, $r_{c}$ , is set to zero, the [[Black Scholes Derivation|underlying asset price]] is $S_{\cdot}$ , and the volatility is $\sigma$ . Recall the [[Mathematical Modeling of Derivative Pricing|Black-Scholes-Merton PDE]] as given in Equation (13.11):  
$$
...
Consequently, we can say that the exchange option is equivalent to $S_{2}$ units of an ordinary European call when the [[Risk Neutral Pricing of Options|underlying asset]] is $S$ , the strike is one, the interest rate is zero, and the volatility is $\sigma^{2}=\sigma_{1}^{2}+\sigma_{2}^{2}-\overset{\cdot}{2}\rho_{12}^{\mathrm{~~}}\sigma_{1}\sigma_{2}.$ . The last two lines above verify that this PDE is the same as the one we previously obtained. Thus, we note that Equation (17.27) solves Equation (17.26).  

This result is useful in better understanding not only the exchange option but also the ordinary European option. The latter can be viewed as an exchange option where the asset exchanged is cash. The exchange option implies a zero interest rate because it can be replicated by holding asset 1 and [[Short Selling|shorting]] asset 2. The [[Short Selling|shorting]] of asset 2 would not have an [[Lecture 1- Probability Distributions of Returns|expected return]] of $r_{c}$ as it would if it were risk free. Rather the holder of asset 2 would demand its [[Lecture 1- Probability Distributions of Returns|expected return]], $\alpha_{2}$ , as compensation. Consequently, the short seller of asset 2, who is trying to replicate the exchange option, would not have an [[Lecture 1- Probability Distributions of Returns|expected return]] of $-r_{c}$ but rather of $-\alpha_{2}$ . In any ordinary European call, the second term in the [[Definitions and Immediate Consequences|pricing equation]] is the present value of the [[Options|exercise price]]. In the exchange option, the second term is also the present value of the [[Options|exercise price]]. The current price of asset 2 is its present value.  

Interestingly, in the same issue of The Journal of Finance directly preceding the Margrabe article, there is an article by Fischer (1978), in which he modeled bonds indexed to [[War Economies and Hyperinflation|inflation]]. He showed that to price such a bond one needs the formula for an option where the [[Options|exercise price]] is stochastic. Such an option is equivalent to an exchange option, and naturally Fisher derives the same formula as Margrabe.  

The traditional exchange option [[Chapter 26 - Pricing Options: Monte Carlo Simulation|Greeks]] are provided in Appendix 17C. We now turn to explore spread options.  

# 17.6 SPREAD OPTIONS3  

We now consider spread options where there is a nonzero [[Options|exercise price]]. In this case, suppose we have a European spread call option in which at expiration the holder can exchange $\alpha_{2}$ units of asset 2 and receive $\alpha_{1}$ units of asset 1 but also must pay a prespecified [[Options|exercise price]], X. We assume $\alpha_{1}$ and $\alpha_{2}$ are both positive. A European spread put option in which at expiration the holder can exchange $\alpha_{1}$ units of asset 1 and receive $\alpha_{2}$ units of asset 2 but again has to pay a prespecified [[Options|exercise price]], $X$ . Note that $X$ can be positive or negative. The payoffs at expiration are  
$$
...
With spread options, neither asset plays the role of the [[Options|exercise price]]. Thus, a spread option is a more complex exchange option with a known [[Options|exercise price]].  

Pearson (1995), Carmona and Durrleman (2003, 2006), Li, Deng, and Zhou (2008), and others have offered approximations when solving for the spread option price when each asset follows [[Black Scholes Derivation|geometric Brownian motion]]. Brooks and Cline (2015) provide a closed-form solution that also incorporates dividends. Based on the results in the previous section as well as the more complex terminal boundary condition, the spread option model based on [[Black Scholes Derivation|geometric Brownian motion]] can be expressed as  
$$
...
The solution in the case of arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] with geometric drift is much more straightforward. Note that the spread is normally distributed and is denoted as  
$$
...
Let $\sigma_{S P}$ denote the standard deviation of changes in the spread. Thus, based on the [[Continuous-Time Stochastic Processes|Brownian motion]] (ABM) model, the spread option prices are  
$$
...
Figure 17.1 illustrates the timeline. Let time $t$ be the day the forward start option is initiated and time $T_{1}$ be the date on which the underlying option is received and $T_{2}$ be the date when the underlying option expires. That is, at time $T_{1}$ $\begin{array}{r}{\mathrm{'}t<T_{1}<T_{2}\mathrm{'}}\end{array}$ ) when the [[A Preview of Alternative Formulations|asset price]] is $S_{1}$ the option will be granted with an [[Options|exercise price]] of $\alpha S_{1}$ . For example, an at-the-money option has $\alpha=1$ . An option with [[Options|exercise price]] $5\%$ higher than the [[A Preview of Alternative Formulations|asset price]] has $\alpha=1.05$ . Because we do not know today what the [[A Preview of Alternative Formulations|asset price]] will be at $T_{1}$ , we cannot prespecify the [[Options|exercise price]]. This would appear to make it difficult to price the option, but in fact it is easy to price the option because we can use the principle of linear homogeneity. The original reference on this is Rubinstein (1991a).  

![](c4648aa022de3947e7162427a1cfb1c67e08f8446baef9e97c060465374bb6b4.jpg)  
FIGURE 17.1 Forward Start Option Timeline  

First let us define $c\big(S_{t},\alpha,T_{1},T_{2}\big)$ as the value of a forward start option at time $t$ in which the [[Options|exercise price]] is determined at time $T_{1}$ and equals $\alpha$ times the value of the underlying at time $T_{1}$ , and the option expires at time $T_{2}$ . Although the option is created and paid for at time $t$ , at time $T_{1}$ , the option formally comes into existence, that is, its [[Options|exercise price]] is now declared. At this point, it has the property of a standard European option, which has a [[Chapter 16 - Black–Scholes Model|time to expiration]] of $\tau_{12}$ . Consequently, we know that it is linearly homogeneous with respect to the [[A Preview of Alternative Formulations|asset price]] and [[Options|exercise price]]. Writing its value as $c\big(S_{1},\alpha S_{1},\tau_{12}\big)$ where subscript 1 denotes $t=1$ , we can state that this option’s value is equal to  
$$
c\big(S_{1},\alpha S_{1},\tau_{12}\big)=S_{1}c(1,\alpha,\tau_{12}).
$$  

The value $c\big(1,\alpha,\tau_{12}\big)$ is known at all times. It is simply the value of an option where the underlying is the value of one unit of the [[Forwards and Futures Notes|currency]], the [[Options|exercise price]] is $\alpha$ , and the [[Chapter 16 - Black–Scholes Model|time to expiration]] is $\tau_{12}$ . One could easily plug these values into the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]] and obtain the value. Even at time $t$ , we know this value. Equation (17.51), however, says that the forward start option value at time $T_{1}$ is the product of $S_{1}$ and this option. At time $t$ , we do not know what $S_{1}$ will be, but we can still price the option at time $t$ .  
...
In other words, holding $c\big(1,\alpha,\tau_{12}\big)$ units of the asset valued at $S_{1}$ will reproduce the option value at time $t$ . Do not, however, confuse this statement with any suggestion of full [[Forward and Futures Contracts|replication]]. Buying $c\big(1,\alpha,\tau_{12}\big)$ units of the asset at time valued at $S_{t}$ will not replicate the final payoff of the option. The [[Forward and Futures Contracts|replication]] strategy is dynamic. We hold a static amount, $c\big(1,\alpha,\tau_{12}\big)$ , of the underlying until time $T_{1}$ . At that point, we have an option with a fixed [[Options|exercise price]] of $S_{1}\alpha$ on an underlying whose value is fluctuating. This option is now an ordinary European call and is replicated using the appropriate number of units of the asset as given by the dynamic delta.  

If instead today we specified a fixed [[Options|exercise price]] of $X$ , then at $T_{1}$ we are awarded an option with an [[Options|exercise price]] of $X$ and a [[Chapter 16 - Black–Scholes Model|time to expiration]] of $\tau_{12}$ . Such an option is trivially equal to purchasing an option today with a [[Chapter 16 - Black–Scholes Model|time to expiration]] of $\tau_{2}$ and an [[Options|exercise price]] of X. If the premium is paid today and the option is received later, it is equivalent to just receiving the option today.  

It is important, however, to contrast these types of options with forward contracts on options. A [[Forward Points in Currency|forward contract]] on an option is an agreement to purchase an option at a later date. The option will have an [[Options|exercise price]] of $X$ and a [[Chapter 16 - Black–Scholes Model|time to expiration]] of $\tau_{12}$ when granted. Being a [[Forward Points in Currency|forward contract]], no money is paid today but a price that will be paid at $T_{1}$ is agreed on by the two parties. It is also a simple process to determine this option’s price. If today we purchase the option with [[Chapter 16 - Black–Scholes Model|time to expiration]] $\tau_{2}$ and [[Options|exercise price]] $X,$ , that is, the option described in the previous paragraph, but borrow the premium, then at time $T_{1}$ , we shall have replicated the payoff of the [[Forward Points in Currency|forward contract]] on the option.  

In other words, let $c\big(S_{t},X,\tau_{2}\big)$ be the price of a call option today struck at $X$ with [[Chapter 16 - Black–Scholes Model|time to expiration]] $\tau_{2}$ . Let $F\big[c\big(S_{t}X\tau_{2}\big),T_{1}\big]$ be the [[Forward Contracts and Forward Prices|forward price]] agreed on today to purchase the option with current price $c\big(S_{t},X,\tau_{2}\big)$ at time $T_{1}$ . The payoff at time $T_{1}$ will be  
$$
c\bigl(S_{1},X,\tau_{12}\bigr)-F\left[c\bigl(S_{t}X\tau_{12}\bigr),T_{1}\right].
$$  

This payoff can be replicated today by purchasing the call option at $c\big(S_{t},X,\tau_{2}\big)$ and borrowing $F\left[c\left(S_{t}X\tau_{2}\right),T_{1}\right]e^{-r_{c}\tau_{1}}$ . At time $T_{1}$ , we are holding the option worth $c\big(S_{t},X,\tau_{12}\big)$ and owe $F\bigl[c\bigl(S_{t}X\tau_{2}\bigr),T_{1}\bigr]$ , which combine to replicate the payoff of the [[Forward Points in Currency|forward contract]]. Because the transaction is a [[Forward Points in Currency|forward contract]], it requires no outlay today. Consequently, the value of the position today, $c(S_{t},X,\tau_{2})-F\big[c\big(S_{t}X\tau_{2}\big),T_{1}\big]e^{-r_{c}\tau_{1}}$ , must equal zero, meaning that  
$$
F\big[c(S_{t}X\tau_{2}),T_{1}\big]=c(S_{1},X,\tau_{2})e^{r_{c}\tau_{1}}.
$$  

Here we see the general rule that the price of a [[Forward Points in Currency|forward contract]] is the price of the underlying compounded to the expiration of the [[Forward Points in Currency|forward contract]]. This result will be derived formally in Chapter 22, when we address the [[Arbitrage Pricing of Derivatives|pricing]] of [[Forward and Futures Contracts|forwards and futures]].  

If the option is written such that the [[Options|exercise price]] is specified as $\alpha{\cal S}_{\cal1}$ , then instead of holding the call worth $c\big(S_{t},X,\tau_{2}\big)$ today, we hold $c(1,\alpha,\tau_{12})$ units of the asset, as determined previously in our derivation of the price of a forward start option. This instrument combines elements of a [[Forward Points in Currency|forward contract]] on an option and a forward start option.  

# 17.8 RECAP AND PREVIEW  

In this chapter, we examined the concept of linear homogeneity and showed how it can be used in option [[Arbitrage Pricing of Derivatives|pricing]]. Linear homogeneity, which occurs in certain types of functions, expresses a relationship between their differentials. Conveniently, the payoff of an option is linearly homogeneous with respect to the [[A Preview of Alternative Formulations|asset price]] and [[Options|exercise price]]. As such, we can use this principle to derive the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]]. In this chapter, we show how it can be used to also derive the value of an exchange option, as well as a forward start option.  
...
By substituting these [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], we confirm that the ABM model has the property of linear homogeneity.  
$$
...
Alternatively, let us simply start by assuming the option price is linearly homogeneous with respect to $S_{t},B_{1t}.$ , and $B_{2t}$ . Note that we are not starting with any specified [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]. Using Euler’s rule, we know that the current option price, which is a function of $S_{t}$ and $t.$ , can be expressed as  
$$
...
which is a second order partial differential equation. This partial differential equation solution, subject to the boundary condition $c_{T}=\operatorname*{max}\bigl(0,S_{T}-\overset{\cdot}{X}\bigr)$ , depends on the specification of the volatility term. The PDE in Equation (17.73) is not the [[Mathematical Modeling of Derivative Pricing|Black-Scholes-Merton PDE]] unless we assume volatility is constant.  

If we assume a generic arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] of the form $d S_{t}=\alpha\big(S_{t},t\big)d t+$ $\sigma d W_{t}$ , we obtain the ABM model which is the solution to  
$$
...
We obtain the first and second [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] in symbolic form. The deltas with respect to the two asset prices were obtained previously but not carried out in detail. They are  
$$
...
Therefore, applying what we know from [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]], the gammas are obtained as  
$$
...
Recall for us to arrive at theta, we first need the derivative with respect to [[Chapter 16 - Black–Scholes Model|time to expiration]] or  
$$
...
Recall we define theta as the derivative with respect to the point in time, that is, $\partial c/\partial t$ and we defined the [[Chapter 16 - Black–Scholes Model|time to expiration]] as $\tau=T-t$ . Hence,  
$$
...
Note that the [[Black Scholes Derivation|risk-free rate]] does not appear in the price equation. Hence, there is no rho.  
...
You are given the following partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]:  
$$
...
2 Assume a stock has a $\$1$ price and a corresponding one year call option is at-themoney. Further, assume no dividends and the [[Black Scholes Derivation|risk-free rate]] is zero. For this case, compare the results of the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]] and ABM model.  
...
4 A hedge fund recently hired a new and highly talented manager. In negotiations, they will receive $20\%$ of any superior performance above the SPY (an exchange-traded fund that seeks to mimic an [[An Asset Allocation Primer|investment]] in the S&P 500 index) over the next year. If they underperform the SPY, then their compensation is zero. Based on this information, identify the valuation model that would provide the fair market value of this compensation scheme. Further, explain the manager’s incentive based on this compensation scheme.  

5 For two constants, $\alpha_{1}>0$ and $\alpha_{2}>0$ , prove the following [[7. Black Scholes Model|put-call parity]] representation:  
$$
...
1. Alternatively, we could assume each asset follows arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] with geometric drift. In this case, the subtraction of two normally distributed variables is also normally distributed, hence, arriving at the [[Arbitrage Pricing of Derivatives|pricing]] model is straightforward.   
