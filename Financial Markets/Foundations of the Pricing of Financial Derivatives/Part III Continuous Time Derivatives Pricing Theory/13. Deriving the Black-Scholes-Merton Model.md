...
- 13. Deriving the Black-Scholes-Merton Model and financial analysis
- 13. Deriving the Black-Scholes-Merton Model in modern finance
- Applications of 13. Deriving the Black-Scholes-Merton Model
- 'Case study: 13. Deriving the Black-Scholes-Merton Model'
...
Define $d S+S$ as the asset price at an instant, $d t.$ , later. Thus, we can write the stochastic process%20Process.md) as $d S+S=S\big[1+\alpha d t+\sigma d{\mathbb{W}}\big].$ Working with the term in brackets, note that we can write it in the following way:  
$$
...
Noting that the time increment until expiration is $\tau$ , we can state that the asset price at expiration is $S_{t+\tau}=S_{T}$ and the stochastic process%20Process.md) for $\mathbb{W}$ is $\Delta\mathbb{W}_{T}=\varepsilon^{*}{\sqrt{\tau}}$ . Thus,  
$$
