...
The [[Swaption Skew|BSM normal model]], as applied in the previous section, has a single. and constant volatility parameter. Taking the model literally, this would. imply that all swaptions can be priced with a single basis-point volatility. Figure 16.2 illustrates, however, that this is not the case: even across swap-. tions with the same expiration and underlying tenor, like either the 2y2y. and 5y10y swaptions in the figure, implied volatilities vary significantly with. strike. As of the [[Arbitrage Pricing of Derivatives|pricing]] date, the two-year [[Teaching Note 4 Interest Rate Derivatives|swap rate]], two years forward is $1.11\%$ (not shown in the figure), which is the ATM strike for 2y2y swap-. tions. The darker gray curve shows, therefore, that 2y2y implied volatilities. increase mostly linearly from 56 basis points at a strike of. $0.11\%$ , which is 100 basis points below the ATM strike, to 125 basis points at a strike of $4.11\%$ , which is 300 basis points above the ATM strike. At the same time, the 10-year [[Teaching Note 4 Interest Rate Derivatives|swap rate]], five years forward is. $2.43\%$ . The lighter gray curve of the figure shows, therefore, that implied volatilities of 5y10y swaptions are roughly 75 basis points for strikes below that ATM strike, and then increase along the curve shown to 95 basis points at a strike 300 basis points over the ATM strike. The phenomenon that basis-point volatility is not constant across strikes is known as the [[Swaption Skew|volatility skew]]. The phenomenon that basis-point volatility is higher for both in- and out-of-the-money options than for at-the-money options, as seen here for 5y10y swaptions, is known as the smile, or, to the extent this effect is not symmetric, [[Lecture Note 6-Implied Volatility|the smirk]].  
...
The existence of a skew does not prevent traders from quoting swap-. tion prices in terms of normal BSM volatility or vice versa, so long as the volatilities vary with strike. But practitioners rely on BSM not just to quote a price or volatility, but also to compute delta, that is, to compute how the value of a swaption changes as its underlying [[Forward Points in Currency|forward rate]] changes. And the usefulness of BSM for this purpose is cast into doubt by Figure 16.2. Without getting into great detail here, the [[A Real-Life Option Pricing Exercise|implied volatility]] of an option at. a given strike can be thought of as the expected gamma-weighted average of instantaneous volatilities over possible paths of the underlying [[Forward Points in Currency|forward rate]] as it changes from its current level to the strike. From this perspective, the [[A Real-Life Option Pricing Exercise|implied volatility]] curves in Figure 16.2 mean that instantaneous volatilities vary in a complex way with the level the [[Forward Points in Currency|forward rate]]. In that case, however, the value of an option changes with the level of rates in two ways: a direct effect, that is, the change in the underlying forward; and an indirect effect, that is, the change in volatility as a result of the change in the level of rates.  

But the [[Swaption Skew|BSM delta]], which is traditionally computed as the change in option value for a change in the underlying at a given volatility, captures only the direct effect.  

The search for a model that can capture the underlying dynamics of. volatility as a function of forward rates has taken two very broad paths. The first is to change the distribution of the [[Forward Points in Currency|forward rate]] away from normal. For example, a constant-volatility lognormal model of rates assumes that volatility is proportional to rates, which might be useful in explaining some manifestations of the skew, like parts of the 2y2y curve in Figure 16.2. In any case, along these lines, the shifted lognormal model allows for the distribution of the underlying [[Forward Points in Currency|forward rate]] to be between normal and lognormal. The defining dynamics of the [[Forward Points in Currency|forward rate]],. $S$ , in this model are,  
$$
\begin{array}{c}{d S_{t}=\phi(S_{t})\sigma\epsilon_{t}\sqrt{d t}}\ {\epsilon_{t}\sim N(0,1)}\ {\phi(S_{t})=a+S_{t}}\end{array}
$$  

with $a\ge0$ . When $a=0$ $\phi(S_{t})=S_{t}$ , the basis-point volatility in Equation. (16.15) is just $\sigma{\cal S}_{t}$ , that is, a constant proportion of the [[Forward Points in Currency|forward rate]]. There-. fore, the distribution of the rate is lognormal. At the other extreme, as $a$ approaches infinity, $\phi(S_{t})$ in (16.17) approaches the constant $a$ , which means. that the volatility in (16.15) approaches a constant and, therefore, that the distribution approaches normality.  

The second approach to finding a model that captures the relationship. between rates and volatility is to allow volatility itself to be a random variable. This approach leads to [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]] models, in which class the SABR model had proven particularly popular. As originally formulated,. however, the model could not handle the negative rates that recently characterized markets in Europe. An adjusted model then emerged, known as the shifted-SABR model, which assumes the following dynamics,.  
$$
...
[[Forward Points in Currency|forward rate]] $S_{t}$ can be as negative as $-b$ with the basis-point volatility of the. model - $(S_{t}+b)^{\beta}\sigma_{t}$ - still positive. Second, the SABR model approaches the normal model as both $\alpha$ and $\beta$ approach 0, and it approaches the lognormal model as $\alpha$ approaches O and as $\beta$ approaches 1. Third, the initial volatility, $\sigma_{0}$ , is most naturally used to match [[Swaptions|ATM swaption]] volatility. Fourth, $\beta$ is typically less than one, because basis-point volatilities, as illustrated in Figures 16.2, do not increase as quickly for high strikes as in a lognormal model. Fifth, the parameters. $\beta$ and $\rho$ control the skew through controlling the relationship between the level of rates and volatility. From Equation (16.18), a higher $\beta$ increases the responsiveness of volatility to the level of the [[Forward Points in Currency|forward rate]]. And from Equation (16.22), a higher $\rho$ increases the correlation between [[Pl Attribution|changes in rates]] and changes in volatility. Sixth, the parameter $\alpha$ controls the smile, or fat tails of the rates distribution, as a higher $\alpha$ increases volatility when volatility is high, that is, $\alpha$ increases volatility when rates. are either higher or lower. Seventh, in practice, because the flexibility of the model enables many different sets of parameters to fit the observed skew, the parameters can also be constrained to fit empirical regularities. One popular choice, for example, is to constrain the parameters to match the empirical backbone, that is, the empirical relationship between ATM basis-point volatilities and forward rates. Eighth, note that, in practice, a different set of parameters are typically chosen for every swaption expiration and underlying tenor. In other words, no BSM-style model can describe the entire volatility cube.  

# Prices, Discount Factors, and Arbitrage.  

# A1.1 DERIVING REPLICATING PORTFOLIOS  

To replicate the 7.625s of 11/15/2022, Table 1.5 uses the 2.875s of 11/15/2021, the 2.125s of 05/15/2022, and the 1.625s of 11/15/2022. Number these bonds from 1 to 3, and let $F_{i}$ be the face amount of bond $i$ in the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]]. Then, the following equations express the requirement that the cash flows of the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] equal those of the 7.625s on each of the [[Preview of the Book|cash flow]] dates. For the cash flows on November 15, 2021,  
$$
...
[[Inverse Matrices Solve Linear Systems|Solving equations]] (A1.1), (A1.2), and (A1.3) for $F_{1},F_{2}$ and $F_{3}$ gives the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]]'s face amounts reported in Table 1.5. Note that, because one bond matures on each date, these equations can be solved one-at-a-time instead of simultaneously.  
...
The only complication in constructing replicating portfolios is to ensure that the matrix C does have an inverse. Essentially, any set of $T$ bonds will do so long as there is at least one bond in the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] making a payment on each of the $T$ dates. All $T$ bonds maturing on the last date would work, for example, but all $T$ bonds maturing on the second-to-last date would not. In the latter case, there would be no bond in the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] making a payment on date $T$  

# A1.2THE EQUIVALENCE OF DISCOUNTING AND ARBITRAGE PRICING  

Proposition: [[Arbitrage Pricing of Derivatives|Pricing]] a bond according to either of the following methods gives the same price:  
...
2. Find the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] of the bond in question using that same set of spanning bonds and calculate the price of the bond as the price of this [[An Asset Allocation Primer|portfolio]].  
...
let p be the vector of prices of each bond in the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]], which.   
...
Under [[Interest Rate Quotations|continuous compounding]], interest is paid every instant, so that the proceeds of an [[An Asset Allocation Primer|investment]] that is continuously compounded over $T$ years grows to the limit of Equation (A2.1) as $_n$ approaches infinity. Taking the logarithm of (A2.1) and rearranging terms,  
$$
n T\ln\left(1+\frac{\hat{r}}{n}\right)=\frac{T\ln(1+\frac{\hat{r}}{n})}{\frac{1}{n}}
$$  

Using l'Hopital's rule, the limit of the right-hand side of (A2.2) as $_n$ becomes large is $\hat{r}T$ . Hence, the limit of (A2.1) is $e^{\hat{r}T}$ , where $e$ is the base of the natural logarithm. Therefore, if interest is continuously compounded at the rate $\hat{\boldsymbol{r}}$ an [[An Asset Allocation Primer|investment]] of one unit of [[Forwards and Futures Notes|currency]] will grow after $T$ years to,  
$$
e^{\hat{r}T}
$$  

Equivalently, the present value of one unit of [[Forwards and Futures Notes|currency]] to be received in $T$ years is,  
$$
e^{-\hat{r}T}
$$  

This section now defines discount factors, spot rates, and forward rates under [[Interest Rate Quotations|continuous compounding]]. Let. $\hat{\boldsymbol{r}}(t)$ be the $t$ -year continuously compounded [[The Foreign Exchange Market Annotations|spot rate]]. Let $f(t-\Delta,t)$ be the [[Forward Points in Currency|forward rate]] from $t-\Delta$ to $t.$ , and  

define $f(t)$ to be the continuously compounded [[Forward Points in Currency|forward rate]] at time $t$ , that is, the limit of $f(t-\Delta,t)$ as $\Delta$ approaches zero.  
...
where $d^{\prime}(t)$ is the derivative of the [[Basic Interest Rate Concepts and Relations|discount function]] with respect to term.  
...
Approximation: The $t$ -year [[The Foreign Exchange Market Annotations|spot rate]] is approximately equal to the average of all forward rates to year. $t$  

Start from Equation (2.21), noting that, because [[Interest Rate Quotations|interest rates]] them-. selves are small numbers, the product of two or more [[Interest Rate Quotations|interest rates]] is particularly small. To illustrate, take the case of the one-year [[The Foreign Exchange Market Annotations|spot rate]], though the argument generalizes easily to longer-term rates,.  
$$
...
Proposition 2: If the [[The Vasicek Model|term structure]] of spot rates is flat, then the [[The Vasicek Model|term structure]] of par rates is flat at that same rate.  

Proof: Denote the semiannually compounded [[Pricing Interest Rate Swaps|par rate]] of maturity $T$ as $C(T)$ . If spot rates are flat at the rate. $\hat{r}$ , then, by definition of. $C(T)$  
$$
...
Proof: By the definition of the [[Pricing Interest Rate Swaps|par rate]], $C(T)$  
$$
...
And, because the [[The Vasicek Model|term structure]] of spot rates is assumed to be increasing,  
$$
...
Note that the spot rates in the summation on the top line are $\hat{\boldsymbol{r}}(t)$ , while those in the summation in the [[Operating Income vs. Net Income What's the Difference|bottom line]] are all $\hat{r}(T)$  
...
Equations (3.6), (3.7), and (3.8) express the relationship between price and yield to maturity when settlement is on a coupon payment date. This. appendix generalizes this relationship to other settlement dates. First, because [[Intra-Year Compounding and Day-Count|accrued interest]] is zero when settlement falls on a coupon payment. date, the [[Accrued Interest|full price]] in the equations in the text do not include any [[Intra-Year Compounding and Day-Count|accrued interest]]. In this section, however, because settlement can fall on other dates, $P$ is taken to include [[Intra-Year Compounding and Day-Count|accrued interest]]..  

Second, the market convention for discounting cash flows that do not occur in regular six-month intervals, using a semiannually compounded rate, is as follows. Let $y$ denote the semiannually compounded yield, and let $\tau$ denote the fraction of a semiannual period until the next coupon payment. For example, if the next coupon payment is in one month, taken to mean one-sixth of a semiannual period, then $\tau=1/6$ . By convention, then, the present value of a unit of [[Forwards and Futures Notes|currency]] at that time is,  
$$
\frac{1}{\left(1+\frac{y}{2}\right)^{\tau}}
$$  

Note that, while reasonably intuitive, this convention cannot really be justified by the logic of compounding conventions. As discussed in the text,. $(1+y/2)^{N}$ represents the final proceeds of an [[An Asset Allocation Primer|investment]] of one unit of cur-. rency semiannually compounded $N$ times. There is no such interpretation. for an exponent that is not a whole number of semiannual periods. In any. case, continuing along these lines, the present value of a unit of [[Forwards and Futures Notes|currency]] to be paid after $\tau+i$ semiannual periods is,  
$$
{\frac{1}{\left(1+{\frac{y}{2}}\right)^{\tau+i}}}
$$  

Finally, then, consider a bond with $2T$ remaining [[Realized Returns|coupon payments]] of. $c/2$ , the first of which is paid after $\tau$ semiannual periods, the second after $\tau+1$ semiannual periods, the third after $\tau+2$ semiannual periods, etc., and the last, along with a principal payment of 100, after $\tau+2T-1$ semiannual periods. Its price is given by,.  
$$
...
By inspection, the right-hand side of Equation (A3.9) is the $_n$ -year return of the bond if all coupons are reinvested at $y$ and if the yield after $_n$ years is $y$ . Breaking that down, the numerator gives the reinvested [[Realized Returns|coupon payments]] at a yield of $y$ ; plus $P_{n}$ , the price of the bond after $_n$ years if the yield is $y$ minus $P_{0}$ the initial price of the bond at the yield $y$ . Hence, the numerator divided by the initial price equals the $_n$ -year return.  
...
Note that the proposition is not an "if and only if," because it is possible - though very unlikely - to have some combination of [[Assets|returns]] on coupons and a final yield that also result in a holding-period return of $y$ per year.  

# A3.3 REALIZED FORWARD SCENARIO  

For simplicity, this section assumes annual coupons and annual compounding.  

Proposition: Under the realized forward scenario, the. $_n$ -year return of a $T$ -year coupon, with [[Pl Attribution|coupon income]] reinvested at the initial forward rates $f(1),f(2),\ldots f(n)$ , is the same as from rolling over a unit of [[Forwards and Futures Notes|currency]] at those. forward rates.  
...
where (A3.11) simply multiplies both sides of (A3.10) by $(1+f(1))\cdots\cdot\cdot(1+$ $f(n))$ ; (A3.12) recognizes that the second and third lines of (A3.11) are $P_{n}$ the price of the bond after $_n$ years, with discounting under the assumption of realized [[Forwards and Futures|forwards]]; and (A3.13) simplifies terms.  

By inspection, the right-hand side of Equation (A3.13) is the $_n$ -year return of the bond if all coupons are reinvested at the short-term rate of the realized forward scenario and if, after. $_n$ years, the bond is priced under. that scenario. But if the right-hand side is the bond's $_n$ -year return under the conditions stated, then, by the left-hand side, that return is equivalent to rolling over a unit of [[Forwards and Futures Notes|currency]] at the initial forward rates for $_n$ years.  
...
Let $P^{i}$ denote the price of asset. $i$ and $P$ the price of a [[An Asset Allocation Primer|portfolio]] of those assets. By definition,  
$$
P=\sum P^{i}
$$  

Let $y$ be the single factor generating [[Pl Attribution|changes in rates]]. Then, taking the derivative of both sides of (A4.1) with respect to $y$  
$$
...
Or, in words, the Dv01 of a [[An Asset Allocation Primer|portfolio]] equals the sum of the individual asset DV01s.  

To derive the [[Key Rates O1s Durations and Hedging|duration]] of a [[An Asset Allocation Primer|portfolio]], start from Equation (A4.2), dividing both sides by $-{\cal P}$  
$$
...
Equation (A4.5) multiplies each term in the summation by one, in the form of $\overset{}{P^{i}}/\overset{}{P^{i}}$ . Equation (A4.6) follows from the definition of [[Key Rates O1s Durations and Hedging|duration]] in the text,.   
Equation (4.11). In words, Equation (A4.6) says that the [[Key Rates O1s Durations and Hedging|duration]] of a port-.   
folio equals the weighted average of the durations of the individual assets,.   
where the weights are the fraction of value of each asset in the [[An Asset Allocation Primer|portfolio]]..  

The proof for the [[PSET II Fixed Income Asset Pricing 1|convexity]] of a [[An Asset Allocation Primer|portfolio]] can be derived along the same lines as the [[Key Rates O1s Durations and Hedging|duration]] of a [[An Asset Allocation Primer|portfolio]]. The result, given here without proof, is,  
$$
...
Let $P(y)$ be the price of an asset as a function of the single factor that describes [[Pl Attribution|changes in rates]]. Then, a second-order [[Profit and Loss Attribution with an OAS|Taylor approximation]] of the price rate function is given by,  
$$
...
where (A4.11) - which is Equation (4.16) - follows from the definitions of. [[An Analytical Decomposition of Forward Rates|duration and convexity]], that is, from Equation (4.10) and a discrete version of Equation (4.14).  

# Regression Hedging and Principal Component Analysis  

# A6.1 REGRESSION HEDGES AND P&L VARIANCE  

This section proves that i) the [[Reverse Regressions|regression hedge]] minimizes the variance of the. P&L of the hedged [[An Asset Allocation Primer|portfolio]]; and ii) the volatility of the regression-hedged. [[An Asset Allocation Primer|portfolio]] equals the Dv01 of the position being hedged times the standard deviation of the regression residuals..  

Begin with [[Level Versus Change Regressions|least-squares estimation]], which finds the parameters $\hat{\alpha}$ and $\hat{\beta}$ to minimize,  
$$
...
To minimize this variance by choosing the Dv01 in the [[Key Rates O1s Durations and Hedging|hedging]] bonds, differentiate (A6.15) with respect to $\overline{{D V01}}^{30}$ , set the result to zero, and solve. for $\overline{{D V01}}^{30}$  
$$
...
But, by inspection of Equation (A6.5), the fraction in (A6.16) is just the estimated slope coefficient in a regression of JNJ yields on 30-year Treasury yields. Hence, the [[Reverse Regressions|regression hedge]] given in Equations (6.8) or (6.10) minimizes the variance of the $\mathrm{P}\&\mathrm{L}$ of the hedged position.  

The minimized P&L variance of the hedged [[An Asset Allocation Primer|portfolio]] can be written. explicitly by substituting Equation (A6.16) into Equation (A6.15) and rearranging terms,  
$$
...
Now consider [[An Asset Allocation Primer|portfolio]] weights or loadings of $-0.5,1.0$ and $-0.6$ on the five-, 10-, and 30-year rates, respectively. By the properties of variance and covariance, and with the specific covariance matrix (A6.24), the variance of this [[An Asset Allocation Primer|portfolio]], denoted ${\sigma}_{p}^{2}$ , is,  
$$
...
Computations like this are more conveniently written with matrix notation. Let the vector of [[An Asset Allocation Primer|portfolio]] weights be w, which, in the present example,  
...
IABLE A6.1 [[Principal Component Analysis|Principal Components]] of USD LIBOR Swap Rates, from June 1, 2020, to July 16, 2021, Using Only. Five-, 10-, and 30-Year Rates. Entries Are in Basis Points.   
...
This section is more precise on a few claims made in the previous section at the cost of some extra mathematics. Let $\mathbf{V}$ denote the $3\times3$ variancecovariance matrix of rates with elements $V_{i j}$ ; let $\mathbf{P}$ denote the $3\times3$ matrix of [[Principal Component Analysis|principal components]], with elements $p_{i j}$ , or, alternatively, with three $3\times1$ column vectors ${\bf p_{i}}$ corresponding to PC $i$ ; let $\mathbf{D}$ denote the $3\times3$ diagonal matrix with diagonal elements $\sigma_{i}^{2}$ , each equal to the variance of $\mathrm{PC}i;$ and let I denote the $3\times3$ [[Prerequisites|identity matrix]]. Then, though not proved here, the construction of the PCs in the previous section guarantees that,  
$$
...
Let $P$ be the [[Accrued Interest|full price]] of some security that depends on $r$ and time. Then, by [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's lemma]],  
$$
d P=P_{r}d r+P_{t}d t+{\frac{1}{2}}P_{r r}\sigma^{2}d t
$$  

where $P_{r},P_{t}$ , and $P_{r r}$ denote the partial [[Vasicek Short Rate Model|first derivatives]] with respect to $r$ and $t$ and the second partial derivative with respect to $r$ . Dividing both sides of (A8.2) by $P$ , taking [[FORWARD RATES AND TERM STRUCTURE|expectations]], and defining $\alpha_{P}$ to be the [[Lecture 1- Probability Distributions of Returns|expected return]] of the security,  
$$
...
Now consider the strategy of investing one unit of [[Forwards and Futures Notes|currency]] in security $P$ and $-P_{r}\mathcal{Q}/P\mathcal{Q}_{r}$ in security Q. From Equations (A8.4) and (A8.5), the return on this [[An Asset Allocation Primer|portfolio]] is,.  
$$
\frac{d P}{P}-\frac{P_{r}Q}{P Q_{r}}\frac{d Q}{Q}=\alpha_{P}d t-\frac{P_{r}Q}{P Q_{r}}\alpha_{Q}d t
$$  

Note that terms with the random variable dw have fallen out of Equation (A8.6). This particular [[An Asset Allocation Primer|portfolio]] is chosen, in fact, so as to hedge completely the risk of $P$ with $\mathcal{Q}$ . In any case, because the [[An Asset Allocation Primer|portfolio]] has no risk, it must earn the instantaneous rate, $r_{0}$  
$$
...
Equation (A8.8) says that the [[Lecture 1- Probability Distributions of Returns|expected return]] of any security above the. instantaneous rate divided by its [[Key Rates O1s Durations and Hedging|duration]] with respect to that rate must equal some function $\uplambda$ . This function cannot depend on any characteristic of the security, because (A8.8) is true for all securities. The function may depend, however, on the interest rate factor and time. Rewriting Equation (A8.8), for any security $P$  
$$
...
This section illustrates the implementation of the [[Vasicek Short Rate Model|Vasicek model]] in a [[Rate and Price Trees|binomial tree]]. The parameters are $r_{0}=2\%$ $\theta=11\%$ $k=0.0165$ ; and $\sigma=$ $0.95\%$ . The step size is one year. The first stages of construction are shown in Figure A9.1. The starting short-term rate, by definition is $2\%$ . By a discrete time approximation to the dynamics of the [[Profit and Loss Attribution with an OAS|risk-neutral process]] in Equation (9.1) of the text, the expected short-term rate after one year is,  
$$
...
FIGURE A9.1  [[Rate and Price Trees|Binomial Tree]] Setup for Three Dates of the [[Vasicek Short Rate Model|Vasicek Model]].  

The next stages of the construction are to find the missing rates and probabilities in Figure A9.1. From the date-1 up-state, the unknowns, $r^{u u}$ and $p$ , must result in the [[Black-Scholes Model|drift and volatility]] specified by the model. Mathematically, the drift condition is,  
$$
...
Let $P_{t}(\tau)$ denote the zero coupon bond price at time $t$ with time-to-expiry $\tau$ Taking the usual expectation of exponential of future [[An Overview of the Vasicek Short Rate Model|short rate]] path conditional on the factors expressed in reduced form, one can show that,  
$$
...
where $\Upsilon(\tau,\alpha)=(\Upsilon_{s}(\tau,\alpha),\Upsilon_{m}(\tau,\alpha),\Upsilon_{l}(\tau,\alpha),)$ where $\Upsilon_{s},\Upsilon_{m}$ and $\Upsilon_{l}$ represent the partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] (a.k.a. loadings) of the changes of the zero coupon. yield of maturity $\tau$ on the short, medium and long rate factors. A similar rep-. resentation for the (continuously compounded) [[Forward Points in Currency|forward rate]] of tenor. $\tau^{\prime}$ as an affine function of the factors in cascade form can be obtained as follows,  
$$
...
Expression (A9.15) can be interpreted as follows: The time $t$ [[Forward Points in Currency|forward rate]] with maturity $\tau$ and tenor $\tau^{\prime}$ can be decomposed into a component that. represents the [[Risk-Neutral Probabilities|risk-neutral expectation]] of the $\tau^{\prime}$ maturity yield prevailing at.  

time $t+\tau$ (first two terms of the right-hand side) minus a component that represents the [[PSET II Fixed Income Asset Pricing 1|convexity]] advantage of receiving (i.e., being long) rates (third term of the right-hand side). The first on the right-hand side component can be shown to be null for $\tau=0$ and converges to $\mu$ as $\tau$ grows toward infinity. For maturities between zero and infinity, the [[Risk-Neutral Probabilities|risk-neutral expectation]] of the zero yield prevailing at that maturity will depend on the value that the factors take at time $t$ , namely, $\boldsymbol{x}_{t}=(\boldsymbol{r}_{t},m_{t},l_{t})$ , and this is what the second term on the right-hand side captures. We can also think of the sum of the first two components on the right-hand side as the fair [[Futures Not Subject to Cash-And-Carry|futures]] rate at time $t$ for a mark-to-market [[Futures Not Subject to Cash-And-Carry|futures]] contract on the zero coupon yield that will prevail at time $t+\tau$ . The shape of the slopes $\Upsilon^{\prime}(\tau,\alpha,\tau^{\prime})$ , evaluated at estimated parameter values, are displayed in Figure 9.7.  

The last term on the right-hand side of (A9.15) is null for. $\tau=0$ and grows at speed equal to the square of maturity. $\tau$ . This [[PSET II Fixed Income Asset Pricing 1|convexity]] term drives. a wedge between the [[Futures Not Subject to Cash-And-Carry|futures]] rate and the [[Forward Points in Currency|forward rate]]. Imagine that an investor can bet on the yield that will prevail at time. $t+\tau$ in two different contracts: the first is a daily-settled [[Futures Not Subject to Cash-And-Carry|futures]] contract. Any increase (decrease) in the market expected [[Forward Rate|future rate]] will be paid (received) today by the long. (short) investor. The second is a standard [[Forward Points in Currency|forward contract]]. Any increase (decrease) in the market expected [[Forward Rate|future rate]] will be paid (received) by the long (short) investor not today, but at time $t+\tau$ . Now, if the [[Futures Not Subject to Cash-And-Carry|futures]] rate and. the [[Forward Points in Currency|forward rate]] were equal, the long investor would be at an advantage in transacting via the [[Forward Points in Currency|forward contract]] rather than the [[Futures Not Subject to Cash-And-Carry|futures]] contract: when. the market [[Forward Rate|future rate]] expectation increases (decreases) their future losses (gains) will be discounted to today at a relatively higher (lower) rate, hence yielding a relatively smaller (larger) present value loss (gain). Therefore, to prevent an [[Arbitrage Pricing of Derivatives|arbitrage]] between forward and [[Futures Not Subject to Cash-And-Carry|futures]] rates, today's [[Forward Points in Currency|forward rate]] should be somewhat lower than today's [[Futures Not Subject to Cash-And-Carry|futures]] rate. The difference between the two is what we call the [[PSET II Fixed Income Asset Pricing 1|convexity]] correction or [[PSET II Fixed Income Asset Pricing 1|convexity]] advantage term, that is, the last term on the right hand side.  

# A9.2.2 A Practical Estimation Method  

There exists a large literature on estimating gaussian [[Financial Mathematics Course|term structure models]], of which the Gauss+ model is a particular case. The methods typically rely on maximum-likelihood estimation procedures which are not easy to implement in general. In the next section we discuss a procedure that is fast and intuitive for the purpose of finding reasonable values for the Gauss $^{\ast+}$ parameter vector $P=(\alpha,\sigma,\mu)$ . Given this parameter vector, and for any time $t$ , we will then extract the factors $\boldsymbol{x}_{t}=(\boldsymbol{r}_{t},m_{t},l_{t})$ in order to have the model zero yields be close to the market zero yields. The [[An Overview of the Vasicek Short Rate Model|short rate]] $r_{t}$ will not need to be filtered because it will be set equal to the observed short policy rate..  
...
bonds or [[Determining the Expression for the Fair Value of the Swap Spread|par swap]] rates. For the purpose of fitting the Gauss+ model, it does. not matter whether these discount factors come from a [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]] (index. or discount curves) or from a bond curve. Here we will use the time series of zero coupon bonds published by the New York Fed, which is based on. applying the well-known Nelson-Siegel-Svensson smooth curve construction procedure to US Treasury notes and bonds.2.  

There is always a trade-off when it comes to selecting the sample for the estimation of the model. Longer samples will lead to more robust estimates. in a statistical sense but will also reflect different market conditions from the ones in which we intend to use the model. We have found that, in practice, a good compromise is to use a sample size of eight years and a decay factor of. 0.8. (This means that in the loss-functions associated with the [[Prerequisites|optimization problems]] that follow we will give a weight of 1 to the last observation, a weight of 0.8 to an observation that is one year old relative to the last observation, a weight of 0.64 to an observation that is two years old relative to. the last observation, and so forth.) Our sample thus begins on January 5,. 2014, and ends on January 21, 2022.  

While the $\mathrm{Gauss+}$ model involves three factors, only two are genuine. factors as we will equal the [[An Overview of the Vasicek Short Rate Model|short rate]] to a given observed [[An Overview of the Vasicek Short Rate Model|short rate]], which tends to change at predictable dates. For this reason, we will take the [[An Overview of the Vasicek Short Rate Model|short rate]] $r_{t}$ to be [[Monetary Policy with Abundantreserves|the fed]] funds target. While taking the general collateral repo rate would be closer to the actual overnight funding rate for US Treasury bonds, this series also exhibits spikes that reflect circumstantial funding conditions and are generally unrelated to [[The Economist Regime Change|monetary policy]], and hence unrelated to [[FORWARD RATES AND TERM STRUCTURE|expectations]] of future paths of the [[An Overview of the Vasicek Short Rate Model|short rate]]..  
...
Estimation involves one step for data preparation, and three steps for parameter optimization. Data preparation consists of netting the effect of the (observed) [[An Overview of the Vasicek Short Rate Model|short rate]] factor from the observed zero coupon yields. This works as follows. Take a candidate parameter value for $\alpha_{r}$ . Then, given the structure of $\Upsilon(\tau,\alpha)$ , subtract $\Upsilon_{s}(\tau,\alpha)\times r_{t}$ from both sides of (A9.13). With an abuse of notation, we will denominate $y_{t}(\tau)$ the zero coupon yield netted out of the [[An Overview of the Vasicek Short Rate Model|short rate]], and we will drop the [[An Overview of the Vasicek Short Rate Model|short rate]] $r_{t}$ from the vector of factors in cascade form, so from now on we have $\boldsymbol{x}_{t}=(m_{t},l_{t})$ . Incidentally, one could show that $\Upsilon_{s}(\tau,\alpha)$ depends only on parameter $\alpha_{r}$  
...
Once the parameter vector $P$ has been estimated, we can solve for the. factors $(m_{t},l_{t})$ to ensure that the model fits exactly the two- and 10-year. forward rates (with tenor one year) on each date. (Filtering methods such as least-squares or Kalman filtering could be employed to extract fitted factors, but we find exact fitting of two points in the curve is preferable in practical trading applications.) We show the fitted factors in Figure 9.8 in the text, along with the 2y [[Forward Points in Currency|forward rate]].3.  

Note that in the text we plot the affine function of $l_{t}$ $L(l_{t})$ below rather than $l_{t}$ itself, so that we can interpret the derived long factor as an approximation of the expectation of what the [[An Overview of the Vasicek Short Rate Model|short rate]] will be in 10 years' time. This is necessary because the extreme persistence (or large half-life, or low mean-reversion parameter) of the factor $l_{t}$ makes the interpretation of the fit-. ted level of $l_{t}$ less intuitive. As you can see, the long factor closely tracks the 10y [[Forward Points in Currency|forward rate]] from before (the difference between the two is explained by the [[PSET II Fixed Income Asset Pricing 1|convexity]] correction adjustment),  
$$
L(l_{t})=\mu(1-e^{-10\alpha_{l}})+l_{t}e^{-10\alpha_{l}}
$$  

The time series properties of the model can be described by graphing its fitted factors over time. As mentioned already, the short-term rate is set each day to [[Monetary Policy with Abundantreserves|the fed]] funds target rate, and the medium- and long-term factors  
...
We will assume henceforth that only the risk of the long factor is priced. This is reasonable for the application we have in mind, which will involve extracting [[FORWARD RATES AND TERM STRUCTURE|expectations]] about the 10-year maturity and, at that maturity, the loadings of the forward relative to the short and medium rate are negligible.. Now, applying [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's lemma]] to the zero coupon bond price as an exponential. affine function of the cascade form factors (A9.9) using (A9.13), and passing. to the true measure, we can write the dynamics of the instantaneous return of a zero coupon bond as follows,.  
$$
\frac{d P(t,\tau)}{P(t,\tau)}=(r_{t}+\lambda_{t}(\tau-t)\Upsilon_{3}(\tau-t,\alpha)\sigma_{l})d t-(\tau-t)\Upsilon(\tau-t,\alpha)\Omega d W_{t}^{*}
$$  

where the loadings vector $\Upsilon(\tau,\alpha)$ was defined in (A9.14), and $\Upsilon_{3}(\tau,\alpha)$ stands for its last element, i.e., the loading of a zero coupon yield with maturity $\tau-t$ on the long rate factor. Also, $\Omega$ was defined in (A9.8) and $\mathbb{W}_{t}^{*}$ stands for the [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] under the true measure.  

The term multiplying dt on the right-hand side of (A9.25) has a clear. interpretation: the [[Lecture 1- Probability Distributions of Returns|expected return]] at time. $t$ of holding $\tau$ maturity zero coupon bond from time $t$ to time $t+d t$ is equal to the riskless rate prevailing at time $t$ , plus a risk-premium term that is equal to the [[Key Rates O1s Durations and Hedging|duration]] of the zero coupon bond (its maturity) times the loading of the yield of a zero coupon bond with maturity $\tau-t$ , times the volatility of the long rate, times the. price of risk prevailing at time t, namely,. $\lambda_{t}$ . We will assume that the price of risk does change over time. We will not specify its dynamics, but we will assume that $\lambda_{t}$ is a very persistent process, so that $E_{t}(\lambda_{t+\Delta t})\approx\lambda_{t}$ for $\Delta t=1$ i.e. one price of risk next-year is expected to be equal to this year's. If we changed measure to the [[Verifying Martingale Property with Q|risk-neutral measure]], this term would disappear from the right-hand side of (A9.25), that is, the expected instantaneous return of our zero coupon bond would equal the riskless rate.  

Recall as well the definition of the [[Forward Points in Currency|forward rate]] at time $t$ with maturity $\tau$ and tenor $\Delta\tau:f_{t}(\tau)=(p(t,\tau)-p(t,\tau+\Delta\tau))/\Delta\tau-\mathrm{we}$ will omit the tenor as. an explicitly argument of $f_{t}()$ . Now, consider the following strategy: At time $t$ , buy one zero coupon bond with maturity $\tau+\Delta\tau$ , and simultaneously sell. one zero coupon bond with maturity $\tau$ . First, we claim that the return of this strategy is equal to the [[Forward Points in Currency|forward rate]] with tenor $\Delta\tau$ prevailing at time $t$ minus the $\Delta\tau$ maturity yield prevailing at time $\tau$ . To see this, denote by. $R_{t}^{\tau}$ the cumulative [[Realized Returns|realized return]] between time $t$ and time $\tau$ . We then have,  
$$
...
where $f_{\tau}(\tau)$ is equal to the zero coupon yield with maturity $\Delta\tau$ prevailing at time $\tau$ which, for a sufficiently small $\Delta\tau$ , approximately equals the [[The Foreign Exchange Market Annotations|spot rate]] prevailing at time $\tau$ . Hence, the [[Lecture 1- Probability Distributions of Returns|expected return]] (under the true measure) of this strategy at time $t$ is equal to the [[Forward Points in Currency|forward rate]] at time $t$ , minus the. expected [[An Overview of the Vasicek Short Rate Model|short rate]] at time $\tau$ . Let's keep this result aside for a moment.  

We will now compute the [[Lecture 1- Probability Distributions of Returns|expected return]] of this strategy (again, under the true measure). Note that the [[Lecture 1- Probability Distributions of Returns|expected return]] of the long and short side cancel out for entire the holding period except for the segment $(t,t+\Delta\tau)$ and $(\tau-\Delta\tau,\tau)$ , because in the model the risk premia depends on [[Hedging Strategies with Forwards|time to maturity]] only. Applying [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's lemma]] to (A9.25), canceling the [[An Overview of the Vasicek Short Rate Model|short rate]], integrating and taking [[FORWARD RATES AND TERM STRUCTURE|expectations]], and dismissing the contribution of the segments $(\tau-\Delta\tau,\tau)$ because only the long factor is priced, we get,.  
$$
...
Note that the expected risk premium (A9.27) is the product of the yet. unknown $\lambda_{t}$ and an amount of risk term. $R P(t,\tau,\Delta\tau)$ that can be easily computed. As we mention in the text, in order to imply $\lambda_{t}$ from the observed forward rates and the parameters of the model, we will assume that there is a maturity $\tau$ such that the (true) expectation of what the [[An Overview of the Vasicek Short Rate Model|short rate]] will be. at maturity $\tau$ is equal to the (true) expectation of the [[An Overview of the Vasicek Short Rate Model|short rate]] will be at. any maturity $\tau^{\prime}>\tau$ . With this assumption, we can specialize this reasoning. to two long maturities $\tau^{\prime}>\tau$ , subtract the resulting equations and solve for. the price of risk $\lambda_{t}$ as follows,  
$$
\lambda_{t}=\frac{f_{t}(\tau^{\prime})-f_{t}(\tau)}{R P(t,\tau^{\prime},\Delta\tau)/\Delta\tau-R P(t,\tau,\Delta\tau)/\Delta\tau}
$$  

Armed with an estimate for. $\lambda_{t}$ , we can then take [[FORWARD RATES AND TERM STRUCTURE|expectations]] on both. sides of the equation (A9.26) at maturity $\tau-\Delta\tau$ and use (A9.27) to solve for the expected rate for a long enough maturity $\tau-\Delta\tau$ . We finally get,.  
$$
E_{t}(r_{\tau})=f_{t}(\tau)-\lambda_{t}R P(t,\tau,\Delta\tau)
$$  

To get an intuition for (A9.29), say that the price of risk $\lambda_{t}$ that we solved before is 0.09. The loading of the 10-year zero coupon bond yield on the long factor is 0.7, and the volatility of the long factor is about 100 basis points, then the 10-year risk premium would be approximately $0.09\times0.01\times10\times$ $0.7=63\mathrm{bps}$ . The [[PSET II Fixed Income Asset Pricing 1|convexity]] advantage term for the 10-year rate is about 24bps. These numbers are very close to the ones obtained by evaluating the model at estimated parameters, and 0.09 is the average price of risk in the sample.5 If we assume that the 10-year [[Forward Points in Currency|forward rate]] is $3\%$ , the implied expected one-year rate, nine years rate under the true measure would then be $0.03+0.0024-0.0063=2.61\%$  

We computed the price of risk in the manner described already for using the 14- and 15-year [[Forwards and Futures|forwards]] to extract $\uplambda$ for every day in our sample, and then using this estimate to imply the expectation of the 10-year rate using the 10-year [[Forward Points in Currency|forward rate]]. Results are not sensitive to using other, longer maturities.  

The implied 10-year rate expectation computed in this manner is plotted in the text against an estimate of the long run expected [[An Overview of the Vasicek Short Rate Model|short rate]] obtained. outside the model, just by just adding the real rate forecast produced by the Cleveland Fed, to a measure of long run [[War Economies and Hyperinflation|inflation]], which we took as the average of the 10-year [[War Economies and Hyperinflation|inflation]] rate forecast produced by the Cleveland Fed, the 10-year forecast produced by the ATsIX model of the Philadelphia Fed,. and an estimate of trend [[War Economies and Hyperinflation|inflation]] obtained with a simple exponential moving. average rule with a decay parameter of 0.987, a value used elsewhere in the. literature.6 Other procedures to assess long rate [[FORWARD RATES AND TERM STRUCTURE|expectations]] are possible. (for example, the Survey of Market Participants conducted by the New York Fed several times a year, or the long-standing Bluechip Financial Forecast survey).  

# A11.1 FORWARD DROP APPROXIMATELY EQUALS CASH CARRY  

Define the following notation, all for 100 face amount of a bond:  
$p_{0}$ : [[Accrued Interest|flat price]] for spot settlement : $c$ : annual coupon payment : $d_{1}$ : number of days from spot settlement to the coupon payment date $d_{2}$ : number of days from the coupon payment date to [[Forward Bond Yield|forward settlement]]. $d=d_{1}+d_{2}$ : number of days from spot to [[Forward Bond Yield|forward settlement]]. $A I_{0},A I_{d}$ : [[Intra-Year Compounding and Day-Count|accrued interest]] as of spot and forward settlements,. respectively : $r{:}$ repo rate from spot to [[Forward Bond Yield|forward settlement]]. : $p_{0}(d)$ : [[Accrued Interest|flat price]] for [[Forward Bond Yield|forward settlement]]  

With this notation, the [[Forward Contracts and Forward Prices|forward price]] of the bond, based on the discussion in the text, can be written as,.  
$$
...
This section highlights the differences between [[Arbitrage Pricing of Derivatives|pricing]] forward and [[Mathematics of the Financial Markets|futures contracts]] in a [[The Vasicek Model|term structure]] model. To focus on this difference, it is assumed here that the [[Futures Not Subject to Cash-And-Carry|futures]] contract has one delivery date and one deliverable bond and that its conversion factor is one. Section A11.4 discusses the [[Arbitrage Pricing of Derivatives|pricing]] of. delivery options in a [[The Vasicek Model|term structure]] model. Also, for simplicity, it is assumed. here that there are no intermediate coupon dates between the spot and [[Forward Bond Yield|forward settlement]] dates.  

Using the methodology of Chapter 7, begin with a recombining, bi-. nomial tree with three dates, labeled 0, 1, and 2, where the forward and [[Mathematics of the Financial Markets|futures contracts]] expire on date 2. Set the following notation:.  
$r_{0},r_{1}^{u}$ , and $r_{1}^{d}$ : the initial one-period rate, and the one-period rates on date 1, in the up and down states, respectively.   $\textbf{\em P}_{0},P_{2}^{u u},P_{2}^{u d}$ and $P_{2}^{d d}$ : the initial [[Accrued Interest|full price]] of the underlying bond, and the date-2 full prices in the "up-up," "up-down," and "down-down" states, respectively..   $\mathbf{\Phi}^{\overline{{\mathbf{\alpha}}}}\mu_{0},p_{2}^{u u},p_{2}^{u d}$ and $\dot{p}_{2}^{d d}$ : fat pries at the indicated dates and states..   
: $P_{0}(2)$ : the initial full [[Forward Contracts and Forward Prices|forward price]]..   
: $p_{0}(2)$ : the initial flat [[Forward Contracts and Forward Prices|forward price]].. $F_{0},F_{1}^{u}, $ $F_{1}^{d}$ : the initial [[Futures Price and the Quality Option Before E|futures price]], and the date-1 [[Futures Not Subject to Cash-And-Carry|futures]] prices in the up and down states, respectively..   $\equiv q,1-q$ : the risk-neutral [[A Primer on Probability Theory and Stochastic  Modelling|transition probabilities]] on all dates to move up and down from the current state, respectively.  
...
Each term of Equation (A11.4) is the probability of a particular path through the tree times the discounted value of the bond price at the end of that path, where discounting is done using the [[Volatility and Convexity|short-term rates]] along. the path. Taking all terms together, therefore, $P_{0}$ is the expected discounted bond price. More generally, then, letting $_n$ be the number of periods, $P_{n}$ the random [[Accrued Interest|full price]] of the bond at period $_n$ , and $r_{i}$ the random short-term rate over period $i$ to $i+1$  
$$
P_{0}=E\left[\frac{P_{n}}{\prod_{i=0}^{n-1}(1+r_{i})}\right]
$$  

The [[Discount Factors|discount factor]] to period. $n,d(n)$ , is just a special case of (A11.5) when the terminal price equals one in all states. Hence,  
$$
d(n)=E\left[\frac{1}{\prod_{i=0}^{n-1}(1+r_{i})}\right]
$$  

Following the discussion in the text, the forward [[Accrued Interest|full price]] of the bond is just the future value, to the forward delivery date, of the full spot price of the bond. Therefore, using Equations (A11.5) and (A11.6),  
$$
P_{0}^{f w d}=\frac{P_{0}}{d(n)}
$$  

Turning to the [[Futures Price and the Quality Option Before E|futures price]], start again with the recombining, risk-. neutral [[Rate and Price Trees|binomial tree]]. At expiration on date 2, the [[Futures Price and the Quality Option Before E|futures price]] equals the flat bond price. (At contract expiration, the bond is delivered at the [[Futures Not Subject to Cash-And-Carry|futures]]. price plus [[Intra-Year Compounding and Day-Count|accrued interest]].) In the up state of date 1, the [[Futures Price and the Quality Option Before E|futures price]] is $F_{1}^{u}$ , which means that the [[Pricing and Hedging Implications of Daily Sett|daily settlement]] payment for a [[Chapter 4 - Futures: Hedging and Speculation|long position]] of one contract on date 2 is ${p}_{2}^{u u}-F_{1}^{u}$ in the up-up state and $p_{2}^{u d}-F_{1}^{u}$ in the up-down state. But because the value of a [[Futures Not Subject to Cash-And-Carry|futures]] contract initiated at any. date is zero, the expected discounted value of the [[Pricing and Hedging Implications of Daily Sett|daily settlement]] payments. from the date 1 up state must be zero. Hence,  
$$
\frac{q(p_{2}^{u u}-F_{1}^{u})+(1-q)(p_{2}^{u d}-F_{1}^{u})}{1+r_{1}^{u}}=0
$$  

And, solving for the [[Futures Price and the Quality Option Before E|futures price]],  
$$
F_{1}^{u}=q{p}_{2}^{u u}+(1-q)p_{2}^{u d}
$$  

Analogously, the [[Futures Price and the Quality Option Before E|futures price]] in the date 1 down state is,  
$$
F_{1}^{d}=q{p}_{2}^{u d}+(1-q)p_{2}^{d d}
$$  

On date 0, the [[Futures Price and the Quality Option Before E|futures price]] is $F_{0}$ , and the expected discounted value of. the date 1 settlement payment must be zero. Hence,  
$$
...
Hence, in a risk-neutral tree, the [[Futures Price and the Quality Option Before E|futures price]] is the expected value of the bond price on the contract [[Risk Neutral Pricing of Options|expiration date]]. More generally,.  
$$
F_{0}=E[\phi_{n}]
$$  
...
Let $A I_{n}$ be the [[Intra-Year Compounding and Day-Count|accrued interest]] on the bond as of date $_n$ . Because [[Intra-Year Compounding and Day-Count|accrued interest]] is not a random variable, $E[P_{n}]=E[\Tilde{P}_{n}]+A I_{n}$ . Next, substitute the definitions of. $P_{0},d(n)$ , and $F_{0}$ from Equations (A11.5), (A11.6), and. (A11.14) into Equation (A11.16) to see that,.  
$$
\mathrm{Co}\nu\left(P_{n},\frac{1}{\prod_{i=0}^{n-1}(1+r_{i})}\right)=P_{0}-(F_{0}+A I_{n})d(n)
$$  

Rearranging terms, substituting in the [[Forward Contracts and Forward Prices|forward price]] from Equation (A11.7), and recognizing that phwd $P_{0}^{f w d}=\bar{p}_{0}^{\bar{f}w d}+A I_{n}$  
$$
...
Finally, because bond prices are negatively correlated with [[Interest Rate Quotations|interest rates]], the covariance term in Equation (A11.18) is negative and, therefore,  
$$
F_{0}<p_{0}^{f w d}
$$  

The [[Futures Price and the Quality Option Before E|futures price]] is less than the [[Forward Contracts and Forward Prices|forward price]].  

# A11.4  FUTURES DELIVERY OPTIONS IN A TERM STRUCTURE MODEL  

Having set up a [[The Vasicek Model|term structure]] model in the form of a tree, the quality option. can be priced as follows. Start at the delivery date. At each node, compute the ratio of the price to conversion factor for each deliverable bond. Find the. bond with the minimum ratio and set the [[Futures Price and the Quality Option Before E|futures price]] equal to that ratio.. Then, given these terminal values of the [[Futures Price and the Quality Option Before E|futures price]], prices on earlier dates can be computed along the lines described in Section A11.2. In the case of the timing, end-of-month, and wild-card options, which are American-style options exercisable by the short, the [[Futures Price and the Quality Option Before E|futures price]] at each node in the tree. is the minimum of the [[Futures Price and the Quality Option Before E|futures price]] with the option exercised and with the. option not exercised.  
...
[[6. A Brief Introduction to Stochastic Calculus|Arbitrage-free pricing]] models usually assume that some set of securities is fairly priced. In the case of [[Futures Not Subject to Cash-And-Carry|futures]], the standard assumption is that the forward prices of all bonds in the [[Mechanics of Us Treasury Note and Bond Futures|deliverable basket]] are fair. Technically, this [[Credit Markets Session 4|calibration]] can be accomplished by assigning a spread to each bond such that its [[Forward Contracts and Forward Prices|forward price]] in the model matches the [[Forward Contracts and Forward Prices|forward price]] in the market. The assumption that bond prices are fair is in the spirit of [[Arbitrage Pricing of Derivatives|pricing]] the [[Futures Not Subject to Cash-And-Carry|futures]] relative to cash. Another approach can be used to decide whether the underlying bonds are fairly priced or not.  

[[Financial Mathematics Course|Term structure models]] commonly used for [[Arbitrage Pricing of Derivatives|pricing]] [[Futures Not Subject to Cash-And-Carry|futures]] fall into two. main categories. First are one- or two-factor short-rate models along the lines of those described in Chapters 7-9. These models are relatively easy to. implement and, for the most part, flexible enough to capture the yield curve dynamics driving [[Futures Not Subject to Cash-And-Carry|futures]] prices. With only one or two factors, however, these. models cannot capture the idiosyncratic price movements of any deliverable bond relative to another.  

The second type of model allows for a richer set of relative price movements across bonds in the [[Mechanics of Us Treasury Note and Bond Futures|deliverable basket]]. These models essentially allow each bond to follow its own price or yield process. There are costs to this flexibility, however. First, ensuring that these models are [[Arbitrage Pricing of Derivatives|arbitrage]] free takes some effort. Second, the user must specify the parameters that describe the stochastic behavior of all bond prices in the basket, for example, a volatility for each of the bonds in the basket along with correlations between each pair of bonds.  

[[Chapter 2 - Futures Markets|Futures traders]] often describe their models in terms of the beta of each bond in the basket relative to a benchmark bond in the basket. The beta. of a bond represents the expected change in yield of that bond given a one-basis-point change in the yield of the benchmark. A bond with a beta of 1.02, for example, implies that the bond's yield is assumed to change $2\%$ more than the yield of the benchmark bond. The beta of a particular bond. can be thought of as the coefficient from a regression of changes in its yield on changes in the benchmark's yield. Note that in a [[Profit and Loss Attribution with an OAS|one-factor model]] the. beta of a bond is simply the ratio of the volatility of that bond yield to the volatility of the benchmark's yield.  

# Short-Term Rates and Their Derivatives  

ing of forward and [[Futures Not Subject to Cash-And-Carry|futures]] rates and the [[The Futures-Forward Difference|futures-forward]] rate difference in [[Financial Mathematics Course|term structure models]]. All [[FORWARD RATES AND TERM STRUCTURE|expectations]] here are also with respect to some risk-neutral or [[Arbitrage Pricing of Derivatives|pricing]] probabilities.  

The first step in this Appendix is to show that the [[Futures Not Subject to Cash-And-Carry|futures]] rate with respect to a term rate, like a [[EURIBOR Forward Rate Agreements and Futures|Euribor futures]] rate in the text, is greater than the corresponding forward term rate.  

The $\beta$ year rate, $t$ years forward,e $r_{t}^{f w d}$ , is the rate that equate the present value at time $t$ of receiving a unit of [[Forwards and Futures Notes|currency]] at time $t+\beta$ with the price of -year zero coupon bond, -years forward, p. Usig continuously compounded rates,  
$$
...
where $F_{0}$ is the [[Futures Price and the Quality Option Before E|futures price]] of that zero coupon bond.  
...
Because of [[Pricing and Hedging Implications of Daily Sett|daily settlement]] payments of [[Mathematics of the Financial Markets|futures contracts]], and following the same argument as in Appendix A11.2, which leads to Equation (A11.14),  
...
By [[Volatility and Convexity|Jensen's inequality]] and Equation (A12.5),  
$$
...
The next step in this appendix is to show that the [[Futures Not Subject to Cash-And-Carry|futures]] rate with respect to an average rate, like a [[One-Month SOFR Futures|one-month SOFR]] ([[SOFR|Secured Overnight Financing Rate]]) [[Futures Not Subject to Cash-And-Carry|futures]] rate or a [[Fed Fund Futures|fed funds futures]] rate, is greater than the [[Futures Not Subject to Cash-And-Carry|futures]] rate with respect to a term rate. This fact, combined with the result just proved, implies that [[Futures Not Subject to Cash-And-Carry|futures]] rates with respect to average rates are also greater than the corresponding forward rates.  

Let $R(t,t+\beta)$ be the integral of the continuously compounded [[Short-Term Rates and the Transition from LIBOR|overnight rates]] from $t$ to $t+\beta$ , so that the average rate is $R(t,t+\beta)/\beta$ . Once again, because of the [[Pricing and Hedging Implications of Daily Sett|daily settlement]] of [[Mathematics of the Financial Markets|futures contracts]], today's [[Futures Not Subject to Cash-And-Carry|futures]] rate on the average rate, $A_{t}^{f u t}$ , is today' expectation of the average at time $t$  
$$
...
where $E_{t}[\cdot]$ gives the expectation as of time $t$ . Applying [[Volatility and Convexity|Jensen's inequality]]. to (A12.10) and rearranging terms,  
$$
...
where the third equation takes [[FORWARD RATES AND TERM STRUCTURE|expectations]] of the second as of today, and the last equality follows from Equation (A12.9). Equation (A12.11) says that the [[Futures Not Subject to Cash-And-Carry|futures]] rate on the average is greater than the [[Futures Not Subject to Cash-And-Carry|futures]] rate on the term rates, as was to be proved.  

The final step in this appendix is to show that the [[Futures Not Subject to Cash-And-Carry|futures]] rate with. respect to a compounded rate, like a three-month [[One-Month SOFR Futures|SOFR futures]] rate, is greater than the [[Futures Not Subject to Cash-And-Carry|futures]] rate with respect to an average rate. Once again,. because of [[Pricing and Hedging Implications of Daily Sett|daily settlement]], today's [[Futures Not Subject to Cash-And-Carry|futures]] rate on a compounded rate, Cfut, is the expectation of that compounded rate in the future. Mathematically, in a continuously compounded version of Equation (12.10) in the text,.  
$$
C_{t}^{f u t}=\frac{1}{\beta}(E[e^{R(t,t+\beta)}]-1)
$$  

By [[Volatility and Convexity|Jensen's inequality]] and the properties of the exponential function,  
$$
E[e^{R(t,t+\beta)}]-1>e^{E[R(t,t+\beta)]}-1>E[R(t,t+\beta)]
$$  

Finally, combining Equations (A12.12), (A12.13), and the definition of the [[Futures Not Subject to Cash-And-Carry|futures]] rate on the average in (A12.9),  
$$
\displaystyle{C_{t}^{f u t}=\frac{1}{\beta}(E[e^{R(t,t+\beta)}]-1)>\frac{1}{\beta}E[R(t,t+\beta)]=A_{t}^{f u t}}
$$  

Hence, as was to be proved, the [[Futures Not Subject to Cash-And-Carry|futures]] rate on a compounded rate is greater than the [[Futures Not Subject to Cash-And-Carry|futures]] on an average rate.  

To summarize, the financial insight is that, because of [[Pricing and Hedging Implications of Daily Sett|daily settlement]], the [[Forward Contracts and Forward Prices|forward price]] on a zero coupon bond is greater than the [[Futures Price and the Quality Option Before E|futures price]] on. that bond. This result, from Chapter 11, plus a [[PSET II Fixed Income Asset Pricing 1|convexity]] effect in moving from price to rate, shows that the [[Futures Not Subject to Cash-And-Carry|futures]] rate with respect to a term rate exceeds the corresponding forward term rate. Then, accounting for the [[PSET II Fixed Income Asset Pricing 1|convexity]] effects of moving to a [[Futures Not Subject to Cash-And-Carry|futures]] rate with respect to an average rate or with respect to a compounded rate, these [[Futures Not Subject to Cash-And-Carry|futures]] rates exceed the [[Futures Not Subject to Cash-And-Carry|futures]] rate with respect to the term rate. The [[Operating Income vs. Net Income What's the Difference|bottom line]], then, is that all of the [[Futures Not Subject to Cash-And-Carry|futures]] rates discussed in the chapter exceed the corresponding term [[Forward Points in Currency|forward rate]]..  

# Interest Rate Swaps  

# A13.1PRICING A EURIBOR SWAP AS OF FEBRUARY 24, 2022  

This section prices a two-year [[Swaptions|fixed-for-floating swap]], where the floating rate is three-month [[EURIBOR Forward Rate Agreements and Futures|Euribor]]. The inputs are the [[The Vasicek Model|term structure]] of STR OIS given in Table A13.1, the two-year [[EURIBOR Forward Rate Agreements and Futures|Euribor]] [[Teaching Note 4 Interest Rate Derivatives|swap rate]] of. $0.078\%$ , and the two-year STR-[[EURIBOR Forward Rate Agreements and Futures|Euribor]] [[Basis Swaps|basis swap]] spread of. $0.138\%$ . All cash flows are. assumed to follow the actual/360 convention, and all of the relevant dates are assumed to be business days..  

The OIS rates in the fourth column of the table are observed in the market. The discount factors are calculated by setting the present value of the fixed payments on each OIS (including the fictional notional amount at maturity) equal to par, which is the value of the floating legs that pay compounded daily STR. Letting $d(t)$ denote the [[Discount Factors|discount factor]] for $t$ years, the equations determining the discount factors, along the same lines as for [[Advanced Usage of QuantLib analytics library|SOFR swaps]] explained in Chapter 2, are the following,.  

TABLE A13.1 STR OIS Rates as of February 24, 2022.   


<html><body><table><tr><td>Term (years)</td><td>Term (date)</td><td>Term (days)</td><td>Rate (%)</td><td>[[Discount Factors|Discount Factor]]</td><td>FwdRate (%)</td></tr><tr><td>0.25</td><td>05/24/2022</td><td>89</td><td>-0.5695</td><td>1.0014099</td><td>-0.1408</td></tr><tr><td>0.50</td><td>08/24/2022</td><td>181</td><td>-0.5580</td><td>1.0028134</td><td>-0.1400</td></tr><tr><td>0.75</td><td>11/24/2022</td><td>273</td><td>-0.5110</td><td>1.0038902</td><td>-0.1073</td></tr><tr><td>1.00</td><td>02/24/2023</td><td>365</td><td>-0.4380</td><td>1.0044606</td><td>-0.0568</td></tr><tr><td>1.25</td><td>05/24/2023</td><td>454</td><td>-0.3380</td><td>1.0042784</td><td>0.1081</td></tr><tr><td>1.50</td><td>08/24/2023</td><td>546</td><td>-0.2330</td><td>1.0035455</td><td>0.0730</td></tr><tr><td>1.75</td><td>11/24/2023</td><td>638</td><td>-0.1400</td><td>1.0024888</td><td>0.1054</td></tr><tr><td>2.00</td><td>02/24/2024</td><td>730</td><td>-0.0600</td><td>1.0012201</td><td>0.1267</td></tr></table></body></html>  
$$
...
Solving this set of equations gives the discount factors in Table A13.1. The resulting forward rates are shown in the table as well but are not. needed for the remaining calculations. In any case, with these discount factors, the two-year [[EURIBOR Forward Rate Agreements and Futures|Euribor]] [[Teaching Note 4 Interest Rate Derivatives|swap rate]] of. $0.078\%$ and the two-year STR versus three-month [[EURIBOR Forward Rate Agreements and Futures|Euribor]] [[Basis Swaps|basis swap]] spread of. $0.138\%$ are related by equating the present value of the fixed side of the [[EURIBOR Forward Rate Agreements and Futures|Euribor]] swap payments (including the fictional notional amount) to one (i.e., the value of floating STR, including the fictional notional amount) plus the present value of the [[Basis Swaps|basis swap]] spread payments. Mathematically,  
$$
...
Note that Equation (A13.2) is the [[Arbitrage Pricing of Derivatives|pricing]] condition. Given the $0.078\%$ [[Teaching Note 4 Interest Rate Derivatives|swap rate]], it can be used to solve for the $0.138\%$ [[Basis Swaps|basis swap]] spread. Or, conversely, given the $0.138\%$ [[Basis Swaps|basis swap]] spread, it can be used to solve for the $0.078\%$ [[Teaching Note 4 Interest Rate Derivatives|swap rate]].  

# A13.2 TWO-CURVE PRICING  

For the purposes of this section, assume for simplicity that payments are annual at times $t=1,\ldots,T$ . For ease of exposition, say that the [[Black Scholes Derivation|risk-free rate]] index is STR and that the non-[[Black Scholes Derivation|risk-free rate]] index is [[EURIBOR Forward Rate Agreements and Futures|Euribor]]. Given a set of discount factors, $d(t)$ , derived from STR OIS; a set of [[EURIBOR Forward Rate Agreements and Futures|Euribor]] swap rates, $c(t)$ ; and STR versus [[EURIBOR Forward Rate Agreements and Futures|Euribor]] [[Basis Swaps|basis swap]] spreads, $x(t)$ , the fair [[Arbitrage Pricing of Derivatives|pricing]] conditions for the [[EURIBOR Forward Rate Agreements and Futures|Euribor]] swaps, discussed in the text and the previous section are, for every $t$  
$$
c(t)\sum_{s=1}^{t}d(s)+d(t)=1+x(t)\sum_{s=1}^{t}d(s)
$$  

Now define a set of adjusted [[EURIBOR Forward Rate Agreements and Futures|Euribor]] forward rates, $L^{\prime}(t)$ , such that the present values of the floating legs of the [[EURIBOR Forward Rate Agreements and Futures|Euribor]] swaps equal their correct values, which are given by the right-hand side of (A13.3),  
$$
\sum_{s=1}^{t}L^{\prime}(s)d(s)+(1+L^{\prime}(t))d(t)=1+x(t)\sum_{s=1}^{t}d(s)
$$  

Given the [[Basis Swaps|basis swap]] spreads, Equation (A13.4) could be used to solve for all of the adjusted [[EURIBOR Forward Rate Agreements and Futures|Euribor]] forward rates, one at a time, starting with $t=1$ and continuing through to. $t=T$ .These $L^{\prime}(t)$ could then be used to price. payments that depend on [[EURIBOR Forward Rate Agreements and Futures|Euribor]]. However, noticing that the right-hand sides of Equations (A13.3) and (A13.4) are the same, these two equations. can be combined,  
$$
c(t)\sum_{s=1}^{t}d(s)+d(t)=\sum_{s=1}^{t}L^{\prime}(s)d(s)+(1+L^{\prime}(t))d(t)
$$  

But given all of the swap rates,. $c(t)$ , Equation (A13.5) can be used iteratively to solve for the $L^{\prime}(t)$ . In other words, so long as the [[EURIBOR Forward Rate Agreements and Futures|Euribor]] swaps are. priced fairly relative to STR OIS, there is no need to know the [[Basis Swaps|basis swap]] spreads.  

In summary, then, the two-curve methodology for [[Arbitrage Pricing of Derivatives|pricing]] [[EURIBOR Forward Rate Agreements and Futures|Euribor]] swaps is as follows. First, solve for the adjusted forward rates, $L^{\prime}(t)$ , as just described. Second, project those adjusted forward rates as future [[EURIBOR Forward Rate Agreements and Futures|Euribor]] floating rates to set floating-leg payments. Third, discount both fixed- and floating-leg payments using the STR discount factors.  

# Corporate Debt and Credit Default Swaps  

# A14.1 CUMULATIVE DEFAULT AND SURVIVAL RATES  

Proposition: If the hazard rate is constant at $\uplambda$ , then the [[A Constant Default Probability Model|cumulative survival probability]] to time $t$ $C S(t)$ , is $e^{-\lambda t}$ , and the cumulative default probability, CD(t), is 1 - e-t.  
...
where Equation (A14.4) is the solution to Equation (A14.3). The cumulative default probability is just one minus the [[A Constant Default Probability Model|cumulative survival probability]],. which gives CD(t) = 1 - e-At .  

[[Bond Equivalent Yield (BEY) - Definition, Formula, and Example|Fixed Income Securities]]: Tools for Today's Markets, Fourth Edition. Bruce Tuckman and Angel 505 Serrat $\copyright$ 2022 Bruce Tuckman and Angel Serrat. Published 2022 by John Wiley & Sons, Inc..  

# A14.2 UPFRONT AMOUNTS  

Continue with the notation of the previous section, and add the following:  
$s^{C D S}$ : [[Cds-Equivalent Bond Spread|CDS spread]] cCDS: CDS coupon : $T$ : maturity of the CDS, in years. n: number of CDS premium payments per year $\overline{{\overline{{\mathbf{\alpha}}}}}t_{i}$ : time of CDS premium payment $\quad;i=1,\ldots,n T_{\cdot}$ with $t_{0}\equiv0$ : $d(t_{i})$ : [[Discount Factors|discount factor]] at time. $t_{i}$ , at risk-free or benchmark rates $\texttt{\textbf{n}}$ : [[Credit Markets Session 3|recovery rate]] UF: upfront amount  

Then, following the logic described in the text, with the detail that premium payments follow the actual/360 [[Intra-Year Compounding and Day-Count|day-count]] convention, the expected discounted value of the fee leg, $V^{f e e}$ , is,  
$$
...
can be computed along the lines of the contingent leg, except that the payment from the bond upon default is $R$ times the [[HSBC-Auto callable Barrier Notes with Step-up Premium|principal amount]], while the payment due to the buyer of protection upon default is. $1-R$ times the [[HSBC-Auto callable Barrier Notes with Step-up Premium|principal amount]]. Therefore, using Equations (A14.5) and (A14.6) - simplified here to assume that [[Realized Returns|coupon payments]] are exactly one-half years apart - the. price of a bond,. $P$ , with a coupon rate,. $c$ given a hazard rate, is given by,.  
$$
...
Assuming that investors are [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|risk neutral]] or that the hazard rate is a [[Financial Mathematics Course|risk-neutral pricing]] rate, investors are indifferent between buying a corporate bond and buying a bond without [[Default Risk and Credit Derivatives 183|default risk]] if the [[Lecture 1- Probability Distributions of Returns|expected return]] of the former, given in Equation (A14.15), is equal to the [[Black Scholes Derivation|risk-free rate]]. Mathematically, then,  
$$
...
This section very briefly discusses three problems that arise in the context. of [[Arbitrage Pricing of Derivatives|pricing]] MBS in [[Financial Mathematics Course|term structure models]]: path dependence, proxies for the mortgage rate, and factors apart from [[Interest Rate Quotations|interest rates]]..  

As for path dependence, the burnout and media effects imply that the value of an MBs depends not only on the current [[6. A Brief Introduction to Stochastic Calculus|term structure of interest rates]], but also on the history of the [[The Vasicek Model|term structure]]. [[Arbitrage Pricing of Derivatives|Pricing]] by backward induction, however, described in earlier chapters of the book, does not naturally accommodate path dependence: at any node of a [[Rate and Price Trees|binomial tree]], for example, there is no memory of how the interest rate process moved from its current state to that particular node of the tree.  

A popular solution for [[Arbitrage Pricing of Derivatives|pricing]] path-dependent claims is [[Pricing a Callable Leveraged Constant Maturity Swap Spread Note|Monte Carlo simulation]]. To price a security in this framework, in a one-factor setting, proceed as follows. First, generate a large number of paths of [[Interest Rate Quotations|interest rates]] at the frequency and to the horizon desired. For this purpose, paths are generated using a particular [[Profit and Loss Attribution with an OAS|risk-neutral process]] for the short-term rate. Second, calculate the cash flows of the security along each path. In the mortgage context, this would include the security's scheduled payments along with its prepayments. Burnout and the media effect can be implemented in this framework, because each path is available in its entirety as cash flows are calculated. Third, starting at the end of each path, calculate the discounted value of the security's cash flows along each path using the [[Interest Rate Quotations|interest rates]] along that path. Fourth, compute the value of the security as the average of the discounted values across paths.  

To connect [[Pricing a Callable Leveraged Constant Maturity Swap Spread Note|Monte Carlo simulation]] with the [[Arbitrage Pricing of Derivatives|pricing]] approach used elsewhere in the book, recall Equation (A11.5), reproduced here for convenience,  
$$
P_{0}=E\left[\frac{P_{n}}{\prod_{i=0}^{n-1}(1+r_{i})}\right]
$$  
...
In the [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] framework, measures of [[Forward Bond Yield|interest rate sensitivity]] can be computed by shifting the initial [[The Vasicek Model|term structure]] in some manner, repeating the valuation process, and calculating the difference between the initial and shifted prices. As a numerical matter, paths should not be regenerated between the initial and shifted valuations, as that would introduce noise into the sensitivity calculations.  

A general drawback of [[Pricing a Callable Leveraged Constant Maturity Swap Spread Note|Monte Carlo simulation]] is that is does not naturally accommodate the valuation of American- or Bermudan-style options,. because the value of holding an option at any state typically requires [[Arbitrage Pricing of Derivatives|pricing]] by backward induction. While methodological advances are now used to overcome this problem,1 the issue does not really arise when valuing the. prepayment option. As it is generally accepted that homeowner behavior is not well explained as the optimal exercise of a [[Lecture Notes Bonds,  Preferred Stock,  and Structured Products|fixed income]] option, a path-dependent prepayment function, which is well-suited to [[Pricing a Callable Leveraged Constant Maturity Swap Spread Note|Monte Carlo simulation]], is the preferred approach..  

The second problem addressed in this section relates to the mortgage. rate. Valuing an MBS along a path requires both the benchmark of discounting rates as well as the mortgage rate. Discounting can be done at benchmark rates plus a spread, but the incentive of a prepayment model depends on the current mortgage rate. The difficulty is that calculating the fair mortgage rate at a single date on a single path of a [[Pricing a Callable Leveraged Constant Maturity Swap Spread Note|Monte Carlo simulation]] is a problem of the same order of magnitude as the original problem of [[Arbitrage Pricing of Derivatives|pricing]] a particular. MBS as of the current state! Common practice, therefore, is to build a simple model of the mortgage rate as a function of benchmark rates and, perhaps,. volatility, for example, an estimated regression model of the mortgage rate as a function of one or more benchmark rates and rate volatility. It may not be trivial to compute long-term benchmark rates along a path of [[Volatility and Convexity|short-term rates]], but this difficulty can usually be overcome with a closed-form solution for long-term rates or with a numerical approximation consistent with the short-term rate process.  

The third problem is that defaults and turnover depend on variables other than [[Interest Rate Quotations|interest rates]], like housing prices, which are not traditionally included in the [[Arbitrage Pricing of Derivatives|pricing]] of [[Bond Equivalent Yield (BEY) - Definition, Formula, and Example|fixed income securities]]. Incorporating these variables, including their correlations with [[Interest Rate Quotations|interest rates]], is yet another challenge of [[Arbitrage Pricing of Derivatives|pricing]] MBS.  
...
1. Given the functional form of a probability distribution (e.g., normal, lognormal), there exist parameters of that distribution such that. $V_{0}$ , the [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|arbitrage-free price]] of any asset today, is given by,.  
$$
{\frac{V_{0}}{N_{0}}}=E_{0}\left[{\frac{V_{t}}{N_{t}}}\right]
$$  

where $N_{t}$ is the price at time $t$ of an asset chosen as the numeraire; $\mathbf{}V_{t}$ is value at time $t$ of an asset being priced today, including reinvested cash flows; and. $E_{t}[\cdot]$ gives [[FORWARD RATES AND TERM STRUCTURE|expectations]] as of time $t$ under the appropriately parameterized probability distribution. Equation (A16.1) is known as the [[Continuous-Time Stochastic Processes|martingale property]] of asset prices. This claim is proved in a special case in Section A16.2 but used more generally here.  
...
Gains process. The gains process of an asset at any time equals the value of that asset at that time plus the value of all its cash flows reinvested to that time. For this purpose, all cash flows are reinvested and then rolled at prevailing short-term [[Interest Rate Quotations|interest rates]].  
...
These definitions allow for the statement of the main result of this section: in the [[Exercises|absence of arbitrage]] opportunities, there exists a parameter-. ization of a given probability distribution, or a [[Arbitrage Pricing of Derivatives|pricing]] measure, such that the normalized gains of any asset today equals the expected value of that asset's normalized gains in the future. Technically, there exist probabilities such that the normalized gains process is a martingale. As the goal here is. intuition rather than mathematical generality, this result is proven in the context of a single-period, [[A Real-Life Option Pricing Exercise|binomial]] process..  
...
In this framework, any asset can be priced by [[Arbitrage Pricing of Derivatives|arbitrage]] relative to the other two assets. The method is just as in Chapter 7. To price asset C by [[Arbitrage Pricing of Derivatives|arbitrage]], construct its [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]], in particular, a [[An Asset Allocation Primer|portfolio]] with $\alpha$ of asset A and $\beta$ of asset B such that,  
$$
\begin{array}{r}{C_{1}^{0}=\alpha A_{1}^{0}+\beta B_{1}^{0}}\ {{}}\ {C_{1}^{1}=\alpha A_{1}^{1}+\beta B_{1}^{1}}\end{array}
$$  

Then, to rule out risk-free [[Class Note 13 The LTCM Meltdown|arbitrage opportunities]], it must be the case that,  
$$
...
Equation (A16.23) just rearranges the terms of (A16.22); combining. (A16.23) with (A16.17) and (A16.18) gives (A16.24); and (A16.24) with (A16.16) gives (A16.25). Hence, as was to be shown, there is a [[Arbitrage Pricing of Derivatives|pricing]] measure, in this case the probability. $p$ , such that the normalized gains processes. of B and C are martingales. And, of course, since nothing distinguishes A from the other assets, a probability with the same properties could have. been found had B or C been chosen as the numeraire instead..  
...
Therefore, the [[Continuous-Time Stochastic Processes|martingale property]] for the bond says that,  
$$
...
So, applying the [[Continuous-Time Stochastic Processes|martingale property]] under the numeraire to a payment of $c(1+r_{2})$ on date 2 requires that,  
$$
...
With this result, the discussion [[Assets|returns]] to the [[Continuous-Time Stochastic Processes|martingale property]] of the bond in (A16.26). Substituting (A16.28) into (A16.26),  
$$
...
The left-hand side of the second line of (A16.29) is the date 0 [[Forward Contracts and Forward Prices|forward price]] of the bond for delivery on date 2. The third line, then, simply denotes this [[Forward Contracts and Forward Prices|forward price]] by $B_{0}(2)$ . Hence, taking the zero coupon bond of maturity. $T$ as a numeraire, the [[Forward Contracts and Forward Prices|forward price]] of a bond for delivery on date $T$ is a martingale.  

Turning now to the price of an option on the bond, consider a call with payoff $(B_{T}-K)^{+}$ . Applying the [[Continuous-Time Stochastic Processes|martingale property]] to the option price and assuming that the forward bond price is lognormal with volatility parameter $\sigma$ , the call option is priced as,  
$$
...
The terminal payoff of a [[EURIBOR Forward Rate Agreements and Futures|Euribor futures]] call option with strike $K$ and expiration time $T$ is, per unit notional,  
$$
[K-f_{T}(T,T+\tau)]^{+}
$$  

Given the [[Pricing and Hedging Implications of Daily Sett|daily settlement]] feature of [[EURIBOR Forward Rate Agreements and Futures|Euribor futures]] options, the numeraire of choice is the money market account, the value of one unit of [[Forwards and Futures Notes|currency]] invested and then rolled every period, at the prevailing short-term rate. Denoting the money market account by $M(t)$ and the short-term rate from time t - 1 to t by rt,  
$$
M(0)=1
$$  
$$
...
The first point to make about the money market account is that it is the numeraire of the risk-neutral short-term rate process used in the [[Financial Mathematics Course|term structure models]] presented earlier in the book. To see this, apply the [[Continuous-Time Stochastic Processes|martingale property]] with the numeraire to an arbitrary gains process. $V_{t}$ at time $t$  
$$
...
The second point to make about the money markets as numeraire is that [[Futures Not Subject to Cash-And-Carry|futures]] prices are martingales under this numeraire. This is proved in Section A16.5.  

Turning now to [[EURIBOR Forward Rate Agreements and Futures|Euribor futures]] options, because they are subject to [[Pricing and Hedging Implications of Daily Sett|daily settlement]] and are [[Mathematics of the Financial Markets|futures contracts]], their prices are also martingales with the money market account as numeraire. Furthermore, if $F_{t}$ is the underlying. [[Futures Price and the Quality Option Before E|futures price]] at time. $t$ , then at the expiration of a put option on the [[Futures Price and the Quality Option Before E|futures price]] (call on rates) at time $T$ , the option is worth. $(F_{T}-K)^{+}$ . Putting together the [[Continuous-Time Stochastic Processes|martingale property]] of the [[Futures Not Subject to Cash-And-Carry|futures]], (A16.38), the [[Continuous-Time Stochastic Processes|martingale property]] of [[Futures Not Subject to Cash-And-Carry|futures]] options, (A16.39), and the [[EURIBOR Futures Options|final settlement price]] of the [[Futures Not Subject to Cash-And-Carry|futures]] options, (A16.40), results in the price of the [[EURIBOR Forward Rate Agreements and Futures|Euribor futures]] put option at time t, denoted vEBPut,  
$$
...
Similarly, for the [[EURIBOR Forward Rate Agreements and Futures|Euribor futures]] call option (put on rates),  
$$
...
As shown in Section A16.5, [[Futures Not Subject to Cash-And-Carry|futures]] prices are a martingale in the [[Verifying Martingale Property with Q|risk-neutral measure]], that is, when the numeraire is the money market account, $M(t)$ Hence, with $F_{t}$ the underlying bond [[Futures Price and the Quality Option Before E|futures price]] at time. $t$  
$$
F_{0}=E[F_{T}]
$$  

By the [[Continuous-Time Stochastic Processes|martingale property]], the price of a put option on the [[Futures Not Subject to Cash-And-Carry|futures]] is,  
$$
...
To continue, make the assumption - defended in the text -- that the [[Discount Factors|discount factor]] is uncorrelated with the [[Futures Price and the Quality Option Before E|futures price]]. Then, Equation (A16.45). becomes,  
$$
...
where (A16.47) follows from the [[Financial Mathematics Course|risk-neutral pricing]] of a zero coupon bond. Finally, applying Section A16.5 to (A16.43), (A16.47) with the assumption that the bond [[Futures Price and the Quality Option Before E|futures price]] has a lognormal distribution,.  
$$
...
Caplets that mature at time $T$ are written on a [[Forward Points in Currency|forward rate]] from time $T$ to $T+\tau$ , whose value, at time $t$ , is denoted by $f_{t}(T,T+\tau)$ . It is first shown that taking a $T+\tau$ -year zero coupon bond as the numeraire makes this [[Forward Points in Currency|forward rate]] a martingale. Let $d_{t}(T)$ be the time- $\cdot t$ price of a zero coupon bond maturing at time $T$ . By the definition of a [[Forward Points in Currency|forward rate]] of term $\tau$  
$$
...
Next, consider a [[An Asset Allocation Primer|portfolio]] that is long a. $T$ -year zero and short a. $T+\tau$ -year zero. Taking the. $T+\tau$ -year zero as the numeraire, the normalized. gains process of this [[An Asset Allocation Primer|portfolio]] is a martingale. Mathematically,  
$$
...
where the last line of (A16.51) just uses the definition of the [[Forward Points in Currency|forward rate]]. Combining (A16.50) and (A16.51) shows that the [[Forward Points in Currency|forward rate]] is a martingale under the chosen numeraire,  
$$
f_{t}(T,T+\tau)=E_{t}[f_{T}(T,T+\tau)]
$$  

Turning to the valuation of the caplet, its normalized gains process is. a martingale as well. Hence, taking [[FORWARD RATES AND TERM STRUCTURE|expectations]] of its normalized gain as of $T+\tau$  
$$
...
Finally, assuming that the [[Forward Points in Currency|forward rate]] $f_{T}(T,T+\tau)$ is normal with variance $\sigma^{2}T$ , and knowing from (A16.52) with $t=0$ that its mean is $f_{0}(T,T+\tau)$ , the results of Section A16.4 apply and,  
$$
...
The underlying of a $T$ -year into $\tau$ -year swaption is the forward [[Determining the Expression for the Fair Value of the Swap Spread|par swap]] rate from $T$ to $T+\tau$ , which, at time $t$ , is denoted by $C_{t}(T,T+\tau)$ . It is first shown that taking an annuity from $T$ to $T+\tau$ as the numeraire makes this forward [[Determining the Expression for the Fair Value of the Swap Spread|par swap]] rate a martingale. Denote the price of this annuity by $A_{t}(T,T+\tau)$  
...
Applying the [[Continuous-Time Stochastic Processes|martingale property]] with this annuity as numeraire,  
$$
...
Hence, as claimed, the forward [[Determining the Expression for the Fair Value of the Swap Spread|par swap]] rate is a martingale under this numeraire.  
...
The last line of (A16.58) follows from (A16.57), the assumption that the forward [[Determining the Expression for the Fair Value of the Swap Spread|par swap]] rate is normal with variance $\sigma^{2}T$ , and the appropriate result from Section A16.4.  
...
As the results in this section are part of the option [[Arbitrage Pricing of Derivatives|pricing]] literature, they are presented here for easy reference but without proof. Let $E^{N}[\cdot]$ and $E^{L N}[\cdot]$  

denote the [[FORWARD RATES AND TERM STRUCTURE|expectations]] operators under the normal and lognormal distributions, respectively, and let $N(\cdot)$ denote the standard normal cumulative distribution.  
...
The initial value of a [[Futures Not Subject to Cash-And-Carry|futures]] contract is zero; subsequent cash flows are from daily settlements; and at maturity, the [[Futures Price and the Quality Option Before E|futures price]] is determined by some final settlement rule. Consider a two-period, three-date framework for simplicity, and let the [[Futures Price and the Quality Option Before E|futures price]] on date. $t$ be $F_{t}$ . Then, the normalized. gains process is,  
$$
...
Since the value of a [[Futures Not Subject to Cash-And-Carry|futures]] contact on date O is zero, the [[Continuous-Time Stochastic Processes|martingale property]] implies that the expectation of the normalized gains at any future date is zero. In particular, for date 1,  
$$
...
As of date 2, the [[Continuous-Time Stochastic Processes|martingale property]] says that  
$$
0=E_{0}\left[\frac{(F_{1}-F_{0})(1+r_{2})+F_{2}-F_{1}}{(1+r_{1})(1+r_{2})}\right]
$$  

Using the [[General Risk-Adjusted Probability Measures|law of iterated expectations]], and the fact that $r_{1}$ is known as of date 0,  
$$
...
Together with (A16.68), (A16.72) shows that the [[Futures Price and the Quality Option Before E|futures price]] is a martin-.   
gale under the [[Appendix 5.C Forward And Futures Prices When Interest Rates Are Random|money-market account]] or [[Verifying Martingale Property with Q|risk-neutral measure]], as desired.  