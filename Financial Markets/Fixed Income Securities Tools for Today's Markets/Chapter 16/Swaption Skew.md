...
where the loadings vector $\Upsilon(\tau,\alpha)$ was defined in (A9.14), and $\Upsilon_{3}(\tau,\alpha)$ stands for its last element, i.e., the loading of a zero coupon yield with maturity $\tau-t$ on the long rate factor. Also, $\Omega$ was defined in (A9.8) and $\mathbb{W}_{t}^{*}$ stands for the Wiener process%20Process.md) under the true measure.  
...
$r_{0},r_{1}^{u}$ , and $r_{1}^{d}$ : the initial one-period rate, and the one-period rates on date 1, in the up and down states, respectively.   $\textbf{\em P}_{0},P_{2}^{u u},P_{2}^{u d}$ and $P_{2}^{d d}$ : the initial full price of the underlying bond, and the date-2 full prices in the "up-up," "up-down," and "down-down" states, respectively..   $\mathbf{\Phi}^{\overline{{\mathbf{\alpha}}}}\mu_{0},p_{2}^{u u},p_{2}^{u d}$ and $\dot{p}_{2}^{d d}$ : fat pries at the indicated dates and states..   
: $P_{0}(2)$ : the initial full forward price..   
: $p_{0}(2)$ : the initial flat forward price.. $F_{0},F_{1}^{u}, $ $F_{1}^{d}$ : the initial futures price, and the date-1 futures prices in the up and down states, respectively..   $\equiv q,1-q$ : the risk-neutral transition probabilities on all dates to move up and down from the current state, respectively.  
...
Fixed Income Securities%20-%20Definition,%20Formula,%20and%20Example.md): Tools for Today's Markets, Fourth Edition. Bruce Tuckman and Angel 505 Serrat $\copyright$ 2022 Bruce Tuckman and Angel Serrat. Published 2022 by John Wiley & Sons, Inc..  
...
$$
\begin{array}{r}{B_{0}(1+r_{1})(1+f)-c(1+f)-c=E_{0}[B_{2}]}\ {\leftB_{0}-\cfrac{c}{1+r_{1}}-\cfrac{c}{(1+r_{1})(1+f)}\right(1+f)=E_{0}[B_{2}]}\ {B_{0}(2)=E_{0}[B_{2}]}\end{array}
$$  
