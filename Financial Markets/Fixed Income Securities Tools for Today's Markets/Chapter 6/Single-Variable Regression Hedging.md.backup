...
This section considers the problem of a [market maker](../../../Financial%20Markets%20and%20Institutions/III.%20Liquidity%20of%20Assets/Class%205-%20Private%20Information,%20Liquidity,%20and%20Securitization/Class%20Note%209%20Bid%20and%20Ask%20Prices%20With%20Private%20Information.md) or a relative value trader on May 14, 2021, who purchases $\$100$ million face amount of the JNJ 2.450s of 09/01/2060 and hedges the resulting [interest rate risk](../../../Fixed%20Income%20Asset%20Pricing/Analysis%20of%20Fixed%20Income%20Securities.md) by selling the US Treasury 1.625s of 11/15/2050. Because there is no 40-year Treasury bond outstanding, the 1.625s of 11/15/2050, with about 30 years to maturity, are selected as the best alternative. Table 6.1 gives the coupons, maturity dates, yields, and Dv01s of these two bonds, along with those of two other bonds that are referenced in the next section.  
...
Figure 6.1 implicitly assumes that the most relevant period for designing an empirical hedge is the recent, immediate past. This assumption is often reasonable, but there are times and situations in which some earlier period seems more relevant. For example, if the Federal Reserve is expected to raise [short-term rates](../Chapter%208/Volatility%20and%20Convexity.md) in the immediate future, a similar episode in the past might be more relevant to the future than the recent past, in which the Federal Reserve left rates unchanged or lowered them. Choosing the length of the observation or estimation window is also part of the art of [regression hedging](Two-Variable%20Regression%20Hedging.md). Too short a window might fail to furnish statistically reliable estimates, but too long a window might include less relevant historical data.  
...
[Least-squares estimation](Level%20Versus%20Change%20Regressions.md) of the unknown parameters finds $\hat{\alpha}$ and $\hat{\beta}$ to minimize the sum of the squares of the residuals over the observation period,  
$$
...
<html><body><table><tr><td>No.ofObservations R-Squared</td><td>82 77.5% 2.00</td></tr><tr><td>[Regression Coefficients](../Chapter%209/A%20Practical%20Estimation%20Method.md)</td><td>Value</td><td>Std. Error</td></tr><tr><td>Constant (@)</td><td>0.060</td><td>0.223</td></tr><tr><td>Chg 30yr Treasury Yield (β)</td><td>0.842</td><td>0.051</td></tr></table></body></html>  
...
All the results in Table 6.2 are in-sample; that is, they are computed from the particular data sample used to estimate the regression model. Relying on these results for [hedging](../Chapter%205/Key%20Rates%20O1s%20Durations%20and%20Hedging.md) assumes that the future will be sufficiently like this particular historical period. The success or failure of this assumption is discussed at the end of the section.  

Turning now to [regression hedging](Two-Variable%20Regression%20Hedging.md), assume for the moment that the yield of the JNJ bonds changes by exactly $\hat{\beta}$ basis points for every one-basis-. point change in the yield of the Treasury bonds. Let $F^{J N J}$ $D V01^{J N J},F^{30}$ and $\dot{D}V01^{30}$ be the face amounts and DV01s of the $\mathrm{JNJ}$ and 30-year Treasury bonds, respectively. Then, the position is hedged against changes in yields if,  
$$
...
The yield-based Dv01 hedge for the JNJ bonds, which is given by Equation (6.9), is given earlier without the slope coefficient of 0.842 as $\$111.2$ million. The regression hedge of (6.9) sells only $\$93.6$ million because it recognizes that the yield of the JNJ bonds does not move as much as the yield of the 30-year Treasury bond. Hence, fewer Treasury bonds need be sold to hedge the [interest rate risk](../../../Fixed%20Income%20Asset%20Pricing/Analysis%20of%20Fixed%20Income%20Securities.md) of the JNJ bonds.  
...
In words, the left-hand side of the equation is the Dv01 of the hedge as. a fraction of the Dv01 of the bonds being hedged. The risk weight of a yield-based DV01 hedge is always $100\%$ - the DV01s of the two sides of the trades are, by construction, equal. In this [regression hedge](Reverse%20Regressions.md), however, the DV01 of the Treasury bonds is only $84.2\%$ of the DV01 of the JNJ bonds. In general, as Equation (6.10) shows, the risk weight of a [regression hedge](Reverse%20Regressions.md) exactly equals the estimated slope coefficient, $\hat{\beta}$  

The best argument for the [regression hedge](Reverse%20Regressions.md) is actually not the earlier. assumption that bond yields change exactly according to the regression model. Write the P&L of the position as,.  
$$
P\&L=-F^{J N J}\frac{D V01^{J N J}}{100}\Delta y_{t}^{J N J}-F^{30}\frac{D V01^{30}}{100}\Delta y_{t}^{30}
$$  

where the negative signs reflect that a positive face amount with a positive change (i.e., increase) in yield lowers $\mathrm{P}\&\mathrm{L}$ . It can then be shown that the [regression hedge](Reverse%20Regressions.md) in Equation (6.8) minimizes the variance (6.11). (See. Appendix A6.1.) In other words, to the extent that $\mathrm{P}\&\mathrm{L}$ variance is an appropriate measure of risk, the [regression hedge](Reverse%20Regressions.md) minimizes the risk of the hedged position.  
...
Applying Equation (6.12) to the case at hand, the DV01 of the JNJ bond position is $\$100$ million $\times0.2124/100$ , or $\$212,400$ , and the standard error. of the regression, reported in Table 6.2, is two basis points per day. Therefore, the standard deviation of the hedged. $\mathrm{P}\&\mathrm{L}$ in the sample is $\$212,400\times2$ or $\$424,800$ per day. Whether this is too much risk or not depends on how. much and how fast the trader is making money buying the JNJ bonds and [hedging](../Chapter%205/Key%20Rates%20O1s%20Durations%20and%20Hedging.md) them. If the trader is making five basis points on the position and holding it for a day, then a standard deviation of two basis points per day likely represents a reasonable risk-return trade-off. If, on the other hand, the trader is making 1.5 basis points and holding the position for a week, a standard deviation of two basis points per day likely ruins the trade from a risk-return perspective.  

This section concludes with an out-of-sample analysis of the estimated. regression model. Figure 6.2 shows the same regression line as estimated in Table 6.2 and graphed in Figure 6.1. The plus signs, however, are the changes in yields over the period May 17, 2021, to July 19, 2021. The regression model, estimated over the earlier period, January 29, 2021, to May 15, 2021, holds up quite well. In fact, the standard error of the residu-. als of the out-of-sample data against the original regression line is 1.5 basis. points, which is actually smaller than the in-sample equivalent. The trader [hedging](../Chapter%205/Key%20Rates%20O1s%20Durations%20and%20Hedging.md) as of May 14, 2021, cannot, of course, run this analysis. But other out-of-sample tests can be informative. A trader might see how a regression model performed over a period before the estimation period, perhaps a period right before that window or perhaps an even earlier period that might be more likely to resemble the future. In any case, poor out-of-sample performance should raise questions about the stability of the [regression coefficients](../Chapter%209/A%20Practical%20Estimation%20Method.md) over time and, therefore, the reliability of the resulting hedge.  
