...
This arbitrage-free relationship allows us to value a put based on the price knowledge of a call with exactly the same exercise terms. The put  call parity function holds regardless of the stochastic process%20Process.md) generating stock prices, but assumes that the underlying stock pays no dividends during the lives of the options.  
...
Cox et al. also propose the following to estimate    $u$   and  $d$   in the binomial approximation to the Wiener process%20Process.md), where    $\sigma$   is the standard deviation of stock returns:  
$$
...
Suppose, for example, that for a particular Wiener process%20Process.md),    $\sigma=0.30$   and    $r_{f}\!=\!0.05$  . Using Equations (9.20) and (9.22) , we estimate  $p,\,u,$   and  $d$   for a single-time-period binomial process as follows:  
$$
...
Thus, we can use the following to estimate    $u$   and    $d$   in the binomial approximation to the Wiener process%20Process.md):  
$$
u=\mathbf{e}^{\sigma}\quad d=\frac{1}{u}
$$  
...
Suppose, for example, that for a particular Wiener process%20Process.md),    $\sigma=0.30$   and    $r_{f}\!=\!0.05$  . Using Equations (9.A.1.7) and (9.A.1.10) , we estimate  $p,\,u,$   and    $d$   for a single time period binomial process as follows:  
$$
\begin{array}{l}{u=\mathrm{e}^{0.3}=1.3498588}\\ {\mathrm{~}}\\ {d={\frac{1}{u}}=0.7408182}\end{array}
$$  
...
which requires us to use Itoˆ’s lemma to solve for  $d c$  . Substituting this Wiener process%20Process.md) for  $d S$   into the preceding equation, we obtain the following:  
$$
