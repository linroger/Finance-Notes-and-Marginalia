...
In this section, we review a few popular short rate models, and their most com-. mon numerical implementation through the use of trinomial trees, the general one-factor. Heath-Jarrow-Morton%20Model.md) framework which subsumes most short rate models, and we finish with a two-factor approach of the LIBOR market model to addressing yield curve tilts..  
...
Hull and White (1990) improve the model by introducing mean reversion%20Process.md) into the drift structure $\mu(t)$ . The differential equation is:.  
$$
...
The Black and Karasinski (1991) model does exactly that. The short rate is log-normal, rather than normal, and it is subject to mean reversion%20Process.md). The functional form is:  
$$
d r=[\theta(t)-\alpha(t)l n r]d t+\sigma(t)d z
$$  

The model is most appealing from the perspective of allowable interest rate movements . mean reversion%20Process.md), no negative values, and a time-dependent volatility structure -- but completely loses analytic tractability. The only way to use it is through a numeric implementation. Most commonly, this is done with the use of a trinomial tree..  
...
The typical trinomial tree is recombinant. Subsequent nodes of a node are the same as some of the subsequent nodes of another node, and there are multiple ways to get to any interior node. The recombinant feature reduces the number of calculation as it reduces the overall number of nodes, and allows the algorithm to handle American exercise options. The drift and mean reversion%20Process.md) of interest rates are controlled jointly by assigning the interest rate levels for the nodes and the probabilities of jumping to those nodes. Parts (a) and (b) in Figures 6.3 show simple recombinant trinomial trees, both with instantaneous forward short-rate drifting up.  
...
Here is the general idea of the HJM framework%20Model.md) in a three-step numerical example. The simplifying assumptions and the notation are as follows..  
