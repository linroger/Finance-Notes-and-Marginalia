...
A classic but still widely used model of interest rate dynamics and the [[Arbitrage Pricing of Derivatives|pricing]] of bonds and interest. rate [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] is the model proposed by [[The Vasicek Model|Vasicek]] (1977). The basic assumptions of the model is that the continuously compounded [[The Gauss Model|short-term interest rate]]. $r_{t}$ has dynamics  
$$
d r_{t}=\kappa\left(\bar{r}-r_{t}\right)d t+\sigma_{r}d z_{t},
$$  

where $\kappa$ $\bar{r}$ , and $\sigma_{r}$ are positive constants, and that the [[Exercises|market price of risk]] associated with the shock $z$ is a constant $\lambda$  

Before we study the consequences of these assumptions, let us see how they can be supported. by a consumption-based equilibrium model. Following Goldstein and Zapatero (1996) assume that [[Pareto-Optimality in Some Incomplete Markets|aggregate consumption]] evolves as  
$$
d c_{t}=c_{t}\left[\mu_{c t}d t+\sigma_{c}d z_{t}\right],
$$  

where $z$ is a one-dimensional standard [[Continuous-Time Stochastic Processes|Brownian motion]], $o_{C}$ is a constant, and the expected [[Exercises|consumption growth rate]] $\mu_{c t}$ follows the process  
$$
d\mu_{c t}=\kappa\left(\bar{\mu}_{c}-\mu_{c t}\right)d t+\theta d z_{t}.
$$  

The [[The Simple Multi-Period Ccapm|representative individual]] is assumed to have a constant [[CCAPM with Alternative Preferences|relative risk aversion]] of $\gamma$ . It follows from (10.16) that the equilibrium [[Real and Nominal Interest Rates and Term Struc|real short-term interest rate]] is.  
$$
r_{t}=\delta+\gamma\mu_{c t}-\frac{1}{2}\gamma(1+\gamma)\sigma_{C}^{2}
$$  

with dynamics $d r_{t}=\gamma d\mu_{c t}$ , which gives (10.31) with $\sigma_{r}=\gamma\theta$ and $\bar{r}=\gamma\bar{\mu}_{c}+\delta-{\textstyle\frac{1}{2}}\gamma(1+\gamma)\sigma_{c}^{2}$ . The [[Exercises|market price of risk]] is $\lambda=\gamma\sigma_{c}$ , a constant.  

The process (10.31) is a so-called [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]]. An [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] exhibits [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]] in the sense that the drift is positive when. $r_{t}<r$ and negative when. $x_{t}>r$ . The process is therefore always pulled towards a long-term level of. $r$ .However, the. random shock to the process through the term. $\sigma_{r}d z_{t}$ may cause the process to move further away from $r$ . The parameter $\kappa$ controls the size of the expected adjustment towards the long-term level. and is often referred to as the [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]] parameter or the speed of adjustment..  

To determine the distribution of the future value of the [[The Gauss Model|short-term interest rate]] define a new process $y_{t}$ as some function of. $r_{t}$ such that $y=(y_{t})_{t\geq0}$ is a generalized [[Continuous-Time Stochastic Processes|Brownian motion]]. It turns out that this is satisfied for $y_{t}=g(r_{t},t)$ , where $g(r,t)=e^{\kappa t}r$ . From [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] we get  
$$
...
This holds for all. $t^{\prime}>t\geq0$ . In particular, we get that the solution to the [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] (10.31) can be written as  
$$
...
where $\varepsilon_{i}\sim N(0,1)$ . Figure 10.1 shows a single path for different combinations of. $r_{0}$ $\kappa$ $r$ , and $\sigma_{r}$ In each sub-figure one of the parameters is varied and the others fixed. The base values of the parameters are $r_{0}=0.08$ $\bar{r}=0.08$ $\kappa=\ln2\approx0.69$ , and $\sigma_{r}=0.03$ . All paths are computed using. the same sequence of random numbers $\varepsilon_{1},\ldots,\varepsilon_{n}$ and are therefore directly comparable. None of. the paths shown involve negative values of the process, but other paths will, see e.g. Figure 10.2.. As a matter of fact, it can be shown that an [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] with probability one will. sooner or later become negative..  

What are the implications of the [[The Vasicek Model|Vasicek]] assumptions for bond prices and the yield curve? The time $t$ price of a zero-coupon bond maturing at time. $s$ is given by  
$$
...
Figure 10.1: Simulated paths for an [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]]. The basic parameter values are $r_{0}=\bar{r}=0.08$ $\kappa=\ln2\approx0.69$ , and $\sigma_{r}=0.03$  
...
Interchange the order of integration in the double integral (this follows from the so-called Fubini Theorem of [[6. A Brief Introduction to Stochastic Calculus|stochastic calculus]])  
$$
...
which is affine in the current [[An Overview of the Vasicek Short Rate Model|short rate]] $r_{t}$ . A model with this property is called an affine term. structure model. It can be shown that $y_{t}^{s}\to y_{\infty}$ for $s\to\infty$ , which explains the notation. The. asymptotic long yield is a constant in the [[Vasicek Short Rate Model|Vasicek model]]. Concerning the shape of the yield curve $s\to y_{t}^{s}$ it can be shown that.  
...
Within the [[Vasicek Short Rate Model|Vasicek model]] it is possible to find closed-form expressions for the prices of many. other interesting assets, such as [[Forward and Futures Contracts|forwards and futures]] on bonds, [[Convexity-Adjusted Models for LIBOR Forwards Qu|Eurodollar futures]], and European options on bonds; see Chapter 12..  

There are many other [[SOFR Derivative Pricing Using a Short Rate Model|affine term structure models]] and they can basically all be supported by. a [[The Classical One-Period Capm|consumption-based asset pricing]] model in the same way as the [[Vasicek Short Rate Model|Vasicek model]]. Assume that the expected growth rate and the variance rate of [[Pareto-Optimality in Some Incomplete Markets|aggregate consumption]] are affine in some state. variables, i.e.  
$$
\mu_{c t}=a_{0}+\sum_{i=1}^{n}a_{i}x_{i t},\qquad\|\pmb{\sigma}_{c t}\|^{2}=b_{0}+\sum_{i=1}^{n}b_{i}x_{i t},
$$  

then the equilibrium [[An Overview of the Vasicek Short Rate Model|short rate]] will be  
$$
...
Of course, we should have. $\begin{array}{r}{b_{0}+\sum_{i=1}^{n}b_{i}x_{i t}\ge0}\end{array}$ for all values of the state variables. The market. price of risk is. $\lambda_{t}=\gamma\sigma_{c t}$ . If the state variables. $x_{i}$ follow processes of the affine type, we have an affine [[The Vasicek Model|term structure]] model..  

For other [[Financial Mathematics Course|term structure models]] developed with the consumption-based approach, see e.g. Bakshi and Chen (1997).  

# 10.5.2 The Cox-Ingersoll-Ross model  

Another widely used model of interest rate dynamics was suggested by Cox, Ingersoll, and Ross (1985b). They assume that the [[The Gauss Model|short-term interest rate]]. $r_{t}$ follows a so-called square root process  
$$
d r_{t}=\kappa\left(\bar{r}-r_{t}\right)d t+\sigma_{r}\sqrt{r_{t}}d\bar{z}_{t},
$$  

where $\kappa$ $r$ , and $\sigma_{r}$ are positive constants. Further they assume that the associated [[Exercises|market price of risk]] is. $\lambda_{t}=\lambda\sqrt{r_{t}}/\sigma_{r}$ , where the. $\lambda$ on the right-hand side is a constant.  

They derive their model as a special case of their [[Real Interest Rates and Aggregate Production|general equilibrium model]] with production which we have reviewed in Section 10.4. The [[The Simple Multi-Period Ccapm|representative individual]] is assumed to have a [[Exercises|logarithmic utility]] so that the [[CCAPM with Alternative Preferences|relative risk aversion]] of the direct utility function is $^{1}$ . In addition, the individual is assumed to have an infinite time horizon, which implies that the [[Dynamic Programming|indirect utility function]] will be independent of time. It can be shown that under these assumptions the [[Dynamic Programming|indirect utility function]] of the individual is of the form. $J(W,x)=A\ln W+B(x)$ . In particular, $J_{W x}=0$ and the [[CCAPM with Alternative Preferences|relative risk aversion]] of the [[Dynamic Programming|indirect utility function]] is also $^{1}$ . It follows from (10.28) that the equilibrium [[Real and Nominal Interest Rates and Term Struc|real short-term interest rate]] is equal to  
$$
r(x_{t})=g(x_{t})-\xi(x_{t})^{2}.
$$  

The authors further assume that the expected rate of return and the variance rate of the return on the productive [[An Asset Allocation Primer|investment]] are both proportional to the state, i.e.  
$$
...
where $\bar{z}$ is a standard [[Continuous-Time Stochastic Processes|Brownian motion]] with correlation $\rho$ with the standard [[Continuous-Time Stochastic Processes|Brownian motion]] $z_{1}$ and correlation $\sqrt{1-\rho^{2}}$ with $z_{2}$ . Then the dynamics of the real [[An Overview of the Vasicek Short Rate Model|short rate]] is $d r_{t}=k d x_{t}$ , which yields  
$$
d r_{t}=\kappa\left(\bar{r}-r_{t}\right)d t+\sigma_{r}\sqrt{r_{t}}d\bar{z}_{t},
$$  
...
The [[Exercises|market price of risk]] associated with the combined shock. $z$ is $\rho\lambda_{1}+\sqrt{1-\rho^{2}}\lambda_{2}$ , which is proportional to $\sqrt{r}$ . These conclusions support (10.40) and the associated form of the [[Exercises|market price of risk]].  

Before we discuss the implications for bond prices and the yield curve, let us look at the properties of the square root process. The only difference to the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] is the square root term in the volatility. The variance rate is now $\sigma_{r}^{2}r_{t}$ which is proportional to the level of the process. A square root process also exhibits [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]]. A square root process can only take on non-negative values. To see this, note that if the value should become zero, then the  

![](65ce9a40fa08e2f820571e190c9b1606d4f949a2add631b935d92ad2ab9f4548.jpg)  
Figure 10.2: A comparison of simulated paths for an [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] and a square root process. For both processes, the parameters $\bar{r}=0.08$ and $\kappa=\ln2\approx0.69$ are used, while. $\sigma_{r}$ is set to 0.03 for the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] and to $0.03/\sqrt{0.08}\approx0.1061$ for the square root process..  
...
Variations in the different parameters will have similar effects as for the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]], which is illustrated in Figure 10.1. Instead, let us compare the paths for a square root process and an [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] using the same drift parameters. $\kappa$ and $\bar{r}$ , but where the. $\sigma_{r}$ parameter for the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] is set equal to the. $\sigma_{r}$ -parameter for the square root process multiplied by the square root of. $\bar{r}$ , which ensures that the processes will have the same variance rate at the long-term level. Figure 10.2 compares two pairs of paths of the processes. In part (a), the initial value is set equal to the long-term level, and the two paths continue to be very close to each other. In part (b), the initial value is lower than the long-term level, so that the variance rates of the two processes differ from the beginning. For the given sequence of random numbers, the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] becomes negative, while the square root process of course stays positive. In this case there is a clear difference between the paths of the two processes.  
...
[[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]], that is we look at $y_{t}=e^{\kappa t}r_{t}$ . By [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]],  
$$
...
Note that the mean is identical to the mean for an [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]], whereas the variance is more complicated for the square root process.  
...
and $\hat{\kappa}=\kappa+\lambda$ and $\nu=\sqrt{\hat{\kappa}^{2}+2\sigma_{r}^{2}}$ . As in the [[Vasicek Short Rate Model|Vasicek model]], the yields are affine in the current [[An Overview of the Vasicek Short Rate Model|short rate]],  
$$
...
Longstaff and Schwartz (1992) study a two-factor version of the model. They assume that the production [[Assets|returns]] are given by  
$$
...
so that the state variable. $x_{2}$ affects both expected [[Assets|returns]] and uncertainty of production, while. the state variable $x_{1}$ only affects the [[Lecture 1- Probability Distributions of Returns|expected return]]. With log utility the [[An Overview of the Vasicek Short Rate Model|short rate]] is again equal to the [[Lecture 1- Probability Distributions of Returns|expected return]] minus the variance,.  
$$
