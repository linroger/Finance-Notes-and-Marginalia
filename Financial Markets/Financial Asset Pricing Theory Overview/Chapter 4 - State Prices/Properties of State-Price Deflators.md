...
After defining state-price deflators, two questions arise naturally: When does a [[Exercises|state-price deflator]] exist? When is it unique? We will answer these questions in the two following subsections..  

# 4.3.1 Existence  

Here is the answer to the existence question:  

Theorem 4.1 A [[Exercises|state-price deflator]] exists if and only if prices admit no [[Arbitrage Pricing of Derivatives|arbitrage]].  

Since we have already concluded that we should only consider no-[[Arbitrage Pricing of Derivatives|arbitrage]] prices, we can safely assume the existence of a [[Exercises|state-price deflator]]..  

Let us take the easy part of the proof first: if a [[Exercises|state-price deflator]] exists, prices do not admit [[Arbitrage Pricing of Derivatives|arbitrage]]. Let us just think of the [[Properties of State-Price Deflators|one-period framework]] so that the [[Exercises|state-price deflator]] is a strictly positive random variable and the price of a random dividend of $D_{i}$ is $P_{i}=\operatorname{E}[\zeta D_{i}]$ . If $D_{i}$ is nonnegative in all states, it is clear that $\zeta D_{i}$ will be non-negative in all states and, consequently, the expectation of $\zeta D_{i}$ will be non-negative. This rules out [[Arbitrage Pricing of Derivatives|arbitrage]] of type (i), cf. the definition of [[Arbitrage Pricing of Derivatives|arbitrage]] in Section 3.4. If, furthermore, the set of states $A=\{\omega\in\Omega:D_{i}(\omega)>0\}$ has strictly positive probability, it is clear that $\zeta D_{i}$ will be strictly positive on a set of strictly positive probability and otherwise non-negative, so the expectation of $\zeta D_{i}$ must be strictly positive. This rules out type (ii) [[Arbitrage Pricing of Derivatives|arbitrage]]. The same argument applies to the [[Multi-Period Valuation Models|discrete-time framework]]. In the [[The Continuous-Time Framework|continuous-time setting]] the argument should be slightly adjusted in order to incorporate the lower bound on the value process that will rule out doubling strategies. It is not terribly difficult, but involves local martingales and super-martingales which we will not discuss here. The interested reader is referred to Duffie (2001, p. 105).  

How can we show the other and more important implication that no [[Arbitrage Pricing of Derivatives|arbitrage]] guarantees the. existence of a state-price vector or [[Exercises|state-price deflator]]? In Chapter 6 we will do that by constructing. a [[Exercises|state-price deflator]] from the solution to the [[Properties of State-Price Deflators|utility maximization problem]] of any individual. The solution will only exist in [[Exercises|absence of arbitrage]]. It is possible to prove the existence of a [[Exercises|state-price deflator]] without formally introducing individuals and solving their [[Prerequisites|utility maximization]] problems.. The alternative proof is based on the so-called [[Properties of State-Price Deflators|Separating Hyperplane Theorem]] and involves no. economics.  

In the [[Properties of State-Price Deflators|one-period framework]] with a finite state space $\Omega=\{1,2,\dots,S\}$ , the alternative argument goes as follows. A given [[An Asset Allocation Primer|portfolio]]. $\pmb{\theta}$ has an initial dividend of $-{\cal P}^{\theta}$ and a terminal dividend given by the $S$ dimensional vector $D^{\theta}$ . Let $M$ denote the set of all. $(S+1)$ dimensional pairs $(-P^{\theta},D^{\theta})$ that is generated by all portfolios, i.e. all $\pmb\theta\in\mathbb{R}^{I}$ . Observe that $M$ is a closed and convex subset of. $L\equiv\mathbb{R}\times\mathbb{R}^{S}$ . Let $K$ be the positive orthant of $L$ , i.e. $K\equiv\mathbb{R}_{+}\times\mathbb{R}_{+}^{S}$ , where $\mathbb{R}_{+}=[0,\infty)$ $K$ is also a closed and convex subset of. $L$ . Note that there is no [[Arbitrage Pricing of Derivatives|arbitrage]] if and only if the only common element of $K$ and $M$ is the zero element. $(0,\mathbf{0})$  

Assume no [[Arbitrage Pricing of Derivatives|arbitrage]], i.e. $K\cap M=\{(0,\mathbf{0})\}$ . By the [[Properties of State-Price Deflators|Separating Hyperplane Theorem]] (see, e.g., Rockafellar 1970) there exists a non-zero linear functional $F:L\rightarrow\mathbb{R}$ with the property that $F(z)=0$ for all $z\in M$ and $F(x)>0$ for all non-zero $x\in K$ .Hence, we can find a strictly positive $\varphi_{0}$ in $\mathbb{R}$ and an $S$ -dimensional vector $\varphi$ with strictly positive elements such that $F(d_{0},d)=\varphi_{0}d_{0}+\varphi\cdot d$ for all $(d_{0},d)$ in $L$ . Since $(-P^{\theta},D^{\theta})\in M$ for any [[An Asset Allocation Primer|portfolio]] $\pmb{\theta}$ , we have that  
$$
...
Just assuming that prices obey the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|law of one price]] would give us a vector. $\psi$ satisfying ${\pmb{\psi}}{\cdot}{\pmb{D}}_{i}={\pmb{P}}_{i}$ for all $i$ . Imposing the stronger no-[[Arbitrage Pricing of Derivatives|arbitrage]] condition ensures us that we can find a strictly positive vector $\psi$ with that property, i.e. a state-price vector. Dividing by state probabilities, $\zeta_{\omega}=\psi_{\omega}/p_{\omega}$ we obtain a [[Exercises|state-price deflator]].  
...
which is not strictly positive. Hence there is no state-price vector for this market. Then there must be an [[Arbitrage Pricing of Derivatives|arbitrage]], but where? We can see that the three assets are priced such that the implicit value of an Arrow-Debreu asset for state 1 is negative. The [[An Asset Allocation Primer|portfolio]] of the three assets that replicates this Arrow-Debreu asset is given by  
$$
...
pays you one if state 1 is realized and nothing in other states. This is clearly an [[Arbitrage Pricing of Derivatives|arbitrage]].  

# 4.3.2 Uniqueness  

Here is a general result on the uniqueness of state-price deflators:  

Theorem 4.2 Assume prices admit no [[Arbitrage Pricing of Derivatives|arbitrage]]. Then there is a unique [[Exercises|state-price deflator]] if and only if the market is complete. If the market is incomplete, several state-price deflators exist.  

In a [[Properties of State-Price Deflators|one-period framework]] the theorem can be explained as follows. In the [[Exercises|absence of arbitrage]], some [[Exercises|state-price deflator]] $\zeta$ exists. If $\varepsilon$ is a random variable with $\mathrm{E}[\varepsilon D_{i}]=0$ for all $i$ , then  
$$
...
If $\varepsilon$ is strictly positive with finite variance, then. $\zeta+\varepsilon$ will be a valid [[Exercises|state-price deflator]]. When can we find such an. $\varepsilon$ ? If the market is complete, the dividends of the basic assets span all random variables so it will be impossible to find a random variable. $\varepsilon$ not identically equal to zero with. $\mathrm{E}[\varepsilon D_{i}]=0$ . Therefore $\zeta$ must be the only [[Exercises|state-price deflator]].  

Let us be a bit more precise. Consider, for simplicity, a [[Properties of State-Price Deflators|one-period framework]] with a finite state space and look for a state-price vector. $\psi$ , i.e. a strictly positive solution to the equation system. $\underline{{\underline{{D}}}}\boldsymbol{\psi}=P$ . If the market is complete and there are no [[Exercises|redundant assets]], the dividend matrix $\underline{{\underline{{\boldsymbol{D}}}}}$ is an $S\times S$ [[Prerequisites|non-singular matrix]] so the only solution to the equation system is.  
$$
\psi^{*}=\underline{{\underline{{D}}}}^{-1}P.
$$  

If there are [[Exercises|redundant assets]], they will be uniquely priced by no-[[Arbitrage Pricing of Derivatives|arbitrage]] so it will be sufficient to look for solutions to. $\underline{{\hat{D}}}\psi=\hat{P}$ , where $\underline{{\underline{{\hat{D}}}}}$ is the dividend matrix and. $\hat{P}$ the price vector of the. [[Marketed Dividends and Market Completeness|non-redundant assets]]. In the case of a complete market the matrix. $\underline{{\underline{{\hat{D}}}}}$ is non-singular. The unique. solution to the equation system is then  
$$
...
If $\psi^{*}$ is strictly positive, we can therefore conclude that it will be a state-price vector. Note that. $\psi^{*}$ is in fact exactly equal to the dividend generated by the [[An Asset Allocation Primer|portfolio]]. $\hat{\pmb{\theta}}^{*}=\left(\underline{{\hat{D}}}\underline{{\hat{D}}}^{\top}\right)^{-1}\hat{\pmb{P}}$ , i.e. if $\psi^{*}$ is strictly positive, it is a state-price vector in the set of [[Marketed Dividends and Market Completeness|marketed dividend]] vectors.  

In the special case of a complete and [[Exercises|arbitrage-free market]] the elements of $\psi^{*}$ will be strictly positive. Why? Since the market is complete, we can construct an Arrow-Debrey asset for any state. The dividend vector of the Arrow-Debreu asset for state $\omega$ is $\begin{array}{r}{\pmb{e}_{\omega}=(0,\ldots,0,1,0,\ldots,0)^{\top}}\end{array}$ where the 1 is the $\omega$ 'th element of the vector. The price of this [[An Asset Allocation Primer|portfolio]] is. $\pmb{\psi}^{*}\cdot\pmb{e}_{\omega}=\psi_{\omega}^{*}$ . To avoid [[Arbitrage Pricing of Derivatives|arbitrage]], $\psi_{\omega}^{*}$ must be strictly positive. This argument works for all. $\omega=1,\ldots,S$ . Hence, $\psi^{*}$ is a state-price vector if the market is complete..  
...
If the market is complete and [[Arbitrage Pricing of Derivatives|arbitrage]]-free we have identified the unique state-price vector $\psi^{*}$ and the unique [[Exercises|state-price deflator]] $\zeta^{*}$ .But it is important to realize the following: $\psi^{*}$ and $\zeta^{*}$ are defined in terms of the prices of the basic assets. Observing the prices and state-contingent dividends of the basic assets, we can extract the [[Exercises|state-price deflator]]. If you want to compute prices of the basic assets from their state-contingent dividends, $\psi^{*}$ and $\zeta^{*}$ are not useful. We need to add more structure to link the state-price vector and deflator to other variables, e.g. the consumption and [[An Asset Allocation Primer|portfolio]] decisions of the individuals in the economy. This is what concrete [[Financial Mathematics Course|asset pricing models]] have to do. Further discussions of $\zeta^{*}$ and what we can learn about state prices from observed prices or [[Assets|returns]] follow later in this chapter.  
...
which is consistent with the results of Example 4.2. The [[An Asset Allocation Primer|portfolio]] generating this dividend vector is $\pmb{\theta}^{*}=(0.14,0.06,0.04)^{\top}$  

Since the market is complete, the unique [[Exercises|state-price deflator]] $\zeta^{*}$ is the one associated with $\psi^{*}$ From Example 4.2, we have $\zeta_{1}^{*}=0.6$ $\zeta_{2}^{*}=0.8$ $\zeta_{3}^{*}=1.2$ . The [[An Asset Allocation Primer|portfolio]] generating this dividend is $\pmb{\theta}^{*}=(0.44,0.36,0.04)^{\top}$  
...
Since $\zeta^{*}>0$ , it is a valid [[Exercises|state-price deflator]]. (Note that this is not the [[Exercises|state-price deflator]] associated with the state-price vector $\psi^{*}$ computed above.) We have infinitely many state-price. deflators for this market. Given any state-price vector. $\psi=(a,0.8-2a,a)^{\top}$ for $0<a<0.4$ , the associated [[Exercises|state-price deflator]] is given by $\zeta_{\omega}=\psi_{\omega}/p_{\omega}$ , i.e.  
$$
\zeta=\left(\stackrel{a/0.5}{(0.8-2a)/0.25}\right)=\left(\stackrel{2a}{3.2-8a}\right).
$$  

Letting $b=2a$ , any [[Exercises|state-price deflator]] is of the form $\zeta=(b,3.2-4b,2b)$ for $0<b<0.8$  

In the multi-period [[Multi-Period Valuation Models|discrete-time framework]] all the above observations and conclusions hold in each period.  

Now consider the [[Exercises|continuous-time framework]] and assume that an instantaneously [[2. Forwards, Swaps, Futures, and Options|risk-free asset]]. is traded. We have seen above that state-price deflators are closely related to market prices of risk. Whenever we have a [[Exercises|market price of risk]]. $\lambda=\left(\lambda_{t}\right)$ , i.e. a nice process satisfying.  
$$
\mu_{t}+\delta_{t}-r_{t}^{f}{\bf1}=\underline{{\underline{{\sigma}}}}{}_{t}\lambda_{t},
$$  

then a [[Exercises|state-price deflator]] can be defined by $\zeta_{0}=1$ and  
$$
...
where $\underline{{\underline{{\hat{\sigma}}}}}_{t}$ is the $k\times d$ matrix obtained from $\underline{{\underline{{\sigma}}}}t$ by removing rows corresponding to [[Exercises|redundant assets]],. i.e. rows that can be written as a linear combination of other rows in the matrix. Similarly, $\hat{\pmb{\mu}}_{t}$ and $\hat{\delta}_{t}$ are the $k$ -dimensional vectors that are left after deleting from and $\delta_{t}$ , respectively, the ${\pmb\mu}_{t}$ elements corresponding to the [[Exercises|redundant assets]]. In the special case where $k=d$ , we have  
$$
\lambda_{t}^{*}=\underline{{\hat{\sigma}}}_{t}^{-1}\left(\hat{\pmb{\mu}}_{t}+\hat{\pmb{\delta}}_{t}-r_{t}^{f}{\bf1}\right).
$$  

Let $\zeta^{\ast}$ be the [[Exercises|state-price deflator]] associated with $\lambda^{*}$ , i.e.  
$$
\zeta_{t}^{*}=\exp\left\{-\int_{0}^{t}r_{s}^{f}d s-\frac12\int_{0}^{t}\|\lambda_{s}^{*}\|^{2}d s-\int_{0}^{t}(\lambda_{s}^{*})^{\top}d z_{s}\right\}.
$$  

In the [[Properties of State-Price Deflators|one-period framework]] the (candidate) [[Exercises|state-price deflator]] $\zeta^{*}$ was shown to be the dividend of some [[An Asset Allocation Primer|portfolio]] of traded assets. What about the [[Exercises|continuous-time framework]]? Consider the selffinancing trading strategy given by the fractions of wealth $\begin{array}{r}{\pmb{\pi}_{t}^{*}=\left(\underline{{\hat{\sigma}}}_{t}\underline{{\hat{\sigma}}}_{t}^{\top}\right)^{-1}\left(\pmb{\hat{\mu}}_{t}+\hat{\delta}_{t}-r_{t}^{f}\pmb{1}\right)}\end{array}$ in the [[Marketed Dividends and Market Completeness|non-redundant assets]] and the fraction $\mathbf{l}-(\pmb{\pi}_{t}^{*})^{\top}\mathbf{1}$ in the instantaneously [[2. Forwards, Swaps, Futures, and Options|risk-free asset]]. The dynamics of the value $V_{t}^{*}$ of this trading strategy is given by  
$$
...
cf. (3.14). It can be shown that $\pi_{t}^{*}$ is the trading strategy with the highest expected continuously compounded growth rate, i.e. the trading strategy maximizing $\mathrm{E}[\ln{(V_{T}^{\pi}/V_{0}^{\pi})}]$ , and it is therefore referred to as the [[Changing the Numeraire Without Changing the Me|growth-optimal trading strategy]]. Consequently, $\lambda_{t}^{*}$ defined in (4.49) is the relative sensitivity vector of the value of the [[Changing the Numeraire Without Changing the Me|growth-optimal trading strategy]]. One can show that $\zeta_{t}^{*}=$ $V_{0}^{*}/V_{t}^{*}$ (see Exercise 4.11), so we have a [[Exercises|state-price deflator]] defined in terms of the value of a trading strategy.  

If $\varepsilon=\left(\varepsilon_{t}\right)$ is a nice. $d$ -dimensional [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] with $\underline{{\underline{{\sigma}}}}_{t}\varepsilon_{t}=0$ for all $t$ , then $\lambda_{t}=\lambda_{t}^{*}+\varepsilon_{t}$ defines a [[Exercises|market price of risk]] since  
$$
...
If the market is incomplete, it will be possible to find such an $\varepsilon_{t}$ and, hence, there will be more than one [[Exercises|state-price deflator]].  

An example of an incomplete market is a market where the traded assets are only immediately affected by $j<d$ of the $d$ [[Some Discrete-Time Stochastic Processes|exogenous shocks]]. Decomposing the $d$ -dimensional standard [[Continuous-Time Stochastic Processes|Brownian motion]] $_{z}$ into $(Z,{\hat{Z}})$ , where $Z$ is $j$ -dimensional and $\hat{Z}$ is $(d-j)$ -dimensional, the dynamics of the traded risky assets can be written as  
$$
...
non-hedgeable risk in [[Interest Rate Quotations|interest rates]], expected [[Assets|returns]], and volatilities and correlations, respectively. Or other variables important for the investor, e.g. his labor income, may be sensitive to $\hat{Z}$ . Let us assume for simplicity that there are $j$ risky assets and the. $j\times j$ matrix $\underline{{\underline{{\sigma}}}}t$ is non-singular (i.e. there are no [[Exercises|redundant assets]]). Then we can define a unique [[Exercises|market price of risk]] associated with the traded risks by the. $j$ -dimensional vector  
$$
...
but for any well-behaved. $(d-j)$ -dimensional process $\hat{\bf A}$ , the process. $\pmb{\lambda}=(\pmb{\Lambda},\hat{\pmb{\Lambda}})$ will be a market. price of risk for all risks. Each choice of $\hat{\bf A}$ generates a valid [[Exercises|market price of risk]] process and hence a valid [[Exercises|state-price deflator]]..  

# 4.3.3 Convex combinations of state-price deflators  

A convex combination of some objects (such as vectors, random variables, [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]], etc.) $x_{1},x_{2},\ldots,x_{L}$ is given by  
$$
x=\sum_{l=1}^{L}\alpha_{l}x_{l},
$$  

where $\alpha_{1},\ldots,\alpha_{L}$ are positive constants summing up to one. The following theorem says that if you have a number of state-price deflators, any convex combination of those deflators will also be a [[Exercises|state-price deflator]]. This is true both in the one-period, the discrete-time, and the [[Exercises|continuous-time framework]]. In particular, it tells you that once you have two different state-price deflators you can generate infinitely many state-price deflators. The proof of the theorem is left as Exercise 4.3.  
...
is also a [[Exercises|state-price deflator]].  

# 4.3.4 The candidate deflator. $\zeta^{*}$ and the Hansen-Jagannathan bound  

The candidate deflator $\zeta^{*}$ from the [[Properties of State-Price Deflators|one-period framework]] is interesting for empirical studies so it is worthwhile to study it more closely. Let us compute the return associated with the dividend $\zeta^{*}$ This return turns out to be important in later sections. For notational simplicity suppose that no assets are redundant so that  
$$
...
Let us first rewrite $\zeta^{\ast}$ and $\theta^{*}$ in terms of the gross [[Assets|returns]] of the assets instead of prices and dividends. Using (3.2), we get  
$$
...
using (3.2) and various rules from matrix algebra. The [[Portfolios and Trading Strategies|portfolio weight vector]] is obtained by substituting $\theta^{*}$ into (3.6). Since  
$$
...
The gross return on this [[An Asset Allocation Primer|portfolio]] is  
$$
