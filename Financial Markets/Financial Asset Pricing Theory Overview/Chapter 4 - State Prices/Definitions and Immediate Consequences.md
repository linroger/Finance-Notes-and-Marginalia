...
This section gives a formal definition of a [[Exercises|state-price deflator]]. Some authors use the name [[Definitions and Immediate Consequences|stochastic discount factor]], [[Definitions and Immediate Consequences|event-price deflator]], or [[Definitions and Immediate Consequences|pricing kernel]] instead of [[Exercises|state-price deflator]]..  

# 4.2.1 The one-period framework  

A [[Exercises|state-price deflator]] is a random variable $\zeta$ with the properties that.  
...
The finite variance assumptions on both the [[Exercises|state-price deflator]] and the dividends ensure that the expectation $\operatorname{E}[\zeta D_{i}]$ is finite.  

We get a similar [[Definitions and Immediate Consequences|pricing equation]] for portfolios:  
$$
...
In terms of [[Definitions and Immediate Consequences|gross rates of returns]], $R_{i}=D_{i}/P_{i}$ , a [[Exercises|state-price deflator]] $\zeta$ has the property that  
$$
...
For a [[Definitions and Immediate Consequences|risk-free portfolio]] with a dividend of 1, the price $P^{f}$ is  
$$
...
Exploiting the definition of a covariance, the [[Arbitrage Pricing of Derivatives|pricing]] condition (4.1) can be rewritten as  
$$
P_{i}=\operatorname{E}[\zeta]\operatorname{E}[D_{i}]+\operatorname{Cov}[D_{i},\zeta].
$$  

A dividend of a given size is valued more highly in a state for which the [[Exercises|state-price deflator]] is high than in a state where the deflator is low. If a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] is traded, we can rewrite this as.  
$$
P_{i}=\frac{\mathrm{E}[D_{i}]+R^{f}\mathrm{Cov}[D_{i},\zeta]}{R^{f}},
$$  

i.e. the value of a future dividend is given by the expected dividend adjusted by a covariance term,. discounted at the [[Black Scholes Derivation|risk-free rate]]. If the dividend is positively [negatively] covarying with the state-. price deflator, the expected dividend is adjusted downwards [upwards]. Defining the dividend-beta of asset $i$ with respect to the [[Exercises|state-price deflator]] as.  
$$
\beta[D_{i},\zeta]=\frac{\mathrm{Cov}[D_{i},\zeta]}{\mathrm{Var}[\zeta]}
$$  

and $\eta=-\mathrm{Var}[\zeta]/\mathrm{E}[\zeta]<0$ , we can rewrite the above [[Definitions and Immediate Consequences|pricing equation]] as  
$$
P_{i}=\frac{\mathrm{E}[D_{i}]-\beta[D_{i},\zeta]\eta}{R^{f}}.
$$  

Some basic finance textbooks suggest that a future uncertain dividend can be valued by taking the expected dividend and discount it by a [[PSET 7- Kohler|discount rate]] reflecting the risk of the dividend. For  

asset $i$ , this [[PSET 7- Kohler|discount rate]] $\hat{R}_{i}$ is implicitly defined by $P_{i}=\mathrm{E}[D_{i}]/\hat{R}_{i}$ and combining this with the above equations, we must have  
$$
...
The risk-adjusted [[PSET 7- Kohler|discount rate]] does not depend on the scale of the dividend in the sense that the risk-adjusted [[PSET 7- Kohler|discount rate]] for a dividend of $k D_{i}$ for any constant $k$ is the same as for a dividend of. $D_{i}$ . Note that the risk-adjusted [[PSET 7- Kohler|discount rate]] will be smaller than $R^{f}$ if the dividend. is negatively covarying with the stare-price deflator. In fact, if $\mathrm{E}[D_{i}]<\beta[D_{i},\zeta]\eta$ , the risk-adjusted. gross [[PSET 7- Kohler|discount rate]] will be negative! While this possibility is rarely realized in textbooks, it is not really surprising. Some assets or investments will have a negative expected future dividend but still a positive value today. This is the case for most [[Risk Sharing|insurance contracts]]. The lesson here is to be careful if you want to value assets by discounting [[Exercises|expected dividends]] by risk-adjusted discount rates.  
...
If a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] is available, the above equation specializes to  
$$
...
where the return-beta $\beta[R_{i},\zeta]$ is defined as $\mathrm{Cov}[R_{i},\zeta]/\mathrm{Var}[\zeta]$ corresponding to the definition of the market-beta of an asset in the traditional CAPM. An asset with a positive [negative] returnbeta with respect to the [[Exercises|state-price deflator]] will have an [[Lecture 1- Probability Distributions of Returns|expected return]] smaller [larger] than the [[A Preview of Alternative Formulations|risk-free return]]. Of course, we can also write the covariance as the product of the correlation and the standard deviations so that  
$$
...
In particular, we can see that the [[Exercises|maximal Sharpe ratio]] is the ratio $\sigma[\zeta]/\operatorname{E}[\zeta]$  

The expressions involving expected [[Assets|returns]] and return-betas above are not directly useful if. you want to value a future dividend. For that purpose the equations with [[Exercises|expected dividends]]. and dividend-betas are superior. On the other hand the return-expressions are better for empirical studies, where you have a historical record of observations of [[Assets|returns]] and, for example, of a potential. [[Exercises|state-price deflator]].  

Example 4.1 Let $\zeta$ be a [[Exercises|state-price deflator]] and consider a dividend given by  
$$
D_{i}=a+b\zeta+\varepsilon,
$$  

where $a,b$ are constants, and where $\varepsilon$ is a random variable with mean zero and $\mathrm{Cov}[\varepsilon,\zeta]=0$ .What is the price of this dividend? We can use the original [[Arbitrage Pricing of Derivatives|pricing]] condition to get  
$$
...
In many specific models the [[Exercises|state-price deflator]] is assumed to have a lognormal distribution. It follows from Appendix B that.  
$$
...
In particular, if a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] exists, the continuously compounded [[Financial Instruments|risk-free rate of return]] is  
$$
r^{f}=\ln R^{f}=-\ln\operatorname{E}[\zeta]=-\operatorname{E}[\ln\zeta]-{\frac{1}{2}}\operatorname{Var}[\ln\zeta].
$$  

For a risky asset with a gross return $R_{i}$ so that the [[Exercises|state-price deflator]] $\zeta$ and the gross return $R_{i}$ are jointly lognormally distributed, the product $\zeta R_{i}$ will also be lognormally distributed. Hence, we find  
$$
...
If we let $r_{i}=\ln R_{i}$ denote the [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return on asset $i$ and apply (4.15), we obtain  
$$
...
Some empirical tests of [[Financial Mathematics Course|asset pricing models]] focus on excess [[Assets|returns]], where [[Assets|returns]] on all assets are measured relative to the return on a fixed benchmark asset or [[An Asset Allocation Primer|portfolio]]. Let. $R$ be the return  
...
If the state space is finite, $\Omega=\{1,2,\dots,S\}$ , we can alternatively represent a [[Introduction|general pricing rule]] by a state-price vector, which is an $S$ -dimensional vector $\psi=(\psi_{1},\ldots,\psi_{S})^{\intercal}$ with the properties  
...
Suppose we can construct an Arrow-Debreu asset for state $\omega$ , i.e. a [[An Asset Allocation Primer|portfolio]] paying $^{1}$ in state $\omega$ and nothing in all other states. The price of this [[An Asset Allocation Primer|portfolio]] will be equal to $\psi_{\omega}$ , the "state price for state $\omega$ The price of a [[Exercises|risk-free dividend]] of $^{1}$ .9 $\begin{array}{r}{P^{f}\equiv\psi\cdot{\bf1}=\sum_{\omega=1}^{S}\psi_{\omega}}\end{array}$ So that the gross [[Financial Instruments|risk-free rate of return]] is  
$$
R^{f}={\frac{1}{\sum_{\omega=1}^{S}\psi_{\omega}}}={\frac{1}{\psi\cdot{\bf1}}}.
$$  

There is a one-to-one correspondence between state-price vectors and state-price deflators. With a finite state space a [[Exercises|state-price deflator]] is equivalent to a vector. $\boldsymbol{\zeta}=(\zeta_{1},\zeta_{2},\ldots,\zeta_{S})^{\intercal}$ and we can rewrite (4.1) as  
$$
...
Conversely, given a state-price vector this equation defines a [[Exercises|state-price deflator]]. With infinitely many states we cannot meaningfully define state-price vectors but we can still define state-price deflators in terms of random variables.  
...
In particular, the gross [[Financial Instruments|risk-free rate of return]] is $1/(0.3+0.2+0.3)=1.25$ corresponding to a $25\%$ risk-free [[Assets|net rate of return]].  

If the state probabilities are 0.5, 0.25, and 0.25, respectively, the [[Exercises|state-price deflator]] corresponding to the state-price vector is given by.  
$$
...
In the discrete-time multi-period framework with time set $\mathcal{T}=\{0,1,2,...,T\}$ a [[Exercises|state-price deflator]] is an [[Stochastic Processes Definition Notation and Te|adapted stochastic process]] $\zeta=(\zeta_{t})_{t\in\mathcal{T}}$ such that  
...
The condition (i) is just a normalization. The condition (ii) is purely technical and will ensure that some relevant [[FORWARD RATES AND TERM STRUCTURE|expectations]] exist. Condition (iv) gives the price at time. $t$ in terms of all the future dividends and the [[Exercises|state-price deflator]]. This condition will also hold for all [[Quantitative Trading Strategies Lecture Notes|trading strategies]]..  

The [[Arbitrage Pricing of Derivatives|pricing]] condition implies a link between the price of an asset at two different points in time, say $t<t^{\prime}$ . From (4.20) we have  
$$
...
Here the fourth equality follows from the [[General Risk-Adjusted Probability Measures|Law of Iterated Expectations]], Theorem 2.1. Conversely, Equation (4.21) implies Equation (4.20).  
...
These equations show that the ratio $\zeta_{t+1}/\zeta_{t}$ acts as a one-period [[Exercises|state-price deflator]] between time $t$ and $t+1$ in the sense of the definition in the [[Properties of State-Price Deflators|one-period framework]], except that the price at the end of the period (zero in the one-period case) is added to the dividend. Of course, given the stateprice deflator process $\zeta=\left(\zeta_{t}\right)$ we know the state-price deflators $\zeta_{t+1}/\zeta_{t}$ for each of the subperiods. (Note that these will depend on the realized value $\zeta_{t}$ which is part of the information available at time $t$ .) Conversely, a sequence of "one-period state-price deflators" $\zeta_{t+1}/\zeta_{t}$ and the normalization $\zeta_{0}=1$ define the entire [[Exercises|state-price deflator]] process.  
...
Similar to the one-period case, the above equations lead to an expression for the [[Theoretical Factors|expected excess return]] of an asset over a single period:.  
...
The same relation holds for net rates of return. Just substitute $R_{i,t+1}=1+r_{i,t+1}$ and $R_{t}^{f}=$ $1+r_{t}^{f}$ on the left-hand side and observe that the ones cancel. On the right-hand side use that $\mathrm{ov}_{t}[R_{i,t+1},\boldsymbol{x}]=\mathrm{Cov}_{t}[1+r_{i,t+1},\boldsymbol{x}]=\mathrm{Cov}_{t}[r_{i,t+1},\boldsymbol{x}$ for any random variable $x$ . Therefore we can work with gross or net [[Assets|returns]] as we like in such expressions. The conditional Sharpe ratio of asset $i$ becomes  
$$
...
In a situation where the next-period [[Exercises|state-price deflator]] $\zeta_{t+1}/\zeta_{t}$ and the next-period gross return $R_{i,t+1}=e^{r_{i,t+1}}$ on a risky asset are jointly lognormally distributed conditional on time $t$ information, it follows from our one-period analysis that the expected excess [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return is  
$$
...
is the continuously compounded [[Financial Instruments|risk-free rate of return]] over the next period.  
...
By definition of a [[Exercises|state-price deflator]], a zero-coupon bond maturing at time $s$ with a face value of 1 will have a time. $t$ price of  
$$
...
Note that this is a [[Financial Instruments|risk-free rate of return]] between time $t$ and time $s$ . Now we can rewrite the.   
...
Each dividend is valued by discounting an appropriately risk-adjusted expected dividend by the [[A Preview of Alternative Formulations|risk-free return]] over the period. This generalizes the result in the [[Properties of State-Price Deflators|one-period framework]].  

Some authors formulate the important [[Arbitrage Pricing of Derivatives|pricing]] condition (iv) as follows: for all basic assets $i=1,\dots,I$ the state-price deflated gains process $G_{i}^{\zeta}=(G_{i t}^{\zeta})_{t\in\mathcal{T}}$ defined by  
$$
...
Similar to the [[Multi-Period Valuation Models|discrete-time framework]] we define a [[Exercises|state-price deflator]] in the [[Exercises|continuous-time framework]] as an [[Stochastic Processes Definition Notation and Te|adapted stochastic process]] $\zeta=\left(\zeta_{t}\right)$ with  
...
Under technical conditions, Equation (4.33) will also hold for all [[Quantitative Trading Strategies Lecture Notes|trading strategies]]. As in the discrete-time case the [[Definitions and Immediate Consequences|pricing equation]] (4.33) implies that for any $t<t^{\prime}<T$  
$$
...
As in the discrete-time case we can derive information about the short-term [[Financial Instruments|risk-free rate of return]] and the expected [[Assets|returns]] on the risky assets from the [[Exercises|state-price deflator]]. First we do this informally by considering a discrete-time approximation with period length. $\Delta t$ and let $\Delta t\rightarrow0$ at some point. In such an approximate model the risk-free one-period gross rate of return satisfies  
$$
\frac{1}{R_{t}^{f}}=\mathrm{E}_{t}\left[\frac{\zeta_{t+\Delta t}}{\zeta_{t}}\right],
$$  

according to (4.25). In terms of the annualized continuously compounded one-period [[Black Scholes Derivation|risk-free rate]] $\boldsymbol{r}_{t}^{f}$ , the left-hand side is given by $\exp\{-r_{t}^{f}\Delta t\}\approx1-r_{t}^{f}\Delta t$ . Subtracting one on both sides of the equation and changing signs, we get  
$$
...
The left-hand side is the continuously compounded short-term [[Exercises|risk-free interest rate]]. The righthand side is minus the relative drift of the [[Exercises|state-price deflator]].  

To obtain an expression for the current [[Lecture 1- Probability Distributions of Returns|expected return]] on a risky asset start with the equivalent of (4.23) in the approximating discrete-time model, i.e.  
$$
...
The left-hand side is the expected excess rate of return over the next instant. The right-hand side is the current rate of covariance between the return on the asset and the relative change of the [[Exercises|state-price deflator]].  

Now let us give a more rigorous treatment. Write the dynamics of a [[Exercises|state-price deflator]] as  
$$
d\zeta_{t}=-\zeta_{t}\left[m_{t}d t+\lambda_{t}^{\top}d z_{t}\right]
$$  

for some relative drift $m$ and some "sensitivity" vector $\lambda$ .First focus on the [[2. Forwards, Swaps, Futures, and Options|risk-free asset]]. By the [[Arbitrage Pricing of Derivatives|pricing]] condition in the definition of a state-price defator, the process $G_{f}^{\zeta}$ defined by $\begin{array}{r}{G_{f t}^{\zeta}=\zeta_{t}\exp\{\int_{0}^{t}r_{u}^{f}d u\}}\end{array}$ has to be a martingale, i.e. have a zero drift. By [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]],  
$$
d G_{f t}^{\zeta}=G_{f t}^{\zeta}\left[\left(-m_{t}+r_{t}^{f}\right)d t-\lambda_{t}^{\top}d z_{t}\right]
$$  

so we conclude that $m_{t}=r_{t}^{f}$ , i.e. the relative drift of a [[Exercises|state-price deflator]] is equal to the negative of the continuously compounded short-term [[Exercises|risk-free interest rate]].  

Next, for any risky asset. $i$ the process $G_{i}^{\zeta}$ defined by (4.35) must be a martingale. From [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] and the dynamics of. $P_{i}$ and $\zeta$ given in (3.3) and (4.38), we get  
$$
...
With a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]], we know that $m_{t}=r_{t}^{f}$ , so setting the drift equal to zero, we conclude that the equation  
$$
...
A (nice) process $\lambda=\left(\lambda_{t}\right)$ satisfying this equation is called a [[Exercises|market price of risk]]. If the price of the $i$ 'th asset is only sensitive to the $j$ 'th exogenous shock, Equation (4.39) reduces to  
$$
...
Therefore, $\lambda_{j t}$ is the compensation in terms of [[General Multi-Period Link Between Consumption a|excess expected return]] per unit of risk stemming. from the $j^{;}$ th exogenous shock. This explains the name [[Exercises|market price of risk]]. Summing up, the dynamics of a [[Exercises|state-price deflator]] is of the form.  
$$
\begin{array}{r}{d\zeta_{t}=-\zeta_{t}\left[r_{t}^{f}d t+\lambda_{t}^{\top}d z_{t}\right],}\end{array}
$$  

where $\lambda$ is a [[Exercises|market price of risk]]. This implies that  
$$
...
If we know or specify the dynamics of the [[Exercises|state-price deflator]] and the dividend stream from an asset, we can in principle price the asset using (4.33). However, only in special cases is it possible to compute the price in closed form. See examples in Section 4.4. In other cases one can use [[Pricing a Callable Leveraged Constant Maturity Swap Spread Note|Monte Carlo simulation]] to approximate the relevant expectation.  