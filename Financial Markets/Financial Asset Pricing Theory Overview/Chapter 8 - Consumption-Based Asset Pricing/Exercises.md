...
Now we allow for a stochastic intensity of usage, i.e. $\theta\:=\:\left(\theta_{t}\right)$ is a stochastic process%20Process.md). Define $X_{t}=(K_{t-1}+D_{t})/K_{t-1}$ $x_{t}=\ln X_{t}$ $c_{t}=\ln C_{t}$ , and $m_{t}=\ln M_{t}$  
