...
We have introduced the [[Exercises|mean-variance frontier]] earlier in Sections 4.6.3 and 6.2.5. Here we provide. an alternative characterization of the [[Mean-Variance Efficient Returns and Pricing Fac|mean-variance efficient returns]] and study the link between [[Exercises|mean-variance]] efficiency and [[Mean-Variance Efficient Returns and Pricing Fac|asset pricing theory]]..  

As before let $\pmb{R}=\left(R_{1},\ldots,R_{I}\right)^{\top}$ denote the vector of gross [[Assets|returns]] on the $I$ traded assets and define $\pmb{\mu}=\mathrm{E}[\pmb{R}]$ and $\underline{{\underline{{\Sigma}}}}=\mathrm{Var}[{\boldsymbol{R}}]$ . A [[An Asset Allocation Primer|portfolio]] $\pi$ is [[Exercises|mean-variance]] efficient if there is an $m\in\mathbb{R}$ so that $\pi$ solves  
$$
...
Following Hansen and Richard (1987) we will show that a return $R$ is [[Exercises|mean-variance]] efficient if and only if it can be written on the form ${\cal R}={\cal R}^{*}+w{\cal R}^{e*}$ for some number $w$ . Here $R^{*}$ is the. return on the [[An Asset Allocation Primer|portfolio]] corresponding to the dividend $\zeta^{\ast}$ defined in (4.46) and. $R^{e*}$ is a particular excess return to be defined shortly. This characterization of the [[Exercises|mean-variance]] portfolios turn out to be preferable for discussing the link between [[2. Forwards, Swaps, Futures, and Options|mean-variance analysis]] and [[Financial Mathematics Course|asset pricing models]].  

# $R^{*}$ and [[2. Forwards, Swaps, Futures, and Options|mean-variance analysis]]  

How does $R^{*}$ fit into the [[Exercises|mean-variance]] framework? The following lemma shows that it is a meanvariance efficient return on the downward-sloping part of the [[Exercises|mean-variance frontier]]. Furthermore, it is the return with minimum second moment. (Recall that the second moment of a random variable $x$ is $\mathrm{E}[x^{2}]$  

Lemma 9.2 $R^{*}$ has the following properties:  

(a) $R^{*}$ is the return that has minimum second moment,   
(b) $R^{*}$ is a [[Exercises|mean-variance]] efficient return located on the downward-sloping part of the [[2. Forwards, Swaps, Futures, and Options|efficient frontier]].  

Proof: The return on a [[An Asset Allocation Primer|portfolio]] $\pi$ is the random variable $R^{\pi}=\pi^{\top}R$ . The second moment of this return is $\begin{array}{r}{\operatorname{E}[(R^{\pi})^{2}]=\pi^{\top}\operatorname{E}[R R^{\top}]\pi}\end{array}$ . Consider the minimization problem  
$$
...
Since $\pi^{*}$ is the [[An Asset Allocation Primer|portfolio]] minimizing the second moment of gross [[Assets|returns]] among all portfolios, it is also the [[An Asset Allocation Primer|portfolio]] minimizing the second moment of gross [[Assets|returns]] among the portfolios with the same mean return as $\pi^{*}$ , i.e. portfolios with $\operatorname{E}[R^{\pi}]=\operatorname{E}[R^{*}]$ . For these portfolios the variance of. return is  
$$
...
It then follows that $\pi^{*}$ is the [[An Asset Allocation Primer|portfolio]] minimizing the [[Arithmetic and Geometric Rates of Return|variance of return]] among all the portfolios.   
having the same mean return as. $\pi^{*}$ . Hence, $\pi^{*}$ is indeed a [[Exercises|mean-variance]] efficient [[An Asset Allocation Primer|portfolio]]. In a.   
(standard deviation, mean)-diagram [[Assets|returns]] with same second moment. $K=\operatorname{E}[(R^{\pi})^{2}]$ yield points.   
on a circle with radius $\sqrt{K}$ centered in (0,0), since $\left(\sigma(R^{\pi})\right)^{2}+\left(\operatorname{E}[R^{\pi}]\right)^{2}=\operatorname{E}[(R^{\pi})^{2}]$ . The return.   $R^{*}$ therefore corresponds to a point on the downward-sloping part of the [[2. Forwards, Swaps, Futures, and Options|efficient frontier]].  

# The constant-mimicking return  

In a market without a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] you may wonder how close you can get to a [[Exercises|risk-free dividend]]. Of course this will depend on what you mean by "close."' If you apply a mean-square measure, the. distance between the dividend $D^{\theta}=\theta^{\top}D$ of a [[An Asset Allocation Primer|portfolio]] $\pmb{\theta}$ and a [[Exercises|risk-free dividend]] of 1 is  
$$
...
where the last equality comes from (4.58). The [[Mean-Variance Efficient Returns and Pricing Fac|constant-mimicking return]] is thus  
$$
...
Excess [[Assets|returns]] and $R^{e*}$  

An excess return is simply the difference between two [[Assets|returns]]. Since any return corresponds to a dividend for a unit initial payment, an excess return can be seen as a dividend for a zero initial payment. Of course, in [[Exercises|absence of arbitrage]], a non-zero excess return will turn out positive in some states and negative in other states.  

Typically, excess [[Assets|returns]] on different portfolios relative to the same "reference"' return are considered. For a reference return $\check{R}=\check{\pi}^{\top}R$ , the set of all possible excess [[Assets|returns]] are given by.  
$$
\underline{{R}}^{e}[\check{R}]=\left\{\boldsymbol{\pi}^{\top}\boldsymbol{R}-\check{R}~|~\boldsymbol{\pi}^{\top}\mathbf{1}=1\right\},
$$  

where as before $\kappa$ is the $I$ -dimensional vector of [[Assets|returns]] on the basis assets, and $\pi$ denotes a [[Portfolios and Trading Strategies|portfolio weight vector]] of these $I$ assets.  

It is useful to observe that the set of excess [[Assets|returns]] is the same for all reference [[Assets|returns]]. An excess return relative to a reference return $\check{R}$ is given by. $\pi^{\top}R-\check{R}=(\pi-\check{\pi})^{\top}R$ for some [[Portfolios and Trading Strategies|portfolio weight vector]] $\pi$ . We can obtain the same excess return relative to another reference return $\dot{R}=\dot{\pi}^{\top}R$ using the [[An Asset Allocation Primer|portfolio]]. $\pi+\dot{\pi}-\check{\pi}$ .(Check for yourself!) Hence, we can simply talk about the set of excess [[Assets|returns]],. $R^{e}$ , without specifying any reference return, and we can write it as  
$$
\underline{{R}}^{e}=\{(\pi^{e})^{\top}{\pmb R}|(\pi^{e})^{\top}{\bf1}=0\}.
$$  

The set of excess [[Assets|returns]] is a linear subspace of the set of all random variables (in the case with $S$ [[Probability Space|possible outcomes]] this is equivalent to. $\mathbb{R}^{S}$ ) in the sense that  
...
2. if $R^{e1}$ and $R^{e2}$ are two excess [[Assets|returns]], then $R^{e1}+R^{e2}$ is also an excess return.  
...
(b) Write the excess return $R^{e}$ as the difference between the return on some [[An Asset Allocation Primer|portfolio]] $\pi$ and $R^{*}$ i.e. $R^{e}=\pi^{\top}R-R^{*}$ .Then  
$$
...
Proof: Define $w_{i}=\left(\mathrm{E}[R_{i}]-\mathrm{E}[R^{*}]\right)/\mathrm{E}[R^{e*}]$ and $\eta_{i}=R_{i}-R^{*}-w_{i}R^{e*}$ . Then (9.24) and $\mathrm{E}[\eta_{i}]=0$ hold by construction.. $\eta_{i}$ is the difference between the two excess [[Assets|returns]] $R_{i}-R^{*}$ and $w_{i}R^{e*}$ and therefore itself an excess return. Now. $\mathrm{E}[R^{*}\eta_{i}]=0$ follows from (9.19) and from (9.21) we have $\mathrm{E}[\eta_{i}R^{e*}]=\mathrm{E}[\eta_{i}]$ , which we know is zero..  

Due to the relations. $\mathrm{E}[R^{*}R^{e*}]=\mathrm{E}[R^{*}\eta_{i}]=\mathrm{E}[R^{e*}\eta_{i}]=0$ , the decomposition is said to be orthogonal.  

Note that the same $w_{i}$ applies for all [[Assets|returns]] with the same expected value. The return variance is  
$$
...
Theorem 9.7 A return. $R_{i}$ is [[Exercises|mean-variance]] efficient if and only if it can be written as  
$$
R_{i}=R^{*}+w_{i}R^{e*}
$$  
...
Varying $w_{i}$ from $-\infty$ to $+\infty$ $R_{i}=R^{*}+w_{i}R^{e*}$ runs through the entire [[Exercises|mean-variance frontier]] in the direction of higher and higher expected [[Assets|returns]] (since $\operatorname{E}[R^{e*}]=\operatorname{E}[(R^{e*})^{2}]>0$  

Note that from (9.18) that the [[Mean-Variance Efficient Returns and Pricing Fac|constant-mimicking return]] can be written as  
$$
R_{\mathrm{cm}}=R^{*}+\frac{\mathrm{E}\left[(R^{*})^{2}\right]}{\mathrm{E}[R^{*}]}R^{e*}
$$  

so the [[Mean-Variance Efficient Returns and Pricing Fac|constant-mimicking return]] is [[Exercises|mean-variance]] efficient.  

# Allowing for a risk-free asset  

Now let us assume that a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] with return. $R^{f}$ exists (or can be constructed as a [[An Asset Allocation Primer|portfolio]] of the basic assets). Then from Lemma 4.1 we have that $R^{f}\operatorname{E}[R^{*}]=\operatorname{E}[R^{*}R^{f}]=\operatorname{E}[(R^{*})^{2}]$ , and hence  
$$
...
so the $w_{i}$ corresponding to the [[A Preview of Alternative Formulations|risk-free return]] is $R^{f}$ itself. With $R^{f}>1$ , we see that $R^{*}+R^{e*}$ corresponds to a point on the frontier below the [[Black Scholes Derivation|risk-free rate]]. (Again we use the fact that $\mathrm{E}[R^{e*}]>0$  

# The minimum-variance return  

With the decomposition in Theorem 9.7, it is easy to find the minimum-variance return. The variance of any [[Exercises|mean-variance]] efficient return is  
$$
...
When a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] exists, this simplifies to $R_{\operatorname*{min}}=R^{f}$ , because $\mathrm{E}[R^{*}]/(1-\mathrm{E}[R^{e*}])=R^{f}$ using (9.29).  

# 9.4.2 Link between mean-variance efficient returns and state-price deflators  

Theorem 9.8 Let. $R$ denote a gross return. Then there exists $a,b\in\mathbb{R}$ so that $\zeta=a+b R$ satisfies $P_{i}=\operatorname{E}[\zeta D_{i}]$ for $i=1,\ldots,I$ if and only if. $R$ is a [[Exercises|mean-variance]] efficient return different from the [[Mean-Variance Efficient Returns and Pricing Fac|constant-mimicking return]].  
...
for some $w\in\mathbb R$ and some excess return $\eta$ withe $\operatorname{E}[\eta_{i}]=\operatorname{E}[R^{*}\eta_{i}]=\operatorname{E}[R^{e*}\eta_{i}]=0$ $R$ is [[Exercises|mean-variance]] efficient if and only if $\eta=0$ . We need to show that, for suitable $a,b\in\mathbb{R}$  
$$
...
Recall that $P_{i}=\operatorname{E}[\zeta D_{i}]$ for all $i$ implies that $\operatorname{E}[\zeta R_{i}]=1$ for all [[Assets|returns]] $R_{i}$ and $\operatorname{E}[\langle R^{e}]=0$ for all excess [[Assets|returns]] $R^{e}$ . In particular,  
$$
...
[[Mean-Variance Efficient Returns and Pricing Fac|constant-mimicking return]], cf. (9.26).  
...
where we have applied various results from earlier. We can now see that we will have $\operatorname{E}[\zeta R_{i}]=1$ for all [[Assets|returns]] $R_{i}$ if and only if $\mathrm{E}[\eta\eta_{i}]=0$ for all excess [[Assets|returns]] $\eta_{i}$ . In particular $\mathrm{E}[\eta^{2}]=0$ , which implies that $\eta=0$  

# 9.4.3 Link between mean-variance efficient returns and pricing factors  

Theorem 9.9 A return $R^{m v}$ is a [[Pricing Factors in a One-Period Framework|pricing factor]], i.e. an $\alpha\in\mathbb R$ exists so that.  
$$
\begin{array}{r}{\operatorname{E}[R_{i}]=\alpha+\beta[R_{i},R^{m v}]\left(\operatorname{E}[R^{m v}]-\alpha\right),\quad i=1,\dots,I,}\end{array}
$$  

if and only if. $R^{m v}$ is a [[Exercises|mean-variance]] efficient return different from the minimum-variance return.  

The constant $\alpha$ must then be equal to the [[Pricing Factors in a One-Period Framework|zero-beta return]] corresponding to $R^{\mathrm{mv}}$ , which is equal to the [[A Preview of Alternative Formulations|risk-free return]] if a [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] exists.  

Proof: (If' part.) First, let us show that if $R^{\mathrm{mv}}$ is [[Exercises|mean-variance]] efficient and different from the minimum-variance return, it will work as a [[Pricing Factors in a One-Period Framework|pricing factor]]. This result is originally due to Roll (1977). For some $w$ , we have $R^{\mathrm{{mv}}}=R^{*}+w R^{e*}$ . Consider a general return $R_{i}$ and decompose as  
$$
R_{i}=R^{*}+w_{i}R^{e*}+\eta_{i}
$$  
...
which verifies that. $R^{\mathrm{mv}}$ is a valid [[Pricing Factors in a One-Period Framework|pricing factor]].  

("Only if' part.) Next, let us show that if a return works as a [[Pricing Factors in a One-Period Framework|pricing factor]], it must be meanvariance efficient and different from the minimum-variance return. This proof is due to Hansen and Richard (1987). Assume that $R^{\mathrm{mv}}$ is a [[Pricing Factors in a One-Period Framework|pricing factor]]. Decomposing as in Theorem 9.6  
$$
R^{\mathrm{mv}}=R^{*}+w R^{e*}+\eta,
$$  

we need to show that. $\eta=0$ (so $R^{\mathrm{mv}}$ is [[Exercises|mean-variance]] efficient) and that $w\neq\mathrm{E}[R^{*}]\mathrm{E}[R^{e*}]/\mathrm{Var}[R^{e*}]$ (so $R^{\mathrm{mv}}$ is different from the minimum-variance return).  
...
One implication of this theorem is that we can always find some [[Assets|returns]] that work as a [[Pricing Factors in a One-Period Framework|pricing factor]], namely the [[Mean-Variance Efficient Returns and Pricing Fac|mean-variance efficient returns]]. Another implication is that the conclusion of the [[The Classical One-Period Capm|classical CAPM]] can be restated as "the market [[An Asset Allocation Primer|portfolio]] is [[Exercises|mean-variance]] efficient."  