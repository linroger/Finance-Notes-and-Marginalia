...
In the continuous-time [[Financial Mathematics Course|asset pricing models]] we will consider in this book, the basic uncertainty in the economy is represented by the evolution of a standard [[Continuous-Time Stochastic Processes|Brownian motion]]. A (one-dimensional) [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] $z~=~(z_{t})_{t\in[0,T]}$ is called a standard [[Continuous-Time Stochastic Processes|Brownian motion]], if it satisfies the following conditions:  

(i) $z_{\mathrm{0}}=0$ (ii) for all. $t,t^{\prime}\geq0$ with $t<t^{\prime}$ .. $z_{t^{\prime}}-z_{t}\sim N(0,t^{\prime}-t)$ [normally distributed increments], (ii) for all $0\leq t_{0}<t_{1}<\dots<t_{n}$ , the random variables. $z_{t_{1}}-z_{t_{0}}$ , ..., $z_{t_{n}}-z_{t_{n-1}}$ are mutually independent [[[Continuous-Time Stochastic Processes|independent increments]]], (iv) $z$ has continuous sample paths.  

The first three conditions are equivalent to the discrete-time case studied above. We can informally. think of $d z_{t}\approx z_{t+d t}-z_{t}\sim N(0,d t)$ as an exogenous shock to the economy at time. $t$ . The state space $\Omega$ is in this case the (infinite) set of all paths of the standard [[Continuous-Time Stochastic Processes|Brownian motion]] $z$ . The [[Information|information filtration]] $(\mathcal{F}_{t})_{t\in[0,T]}$ is generated by the standard [[Continuous-Time Stochastic Processes|Brownian motion]]. $z$ in the sense that, for each $t$ $\mathcal{F}_{t}$ is the smallest $\sigma$ -algebra on which the random variable. $z_{t}$ is measurable. The probability measure $\mathbb{P}$ is fixed by the normality assumption.  

Any continuous-time [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] $X~=~(X_{t})_{t\in[0,T]}$ in the financial models in this book will be defined in terms of the standard [[Continuous-Time Stochastic Processes|Brownian motion]] by an initial constant value $X_{0}$ and an equation of the form  
$$
...
The standard [[Continuous-Time Stochastic Processes|Brownian motion]] is basically the continuous-time version of a [[Some Discrete-Time Stochastic Processes|random walk]] with initial value 0. The standard [[Continuous-Time Stochastic Processes|Brownian motion]] is a [[Chapter 7 Random Walks Risk and Arbitrage|Markov process]] because the increment from today to any future point in time is independent of the history of the process. The standard [[Continuous-Time Stochastic Processes|Brownian motion]] is also a martingale since the expected change in the value of the process is zero. The name [[Continuous-Time Stochastic Processes|Brownian motion]] is in honor of the Scottish botanist Robert Brown, who in 1828 observed the apparently random movements of pollen submerged in water. The often used name [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] is due to Norbert Wiener, who in the 1920s was the first to show the existence of a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] with these properties and who developed a mathematically rigorous analysis of the process. As early as in the year 1900, the standard [[Continuous-Time Stochastic Processes|Brownian motion]] was used in a model for [[Chapter 16 - Black–Scholes Model|stock price]] movements by the French researcher Louis Bachelier, who derived the first option [[Arbitrage Pricing of Derivatives|pricing]] formula.  

The defining characteristics of a standard [[Continuous-Time Stochastic Processes|Brownian motion]] look very nice, but they have some drastic consequences. It can be shown that the sample paths of a standard [[Continuous-Time Stochastic Processes|Brownian motion]] are nowhere differentiable, which broadly speaking means that the sample paths bend at all points in time and are therefore strictly speaking impossible to illustrate. However, one can get an idea of the sample paths by simulating the values of the process at different times. If $\varepsilon_{1},\ldots,\varepsilon_{n}$ are independent draws from a standard $N(0,1)$ distribution, we can simulate the value of the standard [[Continuous-Time Stochastic Processes|Brownian motion]] at time $0\equiv t_{0}<t_{1}<t_{2}<\cdots<t_{n}$ as follows:  
$$
z_{t_{i}}=z_{t_{i-1}}+\varepsilon_{i}{\sqrt{t_{i}-t_{i-1}}},\quad i=1,\ldots,n.
$$  

With more time points and hence shorter intervals we get a more realistic impression of the sample paths of the process. Figure 2.5 shows a simulated sample path for a standard [[Continuous-Time Stochastic Processes|Brownian motion]] over the interval $[0,1]$ based on a partition of the interval into 200 subintervals of equal length.1. Note that since a normally distributed random variable can take on infinitely many values, a. standard [[Continuous-Time Stochastic Processes|Brownian motion]] has infinitely many sample paths that each has a zero probability of. occurring. The figure shows just one possible sample path. Note that the picture resembles typical [[Chapter 16 - Black–Scholes Model|stock price]] charts.  

![](acc192020f69e3747bc620596e7b351894cfbbee7311f58375aa4faf0f0511ac.jpg)  
Figure 2.5: A simulated sample path of a standard [[Continuous-Time Stochastic Processes|Brownian motion]] based on 200 subintervals.  

Another property of a standard [[Continuous-Time Stochastic Processes|Brownian motion]] is that the expected length of the sample path over any future time interval (no matter how short) is infinite. In addition, the expected number of times a standard [[Continuous-Time Stochastic Processes|Brownian motion]] takes on any given value in any given time interval is also infinite. Intuitively, these properties are due to the fact that the size of the increment of a standard [[Continuous-Time Stochastic Processes|Brownian motion]] over an interval of length $\Delta t$ is proportional to $\sqrt{\Delta t}$ , in the sense that the standard deviation of the increment equals $\sqrt{\Delta t}$ . When $\Delta t$ is close to zero, $\sqrt{\Delta t}$ is significantly larger than $\Delta t$ , so the changes are large relative to the length of the time interval over which the changes are measured.  

The expected change in an object described by a standard [[Continuous-Time Stochastic Processes|Brownian motion]] equals zero and the variance of the change over a given time interval equals the length of the interval. This can easily be generalized. As before let $z=(z_{t})_{t\geq0}$ be a one-dimensional standard [[Continuous-Time Stochastic Processes|Brownian motion]] and define a new [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] $X=(X_{t})_{t\geq0}$ by  
$$
X_{t}=X_{0}+\mu t+\sigma z_{t},\quad t\geq0,
$$  

where $X_{0}$ $\mu$ , and $\sigma$ are constants. The constant $X_{0}$ is the initial value for the process $X$ .It follows from the properties of the standard [[Continuous-Time Stochastic Processes|Brownian motion]] that, seen from time $0$ , the value $X_{t}$ is normally distributed with mean $\mu t$ and variance $\sigma^{2}t$ , i.e. $X_{t}\sim N(X_{0}+\mu t,\sigma^{2}t)$  
...
for $\Delta t\rightarrow0$ . The process $X$ is called a generalized [[Continuous-Time Stochastic Processes|Brownian motion]] or a generalized Wiener. process. This is basically the continuous-time version of a [[Some Discrete-Time Stochastic Processes|random walk]] with drift. The parameter $\mu$ reflects the expected change in the process per unit of time and is called the drift rate or simply the drift of the process. The parameter $\sigma$ reflects the uncertainty about the future values of the. process. More precisely, $\sigma^{2}$ reflects the variance of the change in the process per unit of time and. is often called the variance rate of the process.. $\sigma$ is a measure for the standard deviation of the change per unit of time and is referred to as the volatility of the process..  

A generalized [[Continuous-Time Stochastic Processes|Brownian motion]] inherits many of the characteristic properties of a standard [[Continuous-Time Stochastic Processes|Brownian motion]]. For example, also a generalized [[Continuous-Time Stochastic Processes|Brownian motion]] is a [[Chapter 7 Random Walks Risk and Arbitrage|Markov process]], and the sample paths of a generalized [[Continuous-Time Stochastic Processes|Brownian motion]] are also continuous and nowhere differentiable. However, a generalized [[Continuous-Time Stochastic Processes|Brownian motion]] is not a martingale unless $\mu=0$ . The sample paths can be simulated by choosing time points $0\equiv t_{0}<t_{1}<\cdots<t_{n}$ and iteratively computing  
$$
X_{t_{i}}=X_{t_{i-1}}+\mu(t_{i}-t_{i-1})+\varepsilon_{i}\sigma\sqrt{t_{i}-t_{i-1}},\quad i=1,\dots,n,
$$  

where $\varepsilon_{1},\ldots,\varepsilon_{n}$ are independent draws from a [[Verification of Central Limit Theorem|standard normal distribution]]. Figure 2.6 show simulated sample paths for two different values of $\sigma$ but the same $\mu$ . The paths are drawn using. the same sequence of random numbers $\varepsilon_{i}$ so that they are directly comparable. The straight line. represent the deterministic trend of the process, which corresponds to imposing the condition $\sigma=0$ and hence ignoring the uncertainty. The parameter. $\mu$ determines the trend, and the parameter $\sigma$ determines the size of the fluctuations around the trend..  

If the parameters $\mu$ and $\sigma$ are allowed to be time-varying in a deterministic way, the process $X$ is said to be a time-inhomogeneous generalized [[Continuous-Time Stochastic Processes|Brownian motion]]. In differential terms such a process can be written as defined by  
$$
...
Figure 2.6: Simulation of a generalized [[Continuous-Time Stochastic Processes|Brownian motion]] with $\mu=0.2$ and $\sigma=0.5$ or $\sigma=1.0$ .The straight line shows the trend corresponding to $\sigma=0$ . The simulations are based on 200 subintervals.  
...
A (one-dimensional) diffusion process is a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] $X=(X_{t})_{t\geq0}$ for which the change. over an infinitesimally short time interval. $[t,t+d t]$ can be written as  
$$
d X_{t}=\mu(X_{t},t)d t+\sigma(X_{t},t)d z_{t},
$$  

where $z$ is a standard [[Continuous-Time Stochastic Processes|Brownian motion]], but where the drift $\mu$ and the volatility $\sigma$ are now functions of time and the current value of the process.2 This expression generalizes (2.3), where $\mu$ and $\sigma$ were assumed to be constants, and (2.4), where $\mu$ and $\sigma$ were functions of time only. An equation like (2.6), where the [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] enters both sides of the equality, is called a [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]]. Hence, a diffusion process is a solution to a [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]].  
...
A diffusion process is a [[Chapter 7 Random Walks Risk and Arbitrage|Markov process]] as can be seen from (2.6), since both the drift and the volatility only depend on the current value of the process and not on previous values. A diffusion process is not a martingale, unless the drift $\mu(X_{t},t)$ is zero for all $X_{t}$ and $t$ . A diffusion process will have continuous, but nowhere differentiable sample paths. The value space for a diffusion process and the distribution of future values will depend on the functions $\mu$ and $\sigma$ . In Section 2.6.7 we will an example of a diffusion process often used in [[Mathematics of the Financial Markets|financial modeling]], the so-called [[Black Scholes Derivation|geometric Brownian motion]]. Other diffusion processes will be used in later chapters.  

# 2.6.3 Ito processes  

It is possible to define even more general continuous-path processes than those in the class of diffusion processes. A (one-dimensional) [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]. $X_{t}$ is said to be an Ito process, if the. local increments are on the form  
$$
d X_{t}=\mu_{t}d t+\sigma_{t}d z_{t},
$$  

where the drift $\mu$ and the volatility. $\sigma$ themselves are stochastic processes. A diffusion process is. the special case where the values of the drift. $\mu_{t}$ and the volatility. $\sigma_{t}$ are given as functions of $t$ and $X_{t}$ . For a general Ito process, the [[Black-Scholes Model|drift and volatility]] may also depend on past values of the $X$ process and also on past and current values of other adapted processes. It follows that Ito. processes are generally not Markov processes. They are generally not martingales either, unless $\mu_{t}$ is identically equal to zero (and $\sigma_{t}$ satisfies some technical conditions). The processes $\mu$ and $\sigma$ must satisfy certain regularity conditions for the. $X$ process to be well-defined. We will refer. the reader to Oksendal (1998, Ch. 4) for these conditions. The expression (2.8) gives an intuitive understanding of the evolution of an Ito process, but it is more precise to state the evolution in the integral form  
$$
...
Above we have focused on processes having sample paths that are continuous functions of time,. so that one can depict the evolution of the process by a continuous curve. Stochastic processes which have sample paths with discontinuities (jumps) also exist. The jumps of such processes are often modeled by Poisson processes or related processes. It is well-known that large, sudden movements in financial variables occur from time to time, for example in connection with [[Hedge Fund Strategies|stock market]] crashes. There may be many explanations of such large movements, for example a large unexpected change in the productivity in a particular industry or the economy in general, perhaps due to a technological break-through. Another source of sudden, large movements is changes in the political or economic environment such as unforseen interventions by the government or central bank. [[Hedge Fund Strategies|Stock market]] crashes are sometimes explained by the bursting of a bubble (which does not necessarily conflict with the usual assumption of rational investors). Whether such sudden, large movements can be explained by a sequence of small continuous movements in the same direction or jumps have to be included in the models is an empirical question, which is still open. While jump processes may be relevant for many purposes, they are also more difficult to deal with than processes with continuous sample paths so that it will probably be best to study models without jumps first. This book will only address continuous-path processes. An [[Overview of Financial Markets|overview]] of financial models with jump processes is given by Cont and Tankov (2004)..  
...
using the [[General Risk-Adjusted Probability Measures|Law of Iterated Expectations]]. For the variance we have  
$$
...
Theorem 2.3 If $z$ is a [[Continuous-Time Stochastic Processes|Brownian motion]], and $\sigma(u)$ is a deterministic function of time, the random variable $\begin{array}{r}{\int_{t}^{t^{\prime}}\sigma(u)d z_{u}}\end{array}$ is normally distributed with mean zero and variance $\begin{array}{r}{\int_{t}^{t^{\prime}}\sigma(u)^{2}d u}\end{array}$  
...
Theorem 2.4 Suppose the process $M=\left(M_{t}\right)$ is a martingale with respect to a probability measure under which $z=\left(z_{t}\right)$ is a standard [[Continuous-Time Stochastic Processes|Brownian motion]]. Then a unique adapted process $\theta=\left(\theta_{t}\right)$ exists such that  
$$
...
Now the stochastic integral with respect to the standard [[Continuous-Time Stochastic Processes|Brownian motion]] has been defined, we can also define stochastic integrals with respect to other stochastic processes. For example, if $X_{t}$ is a diffusion given by $d X_{t}=\mu(X_{t},t)d t+\sigma(X_{t},t)d z_{t}$ and $\alpha=(\alpha_{t})_{t\in[0,T]}$ is a sufficiently "nice" [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]], we can define.  
$$
...
In [[Financial Mathematics Course|continuous-time models]] a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] for the dynamics of some basic quantity is often.   
taken as given, while other quantities of interest can be shown to be functions of that basic variable.   
To determine the dynamics of these other variables, we shall apply [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]], which is basically.   
...
where $\mu$ and $\sigma$ are real-valued processes, and. $z$ is a one-dimensional standard [[Continuous-Time Stochastic Processes|Brownian motion]].. Let $g(X,t)$ be a real-valued function which is two times continuously differentiable in $X$ and continuously differentiable in. $t$ . Then the process $y=(y_{t})_{t\geq0}$ defined by  
$$
y_{t}=g(X_{t},t)
$$  
...
The [[Black Scholes Derivation|geometric Brownian motion]] is an important example of a diffusion process. A [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] $X=(X_{t})_{t\geq0}$ is said to be a [[Black Scholes Derivation|geometric Brownian motion]] if it is a solution to the. [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]].  
$$
d X_{t}=\mu X_{t}d t+\sigma X_{t}d z_{t},
$$  

where $\mu$ and $\sigma$ are constants. The initial value for the process is assumed to be positive,. $X_{0}>0$ A [[Black Scholes Derivation|geometric Brownian motion]] is the particular diffusion process that is obtained from (2.6) by inserting $\mu(X_{t},t)=\mu X_{t}$ and $\sigma(X_{t},t)=\sigma X_{t}$  
...
which is the relative (percentage) change in the value of the process over the next infinitesimally short time interval $[t,t+d t]$ . If $X_{t}$ is the price of a traded asset, then $d X_{t}/X_{t}$ is the rate of return on the asset over the next instant. The constant $\mu$ is the expected rate of return per period, while $\sigma$ is the standard deviation of the rate of return per period. In this context it is often. $\mu$ which is called the drift (rather than $\mu X_{t}$ ) and $\sigma$ which is called the volatility (rather than $\sigma X_{t}$ ). Strictly speaking, one must distinguish between the relative [[Black-Scholes Model|drift and volatility]] (. $\mu$ and $\sigma$ , respectively) and the absolute [[Black-Scholes Model|drift and volatility]] ( $\mu X_{t}$ and $\sigma X_{t}$ , respectively). An asset with a constant expected rate of return and a constant relative volatility has a price that follows a geometric Brownian. motion. For example, such an assumption is used for the [[Chapter 16 - Black–Scholes Model|stock price]] in the famous Black-ScholesMerton model for stock option [[Arbitrage Pricing of Derivatives|pricing]], cf. Chapter 12. In the framework of consumption-based. capital [[Financial Mathematics Course|asset pricing models]] it is often assumed that the [[Pareto-Optimality in Some Incomplete Markets|aggregate consumption]] in the economy follows a [[Black Scholes Derivation|geometric Brownian motion]], cf. Chapter 8.  

Next, we will find an explicit expression for $X_{t}$ , i.e. we will find a solution to the [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] (2.14). We can then also determine the distribution of the future value of the process. We apply [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] with the function $g(x,t)=\ln x$ and define the process $y_{t}=$ $g(X_{t},t)=\ln X_{t}$ . Since  
$$
...
Hence, the process $y_{t}=\ln X_{t}$ is a generalized [[Continuous-Time Stochastic Processes|Brownian motion]]. In particular, we have  
...
value space of a [[Black Scholes Derivation|geometric Brownian motion]] is. $\S=(0,\infty)$  

Suppose now that we stand at time. $t$ and have observed the current value. $X_{t}$ of a [[Black Scholes Derivation|geometric Brownian motion]]. Which probability distribution is then appropriate for the uncertain future value, say at time $t^{\prime}$ ? Since $z_{t^{\prime}}-z_{t}\sim N(0,t^{\prime}-t)$ , we see from (2.15) that the future value. $X_{t^{\prime}}$ (conditional on $X_{t}$ ) will be lognormally distributed. The probability density function for. $X_{t^{\prime}}$ (given $X_{t}$ ) is given by  
$$
...
We will consider other specific diffusions in later chapters, when we need them. For example, we shall use the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]] defined by  
$$
...
Such processes are used, among other things, to model the dynamics of [[Interest Rate Quotations|interest rates]].  