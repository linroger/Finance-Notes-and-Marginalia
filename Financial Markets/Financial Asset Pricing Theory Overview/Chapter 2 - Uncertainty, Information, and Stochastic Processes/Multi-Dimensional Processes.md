...
So far we have only considered one-dimensional processes, i.e. processes with a value space which is $\mathbb{R}$ or a subset of $\mathbb{R}$ . In most [[Financial Mathematics Course|asset pricing models]] we need to keep track of several processes, e.g. dividend and price processes for different assets, and we will often be interested in [[Multi-Dimensional Processes|covariances and correlations]] between different processes.  

If the [[Some Discrete-Time Stochastic Processes|exogenous shocks]] in the model are one-dimensional, then increments over the smallest. time interval considered in the model will be perfectly correlated. In a discrete-time model where the [[Some Discrete-Time Stochastic Processes|exogenous shocks]]. $\varepsilon_{1},\ldots,\varepsilon_{T}$ are one-dimensional, changes in any two processes between two. subsequent points in time, say. $t$ and $t+1$ , will be perfectly correlated. For example, suppose. $X$ and $Y$ are two general processes defined by  
$$
...
In a continuous-time model where the exogenous shock processe $z=(z_{t})_{t\in[0,T]}$ is one-dimensional, the instantaneous increments of any two processes will be perfectly correlated. For example, if we consider the two [[Multi-Dimensional Processes|Ito processes]] $X$ and $Y$ defined by  
$$
\mathrm{~\boldmath~\sigma~}=\mu_{X t}d t+\sigma_{X t}d z_{t},\quad\mathrm{~\boldmath~}d Y_{t}=\mu_{Y t}d t+\sigma_{Y t}d z_{t},
$$  

then $\operatorname{Cov}_{t}[d X_{t},d Y_{t}]=\sigma_{X t}\sigma_{Y t}d t$ so that the [[Multi-Dimensional Processes|instantaneous correlation]] becomes  
$$
...
$$  
$\sigma_{X t}^{2}$ and $\sigma_{Y t}^{2}$ are the variance rates of $X_{t}$ and $Y_{t}$ , respectively, while the covariance is. $\mathrm{Cov}_{t}[d X_{t},d Y_{t}]=$ $\rho_{t}\sigma_{X t}\sigma_{Y t}$ . If $\sigma_{X t}$ and $\sigma_{Y t}$ are both positive, then. $\rho_{t}$ will be the [[Multi-Dimensional Processes|instantaneous correlation]] between the two processes. $X$ and $Y$  

In many [[Financial Mathematics Course|continuous-time models]], one [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] is defined in terms of a function of two other, not necessarily perfectly correlated, stochastic processes. For that purpose we need the following two-dimensional version of [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]].  
...
where $z_{1}=(z_{1t}$ and $z_{2}~=~(z_{2t})$ are independent standard Brownian motions. Let $g(X,Y,t)$ be $a$ real-valued function for which all the [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] gt, ox, $\textstyle{\frac{\partial g}{\partial t}},{\frac{\partial g}{\partial X}},{\frac{\partial g}{\partial Y}},{\frac{\partial^{2}g}{\partial X^{2}}},{\frac{\partial^{2}g}{\partial Y^{2}}}$ , and! $\frac{\partial^{2}g}{\partial X\partial Y}$ erist and are continuous. Then the process $W=\left(W_{t}\right)$ defined by $W_{t}=g(X_{t},Y_{t},t)$ is an Ito process with  
$$
...
where the dependence of all the partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] on $\left({X}_{t},Y_{t},t\right)$ has been notationally suppressed.  
...
Example 2.1 Suppose that the dynamics of $X$ and $Y$ are given by (2.16) and $W_{t}=X_{t}Y_{t}$ .In order to find the dynamics of $W$ , we apply the above version of [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] with the function $g(X,Y)=X Y$ . The relevant partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are  
$$
...
For the special case, where both $X$ and $Y$ are [[Black Scholes Derivation|geometric Brownian motion]] so that $m_{x}$ $m_{Y}$ $v_{X1}$ $v_{X2}$ $v_{Y1}$ , and $v_{Y2}$ are all constants, it follows that $W_{t}=X_{t}Y_{t}$ is also a [[Black Scholes Derivation|geometric Brownian motion]].  

Example 2.2Define $W_{t}=X_{t}/Y_{t}$ . In this case we need to apply [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] with the function $g(X,Y)=X/Y$ which has [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]  
$$
...
is also a [[Black Scholes Derivation|geometric Brownian motion]]..  

We can apply the two-dimensional version of [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] to prove the following useful result relating expected discounted values and the drift rate..  
...
Proof: Suppose first that the relative drift rate is given by. $m$ so that $d X_{t}=X_{t}[m_{t}d t+v_{t}d z_{t}]$ Let us use [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] to identify the dynamics of the process. $\begin{array}{r}{W_{t}=X_{t}\exp\{-\int_{0}^{t}m_{s}d s\}}\end{array}$ or $W_{t}~=~X_{t}Y_{t}$ , where $\begin{array}{r}{Y_{t}~=~\exp\{-\int_{0}^{t}m_{s}d s\}}\end{array}$ .Note that $d Y_{t}~=~-Y_{t}m_{t}d t$ so that $Y$ is a locally deterministic [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]. From Example 2.1, the dynamics of $W$ becomes  
$$
...
Simultaneously modeling the dynamics of a lot of economic quantities requires the use of a lot of shocks to those quantities. For that purpose we will work with vectors of shocks. In particular for continuous-time modeling, we will represent shocks to the economy by a vector standard [[Continuous-Time Stochastic Processes|Brownian motion]]. We define this below and state [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]] for processes of a general dimension.  

A $K$ -dimensional standard [[Continuous-Time Stochastic Processes|Brownian motion]] ${\boldsymbol{z}}=(z_{1},\dots,z_{K})^{\top}$ is a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]. where the individual components. $z_{i}$ are mutually independent one-dimensional standard Brownian motions. If we let $\mathbf{0}=(0,\ldots,0)^{\top}$ denote the zero vector in. $\mathbb{R}^{K}$ and let. $\underline{{\underline{{I}}}}$ denote the [[Prerequisites|identity matrix]] of dimension $K\times K$ (the matrix with ones in the diagonal and zeros in all other entries), then we can write the defining properties of a $K$ -dimensional [[Continuous-Time Stochastic Processes|Brownian motion]]. $_{z}$ as follows:  

(i) $z_{\mathrm{0}}=\mathbf{0}$ (ii) for all $t,t^{\prime}\geq0$ with $t<t^{\prime}$ .. $z_{t^{\prime}}-z_{t}\sim N(\mathbf{0},(t^{\prime}-t)\underline{{\underline{{I}}}})$ [normally distributed increments], (ii) for all $0\leq t_{0}<t_{1}<\dots<t_{n}$ , the random variables $z_{t_{1}}-z_{t_{0}},\dots,z_{t_{n}}-z_{t_{n-1}}$ are mutually independent [[[Continuous-Time Stochastic Processes|independent increments]]], (iv) $_{z}$ has continuous sample paths in $\mathbb{R}^{K}$  
...
of $K\times K$ -matrices. As before, $_{z}$ is a $K$ -dimensional standard [[Continuous-Time Stochastic Processes|Brownian motion]]. The evolution of.   
...
where ${\pmb\mu}=({\pmb\mu}_{t})$ is a $K$ -dimensional [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] and $\underline{{\underline{{\sigma}}}}=\left(\underline{{\underline{{\sigma}}}}_{t}\right)$ is a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] with values in the space of $K\times K$ --matrices.  

Next, we state a multi-dimensional version of [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's Lemma]], where a one-dimensional process is defined as a function of time and a multi-dimensional process.  
...
We can think of building up the model by starting with. $X_{1}$ . The shocks to $X_{1}$ are represented by. the standard [[Continuous-Time Stochastic Processes|Brownian motion]] $z_{1}$ and its coefficient $\hat{\sigma}_{11}$ is the volatility of $X_{1}$ . Then we extend the model to include $X_{2}$ . Unless the infinitesimal changes to $X_{1}$ and $X_{2}$ are always perfectly correlated we need to introduce another standard [[Continuous-Time Stochastic Processes|Brownian motion]],. $z_{2}$ . The coefficient. $\hat{\sigma}_{21}$ is fixed to match the covariance between changes to $X_{1}$ and $X_{2}$ and then $\hat{\sigma}_{22}$ can be chosen so that $\sqrt{\hat{\sigma}_{21}^{2}+\hat{\sigma}_{22}^{2}}$ equals the volatility of $X_{2}$ . The model may be extended to include additional processes in the same manner.  
