...
Now consider the [[Epstein-Zin Recursive Utility|Epstein-Zin preferences]] introduced in Section 5.7.3, where the [[Utility Indices|utility index]] $\mathcal{U}_{t}$ is specified recursively as  
$$
...
Here is the [[CCAPM with Alternative Preferences|relative risk aversion]], $b$ is the [[Epstein-Zin Recursive Utility|subjective discount factor]], and $\begin{array}{r}{\theta=(1-\gamma)/(1-\frac{1}{\psi})}\end{array}$ $\gamma$   
where $\psi$ is the elasticity of [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]].  

At time $t$ , the objective of the individual is to maximize $\mathcal{U}_{t}$ over all consumption and [[An Asset Allocation Primer|portfolio]]. strategies $(c_{t},c_{t+1},c_{t+2},.~.~)$ and $(\pi_{t},\pi_{t+1},\pi_{t+2},...)$ .Let $J_{t}$ denote the maximum value of $\mathcal{U}_{t}$  

typically referred to as the [[Epstein-Zin Recursive Utility|indirect utility]] or [[Epstein-Zin Recursive Utility|derived utility]]. The dynamic programming idea implies that  
$$
...
The [[Epstein-Zin Recursive Utility|indirect utility]] is clearly depending on wealth. The [[Epstein-Zin Recursive Utility|wealth dynamics]] is given by  
$$
...
Substituting the conjecture and the [[Epstein-Zin Recursive Utility|wealth dynamics]] into (6.66), we obtain  
$$
...
The maximization with respect to the optimal [[An Asset Allocation Primer|portfolio]] is subject to the condition that $\pi_{t}^{\top}\mathbf{1}=1$ i.e. $\pi_{1{t}}+\cdot\cdot\cdot+\pi_{N{t}}=1$ . Hence $\pi_{1t}=1-\left(\pi_{2t}+\cdot\cdot\cdot+\pi_{N t}\right)$ and the [[An Asset Allocation Primer|portfolio]] return can be written. as  
$$
...
We can now see that the optimal decisions of the individual give rise to a next-period [[Exercises|state-price deflator]] given by  
$$
\frac{\zeta_{t+1}}{\zeta_{t}}=b^{\theta}\left(\frac{c_{t+1}}{c_{t}}\right)^{-\theta/\psi}\left(R_{t+1}^{\pi_{t}}\right)^{\theta-1}.
$$  

In particular if $\gamma=1/\psi$ and thus $\theta=1$ , we are back to the traditional [[Exercises|state-price deflator]] for time-additive power utility,. $\zeta_{t+1}/\zeta_{t}=b(c_{t+1}/c_{t})^{-\gamma}$  

# 6.7 Concluding remarks  

This chapter has characterized the [[Dynamic Programming|optimal consumption]] and [[An Asset Allocation Primer|portfolio]] choice of an individual by. the first-order condition of her [[Properties of State-Price Deflators|utility maximization problem]]. The characterization provides a link. between asset prices and the [[Theoretical Factors|optimal consumption plan]] of any individual. In the next chapter we will look at the market equilibrium..  