...
[[The Simple Multi-Period Ccapm|CRRA utility]].(Also known as power utility or [[Utility Functions in Models and in Reality|isoelastic utility]].) Utility functions $u(c)$ in this class are defined for $c\geq0$  
$$
...
The [[CCAPM with Alternative Preferences|relative risk aversion]] is constant across consumption levels. $c$ , hence the name CRRA (Constant [[CCAPM with Alternative Preferences|Relative Risk Aversion]]) utility. Note that $\begin{array}{r}{u^{\prime}(0+)\equiv\operatorname*{lim}_{c\rightarrow0}u^{\prime}(c)=\infty}\end{array}$ with the consequence that an optimal solution will have the property that consumption/wealth. $c$ will be strictly above 0 with probability one. Hence, we can ignore the very appropriate non-negativity constraint on consumption since the constraint will never be binding. Furthermore, $u^{\prime}(\infty)\equiv\mathrm{lim}_{c\rightarrow\infty}u^{\prime}(c)=0$  

Some authors assume a utility function of the form. $u(c)=c^{1-\gamma}$ , which only makes sense for. $\gamma\in(0,1)$ . However, empirical studies indicate that most investors have a [[CCAPM with Alternative Preferences|relative risk aversion]] above 1, cf. the discussion below. The absolute risk tolerance is linear in $c$  
$$
\mathrm{ART}(c)={\frac{1}{\mathrm{ARA}(c)}}={\frac{c}{\gamma}}.
$$  

![](57c5a0ba7d267d50d8ac101b6f5a736c3b27c3d3d7a7998c447ca71b64817d84.jpg)  
Figure 5.1: Some [[The Simple Multi-Period Ccapm|CRRA utility]] functions.  
...
which is the important special case of [[Exercises|logarithmic utility]]. When we consider [[The Simple Multi-Period Ccapm|CRRA utility]],. we will assume the simpler version (5.6), but we will use the fact that we can obtain the optimal strategies of a log-utility investor as the limit of the optimal strategies of the general CRRA investor. as $\gamma\to1$  

Some [[The Simple Multi-Period Ccapm|CRRA utility]] functions are illustrated in Figure 5.1.  

HARA utility. (Also known as [[Utility Functions in Models and in Reality|extended power utility]].) The [[Utility Functions in Models and in Reality|absolute risk aversion]] for [[The Simple Multi-Period Ccapm|CRRA utility]] is hyperbolic in $c$ . More generally a utility function is said to be a HARA (Hyperbolic [[Utility Functions in Models and in Reality|Absolute Risk Aversion]]) utility function if  
$$
...
How do the HARA utility functions look like? First, let us take the case $\alpha=0$ , which implies that the [[Utility Functions in Models and in Reality|absolute risk aversion]] is constant (so-called CARA utility) and $\beta$ must be positive.  
$$
...
for some other constant $k_{2}$ . Applying the fact that increasing affine transformations do not change. decisions, the basic representative of this class of utility functions is the [[Utility Functions in Models and in Reality|negative exponential utility]] function  
$$
u(c)=-e^{-a c},\quad c\in\mathbb{R},
$$  

where the parameter $a=1/\beta$ is the [[Utility Functions in Models and in Reality|absolute risk aversion]]. Constant [[Utility Functions in Models and in Reality|absolute risk aversion]] is certainly not very reasonable. Nevertheless, the [[Utility Functions in Models and in Reality|negative exponential utility]] function is sometimes used for computational purposes in connection with normally distributed [[Assets|returns]], e.g. in one-period models.  
...
where $\gamma=1/\alpha<0$ . We can think of $\bar{c}$ as a satiation level and call this subclass [[Utility Functions in Models and in Reality|satiation HARA utility]] functions. The [[Utility Functions in Models and in Reality|absolute risk aversion]] is  
$$
\mathrm{ARA}(c)={\frac{-\gamma}{{\bar{c}}-c}},
$$  

which is increasing in. $c$ , conflicting with intuition and empirical studies. Some older financial models used the [[The Classical One-Period Capm|quadratic utility function]], which is the special case with $\gamma=-1$ so that $u(c)=$ $-\left(\bar{c}-c\right)^{2}$ . An equivalent utility function is. $u(c)=c-a c^{2}$  
...
which are both decreasing in $c$ . The [[CCAPM with Alternative Preferences|relative risk aversion]] approaches $\infty$ for $c\to\bar{c}$ and decreases to the constant $\gamma$ for $c\to\infty$ . Clearly, for $\bar{c}=0$ , we are back to the [[The Simple Multi-Period Ccapm|CRRA utility]] functions so that these also belong to the HARA family.  

[[Exercises|Mean-variance]] preferences. For some problems it is convenient to assume that the expected utility associated with an uncertain consumption plan only depends on the expected value and the variance of the consumption plan. This is certainly true if the consumption plan is a normally distributed random variable since its probability distribution is fully characterized by the mean and variance. However, it is generally not appropriate to use a normal distribution for consumption (or wealth or [[Some Stylized Empirical Facts About Asset Retur|asset returns]]).  

For a [[The Classical One-Period Capm|quadratic utility function]], $u(c)=c-a c^{2}$ , the expected utility is.  
$$
...
which is indeed a function of the expected value and the variance of the consumption plan. Alas, the [[The Classical One-Period Capm|quadratic utility function]] is inappropriate for several reasons. Most importantly, it exhibits increasing [[Utility Functions in Models and in Reality|absolute risk aversion]].  
...
where $u^{(n)}$ is the $n$ 'th order derivative of $u$ . Taking [[FORWARD RATES AND TERM STRUCTURE|expectations]], we get  
$$
...
Here $\mathrm{E}\left[(c-\mathrm{E}[c])^{n}\right]$ is the central moment of order $n$ . The variance is the central moment of order 2.. Obviously, a greedy investor (which just means that. $u$ is increasing) will prefer higher expected consumption to lower for fixed central moments of order 2 and higher. Moreover, a [[2. Forwards, Swaps, Futures, and Options|risk-averse investor]] (so that $u^{\prime\prime}<0$ ) will prefer lower variance of consumption to higher for fixed expected. consumption and fixed central moments of order 3 and higher. But when the central moments of order 3 and higher are not the same for all alternatives, we cannot just evaluate them on the basis of their expectation and variance. With quadratic utility, the [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] of. $u$ of order 3 and higher are zero so there it works. In general, [[Exercises|mean-variance]] preferences can only serve as an approximation of the true utility function.  

# 5.6.2 What do we know about individuals' risk aversion?  

From our discussion of risk aversion and various utility functions we expect that individuals are risk averse and exhibit decreasing [[Utility Functions in Models and in Reality|absolute risk aversion]]. But can this be supported by empirical evidence? Do individuals have constant [[CCAPM with Alternative Preferences|relative risk aversion]]? And what is a reasonable level of risk aversion for individuals?  

You can get an idea of the [[Risk Aversion|risk attitudes]] of an individual by observing how they choose between. risky alternatives. Some researchers have studied this by setting up "laboratory experiments" in which they present some risky alternatives to a group of individuals and simply see what they. prefer. Some of these experiments suggest that expected [[Expected Utility Representation of Preferences|utility theory]] is frequently violated,. see e.g. Grether and Plott (1979). However, laboratory experiments are problematic for several. reasons. You cannot be sure that individuals will make the same choice in what they know is an experiment as they would in real life. It is also hard to formulate alternatives that resemble the rather complex real-life decisions. It seems more fruitful to study actual data on how individuals have acted confronted with real-life decision problems under uncertainty. A number of studies do that.  

Friend and Blume (1975) analyze data on household asset holdings. They conclude that the data is consistent with individuals having roughly constant [[CCAPM with Alternative Preferences|relative risk aversion]] and that the coefficients of [[CCAPM with Alternative Preferences|relative risk aversion]] are "on average well in excess of one and probably in excess of two' (quote from page 900 in their paper). Pindyck (1988) finds support of a [[CCAPM with Alternative Preferences|relative risk aversion]] between 3 and 4 in a structural model of the reaction of stock prices to fundamental variables.  

Other studies are based on insurance data. Using U.S. data on so-called property/liability. insurance, Szpiro (1986) finds support of [[The Simple Multi-Period Ccapm|CRRA utility]] with a [[Theory Meets Data - Asset Pricing Puzzles|relative risk aversion coefficient]]. between 1.2 and 1.8.  Cicchetti and Dubin (1994) work with data from the U.S. on whether. individuals purchased an insurance against the risk of trouble with their home telephone line. They conclude that the data is consistent with expected [[Expected Utility Representation of Preferences|utility theory]] and that a subsistence [[Utility Functions in Models and in Reality|HARA utility function]] performs better than log utility or [[Utility Functions in Models and in Reality|negative exponential utility]]..  

Ogaki and Zhang (2001) study data on individual food consumption from Pakistan and India. and conclude that [[CCAPM with Alternative Preferences|relative risk aversion]] is decreasing for poor individuals, which is consistent with a subsistence [[Utility Functions in Models and in Reality|HARA utility function]]..  

It is an empirical fact that even though consumption and wealth have increased tremendously over the years, the magnitude of real rates of return has not changed dramatically. As indicated by (5.5) relative risk premia are approximately proportional to the [[CCAPM with Alternative Preferences|relative risk aversion]]. As we. shall see in later chapters, basic [[Mean-Variance Efficient Returns and Pricing Fac|asset pricing theory]] implies that relative risk premia on financial assets (in terms of expected real return in excess of the real [[A Preview of Alternative Formulations|risk-free return]]) will be proportional to the "average" [[CCAPM with Alternative Preferences|relative risk aversion]] in the economy. If the "average' [[CCAPM with Alternative Preferences|relative risk aversion]] was. significantly decreasing (increasing) in the level of consumption or wealth, we should have seen. decreasing (increasing) real [[Assets|returns]] on risky assets in the past. The data seems to be consistent. with individuals having "on average" close to [[The Simple Multi-Period Ccapm|CRRA utility]]..  

To get a feeling of what a given risk aversion really means, suppose you are confronted with two [[Exercises|consumption plans]]. One plan is a sure consumption of $c$ , the other plan gives you $(1-\alpha)\bar{c}$ with probability 0.5 and $(1+\alpha)\bar{c}$ with probability 0.5. If you have a [[The Simple Multi-Period Ccapm|CRRA utility]] function  
...
$u(c)=c^{1-\gamma}/(1-\gamma)$ , the [[Risk Aversion|certainty equivalent]] $c^{*}$ of the risky plan is determined by  
$$
...
Both the [[Risk Aversion|certainty equivalent]] and the risk premium are thus proportional to the consumption level $c$ . The relative risk premium. $\lambda(\bar{c},\alpha)/\bar{c}$ is simply one minus the relative [[Risk Aversion|certainty equivalent]] $c^{*}/\bar{c}$ . These equations assume $\gamma\neq1$ . In Exercise 5.5 you are asked to find the [[Risk Aversion|certainty equivalent]] and risk premium for log-utility corresponding to $\gamma=1$  

Table 5.5 shows the relative risk premium for various values of the [[Theory Meets Data - Asset Pricing Puzzles|relative risk aversion coefficient]]. $\gamma$ and various values of. $\alpha$ , the "size' of the risk. For example, an individual with $\gamma=5$ is willing to sacrifice 2.43% of the safe consumption in order to avoid a fair gamble of. $10\%$ of that consumption. level. Of course, even extremely risk averse individuals will not sacrifice more than they can loose but in some cases it is pretty close. Looking at these numbers, it is hard to believe in. $\gamma$ -values outside, say, [1, 10]. In Exercise 5.6 you are asked to compare the exact relative risk premia shown in the table with the approximate risk premia given by (5.5)..  
...
so that the slope of the indifference curve (also known as the [[The One-Period Ccapm|marginal rate of substitution]]) is  
$$
{\frac{d z}{d c}}=-{\frac{\frac{\partial f}{\partial z}}{\frac{\partial f}{\partial c}}}.
$$  

Unless the indifference curve is linear, its slope will change along the curve. Indifference curves are generally assumed to be convex. The elasticity of substitution tells you by which percentage you need to change $z/c$ in order to obtain a one percent change in the slope of the indifference curve. It is a measure of the curvature or [[PSET II Fixed Income Asset Pricing 1|convexity]] of the indifference curve. If the indifference curve is very curved, you only have to move a little along the curve before its slope has changed by one percent. Hence, the elasticity of substitution is low. If the indifference curve is almost linear, you have to move far away to change the slope by one percent. In that case the elasticity of substitution is. very high. Formally, the elasticity of substitution is defined as  
$$
