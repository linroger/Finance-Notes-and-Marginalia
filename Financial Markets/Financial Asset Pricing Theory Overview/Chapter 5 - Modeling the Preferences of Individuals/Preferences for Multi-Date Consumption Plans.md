...
Above we implicitly considered preferences for consumption at one given future point in time. We need to generalize the ideas and results to settings with consumption at several dates. In one-period models individuals can consume both at time 0 (beginning-of-period) and at time 1 (end-of-period). In multi-period models individuals can consume either at each date in the discrete time set ${\mathcal{T}}=\{0,1,2,\ldots,T\}$ or at each date in the continuous time set $\mathcal{T}=[0,T]$ . In any case a consumption plan is a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] $c=(c_{t})_{t\in\mathcal{T}}$ where each $c_{t}$ is a random variable representing the state-dependent level of consumption at time $t$  

Consider the discrete-time case and, for each $t$ , let $Z_{t}\subseteq\mathbb{R}$ denote the set of all possible consumption levels at date $t$ and define $Z=Z_{0}\times Z_{1}\times\cdot\cdot\cdot\times Z_{T}\subseteq\mathbb{R}^{T+1}$ , then any consumption plan $c$ can again be represented by a probability distribution $\pi$ on the set $Z$ . For finite $Z$ , we can again apply Theorem 5.1 so that under the relevant axioms, we can represent preferences by a [[Utility Indices|utility index]] $\mathrm{\mathcal{U}}$ which to each consumption plan. $(c_{t})_{t\in\mathbb{T}}=(c_{0},c_{1},...,c_{T})$ attaches a real number $\mathcal{U}(c_{0},c_{1},\dots,c_{T})$ with higher numbers to the more preferred [[Exercises|consumption plans]]. If we further impose the [[Expected Utility Representation of Preferences|Substitution Axiom]], Theorem 5.2 ensures an [[Preferences for Multi-Date Consumption Plans|expected utility representation]], i.e. the existence of a utility function $U:Z\rightarrow\mathbb{R}$ so that [[Exercises|consumption plans]] are ranked according to their expected utility, i.e..  
$$
...
In addition it is typically assumed that. $u_{t}(c_{t})=e^{-\partial t}u(c_{t})$ for all $t$ . This is to say that the direct. utility the individual gets from a given consumption level is basically the same for all dates, but the individual prefers to consume any given number of goods sooner than later. This is modeled by the [[Preferences for Multi-Date Consumption Plans|subjective time preference rate]] $\delta$ , which we assume to be constant over time and independent. of the consumption level. More impatient individuals have higher. $\delta$ 's. In sum, the life-time utility. is typically assumed to be given by.  
$$
U(c_{0},c_{1},\dots,c_{T})=\sum_{t=0}^{T}e^{-\delta t}u(c_{t})
$$  

in [[Financial Mathematics Course|discrete-time models]] and  
$$
U((c_{t})_{t\in[0,T]})=\int_{0}^{T}e^{-\delta t}u(c_{t})d t
$$  

in [[Financial Mathematics Course|continuous-time models]]. In both cases, $u$ is a "single-date' utility function such as those discussed in Section 5.6.  

Time-additivity is mostly assumed for tractability. However, it is important to realize that the time-additive specification does not follow from the basic axioms of choice under uncertainty, but. is in fact a strong assumption, which most economists agree is not very realistic. One problem is that time-additive preferences induce a close link between the reluctance to substitute consumption across different states of the economy (which is measured by risk aversion) and the willingness to substitute consumption over time (which can be measured by the so-called elasticity of [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]]). Solving intertemporal [[Prerequisites|utility maximization]] problems of individuals. with [[Consumption-Based Asset Pricing with Incomplete Markets|time-additive CRRA utility]], it turns out that an individual with a high [[CCAPM with Alternative Preferences|relative risk aversion]] will also choose a very smooth consumption process, i.e. she will have a low elasticity of [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]]. There is nothing in the basic theory of choice that links the risk aversion and the elasticity of [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]] together. For one thing, risk aversion makes sense even in an atemporal (i.e. one-date) setting where [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]] is meaningless and, conversely, [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]] makes sense in a multi-period setting without uncertainty in which risk aversion is meaningless. The close link between the two concepts in the [[Information|multi-period model]] with uncertainty is an unfortunate consequence of the assumption of [[General Multi-Period Link Between Consumption a|time-additive expected utility]].  
...
turn out to be computationally tractable. This is closely related to the subsistence HARA utility, but with [[CCAPM with Alternative Preferences|habit formation]] the "subsistence level" $h$ is endogenously determined by past consumption. The corresponding absolute and relative risk aversions are  
$$
...
where $u_{c}$ and $u_{c c}$ are the first- and second-order [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] of $u$ with respect to $c$ . In particular, the [[CCAPM with Alternative Preferences|relative risk aversion]] is decreasing in $c$ . Note that the [[CCAPM with Alternative Preferences|habit formation]] preferences are still consistent with expected utility.  

A related line of extension of the basic preferences is to allow the preferences of an individual. to depend on some external factors, i.e. factors that are not fully determined by choices made by the individual. One example that has received some attention is where the utility which some individual attaches to her consumption plan depends on the [[Exercises|consumption plans]] of other individuals or maybe the [[Pareto-Optimality in Some Incomplete Markets|aggregate consumption]] in the economy. This is often referred to as "keeping up with the Jones'es."' If you see your neighbors consume at high rates, you want to consume at a high rate too. Utility is state-dependent. Models of this type are sometimes said to have an external habit, whereas the [[CCAPM with Alternative Preferences|habit formation]] discussed above is then referred to as internal habit.. If we denote the external factor by $X_{t}$ , a time-additive life-time [[Preferences for Multi-Date Consumption Plans|expected utility representation]] is $\operatorname{E}[\sum_{t=0}^{T}e^{-\delta t}u(c_{t},X_{t})]$ , and a tractable version is $\begin{array}{r}{u(c,X)=\frac{1}{1-\gamma}\left(c-X\right)^{1-\gamma}}\end{array}$ very similar to the subsistence CRRA or the specific [[The Discrete-Time Framework|habit formation utility]] given above. In this case, however, the "subsistence" level is determined by external factors. Another tractable specification is $u(c,X)=$ $\frac{1}{1-\gamma}(c/X)^{1-\gamma}$  

# 5.7.3 Recursive utility  

Another [[Utility Indices|preference]] specification gaining popularity is the so-called recursive preferences or [[Epstein-Zin Recursive Utility|Epstein-Zin preferences]], suggested and discussed by, e.g., Kreps and Porteus (1978), Epstein and Zin (1989, 1991), and Weil (1989). The original motivation of this representation of preferences is that it allows individuals to have preferences for the timing of resolution of uncertainty, which is not consistent with the standard multi-date expected [[Expected Utility Representation of Preferences|utility theory]] and violates the set of behavioral axioms.  

In a [[Multi-Period Valuation Models|discrete-time framework]] Epstein and Zin (1989, 1991) assumed that life-time utility from time $t$ on is captured by a [[Utility Indices|utility index]] $\mathcal{U}_{t}$ (in this literature sometimes called the "felicity") satisfying the recursive relation  
$$
\mathcal{U}_{t}=f(c_{t},z_{t}),
$$  

where $z_{t}=C E_{t}(\mathcal{U}_{t+1})$ is the [[Risk Aversion|certainty equivalent]] of. $\mathcal{U}_{t+1}$ given information available at time $t$ and $f$ is an aggregator on the form.  
$$
f(c,z)=(a c^{\alpha}+b z^{\alpha})^{1/\alpha}.
$$  

The aggregator is identical to the two-good CES utility specification (5.13) and, since $z_{t}$ here refers to future consumption or utility, $\psi=1/(1-\alpha)$ is called the elasticity of [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]]..  

An investor's willingness to substitute risk between states is modeled through $z_{t}$ as the [[Risk Aversion|certainty equivalent]] of a constant [[CCAPM with Alternative Preferences|relative risk aversion]] utility function. Recall that the [[Risk Aversion|certainty equivalent]] for an atemporal utility function $u$ is defined as  
$$
C E=u^{-1}\left(\operatorname{E}[u(x)]\right).
$$  

In particular for [[The Simple Multi-Period Ccapm|CRRA utility]] $u(x)=x^{1-\gamma}/(1-\gamma)$ we obtain  
$$
C E=\left(\mathrm{E}[x^{1-\gamma}]\right)^{\frac{1}{1-\gamma}},
$$  

where $\gamma>0$ is the [[CCAPM with Alternative Preferences|relative risk aversion]].  

To sum up, [[Epstein-Zin Recursive Utility|Epstein-Zin preferences]] are specified recursively as  
$$
...
Hence, it is clear that the [[Utility Indices|utility index]] $\tilde{\mathcal{U}}$ defined for any $t$ by $\dot{\mathcal{U}}_{t}=a^{-1/\alpha}\mathcal{U}_{t}$ is equivalent to the [[Utility Indices|utility index]] $\mathrm{\mathcal{U}}$ , since it is just a scaling. Without loss of generality we could put $a=1$  

Time-additive power utility as a special case. The special case $\gamma=1/\psi$ corresponds to time-additive expected [[The Simple Multi-Period Ccapm|CRRA utility]]. In order to see this, first note that with $\gamma=1/\psi$ , we have $\theta=1$ and thus  
$$
...
Any strictly increasing function of a [[Utility Indices|utility index]] will represent the same preferences. In particular,  
$$
...
represents the same preferences and is clearly equivalent to [[General Multi-Period Link Between Consumption a|time-additive expected utility]]. Note that $b$ plays the role of the [[Epstein-Zin Recursive Utility|subjective discount factor]]..  

The [[Epstein-Zin Recursive Utility|Epstein-Zin preferences]] are characterized by three parameters: the [[CCAPM with Alternative Preferences|relative risk aversion]] $\gamma$ , the elasticity of [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]] $\psi$ , and the [[Epstein-Zin Recursive Utility|subjective discount factor]] $b$ . Relative to the standard time-additive power utility, the Epstein-Zin specification allows the [[CCAPM with Alternative Preferences|relative risk aversion]] (attitudes towards atemporal risks) to be disentangled form the elasticity of [[Preferences for Multi-Date Consumption Plans|intertemporal substitution]] (attitudes towards shifts in consumption over time). Moreover, Epstein and Zin (1989) shows that when $\gamma>1/\psi$ , the individual will prefer early resolution of uncertainty. If $\gamma<1/\psi$ late resolution of uncertainty is preferred. For the standard utility case $\gamma=1/\psi$ , the individual is indifferent about the timing of the resolution of uncertainty. Note that in the relevant case of $\gamma>1$ , the auxiliary parameter $\theta$ will be negative if and only if $\psi>1$ . Empirical studies disagree about reasonable values of $\psi$ . Some studies find $\psi$ smaller than one (e.g. Campbell 1999), other studies find $\psi$ greater than one (e.g. Vissing-Jorgensen and Attanasio 2003).  
