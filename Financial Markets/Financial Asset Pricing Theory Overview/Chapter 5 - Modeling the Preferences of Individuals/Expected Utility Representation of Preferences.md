...
Utility indices are functions of [probability distributions](Exercises.md) on the set of possible consumption levels. With many states of the world and many assets to trade in, the set of such [probability distributions](Exercises.md) will be very, very large. This will significantly complicate the analysis of optimal choice using  
...
We say that a [preference relation](Utility%20Indices.md). $\succeq$ on $\mathcal{P}(Z)$ has an [expected utility representation](Preferences%20for%20Multi-Date%20Consumption%20Plans.md) if there exists a function $u:Z\to\mathbb{R}$ such that  
$$
\pi_{1}\succeq\pi_{2}\quad\Leftrightarrow\quad\sum_{z\in Z}\pi_{1}(z)u(z)\geq\sum_{z\in Z}\pi_{2}(z)u(z).
$$  

Here $\textstyle\sum_{z\in Z}\pi(z)u(z)$ is the expected utility of end-of-period consumption given the consumption probability distribution $\pi$ . The function $u$ is called a von Neumann-Morgenstern utility function or simply a utility function. Note that $u$ is defined on the set $Z$ of consumption levels, which in general has a simpler structure than the set of [probability distributions](Exercises.md) on $Z$ . Given a utility function $u$ , we can obviously define a [utility index](Utility%20Indices.md) by $\begin{array}{r}{\mathcal{U}(\pi)=\sum_{z\in Z}\pi(z)u(z)}\end{array}$  

# 5.4.1 Conditions for expected utility  

When can we use an [expected utility representation](Preferences%20for%20Multi-Date%20Consumption%20Plans.md) of a [preference relation](Utility%20Indices.md)? The next lemma is a first step.  

Lemma 5.2 A [preference relation](Utility%20Indices.md) $\succeq$ has an [expected utility representation](Preferences%20for%20Multi-Date%20Consumption%20Plans.md) if and only if it can be represented by a linear [utility index](Utility%20Indices.md). $\mathfrak{U}$ in the sense that.  
$$
...
Proof: Suppose that $\succeq$ has an [expected utility representation](Preferences%20for%20Multi-Date%20Consumption%20Plans.md) with utility function $u$ .Define $\mathcal{U}:\mathcal{P}(Z)\to\mathbb{R}$ by $\begin{array}{r}{\mathcal{U}(\pi)=\sum_{z\in Z}\pi(z)u(z)}\end{array}$ . Then clearly $\mathrm{\mathcal{U}}$ is a [utility index](Utility%20Indices.md) representing $\succeq$ and $\mathrm{\mathcal{U}}$ is linear since.  
$$
...
Conversely, suppose that $\mathrm{\mathcal{U}}$ is a linear [utility index](Utility%20Indices.md) representing $\succeq$ . Define a function $u:Z\to\mathbb{R}$ by $u(z)=\mathcal{U}(\mathbf{1}_{z})$ . For any $\pi\in{\mathcal{P}}(Z)$ we have  
$$
...
Table 5.3: The [probability distributions](Exercises.md) used in the illustration of the [Substitution Axiom](.md).  

representation of $\succeq$  

The question then is under what assumptions the [preference relation](Utility%20Indices.md). $\succeq$ can be represented by a linear [utility index](Utility%20Indices.md). As shown by von Neumann and Morgenstern (1944) we need an additional axiom, the so-called [Substitution Axiom](.md)..  
...
The [Substitution Axiom](.md) is sometimes called the Independence Axiom or the Axiom of the Irrelevance of the Common Alternative. Basically, it says that when the individual is to compare two. [probability distributions](Exercises.md), she need only consider the parts of the two distributions which are different from each other. As an example, suppose the possible consumption levels are. $Z=\{1,2,3,4\}$ and consider the [probability distributions](Exercises.md) on $Z$ given in Table 5.3. Suppose you want to compare the distributions $\pi_{4}$ and $\pi_{5}$ . They only differ in the probabilities they associate with consumption levels 2, 3, and 4 so it should only be necessary to focus on these parts. More formally observe. that  
$$
\pi_{4}\sim0.5\pi_{1}+0.5\pi_{3}~\mathrm{and}~\pi_{5}\sim0.5\pi_{2}+0.5\pi_{3}.
$$  
$\pi_{1}$ is the conditional distribution of. $\pi_{4}$ given that the consumption level is different from 1 and $\pi_{2}$ is the conditional distribution of. $\pi_{5}$ given that the consumption level is different from 1. The [Substitution Axiom](.md) then says that.  
$$
\pi_{4}\succ\pi_{5}\quad\Leftrightarrow\quad\pi_{1}\succ\pi_{2}.
$$  

The next lemma shows that the [Substitution Axiom](.md) is more restrictive than the [Monotonicity Axiom](Utility%20Indices.md).  

Lemma 5.3 If a [preference relation](Utility%20Indices.md). $\succeq$ satisfies the [Substitution Axiom](.md), it will also satisfy the [Monotonicity Axiom](Utility%20Indices.md)..  
...
First assume that $a>b$ . Observe that it follows from the [Substitution Axiom](.md) that  
$$
a\pi_{1}+(1-a)\pi_{2}\succ a\pi_{2}+(1-a)\pi_{2}
$$  

and hence that $a\pi_{1}+(1-a)\pi_{2}\succ\pi_{2}$ . Also from the [Substitution Axiom](.md) we have that for any $\pi_{3}\succ\pi_{2}$ , we have  
$$
...
Theorem 5.2 Assume that $Z$ is finite and that $\succeq$ is a [preference relation](Utility%20Indices.md) on $\mathcal{P}(Z)$ . Then  can be represented by a linear [utility index](Utility%20Indices.md) if and only $i f\succeq$ satisfies the [Archimedean Axiom](Utility%20Indices.md) and the [Substitution Axiom](.md).  

Proof: First suppose the [preference relation](Utility%20Indices.md). $\succeq$ satisfies the [Archimedean Axiom](Utility%20Indices.md) and the [Substitution Axiom](.md). Define a [utility index](Utility%20Indices.md). $\mathcal{U}:\mathcal{P}(Z)\to\mathbb{R}$ exactly as in the proof of Theorem 5.1, i.e.. $\mathcal{U}(\pi)=\alpha_{\pi}$ , where $\alpha_{\pi}\in[0,1]$ is the unique number such that.  
$$
\boldsymbol{\pi}\sim\alpha_{\pi}\mathbf{1}_{z^{u}}+(1-\alpha_{\pi})\mathbf{1}_{z^{l}}.
$$  

We want to show that, as a consequence of the [Substitution Axiom](.md), $\mathrm{\mathcal{U}}$ is indeed linear. For that purpose, pick any two [probability distributions](Exercises.md) $\pi_{1},\pi_{2}\in\mathcal{P}(Z)$ and any number $a\in[0,1]$ . We want to show that $\mathcal{U}\left(a\pi_{1}+(1-a)\pi_{2}\right)=a\mathcal{U}(\pi_{1})+(1-a)\mathcal{U}(\pi_{2})$ . We can do that by showing that  
$$
...
This follows from the [Substitution Axiom](.md):  
$$
...
Now let us show the converse, i.e. if $\succeq$ can be represented by a linear [utility index](Utility%20Indices.md) $\mathrm{\mathcal{U}}$ , then it must satisfy the [Archimedean Axiom](Utility%20Indices.md) and the [Substitution Axiom](.md). In order to show the [Archimedean Axiom](Utility%20Indices.md), we pick $\pi_{1}\succ\pi_{2}\succ\pi_{3}$ , which means that $\mathcal{U}(\pi_{1})>\mathcal{U}(\pi_{2})>\mathcal{U}(\pi_{3})$ , and must find numbers $a,b\in(0,1)$ such that  
$$
...
In order to show the [Substitution Axiom](.md), we take $\pi_{1},\pi_{2},\pi_{3}\in\mathcal{P}(Z)$ and any number $a\in(0,1]$ We must show that $\pi_{1}\succ\pi_{2}$ if and only if $a\pi_{1}+(1-a)\pi_{3}\succ a\pi_{2}+(1-a)\pi_{3}$ , i.e.  
$$
...
The next theorem shows which utility functions that represent the same [preference relation](Utility%20Indices.md). The proof is left for the reader as Exercise 5.1..  

Theorem 5.3 A utility function for a given [preference relation](Utility%20Indices.md) is only determined up to a strictly increasing affine transformation, i.e. if $u$ is a utility function for $\succeq$ , then v will be so if and only if there exist constants $a>0$ and b such that $v(z)=a u(z)+b$ for all $z\in Z$  

If one utility function is an affine function of another, we will say that they are equivalent. Note that an easy consequence of this theorem is that it does not really matter whether the utility is positive or negative. At first, you might find negative utility strange but we can always add a sufficiently large positive constant without affecting the ranking of different [consumption plans](Exercises.md)..  

Suppose $\mathrm{\mathcal{U}}$ is a [utility index](Utility%20Indices.md) with an associated utility function $u$ . If $f$ is any [strictly increasing transformation](Utility%20Indices.md), then $V=f\circ\mathcal{U}$ is also a [utility index](Utility%20Indices.md) for the same preferences, but $f\circ u$ is only the utility function for $V$ if $f$ is affine.  

The expected utility associated with a probability distribution $\pi$ on $Z$ is $\textstyle\sum_{z\in Z}\pi(z)u(z)$ . Recall that the [probability distributions](Exercises.md) we consider correspond to [consumption plans](Exercises.md). Given a consumption plan, i.e. a random variable. $c$ , the associated probability distribution is defined by the probabilities  
$$
...
Infinite $Z$ .What if $Z$ is infinite, e.g. $Z=\mathbb{R}_{+}\equiv\left\lvert0,\infty\right\rvert$ ? It can be shown that in this case a [preference relation](Utility%20Indices.md) has an [expected utility representation](Preferences%20for%20Multi-Date%20Consumption%20Plans.md) if the [Archimedean Axiom](Utility%20Indices.md), the [Substitution Axiom](.md), an additional axiom ("the sure thing principle"), and "some technical conditions" are satisfied. Fishburn (1970) gives the details.  
...
no problem if $\mathbb{R}_{+}\subseteq Z$ $u$ is concave, and [consumption plans](Exercises.md) have finite [expectations](../../../Fixed%20Income%20Asset%20Pricing/Fixed%20Income%20Lecture%20Notes/FORWARD%20RATES%20AND%20TERM%20STRUCTURE.md): $u$ concave $\Rightarrow u$ is differentiable in some point $b$ and  
$$
u(z)\leq u(b)+u^{\prime}(b)(z-b),\quad\forall z\in Z.
$$  

If the consumption plan $c$ has finite [expectations](../../../Fixed%20Income%20Asset%20Pricing/Fixed%20Income%20Lecture%20Notes/FORWARD%20RATES%20AND%20TERM%20STRUCTURE.md), then  
$$
\operatorname{E}[u(c)]\leq\operatorname{E}[u(b)+u^{\prime}(b)(c-b)]=u(b)+u^{\prime}(b)\left(\operatorname{E}[c]-b\right)<\infty.
$$  

Table 5.4: The [probability distributions](Exercises.md) used in the illustration of the Allais Paradox.   
...
Subjective probability. We have taken the probabilities of the states of nature as exogenously given, i.e. as objective probabilities. However, in real life individuals often have to form their own probabilities about many events, i.e. they form subjective probabilities. Although the analysis is a bit more complicated, Savage (1954) and Anscombe and Aumann (1963) show that the results we developed above carry over to the case of subjective probabilities. For an [introduction](../../../Financial%20Markets%20and%20Institutions/III.%20Liquidity%20of%20Assets/Class%209-%20Bailouts%20and%20Bank%20Failures/Squam%20Lake%20Group%20Introduction.md) to this analysis, see Kreps (1990, Ch. 3).  

# 5.4.3 Are the axioms reasonable?  

The validity of the [Substitution Axiom](.md), which is necessary for obtaining the [expected utility representation](Preferences%20for%20Multi-Date%20Consumption%20Plans.md), has been intensively discussed in the literature. Some researchers have conducted experiments in which the decisions made by the participating individuals conflict with the [Substitution Axiom](.md).  

The most famous challenge is the so-called Allais Paradox named after Allais (1953). Here is one example of the paradox. Suppose $Z=\{0,1,5\}$ . Consider the [consumption plans](Exercises.md) in Table 5.4.. The [Substitution Axiom](.md) implies that. $\pi_{1}\succ\pi_{2}\Rightarrow\pi_{4}\succ\pi_{3}$ . This can be seen from the following:.  
$$
