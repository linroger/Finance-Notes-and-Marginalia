...
- **7. Harmonic Mean Return**
  $$\overline{X}_H = \frac{T}{\sum_{t=1}^{T} \frac{1}{X_t}} \quad (X_t > 0)$$
...
- **3. Conditional Expected Value**
  ...
  | Variable | Definition |
  |----------|------------|
  | $P(X_i | S)$ | Probability of $X_i$ given event $S$ |
...
- **2. Estimated Slope**
  $$\hat{b}_1 = \frac{\sum_{i=1}^{n} (X_i - \overline{X})(Y_i - \overline{Y})}{\sum_{i=1}^{n} (X_i - \overline{X})^2} = \frac{s_{XY}}{s_X^2}$$
...
- **7. Hypothesis Test of Slope**
  $$t = \frac{\hat{b}_1 - B_1}{s_{\hat{b}_1}} \quad ; \quad df = n - k - 1$$
  $$s_{\hat{b}_1} = \frac{s_e}{\sqrt{\sum_{i=1}^{n} (X_i - \overline{X})^2}}$$
  | Variable | Definition |
  |----------|------------|
...
- **8. Prediction Interval**
  $$\hat{Y}_f \pm t_{\alpha/2, df} \times s_f$$
