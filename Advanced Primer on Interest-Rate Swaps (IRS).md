---
title: Advanced Primer on Interest-Rate Swaps IRS
tags:
- apt
- arbitrage
- basel
...
- Backtesting and stress testing methodologies
...
- $P(t, T)$: Price $\$a_t$$ time $t$ of zero-coupon bond maturing $$a_t$$ $T$
...
$\$df(t, T) = \\alpha(t, T)$$d_t$$ + \\sigma(t, T)dW_t$$
...
A forward-starting swap begins $\$a_t$$ a future date $T_{\text{start}} > t_0$. Applications include:
...
A swaption grants the right (not obligation) to enter a swap $\$a_t$$ a predetermined rate. Under the Black model:
