...
In Chapter 1, we stated the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|fundamental theorem]] of calculus as follows:  
...
This proof is of special interest as it has direct connections to our presentation of [[6. A Brief Introduction to Stochastic Calculus|stochastic calculus]] later in the book. Since the function $f(x)$ is continuous, then $f(x_{i}^{*})\approx f(x_{i})$ when $\Delta x$ is small. In fact the error in this approximation is negligible. We will not prove this fact as it is beyond the level of this text. So, we can replace. $x_{i}^{\phantom{\dagger}}$ with $x_{i}$ and need to prove that:.  
