...
These lecture notes introduce [[6. A Brief Introduction to Stochastic Calculus|fixed income derivative securities]] and the modeling philosophy used to price these securities. We will use [[6. A Brief Introduction to Stochastic Calculus|binomial lattice models]] for doing this and the securities we will consider include bond [[Chapter 1 - Derivative Securities|futures and forwards]],  caplets and caps,  floorlets and [[Caps and Floors|floors]],  and swaps and swaptions.

Fixed-income models are inherently more complex than security models as we need to model the evolution of the entire term-structure of [[Interest Rate Quotations|interest rates]]. The short-rate,  rt,  is the variable of interest in many [[Lecture Notes Bonds,  Preferred Stock,  and Structured Products|fixed income]] models and we will focus on this in our [[Binomial Option Pricing Model|lattice models]]. Our [[Arbitrage Pricing of Derivatives|pricing]] "philosophy" will be to simply specify [[Financial Instruments|risk-neutral probabilities]] for the short-rate,  rt,  and to do so without any reference to the true dynamics of the short-rate.

This is in contrast to the way we developed [[Financial Mathematics Course|risk-neutral pricing]] in the [[Binomial Option Pricing|binomial model]] for stocks. In the [[Binomial Option Pricing Model|lattice models]] of these notes,  however,  we will will price securities in such a way that guarantees no-[[Arbitrage Pricing of Derivatives|arbitrage]].

We also note that since the [[Squam Lake Group Letter|financial crisis]] of 2008 / 2009 there have been substantial developments in [[Hull White Term Structure Simulations with QuantLib Python|term structure modeling]]. In particular (and in contrast to the pre-crisis world) it is now widely recognized that the [[Arbitrage Pricing of Derivatives|pricing]] of LIBOR-based [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] such as swaps,  swaptions,  caps,  [[Caps and Floors|floors]] etc. require a so-called multi-curve framework with a separate curve necessary for each tenor. We do not develop the multi-curve framework in these notes as our goal is simply to learn the basics of [[Hull White Term Structure Simulations with QuantLib Python|term structure modeling]] and [[6. A Brief Introduction to Stochastic Calculus|fixed income derivatives]] [[Arbitrage Pricing of Derivatives|pricing]].

## 1 Binomial-Lattice Models

We begin with [[A Real-Life Option Pricing Exercise|binomial]]-[[Binomial Option Pricing Model|lattice models]] of the [[An Overview of the Vasicek Short Rate Model|short rate]]. These models may be viewed as models in their own right or as approximations to more sophisticated continuous-time one-factor short-rate models. These models will also be used to introduce various interest rate [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] that are commonly traded in the [[Financial Markets and Institutions Lecture Notes|financial markets]].

## 1.1 Constructing An Arbitrage-Free Lattice

Consider the [[6. A Brief Introduction to Stochastic Calculus|binomial lattice]] below where we specify the [[An Overview of the Vasicek Short Rate Model|short rate]],  ri,  j,  that will apply for the single period beginning at node N(*i,  j*). This means for example that if $1 is deposited in the cash account at t = i,   state j,
...
t = 0 t = 1 t = 2 t = 3 t = 4
We use [[7. Black Scholes Model|martingale pricing]] on this lattice to compute security prices. For example,  if Si(j) is the value of a non-dividend / coupon1 paying security at time i and state j,  then we *insist* that
$$S_{i}(j)\;=\;\frac{1}{1+r_{i,  j}}\left[q_{u}\;S_{i+1}(j+1)+q_{d}\;S_{i+1}(j)\right]\tag{1}$$$$(1)$$

where qu and qd are the probabilities of upand down-moves,  respectively. If the security pays a coupon should be included in the right-hand side of (1). Such a model is [[Arbitrage Pricing of Derivatives|arbitrage]]-free *by construction*.
...
and the short-rate lattice are specified. In the [[An Overview of the Vasicek Short Rate Model|short rate]]-lattice below (where the [[An Overview of the Vasicek Short Rate Model|short rate]] increases by a factor of u = 1.25 or decreases by a factor of d =0.\1 in each period),  we assume that the Q-probability of each branch is0.\1 and node-independent. We can then use martingale-[[Arbitrage Pricing of Derivatives|pricing]] to compute the prices of zero-coupon bonds.

| [[An Overview of the Vasicek Short Rate Model|Short Rate]] Lattice   | 0.183   |       |       |       |       |
|----------------------|---------|-------|-------|-------|-------|
|                      | 0.146   | 0.132 |       |       |       |
|                      | 0.117   | 0.105 | 0.095 |       |       |
...
Example 1 ([[Arbitrage Pricing of Derivatives|Pricing]] a Zero-Coupon Bond)
...
Note that given the price of the 4-period zero-coupon bond,  we can now find the 4-period [[The Foreign Exchange Market Annotations|spot rate]],  s4. It satisfies 77.22 = 1/(1 + s4)
...
We now introduce and price several interest-rate [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] using the straightforward [[7. Black Scholes Model|martingale pricing]] methodology. (After we derive the *forward equations* in a later section,  we will see an even easier and more efficient2 method for [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|pricing derivatives]].) The following examples will be based on the short-rate lattice and the corresponding zero-coupon bond of Example 1.

Example 2 ([[Arbitrage Pricing of Derivatives|Pricing]] a European Call Option on a Zero-Coupon Bond)
...
Example 3 ([[Arbitrage Pricing of Derivatives|Pricing]] an American Put Option on a Zero-Coupon Bond) We want to compute the price of an American put option on the same zero-coupon bond. The [[Risk Neutral Pricing of Options|expiration date]] is t = 3 and the strike is $88. Again the price is computed by backwards induction on the lattice below,  where the maximum of the *continuation value* and exercise value is equal to the option value at each node.

## Futures Contracts On Bonds

Let Fk be the date k price of a [[Futures Not Subject to Cash-And-Carry|futures]] contract written on a particular underlying security in a complete3 market model. We assume that the contract expires after n periods and we let Sk denote the time k price of the security. Then we know that Fn = Sn,  i.e.,  at expiration the [[Futures Price and the Quality Option Before E|futures price]] and the security price must coincide.

We can compute the [[Futures Price and the Quality Option Before E|futures price]] at t = n − 1 by recalling that anytime we enter a [[Futures Not Subject to Cash-And-Carry|futures]] contract,  the initial value of the contract is 0. Therefore the [[Futures Price and the Quality Option Before E|futures price]],  Fn−1,  at date t = n − 1 must satisfy
$$\begin{array}{r c l}{{0}}&{{=}}&{{\mathrm{E}_{n-1}^{Q}\left[\frac{F_{n}-F_{n-1}}{B_{n}}\right]}}\end{array}$$

2Of course the forward equations are themselves derived using [[7. Black Scholes Model|martingale pricing]] so they really add nothing new to the theory.

3A complete market is assumed so that we can uniquely price any security and more generally,  the [[Futures Price and the Quality Option Before E|futures price]] process. We could,  however,  have assumed markets were incomplete and still compute the [[Futures Price and the Quality Option Before E|futures price]] process as long as certain securities were replicable.
...
k
[Fk+1] for 0 ≤ *k < n*. We can then use the [[General Risk-Adjusted Probability Measures|law of iterated expectations]] to see that F0 = EQ
0
[Fn],  implying in particular that the [[Futures Price and the Quality Option Before E|futures price]] process is a martingale. Since Fn = Sn we have
$$\overline{{{\begin{array}{c}{F_{0}\ =\ \mathrm{E}_{0}^{Q}\left[S_{n}\right].}\end{array}}}$$$$\left(2\right)$$

[Sn]. (2)
Remark 1 Note that the above argument holds regardless of whether or not the underlying security pays dividends or coupons as long as the settlement price,  Sn*,  is ex-dividend. In particular,  we can use (2) to price* [[Futures Not Subject to Cash-And-Carry|futures]] on zero-coupon and coupon bearing bonds. Remark 2 It is important to note that (2) applies only when the EMM,  Q*,  is the EMM corresponding to when* the cash account is numeraire.

## Forward Contracts On Bonds

Now let us consider the date 0 price,  G0,  of a [[Forward Points in Currency|forward contract]] for delivery of the same security at the same date,  t = n. We recall that G0 is chosen in such a way that the contract is initially worth zero. In particular,  [[7. Black Scholes Model|martingale pricing]] implies
...
$$G_{0}={\frac{\operatorname{E}_{0}^{Q}\left[S_{n}/B_{n}\right]}{\operatorname{E}_{0}^{Q}\left[1/B_{n}\right]}}$$$$(3)$$

Remark 3 If Bn and Sn are Q-independent,  then G0 = F0. In particular,  if [[Interest Rate Quotations|interest rates]] are deterministic,  we have G0 = F0.

Remark 4 *If the underlying security does not pay dividends or coupons ([[Primary vs. Secondary Commodities|storage costs]] may be viewed as* negative dividends),  then we obtain G0 = S0/E
Q
0
[1/Bn] = S0/d(0,  n).

Example 4 ([[Arbitrage Pricing of Derivatives|Pricing]] a [[Forward Points in Currency|Forward Contract]] on a Coupon-Bearing Bond)
We now price a [[Forward Points in Currency|forward contract]] for delivery at t = 4 of a 2-year 10% coupon-bearing bond where we assume that delivery takes place just after a coupon has been paid. In the lattice below we use backwards induction to compute the t = 4 price (ex-coupon) of the bond. We then use (3) to price the contract,  with the numerator given by the value at node (0,  0) of the lattice and the denominator equal to the 4-year [[Discount Factors|discount factor]]. Note that between t = 0 and t = 4 in the lattice below,  coupons are (correctly) ignored.
...
| [[Forward Contracts and Forward Prices|Forward Price]] = 100*79.83 / 77.22 |         | = 103.38 |         |         |        |        |

Example 5 ([[Arbitrage Pricing of Derivatives|Pricing]] a [[Futures Not Subject to Cash-And-Carry|Futures]] Contract on a Coupon-Bearing Bond)
The t = 0 price of a [[Futures Not Subject to Cash-And-Carry|futures]] contract expiring at t = 4 on the same coupon-bearing bond is given at node (0,  0)
in the lattice below. This lattice is constructed using (2). Note that the forward and [[Futures Price and the Quality Option Before E|futures price]] are close but not equal.

| [[Futures Price and the Quality Option Before E|Futures Price]]   | 91.66   |        |        |        |
|-----------------|---------|--------|--------|--------|
|                 | 95.05   | 98.44  |        |        |
|                 | 98.09   | 101.14 | 103.83 |        |
...
+. That is,  the caplet is a call option on the [[An Overview of the Vasicek Short Rate Model|short rate]] prevailing at time τ − 1,  settled at time τ.
...
Remark 5 *In practice caplets are usually based on* [[A Guide to the Front End and [[Basis Swaps|Basis Swap]] Markets#[[Short-Term Rates and the Transition from LIBOR|London Interbank Offered Rate]] (LIBOR)|LIBOR]] (London Inter-Bank Offered Rates).

Caps: A cap is a string of caplets,  one for each time period in a fixed interval of time and with each caplet having the same [[Call and Put Payoffs at Expiry|strike price]],  c.

Floorlets: A *floorlet* is similar to a caplet,  except it has payoff (c − rτ−1)

- and is usually settled in arrears at time τ.

[[Caps and Floors|Floors]]: A *floor* is a string of floorlets,  one for each time period in a fixed interval of time and with each floorlet having the same [[Call and Put Payoffs at Expiry|strike price]],  c.
...
$$\begin{array}{r c l}{{P_{e}(k+1,  s)}}&{{=}}&{{\frac{P_{e}(k,  s-1)}{2(1+r_{k,  s-1})}}}\\ {{}}&{{}}&{{}}\\ {{P_{e}(k+1,  0)}}&{{=}}&{{\frac{1}{2}\frac{P_{e}(k,  0)}{(1+r_{k,  0})}}}\\ {{}}&{{}}&{{}}\\ {{P_{e}(k+1,  k+1)}}&{{=}}&{{\frac{1}{2}\frac{P_{e}(k,  k)}{(1+r_{k,  k})}}}\end{array}$$$$+\ \frac{P_{e}(k,  s)}{2(1+r_{k,  s})},  \quad0<s<k+1$$
...
1. Once you compute the state prices they can be stored and used repeatedly for [[Arbitrage Pricing of Derivatives|pricing]] [[Financial Mathematics Course|derivative securities]] without needing to use backwards iteration each time.
1. In particular,  the [[The Vasicek Model|term structure]] can be computed using O(T
1. operations by first computing the state prices. In contrast,  computing the [[The Vasicek Model|term structure]] by first computing all zero-coupon bond prices using backwards iteration takes O(T
1. operations. While this makes little difference given today's computing power,  writing code to price [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] in [[Binomial Option Pricing Model|lattice models]] is often made easier when we price with the state prices.
1. They can be useful for calibrating [[Binomial Option Pricing Model|lattice models]] to observed market data.

It should be noted that when [[An Introduction to Interest Rate Term Structure in QuantLib Python|pricing securities]] with an [[Bond Futures Options|early exercise]] feature,  state prices are of little help and it is still necessary to use backwards iteration.

## 2.1 [[7. Black Scholes Model#4 Delta Hedging|Hedging In The Binomial Lattice]]

It is very easy to hedge in a [[A Real-Life Option Pricing Exercise|binomial]]-lattice model. For example,  if we wish to hedge a payoff that occurs at t = s,  then we can use any two securities (that do not expire before t = s) to do this. In particular,  we could use the cash account and a zero-coupon bond with maturity *t > s* as our [[Key Rates O1s Durations and Hedging|hedging]] instruments. At each node and time,  we simply choose our holdings in the cash account and zero-coupon bond so that their combined value one period later exactly matches the value of the payoff to be hedged. This of course is exactly analogous to [[Key Rates O1s Durations and Hedging|hedging]] in the classic [[Binomial Option Pricing|binomial model]] for stocks.

That we can hedge with just two securities betrays a weakness of the [[6. A Brief Introduction to Stochastic Calculus|binomial lattice]] model and 1-factor short-rate models more generally. In practice,  the value of an interest-rate derivative will be sensitive to changes in the entire term-structure and it is unlikely that such changes can be perfectly hedged using just two securities. It is possible in some circumstances,  however,  that good *approximate* hedges can be found using just two securities. In the example below,  we hedge using elementary securities.
...
Example 7 (a) Consider the [[An Overview of the Vasicek Short Rate Model|short rate]] lattice of Figure 4.1 where [[Interest Rate Quotations|interest rates]] are expressed as percentages.

You may assume the [[Financial Instruments|risk-neutral probabilities]] are given by q = 1 − q = 1/2 at each node.

Use the forward equation to compute the elementary prices at times t = 0,  t = 1 and t = 2.

(b) Consider the different [[An Overview of the Vasicek Short Rate Model|short rate]] lattice of Figure 4.2 where again you may assume q = 1 − q = 1/2. The short rates and elementary prices are given at each node. Using only the elementary prices,  find the price of a zero-coupon bond with face value $100 that matures at t = 3.

(c) Compute the price of a European call option on the zero-coupon bond of part (b) with strike = $93 and [[Risk Neutral Pricing of Options|expiration date]] t = 2.

(d) Again referring to Figure 4.2,  suppose you have an obligation whose value today is $100 and whose value at node (1,   1) (i.e. when r = 7.2%) is $95. Explain how you would immunize this obligation using date 1 elementary securities? (e) Consider a forward-start swap that begins at t = 1 and ends at t = 3. The [[Fundamentals of Swaps|notional principal]] is $1 million,  the fixed rate in the swap is 5%,  and payments at t = i (i=2,  3) are based on the fixed rate minus the floating rate that prevailed at t = i − 1. Compute the value of this [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]].
...
$$\begin{array}{r c l}{{\frac{1}{(1+s_{i})^{i}}}}&{{=}}&{{\sum_{j=0}^{i}P_{e}(i,  j)}}\end{array}$$$$=\frac{P_{e}(i-1,  0)}{2(1+a_{i-1})}+\sum_{j=1}^{i-1}\left(\frac{P_{e}(i-1,  j)}{2(1+a_{i-1}e^{bj})}+\frac{P_{e}(i-1,  j-1)}{2(1+a_{i-1}e^{b(j-1)})}\right)+\frac{P_{e}(i-1,  i-1)}{2(1+a_{i-1}e^{b(i-1)})}.\tag{4}$$
...
| [[Arbitrage Pricing of Derivatives|Pricing]] a 2-8 [[Chapter 39 - Swaptions, Forward Swaps, and MBS|Payer Swaption]] | 0.0479  | 0.0360  |         |        |        |        |        |        |        |              |
...
6An exercise in Assignment 1 discusses whether or not the BDT model together with our method of [[Credit Markets Session 4|calibration]],  is a good approach to [[Arbitrage Pricing of Derivatives|pricing]] swaptions.