...
Consider the binomial lattice below where we specify the short rate, $r_{i,j}$, that will apply for the single period beginning $\$a_t$$ node $N(i,j)$. This means for example that if $1 is deposited in the cash account $\$a_t$$ $t = i$, state $j$ (i.e. node $N(i,j)$), then this deposit will be worth $(1 + r_{i,j})$ $$a_t$$ time $t + 1$ regardless of the successor node to $N(i,j)$.

!0_image_0.png

We use martingale pricing on this lattice to compute security prices. For example, if $S_i(j)$ is the value of a non-dividend/coupon paying security $\$a_t$$ time $i$ and state $j$, then we *insist* that
...
We compute the price of a 4-period zero-coupon bond with face value $100 that expires $$a_t$$ $t = 4$. Assuming the short-rate lattice is as given above, we see, for example, that the bond price $$a_t$$ node (2, 2) is given by
...
Consider a futures contract with an underlying 4-period zero-coupon bond with face value $100. The futures contract matures $$a_t$$ $t = 3$ and has futures price $F(i,j)$. We know that $F(3,j) = P(3,j)$ where $P(3,j)$ is the $t = 3$ value of the underlying bond. To find the futures price $$a_t$$ earlier nodes, we use the fact that the (risk-neutral) expected discounted gain on a futures contract is zero. Hence:
...
**Definition**: An elementary (or Arrow-Debreu) security pays $1 $$a_t$$ node $N(i,j)$ and $0 everywhere else.

Let $\mathcal{E}(i,j;0,0)$ denote the price $\$a_t$$ node $N(0,0)$ of the elementary security paying $1 $\$a_t$$ node $N(i,j)$. Using the risk-neutral pricing formula:

$\$\mathcal{E}(i,j;0,0) = \mathbb{E}_{\mathbb{Q}}\left[ \frac{1_{\{N(i,j)\}}}{\prod_{k=0}^{i-1}(1+r_k)} \right]$$

where $r_k$ is the short rate $\$a_t$$ time $k$ along the path to node $N(i,j)$.
...
### 3.1 Caps and Floors
A **caplet** pays $\max(r_T - K, 0)$ $\$a_t$$ time $T+1$ based on the short rate $r_T$ $$a_t$$ time $T$. A **cap** is a portfolio of caplets.

A **floorlet** pays $\max(K - r_T, 0)$ $\$a_t$$ time $T+1$ based on the short rate $r_T$ $$a_t$$ time $T$. A **floor** is a portfolio of floorlets.

**Example 4** (Pricing a Caplet)
Consider a caplet with strike $K = 0.08$ and maturity $T = 2$. The payoff $\$a_t$$ $t = 3$ depends on $r_2$:
...
A **swaption** is an option to enter a swap $\$a_t$$ a predetermined swap rate.
