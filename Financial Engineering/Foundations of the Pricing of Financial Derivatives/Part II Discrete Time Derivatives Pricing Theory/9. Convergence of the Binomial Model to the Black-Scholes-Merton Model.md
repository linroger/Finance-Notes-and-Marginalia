...
where $S_t$ is the current asset price at time $t$, $X$ is the exercise price, $r_c$ is the continuously compounded risk-free rate, $\tau$ is the time to expiration, and $\sigma[^2]$ is the variance of the continuously compounded return on the asset. $N(d_i)$ is the cumulative normal probability for $i=1$ and 2 as defined.$[^3]$  
