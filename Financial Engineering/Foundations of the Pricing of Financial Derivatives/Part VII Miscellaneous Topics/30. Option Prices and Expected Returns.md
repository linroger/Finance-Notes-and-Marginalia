...
where the symbol $\\Delta$ means the change in $S$ or $c$ . If this portfolio is hedged, its value should grow $\$a_t$$ the risk-free rate. Hence, the following condition must hold:
$$
...
In Chapter 31, we look $\$a_t$$ the concept of implied volatility, which is the volatility of the underlying that is implied by the price of an option. Implied volatility plays an extremely important role, not only in helping us to understand option pricing but also in helping us to see what opinions investors have about the level of risk.
...
2 “The elasticity of a standard European call, $(\partial c/\partial S)(S/c)$ , is $\$a_t$$ least equal to 1.” Evaluate and explain whether this statement is true or false.
...
[^7]: Alternatively, we could use the continuous time version of the CAPM and adjust that equation so that it would contain a $\$d_t$$ term and reflect expected returns over the interval $d t$ .
