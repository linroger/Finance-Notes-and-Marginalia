...
n this chapter, we will take what we have already learned about stochastic processes and adapt it to the case of an asset. We will show how certain properties of the asset’s return behavior are derived. Finally, we will show how to obtain the formula for the future [[A Preview of Alternative Formulations|asset price]] in terms of a base price and information on what happened in the intervening period. In examining the diffusion processes of assets, let us start by noting that there are many excellent treatments of the subject of this chapter and those in this entire unit. For further study, we recommend several that are particularly well written such as Shimko (1992), Neftci (2006), Ross (1999), and Malliaris and Brock (1982). In this chapter, we use the term asset to represent any of a variety of potential exposures, such as stocks, exchange rates, or combinations of assets, one of which we will introduce later in this chapter.  

Recall that in Chapter 10, we obtained the [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] for the [[A Preview of Alternative Formulations|asset price]] in a heuristic manner. We started with four desirable properties for such a process summarized succinctly here:  

1. The [[A Preview of Alternative Formulations|asset price]] may have a nonzero drift.   
2. Changes in the [[A Preview of Alternative Formulations|asset price]] are random.   
3. Over time, the [[A Preview of Alternative Formulations|asset price]] becomes more unpredictable.   
4. Over time, the [[A Preview of Alternative Formulations|asset price]] is nonnegative.  

Based on these properties, we proposed [[Black Scholes Derivation|geometric Brownian motion]] (GBM) with geometric drift. The GBM process has all four properties and can be expressed in two ways,  
$$
...
Given the fact that $d S_{t}/S_{t}$ is just a linear transformation of a normally distributed random variable $d\mathbb{W}_{t}$ , then it is also normally distributed.2 In this chapter, we formally derive this [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] and some important results related to it.  

Alternatively, we can model some underlying exposure with arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] (ABM). There are two forms of ABM, either with geometric drift or arithmetic drift. Recall from Chapter 10, we introduced the $\$5$ symbol to emphasize the unit of measure is the same as the [[A Preview of Alternative Formulations|asset price]] and not percentage. Thus, arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] with geometric drift (ABMGD) can be expressed as  
$$
d S_{t}=\alpha S_{t}d t+\sigma_{\S}d\boldsymbol{\mathrm{W}}_{t},
$$  

or, depending on context, arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] with arithmetic drift (ABMAD) can be expressed as  
$$
d S_{t}=\alpha_{\S}d t+\sigma_{\S}d\mathbb{V}_{t}.
$$  

These ABM processes resolve the undesirable property evident in Equation (12.1) because the underlying exposure can be positive, zero, or negative. Also, at times we may wish to have the underlying instrument growing geometrically (e.g., stocks) and at other times arithmetically (e.g., spreads such as refined petroleum and crude oil). Note that in this case relative return may be undefined if $S_{t}=0$ or lacking financial interpretation if $S_{t}<0$ . For the ABM [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]], we are interested in $d S_{t}$ and not $d S_{t}/S_{t}$ . Again, volatility, and perhaps mean, is measured in [[Forwards and Futures Notes|currency]] units and not percentage.  
...
This return can be expressed by linking the successive relative [[Assets|returns]],  
$$
...
We see that the log return for the period of time 0 to time $T$ is the sum of the log [[Assets|returns]] of the subperiods from time 0 to time $T.$ Now recall that the central limit theorem says that a random variable that is defined as the sum of a series of other random variables from any distribution that is constant approaches a normal distribution. Thus, we know that the return from time 0 to time $T$ is normally distributed, provided the distribution of the intermediate [[Assets|returns]] is constant. We can also propose that each subperiod is infinitesimally small such that it, too, is made up of a series of component [[Assets|returns]] over infinitesimally small subperiods. Hence, it is reasonable to propose that the return over any arbitrary “short” period from $t$ to $t+d t$ is normally distributed with $d t$ period expectation of $\mu_{\ensuremath{p}}$ and variance of $\sigma_{{p}}^{2}$ , which is formally written as  
$$
\ln\left(\frac{S_{t+d t}}{S_{t}}\right)\sim N\left(\mu_{p},\sigma_{p}^{2}\right).
$$  

It is important to emphasize that when the price is lognormally distributed, the log of the relative return is normally distributed. Thus, the [[A Preview of Alternative Formulations|asset price]] is lognormally distributed. By definition, the log return is defined in the following manner:  
$$
\ln\left({\frac{S_{t+d t}}{S_{t}}}\right)=\ln S_{t+d t}-\ln S_{t}=d\ln S_{t}.
$$  

In other words, the log return is the change in the log values of the [[A Preview of Alternative Formulations|asset price]] at the beginning and ending of the holding period. We then propose that the log return follows the GBM [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]  
$$
...
We wish to find the [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] for $S_{t}$ , so we can use Itô’s lemma. Temporarily dropping the time subscript, we obtain  
$$
d S={\frac{\partial S}{\partial G}}d G+{\frac{1}{2}}{\frac{\partial^{2}S}{\partial G^{2}}}d G^{2}.
$$  

The partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are easily obtained as  
$$
...
Dividing both sides by $S_{t}$ and adding the time subscript, we now have the [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] for $d S_{t}/S_{t}$ ,  
$$
...
Thus a price change can be expressed by linking the successive [[Assets|returns]],  
$$
...
The equations for the relative return and log return are stochastic processes, as well as stochastic differential equations. A differential equation has a potential solution, which is a function such that the [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] conform to the differential equation. In this context, a solution would be the [[A Preview of Alternative Formulations|asset price]] at some future time $t.$ , expressed in terms of the [[A Preview of Alternative Formulations|asset price]] at a previous time such as time 0.  

To solve the [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]], (12.23), we want to obtain a future price $S_{t}$ in terms of a current price, say $S_{0}$ . We take the equation for the log return, Equation (12.13) using $d G_{\mathrm{t}}$ as the LHS, and integrate over the interval 0 to $t$ ,  
$$
...
In fact, in this case, the stochastic integral is so simple, it is the same as the Riemann integral. The variable $\mathbf{}\mathbf{}\mathbf{}W_{t}$ is the value of the [[Continuous-Time Stochastic Processes|Brownian motion]] process at time $t$ . It is quite common that $\mathbb{W}_{0}$ is set at zero, so let us do it. So now we have $\sigma\mathbb{W}_{t}$ as the solution to Equation (12.36). Then $G_{t}-G_{0}=\mu t+\sigma\mathbb{W}_{t}$ is the solution to Equation (12.34). To get this solution in terms of $S_{t}$ and $S_{0}$ , we exponentiate this result,  
$$
...
Equation (12.38) is presumably the solution to the [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]], (12.23). Now, to be absolutely certain that we have the correct solution, we need to check it by applying Itô’s lemma to $S_{t}$ , inserting the [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], and determining if we obtain (12.23). Our [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] for $S_{t}$ obtained from Itô’s lemma is  
$$
...
We then insert the partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] that we obtain by differentiating the solution, Equation (12.38). The partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are  
$$
...
This is the original [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]], Equation (12.23). Thus, our solution is correct.  
...
Let us see how solving a [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] (SDE) is different from solving an ordinary differential equation (ODE). Consider the ordinary differential equation:  
$$
...
The partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are $\partial x_{t}/\partial y_{t}=1/y_{t}$ and $\partial^{2}x_{t}/\partial y_{t}^{2}=-(1/y_{t}^{2})$ . We also have $d y_{t}=$ $\boldsymbol{y}_{t}d\boldsymbol{W}_{t}$ and $d y_{t}^{2}=y_{t}^{2}d t$ , due to the properties of $d\mathbb{W}_{t}$ . Substituting these results, we obtain  
$$
...
to the [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]], we shall now use it to obtain the expected [[A Preview of Alternative Formulations|asset price]] at t. Such a concept is commonly seen in real markets, such as when someone asks an analyst, “Where do you think the Dow will be in a year?” Using Equation (12.49), we express the problem as follows:  
$$
...
Note that this result is also equal to $E(S_{t})=S_{0}e^{\alpha t}$ because $\alpha=\mu+\sigma^{2}/2$ . This is an intuitively simple result. It says that the expected future [[A Preview of Alternative Formulations|asset price]] is the current [[A Preview of Alternative Formulations|asset price]] compounded at the expected rate of return.  
...
The key focus of this book is addressing approaches to valuing [[Financial Instruments PSET Solutions|financial derivatives]]. Based on materials covered thus far, there appears to be a key decision to be made in our journey. Do we go with GBM or ABM?  
...
Osborne (1959) provided one of the original analyses of continuously compounded stock [[Assets|returns]]. Based on the observation that for percentage changes less than $15\%$ there is not a material difference between modeling with the normal or lognormal distributions, he opted for the lognormal distribution based on the lognormal appearance of the cross-sectional distribution of stock prices.10 Sprenkle (1961) argued in favor of the lognormal distribution due to it having no chance of negative values. Alexander (1961) raised some concerns regarding this choice due to time series financial data exhibiting fat tails.  

Samuelson (1965) seems to have created the phrase “[[Black Scholes Derivation|geometric Brownian motion]].” He opted for GBM over ABM primarily due to the nonnegativity of the lognormal distribution and some technical problems with Bachelier’s (1900) model.11 Following Samuelson (1965), Black and Scholes (1973) as well as [[Credit Markets Session 5|Merton]] (1973) chose GBM.  
...
■ Positive values, $S_{t}>0$ . With GBM, there is a zero probability of the [[A Preview of Alternative Formulations|asset price]] being zero in the future. ABM solves this problem. Based on US Census data, approximately $0.7\%$ of businesses file for some form of bankruptcy each year. Thus, some percentage of these bankruptcy filing companies result in equity prices being permanently zero. Note there is a fundamental difference between legal bankruptcy, such as a court filing, and economic bankruptcy, such as the stock being deemed worthless. Many legal bankruptcies do not result in economic bankruptcy.   
■ Portfolios, $\Pi_{t}=\sum_{i=1}^{n}N_{i}S_{i,t}$ . With GBM, a simple [[An Asset Allocation Primer|portfolio]] of assets follows no known statistical distribution. Thus, internal coherence of risk measures is not feasible due to the lack of the additive property of the lognormal distribution. ABM solves this problem.   
■ Time-series independence. With GBM, changes in the [[Some Stylized Empirical Facts About Asset Retur|asset returns]] are assumed to be independent of the [[A Preview of Alternative Formulations|asset price]] itself. ABM suffers from dependence particularly over long periods of time. For example, if a [[Chapter 16 - Black–Scholes Model|stock price]] doubles in value, the absolute volatility of price changes does not automatically double with arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]].  

■ Factor models, $S_{t}=f(x_{1},x_{2},\ldots,x_{n})$ . An asset can have multiple [[PSET 6- Financial Instruments|risk factors]]. ABM can easily handle multiple factors, whereas with GBM it is a difficult challenge. Mathematically, the two approaches can be expressed as  
$$
...
■ Risk. Risk can be represented based on relative measures $(\%)$ or absolute measures ( $\$5$ or other [[Forwards and Futures Notes|currency]]). Stock splits require ABM to manually adjust absolute volatility, but there is a [[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]] effect that is implicitly handled by ABM. The [[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]] effect is the intuitive observation that as a [[Chapter 16 - Black–Scholes Model|stock price]] declines it becomes more highly leveraged. Thus, by definition the [[Chapter 16 - Black–Scholes Model|stock price]] would be expected to be more volatile in a relative sense. GBM automatically adjusts for stock splits, but the [[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]] effect is ignored.   
...
■ [[A Real-Life Option Pricing Exercise|Binomial]] convergence. GBM can be derived from a multiplicative [[A Real-Life Option Pricing Exercise|binomial]] process. ABM implies an additive [[A Real-Life Option Pricing Exercise|binomial]] process. Both lattices can be modeled to converge to the appropriate terminal distributions.   
■ Homogeneity of degree 1. GBM-based models typically are homogeneous of degree 1, whereas ABM-based models are not.12 For example, within an ABM framework stock splits will result in option [[Arbitrage Pricing of Derivatives|pricing]] models that do not properly adjust. Scaling absolute volatility, however, can easily solve this problem.   
...
[[A Preview of Alternative Formulations|asset price]] in terms of a base price and information on what happened in the intervening period. We also examined and compared both arithmetic [[Continuous-Time Stochastic Processes|Brownian motion]] and GBM.  

In the following chapter, we shall do what we have been alluding to many times: derive the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton model]].  

# QUESTIONS AND PROBLEMS  

1 [[Black Scholes Derivation|Geometric Brownian motion]] with geometric drift is often represented in one of two ways:  
$$
...
3 Volatility is often reported as a standard deviation, $\sigma$ . If we assume the standard deviation itself follows GBM with geometric drift, what is the [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] of the variance as well as its mean and variance?  

4 Assuming $S_{t}$ follows [[Black Scholes Derivation|geometric Brownian motion]], explain the relationship between $E\big[\mathrm{ln}\big(S_{T}/S_{0}\big)\big]$ and $\ln\left[E\bar{(S_{T})}/S_{0}\right]$ .  

5 Based on a historical analysis of a tech company, the annualized average arithmetic return was $29.4\%$ and the annualized average geometric return was $25.95\%$ . The annualized standard deviation of arithmetic [[Assets|returns]] was $26.2\%$ . Based on the analysis contained in this chapter, how can you explain the relationship between these two annualized [[Assets|returns]]?  
...
11. Without limited liability, a call option can be higher than the corresponding [[Chapter 16 - Black–Scholes Model|stock price]] because the call enjoys limited liability, whereas the underlying stock does not.   
