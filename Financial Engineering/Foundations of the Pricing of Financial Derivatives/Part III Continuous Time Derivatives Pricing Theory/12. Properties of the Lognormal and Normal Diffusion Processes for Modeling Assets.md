...
Recall the random variable $dW_{t}$ is the transformed Brownian motion, which we called a Wiener process%20Process.md). Recall that $dW_{t}$ is normally distributed with $E(dW_{t})=0$, $\text{var}(dW_{t}) = dt$. Also, we noted that $(dW_{t})^{2}$ is non-stochastic and equal to $dt$. Using these results, the expectation and variance of Equation (12.1) are  
$$
\begin{aligned}
E\left(\frac{dS_{t}}{S_{t}}\right) &= \alpha dt \\
\text{var}\left(\frac{dS_{t}}{S_{t}}\right) &= \sigma^{2} dt
\end{aligned}
$$  
...
The relative return on the asset from a starting point of time 0 to a point of time $dt$ is  
$$
\frac{S_{dt}}{S_{0}}
$$  
