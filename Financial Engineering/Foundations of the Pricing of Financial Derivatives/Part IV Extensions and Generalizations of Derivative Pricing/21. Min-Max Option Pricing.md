...
Note that the [[Preview of the Book|cash flow]] yields related to the underlying assets do not directly influence this parity. This type of max-min parity will continue to be useful because all we need to do is to derive a [[Arbitrage Pricing of Derivatives|pricing]] model for one of the two min-max options, and the price of the other can be obtained using the previous equation.  
...
Intuitively, the call on the max will be worth at least the call on the min as $\operatorname*{max}\bigl(S_{1T},S_{2T}\bigr)\geq\operatorname*{min}\bigl(S_{1T},S_{2T}\bigr).$ Specifically, $c_{\mathrm{max}}\geq c_{\mathrm{min}}$ . We demonstrate this intuition in Table 21.2. Thus, if $c_{\operatorname*{max}}\geq c_{\operatorname*{min}}$ , then a [[An Asset Allocation Primer|portfolio]] can be created such that there is a positive probability of receiving money and no chance of losing. Clearly, this would require a nonnegative [[An Asset Allocation Primer|investment]]; otherwise, we have [[Arbitrage Pricing of Derivatives|arbitrage]] resulting in disequilibrium.  

Following a similar argument, the put on the min will be worth at least the put on the max. Specifically, $p_{\operatorname*{min}}\geq p_{\operatorname*{max}}$ . We demonstrate this intuition in Table 21.3. Thus, if $p_{\operatorname*{min}}<p_{\operatorname*{max}}$ , then again there is a [[An Asset Allocation Primer|portfolio]] that can be created such that there is a positive probability of receiving money and no chance of losing. Clearly, this would require a nonnegative [[An Asset Allocation Primer|investment]]; otherwise, we have [[Arbitrage Pricing of Derivatives|arbitrage]] resulting in disequilibrium.  
...
Consider the following comparison whereby a put on the min is shown to be equivalent to a [[Chapter 4 - Futures: Hedging and Speculation|long position]] in risk-free bonds with face value $X$ maturing at the option’s expiration, a short position in a call on the minimum struck at zero, whose price we denote as $c_{\operatorname*{min},X=0}$ , and a [[Chapter 4 - Futures: Hedging and Speculation|long position]] on a call on the min with [[Options|exercise price]] X. Note first that a call on the minimum struck at zero is an instrument that always pays off the lower-priced asset and there are effectively two exercise prices, $X=0$ and $X>0$ . We denote the current price of a call on the min struck at zero as $c_{\operatorname*{min},X=0}$ . We illustrate these results in Table 21.5.  
...
Finally, we can easily obtain the price of the put on the maximum. Let us compare it to a [[Chapter 4 - Futures: Hedging and Speculation|long position]] in a pure discount bond, a call on the maximum struck at zero, and a long call on the maximum with an [[Options|exercise price]] of $X$ illustrated in Table 21.6.  
...
TABLE 21.5 [[7. Black Scholes Model|Put-Call Parity]] for Options on the Min   
...
TABLE 21.6 [[7. Black Scholes Model|Put-Call Parity]] for Options on the Max   
...
where $\alpha_{i}$ and $\sigma_{i}$ are the [[Black-Scholes Model|drift and volatility]] of asset $i_{-}$ , and $d\mathbb{W}_{i}$ is the [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] driving asset $i$ , with $i=1,2$ . The correlation between assets 1 and 2 is $\rho_{12}$ . Note that $c_{m i n}$ is a function of $s_{1},s_{2}$ , and $t$ . Thus, based on Itô’s lemma, we know  
$$
...
Consider a hedged [[An Asset Allocation Primer|portfolio]] where we are short one call on the minimum and long $(\partial c_{\operatorname*{min}}/\partial S_{1})$ shares of $S_{1}$ and also long $(\partial c_{\operatorname*{min}}/\partial S_{2})$ shares of $S_{2}$ . Thus, the current value of the [[An Asset Allocation Primer|portfolio]], $V.$ , is  
$$
V=-c_{\mathrm{min}}+\frac{\partial c_{\mathrm{min}}}{\partial S_{1}}S_{1}+\frac{\partial c_{\mathrm{min}}}{\partial S_{2}}S_{2}.
$$  

Now let us assume constant dividend yields and denote them $\delta_{1}$ and $\delta_{2}$ . Recall the dividend is assumed to be paid continuously based on the current [[A Preview of Alternative Formulations|asset price]]. Thus, the change in [[An Asset Allocation Primer|portfolio]] value is  
$$
...
Because the [[An Asset Allocation Primer|portfolio]] is risk free, we also know that  
$$
...
Thus, we seek a solution to this PDE that satisfies the [[Appendix 21.C Solutions for Black-Scholes PDE|boundary conditions]].2  

Recall from Chapter 13 that the univariate Feynman-Kac theorem provides a link between the PDE solution and the [[FORWARD RATES AND TERM STRUCTURE|expectations]] solution. There is a multivariate version that provides the same link that can be applied here. Appendix 21.A provides a formal  
...
Figure 21.1 illustrates the sensitivity of min-max options with respect to changes in [[A Preview of Alternative Formulations|asset price]] 2 $(S_{2})$ . Note that as [[A Preview of Alternative Formulations|asset price]] 2 declines, the call on max converges to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] call value for asset 1 (14.23 with these parameters), the put on the max converges to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] put value for asset 1 (9.35 with these parameters), the call on the min converges to zero, and the put on the min converges to the lower boundary condition for asset 2. Further, as [[A Preview of Alternative Formulations|asset price]] 2 increases, the call on min converges to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] call value for asset 1, the put on the min converges to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] put value, the call on the max converges to the lower boundary condition for asset 2, and the put on the max converges to zero.  

Figure 21.2 illustrates the sensitivity of min-max options with respect to changes in the correlation between asset 1 and asset 2. Note that as the correlation declines, the call on max and the put on the min increase in value as the likelihood of favorable moves in one of the two assets increases due to the lower correlation. Further, the call on the min and the put on the max decrease in value as the likelihood of an unfavorable move in one of the two assets increases. Further, note that as the correlation tends to $+1$ , the call on the min and call on the max converge to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] call value (14.23, same for both underlying calls) and the put on the min and put on the max converge to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] put value (9.35, same for both underlying puts).  

![](79eb3e3711dab2ef9a6d1505db75aab6ba42f0476f55a9df43bce6eff7ff6488.jpg)  
Figure 21.3 illustrates the sensitivity of min-max options with respect to changes in the [[Chapter 16 - Black–Scholes Model|time to expiration]]. The familiar time value decay is evident with all four min-max options   
FIGURE 21.1 Min-Max Option Value Sensitivity to [[A Preview of Alternative Formulations|Asset Price]] 2  
...
FIGURE 21.3 Min-Max Option Value Sensitivity to [[Chapter 16 - Black–Scholes Model|Time to Expiration]]  

converging to zero as [[Chapter 16 - Black–Scholes Model|time to expiration]] tends toward zero. The call on the max and the put on the min have greater time value decay because they have greater value for longer maturities.  
...
Figure 21.4 illustrates the sensitivity of min-max options with respect to changes in asset 2 volatility. As asset 2 volatility tends toward zero, the put on the min converges to the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] put value for asset 1 and the put on the max converges to zero. Min-max call options are less transparent. When volatility converges to zero, the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]]-[[Credit Markets Session 5|Merton]] call value converges to the lower bound, which in this case is positive ([[A Preview of Alternative Formulations|asset price]] 2 less the present value of the [[Options|exercise price]]). Based on Equation (21.3), we know the sum of min-max call options should equal the sum of the underlying calls. Thus, we observe a nonzero call on min and a call on max in excess of the BlackScholes-[[Credit Markets Session 5|Merton]] call value (14.23).  

# 21.3 OTHER RELATED OPTIONS  

There have been numerous extensions of the basic formula. Johnson (1987) and Rich and Chance (1993) develop the formula under the condition of more than two assets. Also, several other useful results have been obtained by Rubinstein (1991b). He first establishes a formula for an option that pays off the better of two risky assets or a fixed amount of cash. Letting $X$ be the fixed amount of cash, we write this payoff as $\operatorname*{max}\bigl(S_{1T},S_{2T},X\bigr)$ . Rubinstein then derives the [[Arbitrage Pricing of Derivatives|pricing]] formula for this option. Let us denote this price as $c_{12X}$ , where the subscripts denote [[A Preview of Alternative Formulations|asset price]] 1, [[A Preview of Alternative Formulations|asset price]] 2, and some fixed amount X. Then note the following relationship, $\operatorname*{max}\bigl(S_{1T},S_{2T},X\bigr)-X=\operatorname*{max}\bigl[0,\operatorname*{max}\bigl(S_{1T},S_{2T}\bigr)-X\bigr]$ . This equivalence implies that a [[Chapter 4 - Futures: Hedging and Speculation|long position]] in Rubinstein’s option paying the best of two assets or $X$ and a short position worth the present value of $X$ is equivalent to a call on the max struck at $X$ . Thus,  
$$
...
In this chapter, we looked at options in which there is more than one [[Risk Neutral Pricing of Options|underlying asset]]. Specifically, we examined options on the greater or lesser valued or greater or lesser performing of two assets. These assets have a second feature not found in standard options in that they enable the user to bet or hedge on which asset will be worth more or less or generate a greater or lesser rate of return.  

In Chapter 22, we look at two different types of [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]: [[Forward and Futures Contracts|forwards and futures]]. We do this because they complete our knowledge base in the [[Arbitrage Pricing of Derivatives|pricing]] of [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. Yet, they do it in a different manner. As we shall see, they do not require the dynamic [[Key Rates O1s Durations and Hedging|hedging]] we are required to use with options.  
...
Note that $\mu,\sigma,b,V,g.$ , and $\rho$ , are known functions. Then the [[Vasicek Short Rate Model|Feynman-Kac formula]] provides the solution as a conditional expectation expressed as  
$$
...
With $W^{Q}(t)$ a [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] under $\mathcal{Q}$ and the initial condition for $X(t)$ is $X(t)=x$ .  

# APPENDIX 21B  

# An Alternative Derivation of the Min-Max Option Model  

Let us form a hedge [[An Asset Allocation Primer|portfolio]] currently valued at $V$ by placing $x\%$ of our wealth in asset 1, $y\%$ of our wealth in asset 2, and $100\%-x\%-y\%$ of our wealth in the [[2. Forwards, Swaps, Futures, and Options|risk-free asset]]. For example, let $x$ and $y$ be expressed as percentages (e.g., $x=0.3$ or $30\%$ , $y=0.6$ or $60\%$ , $1-x-y=0.1$ or $10\%$ ). To express the current value of this [[An Asset Allocation Primer|portfolio]] consider that we invest $x V$ dollars in asset 1, which will buy $x V/S_{1}$ shares. We invest $y V$ dollars in asset 2, which will buy $y V/S_{2}$ shares. We then invest $\boldsymbol{V}-\boldsymbol{x}\boldsymbol{V}-\boldsymbol{y}\boldsymbol{V}$ dollars in the [[2. Forwards, Swaps, Futures, and Options|risk-free asset]]. The current value of our [[An Asset Allocation Primer|portfolio]] can, therefore, be expressed as  
$$
V=\left({\frac{x V}{S_{1}}}\right)S_{1}+\left({\frac{y V}{S_{2}}}\right)S_{2}+(1-x-y)V.
$$  

The change in the value of this [[An Asset Allocation Primer|portfolio]] can be expressed as  
$$
...
This equation will apply to any min-max options. The differences in the various minmax options will come from the [[Appendix 21.C Solutions for Black-Scholes PDE|boundary conditions]], which are Equations (21.4) and (21.5). We shall use Equation (21.41) to price a call on the minimum. Therefore its price, the solution to the above PDE, is subject to the boundary condition of Equation (21.2), which we repeat here,  
$$
c_{\operatorname*{min},T}=\operatorname*{max}\bigl[0,\operatorname*{min}\left(S_{1T}S_{2T}\right)-X\bigr].
$$  

Stulz obtains the solution reported in the chapter by discounting the expected payoff under [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|risk neutrality]].  
...
2 A [[An Asset Allocation Primer|portfolio]] of two call options on different stocks with the same maturity and [[Options|exercise price]] can be replicated with options on the maximum and/or minimum. Identify the replicating min and/or max option combination and prove that this [[An Asset Allocation Primer|portfolio]] is the proper [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]].  

3 Explain how a dividend yield will influence the boundary condition given in the previous problem.  

4 Based on [[Forward Contracts and Forward Prices|arbitrage arguments]], show that pmax + pmin = p1 + p2.  

5 Based on the previous problems as well as standard [[7. Black Scholes Model|put-call parity]], we have the following relationships:  
$$
...
Demonstrate, by substitution, that $c_{\mathrm{max},X=0}+c_{\mathrm{min},X=0}=S_{1}+S_{2}$ . Prove with an [[Arbitrage Pricing of Derivatives|arbitrage]] table that this equality holds.  
...
5. The values of the partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] of $V$ with respect to $S_{1}$ and $S_{2}$ are not known at the present. With the [[Arbitrage Pricing of Derivatives|pricing]] model that solves this PDE, we can then derive these partial [[Chapter 9 Arbitrage and Hedging With Options|derivatives]].  