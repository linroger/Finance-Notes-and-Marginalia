...
The price quotes for each tranche are in basis points and have a day-count convention of ‘30/360’. But the equity tranche is quoted diferently. The equity tranche payer on the CDX-USA index pays $26\\%$ of the notional principal (in the CDX index) $\$a_t$$ the initiation of the equity single tranche trade and then pays 400 bps per year thereafter. For the equity tranche on the iTraxx (Europe) index the equivalent fgures are $11\%$ of the principal (in the iTraxx index), $$a_t$$ the initiation of the equity single tranche trade and then 400 bps per year thereafter.
