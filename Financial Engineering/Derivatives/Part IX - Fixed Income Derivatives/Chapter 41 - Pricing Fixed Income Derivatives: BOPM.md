...
A trinomial lattice for $t=2$ periods and with each period equal to 1 year is shown in Figure 41.3. The (continuously compounded) short-rate is given at each node and the probabilities of an ‘up’, ‘fat’ and ‘down’ move are assumed to be $q_{i}=(0.25,0.50,0.25)$ respectively. Broadly speaking, these probabilities are chosen so that the lattice exhibits mean reversion%20Process.md) and it matches the ‘real world’ variance of the short-rate.  
