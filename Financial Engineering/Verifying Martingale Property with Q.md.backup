...
In Section 5.2.2.2, we verified that the [stock price](Derivatives/Part%20IV%20-%20Options/Chapter%2016%20-%20Black–Scholes%20Model.md) relative to the bond (the bond as the numeraire) satisfies the [martingale property](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) from time O to 1. Here, we will do the same for the [stock price](Derivatives/Part%20IV%20-%20Options/Chapter%2016%20-%20Black–Scholes%20Model.md) from time 1 to 2 and then in subsequent periods. We will also demonstrate that the required measure that makes the [discounted stock price](.md) into a martingale is the risk-neutral probability $\mathbb{Q}$ . This appendix is essentially a continuation of Section 5.2.2.2.  

Suppose, we are given the [stock price](Derivatives/Part%20IV%20-%20Options/Chapter%2016%20-%20Black–Scholes%20Model.md) ${\cal S}_{\boldsymbol{O},\boldsymbol{I};\boldsymbol{u}}$ at time 1 after an upjump at time 1. Denote $S_{O,2:u,u}$ and $S_{O,2:u,d}$ as the stock prices at time 2 given the outcomes $\omega_{\mathrm{u,u}}$ (consecutive upjumps at time 1 and 2) and $\omega_{\mathrm{u,d}}.$ respectively. Denote $q_{I,2;u,u}$ and $q_{I,2;u,d}$ as the [risk-neutral probabilities](../Financial%20Instruments/Financial%20Instruments.md) that at time 1, the stock will have an upjump or downjump at time 2 given that it had an upjump at time 1. Let $B_{I.2}$ ([discount function](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%2010%20-%20The%20Economics%20of%20the%20Term%20Structure%20of%20Interest%20Rates/Basic%20Interest%20Rate%20Concepts%20and%20Relations.md)) be the time 1 value of a bond that pays 1 at time 2. Analogous to equation (5.15), the forward prices from time 1 to 2 given the [stock price](Derivatives/Part%20IV%20-%20Options/Chapter%2016%20-%20Black–Scholes%20Model.md). ${\cal S}_{\boldsymbol{O},\boldsymbol{I};u}$ at time 1, must satisfy the [system of equations](Fixed%20Income%20Derivatives/A%20Primer%20on%20Probability%20Theory%20and%20Stochastic%20%20Modelling.md):  
$$
...
If we divide the matrix equation by $B_{I,2}$ , and define the [risk-neutral probabilities](../Financial%20Instruments/Financial%20Instruments.md) $q_{I,2;u,u}{=}\psi_{I,2;u,u}\left/B_{I,2}\right.$ and $q_{l,2;u,d}{=}\psi_{I,2;u,d}/B_{I,2}$ as required by equation (5.12) we get:  
$$
...
We can repeat the calculation given the [stock price](Derivatives/Part%20IV%20-%20Options/Chapter%2016%20-%20Black–Scholes%20Model.md) $S_{O,I;d}.$ Analogous to equations (3) and (4), we would obtain:  
$$
...
Equations (4) and (6) together verify the [martingale property](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) going from time 1 to 2 that the. expected value of the stock at time 2 given its value at time 1 (in bond numeraire) is equal to the value of the stock at time 1 (in bond numeraire)..  

Next, we study the stock model going from time 0 to 2. Let, $q_{O,2;u,d},q_{O,2;d,u},$ and $q_{O,2;d,d}$ denote the [risk-neutral probabilities](../Financial%20Instruments/Financial%20Instruments.md) at time O that the [stock price](Derivatives/Part%20IV%20-%20Options/Chapter%2016%20-%20Black–Scholes%20Model.md) will experience the outcomes $\omega_{\mathrm{u,u}},\omega_{\mathrm{u,d}},\omega_{\mathrm{d,u}},$ and $\omega_{\mathrm{d,d}}$ , respectively, in going from time O to time 2. Let. $B_{O.2}$ be the time 0 value of a bond that pays 1 at time 2. Note that this bond value must satisfy the equation $B_{O,2}{=}B_{O,I}B_{I,2}$ Recall that $\Psi_{0,2;\mathrm{u,u}}$ is the price of the pure security at time O that pays 1 at time 2 after the upjump in the price of the stock from time 0 to time 1 and also from time 1 to time 2. The prices $\Psi_{\mathrm{0},2;\mathrm{u},\mathrm{d}}$ $\Psi_{\mathrm{0},2;\mathrm{d},\mathrm{u}}.$ and $\Psi0{,}2{;}\mathrm{d,d}$ are defined analogously. Similar to previous analysis, we have:  
$$
...
If we divide both sides of equation (7) by $B_{O,2}$ and use equation (5.12) to define the [risk-neutral probabilities](../Financial%20Instruments/Financial%20Instruments.md), we obtain:  
$$
...
Since $E_{\mathbb{Q}}[S_{\mathrm{n}}|S_{O},S_{1},...,S_{k}]=S_{\mathrm{k}}\forall k<n$ if the stock prices are expressed in the units of the. bond as the numeraire, it is clear that. $S_{\mathrm{t}}$ is a martingale, despite its having been a submartingale when priced relative to [currency](../Financial%20Instruments/Lecture%20Notes-%20Financial%20Instruments/Teaching%20Note%201-%20Forward%20Rates%20Agreement/Forwards%20and%20Futures%20Notes.md)..  

# Chapter 5 Additional Readings  

# Brownian Motion as a Limit of a Discrete Random Walk  

Think of the discrete [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md) and [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) taking place on the $\mathbf{Z}$ -axis, where we start at the origin $(z{=}O)$ at time 0. We will then study these two processes as a function of time. We can also graph each of these two processes if we wish with position z as the vertical axis versus time $t$ as the horizontal axis, analogous to the graph of a [stock price](Derivatives/Part%20IV%20-%20Options/Chapter%2016%20-%20Black–Scholes%20Model.md) $(S)$ versus time $(t)$ . The graph of the discrete [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md) versus time for a specific history will consist of a 1arge number of discrete points in the. $t{-}z$ plane, while the graph of [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) versus time for a specific history will consist a continuous zig-zag graph in the $t{-}z$ plane. To create [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) from the discrete [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md) with. $p=1/2$ , first divide each unit of time on the. continuous time axis into $n$ equal subintervals, so that the width of each subinterval is $\Delta t=1/n$ This results in the discrete times $0,1/n,2/n,...,k/n,...$ on the continuous time axis. Define the. following discrete [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md) as a function of time $k$ (the $k^{\mathrm{th}}$ discrete time on the continuous time axis at time $k/n$ ) by:  
$$
\begin{array}{r}{X_{k}^{\left(n\right)}=\sum_{i=1}^{k}{\frac{Z_{i}}{\sqrt{n}}}.}\end{array}
$$  

Assume that each $Z_{i}$ is a random variable that equals 1 with probability $\%$ and equals -1 with probability $\%$ at time $i,$ and that the $Z_{i}^{\phantom{}}s$ are pairwise independent of one another. This discrete [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md), startig at , takes independent smal steps of length $\pm{\frac{1}{\sqrt{n}}}$ at each moment of time rom $(i{-}1)/n$ $i/n$ $i$ goes from I to $k$ $X_{k}^{(n)}$ at time $k/n$ Since the [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md) $X_{k}^{(n)}$ takes ever-smaller seps aproaching length 0 as $n\to\infty$ it is approaching a continuous path as $n\to\infty$ for each specific history. This justification for the existence and continuity of the path isn't rigorous. A rigorous argument is beyond the level of this text.  

We next claim that as. $n\to\infty$ , the [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md) will converge to a process that has a normal distribution (which is one the requirements to be [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md)).To prove this, pick a particular time $t$ on the continuous non-negative real line. Pick the integer $k,$ $k\leq n t$ so that it best approximates $k/n\approx t.$ Note that $\left|k/n-t\right|<l/n$ so that the approximation improves as. $n$ approaches $\infty$ . Also note that. $k\approx n t$ , so that $k\to\infty$ as $n\to\infty$ Define the sample mean random variable  
$$
...
$$  
$k\to\infty$ Since $Z\sqrt{t}{\sim}N(0,t)$ , this shows thate $X_{k}^{(n)}$ approaches a normal distribution, say $X_{t}$ with mean O and variance $t.$ We have completed the main step to show that our [random walk](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Some%20Discrete-Time%20Stochastic%20Processes.md). converges to a process that has a normal distribution with mean O and variance $t.$  
...
and $j+l\le k\le l+l\le m$ then the random variables $X_{k}^{(n)}-X_{j}^{(n)}$ and $X_{m}^{(n)}-X_{l}^{(n)}$ are independent. This is easily seen by observing that the $Z_{i}^{\phantom{}}s$ in the first sum are independent of the $Z_{i}$ 's inthe scond sm i thi case. tut $X_{k}^{(n)}-X_{j}^{(n)}$ approaches $X_{t}-X_{s}$ $X_{l}^{(n)}-X_{m}^{(n)}$ approaches $X_{u}-X_{\nu}$ as $n{\longrightarrow}\infty$ . Thus, $X_{t}-X_{s}$ and $X_{u}-X_{\nu}$ should be independent of one another. Each of the requirements in order that. $X_{k}^{(n)}$ will converge to the standard [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md). process as $n\to\infty$ have been addressed, and we conclude that the convergent process $X_{t}$ is in fact standard [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md).  

# Geometric Brownian Motion as a Limit of a Discrete Stochastic Process  

In the same way that we derived [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) as a limit of a discrete [stochastic process](../The%20Ornstein-Uhlenbeck%20(OU)%20Process.md), we divide each unit of time into $n$ equal subintervals, so that the width of each subinterval is $\Delta t=$ $l/n$ This results in the discrete times $0,I/n,2/n,...,k/n,...$ on the continuous time axis. Define the price of a sock s(m) as as a discrete [stochastic process](../The%20Ornstein-Uhlenbeck%20(OU)%20Process.md) as a function of time $k$ (the $k^{t h}$ discrete time on the continuous time axis at time $k/n$ ) so that:  
$$
...
Similar to the derivation of [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) from the discrete model, pick a particular time $t$ on the continuous time axis. Choose the integer $k$ so that $k/n\leq t$ and $k/n$ best approximates t. As before, $\left|k/n-t\right|\leq1/n$ and so $k/n\to t$ as $n\to\infty$ . It is also not hard to show that the error. terms are bounded by $C(t,\mu,\sigma)/\sqrt{n}$ where $C(t,\mu,\sigma)$ is a value independent of $n$ . Thus, the error  

terms approach O as $n\to\infty$ . We showed in the derivation of [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md) from the discrete model that D- converges to $Z_{t}\left(\upomega\right)$ where $Z_{t}$ is standard [Brownian motion](../Financial%20Markets/Financial%20Asset%20Pricing%20Theory%20Overview/Chapter%202%20-%20Uncertainty,%20Information,%20and%20Stochastic%20Processes/Continuous-Time%20Stochastic%20Processes.md). These observations show $l n S_{k}^{(n)}(\omega)$ converges toe  
$$
...
Let's label this resulting continuous [stochastic process](../The%20Ornstein-Uhlenbeck%20(OU)%20Process.md) as $S_{t}(\upomega)$ . Since this result holds for all. possible histories $\mathbf{\rho}_{\omega}$ in the sample space, then  
$$
...
First, we state the Tower Property: Suppose $X_{t}$ is a [stochastic process](../The%20Ornstein-Uhlenbeck%20(OU)%20Process.md) with an adapted filtration $\{\mathcal{F}_{t}\}.I f s{\le}t,$ then for any $T\geq t,$ we have  
$$
