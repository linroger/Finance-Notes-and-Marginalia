...
4.1 [[Squam Lake Group Introduction|Introduction]] . 460   
4.2 A Framework for Swaps ... . 461   
14.2.1 Equivalence of Cash Flows . 464   
14.2.2 [[Arbitrage Pricing of Derivatives|Pricing]] the Swap . 464   
14.2.2.1 Interpretation of the [[Teaching Note 4 Interest Rate Derivatives|swap rate]] . 466   
14.2.3 Some Applications.. 468   
14.2.3.1 Another formula .. 469   
14.2.3.2 Marking to market .... 470   
14.3 [[Hull White Term Structure Simulations with QuantLib Python|Term Structure Modeling]] .. . 471   
14.3.1 Determining the Forward Rates from Swaps .. 471   
14.3.2 Determining the $B(t_{0},\ t_{i})$ from Forward Rates 472   
14.3.3 Determining the [[Teaching Note 4 Interest Rate Derivatives|Swap Rate]] . 472   
14.3.4 Real-World Complications . 472   
14.3.4.1 Remark... 473   
4.4 [[The Gauss Model|Term Structure Dynamics]].. 473   
14.4.1 The Framework. 474   
14.4.2 Normalization and Forward Measure... 475   
14.4.2.1 [[Verifying Martingale Property with Q|Risk-neutral measure]] is inconvenient.. 475   
14.4.2.2 The forward measure. 477   
14.4.2.3 [[Arbitrage Pricing of Derivatives|Arbitrage]]-free SDEs for forward rates . 478   
14.4.3 [[Arbitrage Pricing of Derivatives|Arbitrage]]-Free Dynamics ... 479   
14.4.3.1 Review .. 481   
14.4.4 A [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] Implementation . 482   
...
14.6.2 [[Arbitrage Pricing of Derivatives|Pricing]] CMS.. 493   
4.7 In-Arrears Swaps and [[PSET II Fixed Income Asset Pricing 1|Convexity]] ..... 494   
14.7.1 Valuation .. 495   
14.7.2 Special Case .. 497   
4.8 Cross-[[Financial Instruments PSET Solutions|Currency Swaps]].. . 498   
14.8.1 [[Arbitrage Pricing of Derivatives|Pricing]] . 499   
...
[[Exercises|Exercises]] ... .. 502   
EXCEL [[Exercises|Exercises]] . 503   
MATLAB Exercise . 505  

# 14.1 INTRODUCTION  

This chapter extends the discussion of swap-type instruments and outlines a simple framework for fixed-income security [[Arbitrage Pricing of Derivatives|pricing]]. [[Hull White Term Structure Simulations with QuantLib Python|Term structure modeling]] is treated within this framework. The chapter also introduces the recent models that are becoming a benchmark in this sector.  

Until the late 1990s, short-rate modeling was the most common approach in [[Arbitrage Pricing of Derivatives|pricing]] and riskmanaging fixed-income securities. The publication in 1992 of the Heath Jarrow [[Credit Markets Session 5|Merton]] (HJM) approach enabled [[Arbitrage Pricing of Derivatives|arbitrage]]-free modeling of multifactor-driven [[Financial Mathematics Course|term structure models]], but markets continued to use short-rate modeling. A few years ago the situation changed. The Forward LIBOR or Brace Gatarek Musiela (BGM) Model published in 1997 became the market standard for [[Arbitrage Pricing of Derivatives|pricing]] and [[Financial Mathematics Course|risk management]].  

During the GFC, the LIBOR rate rose significantly and diverged from the overnight indexed swap (OIS) rate raising questions about whether LIBOR is an appropriate riskless discount. In Chapter 24, we will discuss recent models and market developments such as the use of OIS curve as a risk-free curve for discounting. The use of the OIS curve implies that more than one zero curve must be modeled for the purpose of [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|pricing derivatives]]. The LIBOR curve is used to determine payoffs and the OIS zero curve is used for discounting. This chapter will discuss the Forward LIBOR Model and we will deal with more complex and recent issues such as approaches that simultaneously or separately model the LIBOR and the OIS curve in Chapter 24.  

This chapter will approach the issues from a practical point of view using swap markets and swap [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] as a background. We are interested in providing a framework for analyzing the mechanics of swaps and swap [[Chapter 9 Arbitrage and Hedging With Options|derivatives]], for decomposing them into simpler instruments, and for constructing synthetics. Recent models of fixed-income modeling can then be built on this foundation very naturally.  

It is worth reviewing the basic principles of swap engineering laid out in Chapter 4. First of all, swaps are almost always designed such that their value at initiation is zero. This is a characteristic of modern swap-type “spread instruments,” and there is no surprise here. Second, what makes the value of the swap equal to zero is a spread or an interest rate that is chosen with the purpose that the initial value of the swap vanishes. Third, swaps encompass more than one settlement date. This means that whatever the value of the [[Teaching Note 4 Interest Rate Derivatives|swap rate]] or swap spread, these will in the end be some sort of “average of shorter term floating rates or spreads.” This not only imposes simple [[Arbitrage Pricing of Derivatives|arbitrage]] conditions on relevant market rates but also provides an opportunity to trade the volatility associated with such averages through the use of options on swaps. Since swaps are very liquid, they form an excellent underlying for swaptions. Swaptions, in turn, are related to interest rate volatilities for the underlying subperiods, which will relate to cap/floor volatilities. This structure is conducive to  

![](f5787f5bbf4d20515b24c53782c2e3794a89d88baafb9f68cdfefcc99a607f79.jpg)  

# FIGURE 14.1  

Three-period [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]].  
...
Finally, the chapter will further use the developed framework to illustrate the advantages of measure change technology. Switching between various $T_{\mathbf{\delta}}$ -forward measures, we show how [[PSET II Fixed Income Asset Pricing 1|convexity]] effects can be calculated.  

Most of the discussion will center on a three-period swap first, and then generalize the results. We begin with this simple example, because with a small number of cash flows the analysis becomes more manageable and easier to understand. Next, we lay out a somewhat more technical framework for engineering [[Lecture 7-Risk and Return of Bonds|fixed-income instruments]]. Eventually, this is developed into the Forward LIBOR Model. Within our framework, measure changes using Girsanov-type transformations emerge as fundamental tools of financial engineering. The chapter discusses how measures can be changed sequentially during a numerical [[Arbitrage Pricing of Derivatives|pricing]] exercise as was done in the simulation of the Forward LIBOR Model. These tools are then applied to CMS, which are difficult to price with traditional models.  

# 14.2 A FRAMEWORK FOR SWAPS  

We work with forward fixed payer interest rate swaps and their “spot” equivalent. These are vanilla products in the sense that contracts are predesigned and homogeneous. They are liquid, the [[Bid Ask and Transaction Prices in a Specialist Market With Heterogeneously Informed Traders|bid ask]] spreads are tight, and every market player is familiar with their properties and related conventions.  

To simplify the discussion we work with a three-period [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]], shown in Figure 14.1. It is worth repeating the relevant parameters again, given the somewhat more technical approach the chapter will adopt.  
...
2. The swap maturity is three periods. The swap ends at time $T=t_{4}$ . The [[Currency Swaps|swap contract]] is signed at time $t_{0}$ but starts at time $t_{1}$ , hence the term [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]] is used.1  
...
[[Preview of the Book|Cash flow]] diagrams of three FRAs paid in arrears.  
...
According to this, if these FRAs are liquid at time $t_{0}$ , the known [[Preview of the Book|cash flow]] sequence in Eq. (14.5) is perceived by the markets as the correct exchange against the unknown, floating payments in Eq. (14.4). If we then consider the [[Chapter 34 - Pricing Interest Rate Swaps|swap cash flows]] shown in Figure 14.1, we note that exactly the same floating [[Preview of the Book|cash flow]] sequence as in Eq. (14.4) is exchanged for the known and fixed swap leg  
$$
...
This has an important implication. It means that the time $t_{0}$ values of the two [[Preview of the Book|cash flow]] sequences are the same. Otherwise, one party would demand an initial cash payment. Given that the cash flows are known as of time $t_{0}$ , their equivalence provides an equation that can be used in [[Arbitrage Pricing of Derivatives|pricing]], as we will see next. This argument will be discussed further using the Forward LIBOR Model.  

# 14.2.2 PRICING THE SWAP  

We have determined two known [[Preview of the Book|cash flow]] sequences the market is willing to exchange at no additional cost. Using this information, we now calculate the time $t_{0}$ values of the two cash flows. To  

do this, we use the second component of our framework, namely, the discount bond prices given in Figure 14.2.  

Suppose the pure discount bonds with [[Arbitrage Pricing of Derivatives|arbitrage]]-free prices $B(t_{0},t_{i}),i=1,2,3,4$ are liquid and actively traded. We can then use $\{B(t_{0},\ t_{2}),B(t_{0},\ t_{3}),B(t_{0},\ t_{4})\}$ to value cash flows settled at times $t_{2},t_{3}$ , and $t_{4}$ , respectively.4 In fact, the time $t_{0}$ value of the sequence of cash flows,  
$$
\{F(t_{0},t_{1})N\delta,F(t_{0},t_{2})N\delta,F(t_{0},t_{3})N\delta\}
$$  

is given by multiplying each [[Preview of the Book|cash flow]] by the [[Discount Factors|discount factor]] that corresponds to that particular settlement date and then adding. We use the default-free bond prices as our discount factors and obtain the value of the fixed FRA cash flows  
$$
...
The time $t_{0}$ value of the fixed [[Chapter 34 - Pricing Interest Rate Swaps|swap cash flows]] can be calculated similarly  
$$
...
This equality has at least two important implications. First, it implies that the value of the swap at time $t_{0}$ is zero. Second, note that equality can be used as an equation to determine the value of one unknown. As a matter of fact, [[Arbitrage Pricing of Derivatives|pricing]] the swap means determining a value for $s_{t_{0}}$ such that the equation is satisfied. Taking $s_{t_{0}}$ as the unknown, we can rearrange Eq. (14.12), simplify, and obtain  
$$
s_{t_{0}}=\frac{B(t_{0},t_{2})F(t_{0},t_{1})+B(t_{0},t_{3})F(t_{0},t_{2})+B(t_{0},t_{4})F(t_{0},t_{3})}{B(t_{0},t_{2})+B(t_{0},t_{3})+B(t_{0},t_{4})}
$$  

This [[Arbitrage Pricing of Derivatives|pricing]] formula can easily be generalized by moving from the three-period setting to a vanilla (forward) swap that makes $n$ payments starting at time $t_{2}$ . We obtain  
$$
...
The formula that gives the [[Arbitrage Pricing of Derivatives|arbitrage]]-free value of the (forward) swap has a nice interpretation. For simplicity, revert to the three-period case. Rewrite Eq. (14.13) as  
$$
...
According to this expression, we see that the “correct” (forward) [[Teaching Note 4 Interest Rate Derivatives|swap rate]] is a weighted average of the FRA paid-in-arrears rates during the life of the swap:  
$$
...
Thus, the (forward) [[Teaching Note 4 Interest Rate Derivatives|swap rate]] is an average paid-in-arrears FRA rate. We emphasize that this is true as long as the FRAs under consideration are paid in arrears. There are, on the other hand, so-called LIBOR-in-arrears FRAs where a [[Chapter 10 - Bonds: Duration and Convexity|convexity adjustment]] needs to be made for the argument to hold.5  
...
According to these formulas, three important components of our [[Chinese Financial System|pricing framework]]—the swap market, the FRA market, and the bond market—are interlinked through nonlinear functions of forward rates. The important role played by the forward rates in these formulas suggests that obtaining [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics of the latter is required for the [[Arbitrage Pricing of Derivatives|pricing]] of all swap and swap-related [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. The Forward LIBOR Model does exactly this. Because this model is set up in a way as to fit market conventions, it is also practical.  
...
<html><body><table><tr><td>Term</td><td>[[Bid-Ask Prices with Adverse SelectionPrivate Information|Bid-Ask]]</td></tr><tr><td>0X6 4.05-4.07</td><td></td></tr><tr><td>6X12</td><td>4.15-4.17</td></tr><tr><td>12X18</td><td>4.32-4.34</td></tr><tr><td>18 ×24 4.50-4.54</td><td></td></tr></table></body></html>  

Also, suppose the following treasury strip prices are observed:  

<html><body><table><tr><td>Maturity</td><td>[[Bid-Ask Prices with Adverse SelectionPrivate Information|Bid-Ask]]</td></tr><tr><td>12 months</td><td>96.00-96.02</td></tr><tr><td>18 months</td><td>93.96-93.99</td></tr><tr><td>24 months</td><td>91.88-91.92</td></tr></table></body></html>  

We can ask two questions. First, are these data [[Arbitrage Pricing of Derivatives|arbitrage]]-free so that they can be used in obtaining an [[Arbitrage Pricing of Derivatives|arbitrage]]-free [[Teaching Note 4 Interest Rate Derivatives|swap rate]]? Second, if they are, what is the implied [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]] rate for the period that starts in 6 months and ends in 24 months?  

The answer to the first question can be checked by using the following [[Arbitrage Pricing of Derivatives|arbitrage]] equality, written for discount bonds with par value $\$100$ , as market convention suggests:  
$$
...
The relevant equalities hold for other discount bond prices as well. This means that the data are [[Arbitrage Pricing of Derivatives|arbitrage]] free and can be used in finding an [[Arbitrage Pricing of Derivatives|arbitrage]]-free [[Teaching Note 4 Interest Rate Derivatives|swap rate]] for the abovementioned forward start swap.  
...
The asking [[Teaching Note 4 Interest Rate Derivatives|swap rate]] is  
$$
...
Now, we would like to obtain an alternative way of looking at the same [[Teaching Note 4 Interest Rate Derivatives|swap rate]] by modifying the formula. We start the discussion with the [[Arbitrage Pricing of Derivatives|arbitrage]] relation between the discount bond prices, $B(t_{0},t_{i})$ , and the forward rates, $F(t_{0},t_{i})$ , obtained earlier in Chapter 3:  
$$
...
The equation equates two cash flows. $B(t_{0},t_{1})$ is the value of 1 dollar to be received at time $t_{1}$ . Thus, the position needs to be long a $t_{1}$ -maturity discount bond. Second, there appear to be [[Realized Returns|coupon payments]] of constant size, $\delta s_{t_{0}}$ at times $t_{2}$ , $t_{3}$ , $t_{4}$ and then a payment of 1 dollar at time $t_{4}$ .7 Thus, this seems to be a short (forward) position in a $t_{4}$ -maturity coupon bond with coupon rate $s_{t_{0}}$ .  

To summarize, this particular forward fixed receiver [[Primer on Interest Rate Swaps|interest rate swap]] is equivalent to  

Fixed-payer forward sw $\mathrm{{ap}=\{B u y\ }}t_{1}$ discount bond; forward sell $t_{4}$ -maturity coupon bond  

This synthetic will replicate the value of the [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]]. Note that the floating cash flows do not have to be replicated. This is because, in a [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]], the floating cash flows are related to deposits (loans) that will be made in the future, at [[Interest Rate Quotations|interest rates]] to be determined then.  
...
Now, let a short but noninfinitesimal time, $\Delta$ , pass. There will be a new [[Teaching Note 4 Interest Rate Derivatives|swap rate]] $s_{t_{0}}+\Delta$ , which, in all probability, will be different than $s_{t_{0}}.$ This means that the market is now willing to pay the new known cash flows  
$$
...
This way of writing the expression shows the profit and loss from the point of view of a fixed receiver. It should be noted that here the weights $\omega_{i}$ have time subscripts, since they will change as time passes. Thus, managing a swap book will depend nonlinearly on the [[Forward Points in Currency|forward rate]] dynamics.9  

# 14.3 TERM STRUCTURE MODELING  

The framework outlined in this chapter demonstrates the links between swap, bond, and FRA markets. We will now discuss the [[The Vasicek Model|term structure]] implications of the derived formulas. The set of formulas we studied implies that, given the necessary information from two of these markets, we can, in principle, obtain [[Arbitrage Pricing of Derivatives|arbitrage]]-free prices for the remaining market.10 We discuss this briefly, after noting the following small, but significant, modification. [[Financial Mathematics Course|Term structure models]] concern forward rates as well as spot rates. As a matter of fact, traditional yield curve construction is done by first obtaining the spot yields and then moving to forward rates.  

Following this tradition, and noting that spot swaps are more liquid than forward swaps, in this section we let $s_{t_{0}}^{n}$ denote the spot [[Teaching Note 4 Interest Rate Derivatives|swap rate]] with maturity $n$ years. Without loss of generality, we can assume that swap maturities are across years $n=1,...,30$ , so that the longest dated swap is for 30 years.11 The discussion will be conducted in terms of spot swap rates.  

# 14.3.1 DETERMINING THE FORWARD RATES FROM SWAPS  

Given a sufficient number of [[Arbitrage Pricing of Derivatives|arbitrage]]-free values of observed spot swap rates $\left\{s_{t_{0}}^{n}\right\}$ and using the equalities  
$$
...
to calculate the [[Arbitrage Pricing of Derivatives|arbitrage]]-free values of the relevant pure discount bond prices. In each case, we can derive the values of $B(t_{0},t_{i})$ from the observed $\{F(t_{0},t_{\mathrm{i}})\}$ and $\{s_{t_{0}}^{n}\}$ . This procedure would price the FRAs and bonds off the swap markets. It is called the curve algorithm.  

# 14.3.3 DETERMINING THE SWAP RATE  

We can proceed in the opposite direction as well. Given [[Arbitrage Pricing of Derivatives|arbitrage]]-free values of forward rates, we can, in principle, use the same formulas to determine the swap rates. All we need to do is (i) calculate the discount bond prices from the forward rates and (ii) substitute these bond prices and the appropriate forward rates in our formula:  
$$
s_{t_{0}}^{n}=\frac{\sum_{i=0}^{n-1}B(t_{0},t_{i+1})F(t_{0},t_{i})}{\sum_{i=0}^{n-1}B(t_{0},t_{i+1})}
$$  

Repeating this for all available $s_{t_{0}}^{n}$ $,n=1,...,30$ , we obtain the [[Arbitrage Pricing of Derivatives|arbitrage]]-free [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]] and discounts. In this case, we would be going from the spot and forward LIBOR curve to the (spot) [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]].  
...
Second, observed quotes on forward rates do not necessarily come from paid-in-arrears FRAs. Market-traded FRAs settle at the time the LIBOR rate is observed, not at the end of the relevant period. The FRA rates generated by these markets will be consistent with the formulas introduced earlier. On the other hand, some traders use [[Chapter 12 - Hedging with Interest Rate Futures|interest rate futures]], and, specifically, [[The Eurocurrency and Eurobond Markets Annotations|Eurocurrency]] [[Futures Not Subject to Cash-And-Carry|futures]], in [[Key Rates O1s Durations and Hedging|hedging]] their swap books. [[Contango And Backwardation In Arbitrage-Free Futures-Markets|Futures markets]] are more transparent than the FRA markets and have a great deal of [[Class Note 10 Liquidity and Class Note 10 Liquidity and Liquidity Managementliquidity management|liquidity]]. But the forward rates determined in [[Contango And Backwardation In Arbitrage-Free Futures-Markets|futures markets]] require [[PSET II Fixed Income Asset Pricing 1|convexity]] adjustments before they can be used in the swap formulas discussed in this chapter.  

Third, the [[Class Note 10 Liquidity and Class Note 10 Liquidity and Liquidity Managementliquidity management|liquidity]] of FRA and swap rates depends on the maturity under consideration. As mentioned earlier, FRAs are more liquid for the shorter end of the curve, whereas swaps are more liquid at the longer end. This means that it may not be possible to go from FRA rates to swap rates for maturities over 5 years. Similarly, for very short maturities there will be no observed quotes for swaps.  

# 14.3.4.1 Remark  

Another important point needs to be mentioned here. In this chapter, for simplicity, our treatment has followed the pre-GFC convention in academic work of using the term “LIBOR” as if it relates to a default-free loan. In practice, LIBOR rates $L_{t_{i}}$ are not risk free. Admittedly, this was implicit in the fixing process of the LIBOR rate. The banks that contributed to the rate had, in general, ratings of AA or $\mathrm{\AA-}$ , and the interest rate that they paid reflected this level of [[Quantitative Trading Strategies Lecture Notes|credit risk]]. However, the probability of such highly rated banks defaulting was considered small and therefore market participants and academics considered LIBOR a reasonable proxy for the [[Black Scholes Derivation|risk-free rate]]. Before the GFC, LIBOR was a key indicator of what banks were willing to lend to each other, and 3-month LIBOR in particular emerged as a key benchmark in the interest rate swaps market. As discussed in Chapter 3 during the GFC, the LIBOR rate rose dramatically and diverged from the OIS as fears of counter-party risk became acute. This led to the realization that LIBOR could not be used as a measure of the [[Black Scholes Derivation|risk-free rate]]. Following the GFC, market participants started using the OIS as the [[PSET 7- Kohler|discount rate]] for the [[Arbitrage Pricing of Derivatives|pricing]] of (collateralized) interest rate swaps. In June 2010, LCH.Clearnet announced that it would use the OIS instead of LIBOR to discount its $\$218$ trillion [[Primer on Interest Rate Swaps|interest rate swap]] [[An Asset Allocation Primer|portfolio]]. The intuition is that in fully collateralized swaps, financial counterparties are exposed to overnight risk to other financial counterparties only and not the risk of the full term of a particular loan or instrument. Academic models have been developed to incorporate credit and counter-party risk more explicitly into [[Solutions|derivatives pricing models]]. Such models and recent market practice are discussed in detail in Chapter 24.  

Thus, if a financial engineer follows the procedures described here, the resulting curve will be the [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]] and not the treasury or sovereign curve. This [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]] will be “above” the sovereign or treasury curve, and the difference will be the curve for the swap spreads.  

# 14.4 TERM STRUCTURE DYNAMICS  

In the remainder of this chapter, we will see that the Forward LIBOR Model is the correct way to approach [[The Gauss Model|term structure dynamics]]. The model is based on the idea of converting the dynamics of each [[Forward Points in Currency|forward rate]] into a Martingale using some properly chosen forward measure. According to the linkages between sectors shown in this chapter, once such dynamics are obtained, we can use them to generate dynamics for other [[Lecture 7-Risk and Return of Bonds|fixed-income instruments]].  

Most of the derivation associated with the Forward LIBOR Model is an application of the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|fundamental theorem]] of [[Fixed Income Asset Pricing|asset pricing]] discussed in Chapter 12. Thus, we continue to use the same finite-state world discussed in Chapter 12. The approach is mostly straightforward. There is only one aspect of Forward LIBOR or swap models that makes them potentially difficult to follow. Depending on the instruments, [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics of different forward rates may have to be expressed under the same forward measure. The methodology then becomes more complicated. It requires a judicious sequence of Girsanov-style measure changes to be applied to [[Forward Points in Currency|forward rate]] dynamics in some recursive fashion. Otherwise, [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics of individual forward rates would not be correctly represented.  

The Girsanov theorem is a powerful tool, but it is not easy to conceive such successive measure changes. Doing this within a discrete framework, in a discrete setting, provides a great deal of motivation and facilitates understanding of [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics. This is the purpose behind the second part of this chapter.  
...
4. An FRA contracted at time $t_{0}$ and settled at time $t_{2}$ , where the buyer receives/pays the differential between the fixed rate $F(t_{0},t_{1})$ and the floating rate $L_{t_{1}}$ at time $t_{2}$ . We let the notional amount of this instrument equal 1 and abbreviate the [[Forward Points in Currency|forward rate]] to $F_{t_{0}}$ . The final payoff can be written as  
$$
...
where the $B_{t_{2}}^{i j}$ is the (random) value of the $t_{3}$ maturity discount bond at time $t_{2}$ . This value will be state dependent at $t_{2}$ because the bond matures one period later, at time $t_{3}$ . Looked at from time $t_{0}$ , this value will be random. Clearly, with this $D$ matrix we have simplified the notation significantly. We are using only four states of the world, expressing the [[Forward Points in Currency|forward rate]] $F(t_{0},t_{2})$ simply as $F_{t_{0}}$ , and the $B(t_{2},t_{3})^{i j}$ simply as $B_{t_{2}}^{i j}$ .  

If the FRA, the savings account, and the two bonds do not admit any [[Class Note 13 The LTCM Meltdown|arbitrage opportunities]], the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|fundamental theorem]] of [[Fixed Income Asset Pricing|asset pricing]] permits the following linear representation:  
$$
...
where $\{Q^{i j},i,j=u,d\}$ are the four state prices for period $t_{2}$ . Under the no-[[Arbitrage Pricing of Derivatives|arbitrage]] condition, the latter exist and are positive  
$$
Q^{i j}>0
$$  
...
This matrix equation incorporates the ideas that (i) the fair market value of an FRA is zero at initiation, (ii) 1 dollar is to be invested in the savings account originally, and (iii) the bonds are default free. They mature at times $t_{2}$ and $t_{3}$ . $R_{t_{2}}^{i,j}$ , finally, represents the gross [[Assets|returns]] to the savings account as of time $t_{2}$ . Because the interest rate that applies to time $t_{i}$ is paid in arrears, at time $t_{i}+\delta$ , we can express these gross [[Assets|returns]] as functions of the underlying LIBOR rates in the following way:  
$$
...
As usual, define the [[Verifying Martingale Property with Q|risk-neutral measure]] $\{\tilde{p}_{i j}\}$ using the first row of the matrix equation:  
$$
...
We know from Chapter 12 that, under the condition that every asset’s price is [[Arbitrage Pricing of Derivatives|arbitrage]] free, $\{Q^{i j},i,j=u,d\}$ exist and are all positive, and $\tilde{p}_{i j}$ will be the [[Financial Instruments|risk-neutral probabilities]]. Then, by using the last row of the system in Eq. (14.58) we can write the following equality:  
$$
...
That is to say, under the [[Verifying Martingale Property with Q|risk-neutral measure]], $\tilde{P}$ , the [[Forward Points in Currency|forward rate]] for time $t_{i}$ is a biased “forecast” of the future LIBOR rate $L_{t_{i}}$ . In fact, it is not very difficult to see that the $E_{t}^{\tilde{p}}[L_{t_{i}}]$ is the [[Futures Not Subject to Cash-And-Carry|futures]] rate that will be determined by, say, a Eurodollar contract at time $i$ . The “bias” in the [[Forward Points in Currency|forward rate]], therefore, is associated with the [[Chapter 10 - Bonds: Duration and Convexity|convexity adjustment]].  
...
where $\boldsymbol{a}(\boldsymbol{F}_{t},t)$ is the non-zero expected rate of change of the [[Forward Points in Currency|forward rate]] under the probability $\Tilde{P}_{-}$  

The fact that $F_{t}$ is not a Martingale with respect to probability $\tilde{P}$ makes the [[Verifying Martingale Property with Q|risk-neutral measure]] an inconvenient working tool for [[Arbitrage Pricing of Derivatives|pricing]] and [[Financial Mathematics Course|risk management]] in the fixed-income sector. Before we can use Eq. (14.71), we need to calibrate the drift factor $a(.)$ . This requires first obtaining a functional form for the drift under the probability $\tilde{P}$ . The original HJM article does develop a functional form for such drifts using continuously compounded instantaneous forward rates. But, this creates an environment quite different from LIBOR-driven markets and the associated actuarial rates Lti used here.16  

On the other hand, we will see that in the interest rate sector, [[Arbitrage Pricing of Derivatives|arbitrage]]-free drifts become much easier to calculate if we use the Forward LIBOR Model and switch to appropriate forward measures.  
...
This implies that, for an asset that settles at time $T$ and has no other payouts, the general [[Definitions and Immediate Consequences|pricing equation]] is given by  
$$
...
which is a Martingale with respect to the measure $\Tilde{P}^{T}$ . In fact, $B(t,T)$ being the [[Discount Factors|discount factor]] for time $T.$ , and, hence, being $^{<1}$ , $Z_{t}$ is nothing more than the $T_{\mathbf{\delta}}$ -forward value of the $C_{t}$ . This means that the forward measure $\Tilde{P}^{T}$ operates in terms of Martingales that are measured in time $T$ dollars. The advantage of the forward $\Bar{P}^{T}$ measure becomes clear if we apply the same transformation to price the FRA as was done earlier for the case of the [[Verifying Martingale Property with Q|risk-neutral measure]].  

# 14.4.2.3 Arbitrage-free SDEs for forward rates  

To get [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics for forward rates, we now go back to the simple model in Eq. (14.58). Divide the fourth row of the system by $B(t_{0},t_{2})$ and rearrange,  
$$
...
This means that, under the measure $\tilde{P}^{t_{2}}$ , the [[Forward Points in Currency|forward rate]] for the period $[t_{1},~t_{2}]$ will be an unbiased estimate of the corresponding LIBOR rate.  
...
where $W_{t}$ is a [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] under the measure $\tilde{P}^{T+\delta}.\Delta W_{t}$ is the [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] increment:  
$$
\Delta W_{t}=W_{t+\Delta}-W_{t}
$$  

This (approximate) equation has no drift component since, by [[Forward Contracts and Forward Prices|arbitrage arguments]], and writing for the general $t,T,$ we have  
$$
...
However, note that the [[Forward Points in Currency|forward rate]] for the period $[T-\delta,T]$ given by  
$$
1+\delta F(t,T-\delta)=\frac{B(t,T-\delta)}{B(t,T)}
$$  

is not a Martingale under the same forward measure $\tilde{P}^{T^{+\delta}}$ . Instead, this [[Forward Points in Currency|forward rate]] is a Martingale under its own measure $\Tilde{P}^{T}$ which requires normalization by $B(t,T)$ . Thus, we get a critical result for the Forward LIBOR Model:  

Each [[Forward Points in Currency|forward rate]] F(t, Ti) admits a Martingale representation under its own forward measure P\~Ti1δ.  

This means that each [[Forward Points in Currency|forward rate]] dynamics can be approximated individually by a difference equation with no drift given the proper normalization. The only parameter that would be needed to characterize such dynamics is the corresponding [[Forward Points in Currency|forward rate]] volatility.  
...
“dynamics” of the [[Forward Points in Currency|forward rate]] $F(t_{0},t_{2})$ . In our simplified setup, we will observe only two future values of this [[Forward Points in Currency|forward rate]] at times $t_{1}$ and $t_{2}$ . These are given by  
$$
...
The superscript in $\triangle_{\ast t.}W_{t_{i}}^{t_{3}},~i=1,2$ , indicates that the [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] increments have zero mean under the probability $\tilde{P}^{t_{3}}$ . These equations show how the “current” value of the [[Forward Points in Currency|forward rate]] $F(t_{0},t_{2})$ first changes to become $F(t_{1},t_{2})$ and then ends up as $F(t_{2},t_{2})$ . The latter is also $L_{t_{2}}$ .  

For the “nearer” [[Forward Points in Currency|forward rate]] $F(t_{0},t_{1})$ , we need only one equation18 defined under the normalization with the bond $B(t_{0},t_{2})$ (i.e., the $\tilde{P}^{t_{2}}$ measure) and the associated zero drift:  
$$
F(t_{1},t_{1})-F(t_{0},t_{1})=\sigma_{1}F(t_{0},t_{1})\Delta W_{t_{1}}^{t_{2}}
$$  

Similarly, the superscript in $\Delta W_{t_{1}}^{t_{2}}$ indicates that this [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] increment has zero mean under the probability $\tilde{P}^{t_{2}}$ . Here, the $F(t_{1},t_{1})$ is also the LIBOR rate $L_{t_{1}}$ . We reemphasize that each dynamic is defined under a different probability measure. Under these different forward measures, each forward LIBOR process behaves like a Martingale.19 Consequently, there are no drift terms in either equation.  

Fortunately, as long as we can work with these equations separately, no [[Arbitrage Pricing of Derivatives|arbitrage]]-free drift terms need to be estimated or calibrated. The only parameters we need to determine are the volatilities of the two forward rates: $\sigma_{2}$ for the [[Forward Points in Currency|forward rate]] $F(t_{0},t_{2})$ and $\sigma_{1}$ for the [[Forward Points in Currency|forward rate]] $F(t_{0},t_{1})$ .20  
...
Yet, when we evaluate the [[FORWARD RATES AND TERM STRUCTURE|expectations]] under $\tilde{P}^{t_{2}}$ , we get  
$$
E_{t_{0}}^{\tilde{P}^{t_{2}}}\left[\Delta W_{t_{1}}^{t_{2}}\right]=0
$$  
...
The results thus far indicate that for the [[Arbitrage Pricing of Derivatives|pricing]] and risk managing of equity-linked assets, the riskneutral measure $\tilde{P}$ may be quite convenient since it is easily adaptable to lognormal models where the [[Arbitrage Pricing of Derivatives|arbitrage]]-free drifts are simple and known functions of the [[Exercises|risk-free interest rate]]. As far as equity products are concerned, the assumption that short rates are constant is a reasonable approximation, especially for short maturities. Yet, for contracts written on future values of [[Interest Rate Quotations|interest rates]] (rather than on asset prices), the use of the $\tilde{P}$ leads to complex [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics that cannot be captured easily by Martingales and, hence, the corresponding [[Arbitrage Pricing of Derivatives|arbitrage]]-free drift terms may be difficult to calibrate.  

Appropriate forward measures, on the other hand, result in Martingale equalities and lead to dynamics convenient for the calculation of [[Arbitrage Pricing of Derivatives|arbitrage]]-free drifts, even when they are not zero. Forward (and swap) measures are the proper working probabilities for fixed-income environments.  

# 14.4.4 A MONTE CARLO IMPLEMENTATION  

Suppose we want to generate [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] “paths” from the two discretized SDEs for two forward rates, $F(t_{i},t_{1})$ and $F(t_{i},t_{2})$ ,  
$$
...
where the superscript on the left-hand side indicates that these values are for the first [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] trajectory. Proceeding sequentially, all the terms on the right-hand side will be known. This gives the first simulated “path” $\{F(t_{0},\dot{t_{2}}),F(t_{1},t_{2})^{1},F(t_{2},t_{2})^{1}\}$ . We can repeat this algorithm to obtain $M$ such paths for potential use in [[Arbitrage Pricing of Derivatives|pricing]].  
...
Thus, we get the correct result under the $\tilde{P}^{t_{2}}$ , after the mean correction. It is obvious that we need to determine these correction factors before the randomly generated [[Continuous-Time Stochastic Processes|Brownian motion]] increments can be used in all equations.  

Yet, note the following simple case. If the instrument under consideration has additive cash flows where each [[Preview of the Book|cash flow]] depends on a single [[Forward Points in Currency|forward rate]], then individual zero-drift equations can be used separately to generate paths. This applies for several liquid instruments. For example, FRAs and especially swaps have payment legs that depend on one LIBOR rate only. Individual zero-drift equations can be used for valuing each leg separately, and then these values can be added using observed zero-coupon bond prices, $B(t,\ T_{i})$ . However, this cannot be done in the case of CMSs, for example, because each settlement leg will depend nonlinearly on more than one [[Forward Points in Currency|forward rate]].  

We now discuss further how mean corrections can be conducted so that all forward rates are projected using a single forward measure. This will permit [[Arbitrage Pricing of Derivatives|pricing]] instruments where individual cash flows depend on more than one [[Forward Points in Currency|forward rate]].  
...
denote settlement dates of some basic [[Primer on Interest Rate Swaps|interest rate swap]] structure and limit our attention to forward rates for successive forward loans contracted to begin at $t_{i}.$ , and paid at $t_{i+1}$ . An example is shown in Figure 14.4.  

Result 1  

The [[Forward Points in Currency|forward rate]] at time $t$ , for a LIBOR-based forward loan that starts at time $t_{i}$ and ends at time $t_{i}+\delta$ , denoted by $F(t,t_{i})$ , admits the following [[Arbitrage Pricing of Derivatives|arbitrage]] relationship:  
$$
...
The left side of this equality is a gross forward return. The right side, on the other hand, is a traded [[A Preview of Alternative Formulations|asset price]], $B(t,t_{i})$ , normalized by another [[A Preview of Alternative Formulations|asset price]], $B(t,{t_{i+1}})$ . Hence, the ratio will be a Martingale under a proper measure—here, the forward measure denoted by $\tilde{P}^{t_{i+1}}$ .  

Result 2  

In a discrete state setting with $k$ states of the world, assuming that all asset prices are [[Arbitrage Pricing of Derivatives|arbitrage]] free, and that the time $t_{i}$ state prices $Q^{j},j=1,...,k.$ , with $0<Q^{j}$ exist, the time $t_{i}$ values of the forward measure $\tilde{P}^{t_{i}}$ are given by23  
$$
...
Consider a one-factor setting, where a single [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] causes fluctuations in the two forward rates. Suppose that in this setting, starting from time $t$ , with $t<t_{i},i=1,...,n,a$ small time interval denoted by $h$ passes with $t+h<t_{i}$ . By imposing a Gaussian volatility structure, we can write down the individual discretized [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics for two successive forward rates $F\left(t,t_{i}\right)$ and $F(t,t_{i+1})$ as  
$$
...
Changes in these forward rates have zero mean under their own forward measure and, hence, are written with zero drift. This means that the unique real-world [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] $W_{t+h}$ is now denoted by $\Delta W_{t+h}^{1}$ and $\Delta W_{t+h}^{2}$ in the two equations. These are normally distributed, with mean zero and variance $h$ only under their own forward measures, $\tilde{P}^{t_{i+1}}$ and $\tilde{P}^{t_{i+2}}$ .  
...
The individual Martingale dynamics are very convenient from a financial engineering point of view. The respective drift components are zero and, hence, they need not be modeled during [[Arbitrage Pricing of Derivatives|pricing]]. The only major task of the market practitioner is to get the respective volatilities $\sigma^{i}$ and $\bar{\sigma}^{i+1}$ .  

However, some securities’ prices may depend on more than one [[Forward Points in Currency|forward rate]] in a nonlinear fashion and their value may have to be calculated as an expectation under one single measure. For example, suppose a security’s price, $S_{t}$ , depends on $F(t,t_{i})$ and $F(t,t_{i+1})$ through a [[Arbitrage Pricing of Derivatives|pricing]] relation such as:  
$$
S_{t}=E_{t}^{\tilde{P}}[g(F(t,t_{i}),F(t,t_{i+1}))]
$$  

where $g(.)$ is a known nonlinear function. Then, the expectation has to be calculated under one measure only. This probability can be either the time $t_{i+1}$ or the time $t_{i+2}$ forward measure. We then have to choose a [[Forward Points in Currency|forward rate]] equation with Martingale dynamics and carry out a mean correction to get the correct [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics for the other. The forward measure of one of the Martingale relationships is set as the working probability distribution, and the other equation(s) is obtained in terms of this unique probability by going through successive measure changes. We discuss this in detail below.  

# 14.5.1 THE MECHANICS OF MEASURE CHANGES  

We have the following [[FORWARD RATES AND TERM STRUCTURE|expectations]] concerning $\Delta W_{t+h}^{1}$ and $\Delta W_{t+h}^{2}$ , defined in Eqs. (14.109) and (14.110)  
$$
...
The value of $\lambda_{t}$ gives the correction factor that we need to use in order to obtain the correct [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics, if the working measure is $\tilde{P}^{t_{i+2}}$ . Calculating this factor implies that we can change measures in the dynamics of $F(t,t_{i})$ .  
...
As can be seen from this expression, the new dynamics have a non-zero drift and $F(t,\ t_{i})$ is not a Martingale under the new measure. Yet, this process is [[Arbitrage Pricing of Derivatives|arbitrage]] free and easy to exploit in [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] type approaches. Since both dynamics are expressed under the same measure, the set of equations that describe the dynamics of the two forward rates can be used in [[Arbitrage Pricing of Derivatives|pricing]] instruments that depend on these forward rates. The same pseudo-random numbers can be used in the two SDEs. Finally, the reader should remember that the discussion in this section depends on the discrete approximation of the SDEs.  

# 14.5.2 GENERALIZATION  

A generalization of the previous heuristic discussion leads to the Forward LIBOR Model. Suppose the setting involves $n$ forward rates, $F(t_{0},t_{i}),i=0,...,n-1$ , that apply to loans which begin at time $t_{i\cdot}$ , and end at $t_{i+1}=t_{i}+\delta$ . The $F(t_{0},t_{0})$ is the trivial [[Forward Points in Currency|forward rate]] and is the spot LIBOR with tenor $\delta$ . The terminal date is $t_{n}$ .  

Similar to the discussion in the previous section, assume that there is a single factor.26 Using the $t_{i+1}$ forward measure we obtain [[Arbitrage Pricing of Derivatives|arbitrage]]-free Martingale dynamics for each [[Forward Points in Currency|forward rate]] $F(t,t_{i})$ :  
$$
...
These [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics are very useful since they do not involve any [[Arbitrage-Free Interest Rate Models|interest rate modeling]] and are dependent only on the correct specification of the respective volatilities. However, when  

more than one [[Forward Points in Currency|forward rate]] determines a security’s payoff in a nonlinear fashion, the process may have to be written under a unique working measure.  

Suppose we chose $\tilde{P}^{t_{n}}$ as the working measure.28 The heuristic approach discussed in the previous section can be generalized to obtain the following [[Arbitrage Pricing of Derivatives|arbitrage]]-free system of SDEs that involve recursive drift corrections in a one-factor case:  
$$
...
The forward measure and measure change technology are relevant for the [[Arbitrage Pricing of Derivatives|pricing]] of many instruments. But there is one instrument class that has recently become quite popular with market participants and that can be priced with this technology. These are CMSs. They have properties that would illustrate some subtleties of the methods used thus far. In order to price them, forward rates need to be projected jointly.  

First, we present a reading that illustrates some of the recent interest in this instrument class.  

# EXAMPLE  

European [[Chapter 4 Institutional Trading|institutional investors]] mindful of rising [[Interest Rate Quotations|interest rates]] have been turning to constant maturity swap products to hedge their long-dated liabilities over the past few months. Whereas mainstream interest rate swaps tend to reference LIBOR, CMS pay-out structures are linked to the 10-year [[Teaching Note 4 Interest Rate Derivatives|swap rate]] instead—a useful hedge for investors with long-dated liabilities such as insurance companies.  

As the 10-year euro swap cratered from a peak of $3.77\%$ in April 2011 to a low of $I.44\%$ in early May this year, these firms piled into CMS structures to shield themselves against falling [[Interest Rate Quotations|interest rates]]. The spectre of the US Federal Reserve scaling back its asset purchases was enough to put paid to this trend. By the end of June, the 10-year [[Teaching Note 4 Interest Rate Derivatives|swap rate]] had rocketed to above $2\%$ and CMS products are now gaining traction as a hedge against further rises.  

“Buying of CMS caps has picked up in the first half of the year because of the change of sentiment in the market, which has put rising rates to the forefront of investors’ minds,” said Ivan Jossang, head of EMEA rates structuring at [[Lessons From The Crisis|Morgan Stanley]].  

Caps have traditionally been popular with investors due to the relative simplicity of the structure. Take the example of an investor buying a cap with a $3\%$ strike. If rates rise to $4\%$ by the time the cap expires, the investor will be paid 100 bp on the notional it originally agreed. “CMS is the main light [[Exotic Interest Rate Options|exotic]] product that  

survived the crisis, and is the most important building block for pay-offs in rates,” said Adrian Bracher,   
European head of rates structuring at [[The Economist Margin Call of the Wild|Credit Suisse]]. As with many [[Exotic Interest Rate Options|exotic]] products, CMS flourished in the run-up to the [[Squam Lake Group Letter|financial crisis]] and the market has   
...
[[Key Rates O1s Durations and Hedging|hedging]] if rates go up or down. [. . .] Bank [[Key Rates O1s Durations and Hedging|hedging]] of CMS trades has seriously impacted on the swaptions market, making the 10-year euro   
...
that 3-month [[EURIBOR Forward Rate Agreements and Futures|Euribor]], CMS can act as a useful yield enhancement tool, often overlaid on government bonds   
...
CMSs are instruments that build on the plain vanilla swaps in an interesting way. In a vanilla swap, a [[Interest Rate Swaps|fixed swap rate]] is exchanged against a floating LIBOR that is an interest rate relevant for that particular settlement period only. In a CMS, this will be generalized. The [[Pricing Interest Rate Swaps|fixed leg]] is exchanged against a [[Pricing Interest Rate Swaps|floating leg]], but the [[Pricing Interest Rate Swaps|floating leg]] is not a “one-period” rate. It is itself a multiperiod [[Teaching Note 4 Interest Rate Derivatives|swap rate]] that will be determined in the future.  

There are many versions of such exchanges, but as an example we consider the following. Suppose one party decides to pay $4\%$ during the next 3 years against receiving a 2-year [[Teaching Note 4 Interest Rate Derivatives|swap rate]] that will be determined at the beginning of each one of those years. The future swap rates are unknown at time $t_{0}$ and can be considered as floating payments, except they are not floating payments that depend on the perceived volatility for that particular year only. They are themselves averages of 1-year rates. Clearly, such swaps have significant nonlinearities and we cannot do the same engineering as in the case of a plain vanilla swap.  

An example of CMS is shown in Figure 14.5. The reader can see that what is being exchanged at each settlement date against a fixed payment is a floating rate that is a function of more than one [[Forward Points in Currency|forward rate]]. Under these conditions it is impossible to project individual forward rates using individual zero-drift stochastic differential equations defined under different forward measures. Each leg of the CMS depends on more than one [[Forward Points in Currency|forward rate]] and these need to be projected jointly, under a single measure.  
...
We now show that the paid-in-arrears FRA and market-traded FRAs lead to the same [[Forward Points in Currency|forward rate]]. First, remember that under the $\tilde{P}^{t_{i+1}}$ forward measure for paid-in-arrears FRAs, we have:  
$$
...
Would the [[Forward Points in Currency|forward rate]] implied by this contract be the same as the paid-in-arrears FRAs?  

The answer is yes. Using the measure change technology, we discuss how this can be shown. The idea is to begin with the expectation of this settlement amount under the $\tilde{P}^{t_{i}}$ measure and show that it leads to the same [[Forward Points in Currency|forward rate]]. Thus, begin with  
$$
E_{t_{0}}^{\tilde{P}^{t_{i}}}\left[\frac{N\delta[F(t_{0},t_{i})-L_{t_{i}}]}{(1+\delta L_{t_{i}})}\right]
$$  

Setting this equal to zero, and rearranging, leads to the [[Definitions and Immediate Consequences|pricing equation]]  
$$
F(t_{0},t_{i})=\frac{E_{t_{0}}^{\tilde{P}^{t_{i}}}[N\delta L_{t_{i}}/(1+\delta L_{t_{i}})]}{E_{t_{0}}^{\tilde{P}^{t_{i}}}[N\delta/(1+\delta L_{t_{i}})]}
$$  

Now we switch measures on the right-hand side of Eq. (14.142). We have two [[FORWARD RATES AND TERM STRUCTURE|expectations]] and we will switch measures in both of them. But first, let $N=1$ and, similarly, $\delta=1$ .  
...
It is market convention to replace the random variable $L_{t_{i}}$ with the corresponding [[Forward Points in Currency|forward rate]] of time $t_{1}$ . We get  
$$
...
[[Arbitrage Pricing of Derivatives|Pricing]] CMS is known to involve [[PSET II Fixed Income Asset Pricing 1|convexity]] adjustments. Staying within the context of the simple framework used in this chapter, the industry first obtains the $t_{1}\times t_{2}$ and $t_{2}\times t_{3}$ swaption volatilities. Then, knowing that the swap is a Martingale under the “annuity” measure treated in Chapter 17, various transformations under specific assumptions are performed and then the [[PSET II Fixed Income Asset Pricing 1|convexity]] correction to the [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]] rate is estimated. In other words, the industry calculates the $\in{\mathfrak{_{t}}}$ in the equation  
$$
{\mathrm{cms}}_{t}=s_{t}^{f}+\in{}_{t}
$$  

where $c m s_{t}$ is the CMS rate, $s_{t}^{f}$ is the relevant [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]] rate, and $\in{\mathfrak{\Gamma}}_{t}$ is the [[PSET II Fixed Income Asset Pricing 1|convexity]] correction.  
...
Consider a two-period forward CMS where a fixed CMS rate $x_{t_{0}}$ is paid at times $t_{2}$ and $t_{3}$ against the floating two-period cash [[Teaching Note 4 Interest Rate Derivatives|swap rate]] at these times. The present value of the cash flows under the $\tilde{P}^{t_{3}}$ forward probability is given by  
$$
...
Hence, to find the value of the CMS rate $x_{t_{0}}$ , all we need to do is write down the dynamics of the forward LIBOR processes, $\textit{F}(t_{0},t_{1})$ and $F\left(t_{0},t_{2}\right)$ , under the same forward measure $\mathbf{\bar{\boldsymbol{P}}}^{t_{3}}$ as done earlier, and then select [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] paths.  

It is clear that proceeding in this way and obtaining [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] paths from the [[Arbitrage Pricing of Derivatives|arbitrage]]-free forward LIBOR dynamics requires calibrating the respective volatilities $\sigma^{i}$ . But once this is done, and once the correction factors are included in the proper equations, the [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] paths can be selected in a straightforward manner. The CMS rate can then be calculated from  
$$
...
where the swap rates $s_{t_{i}}^{j}$ themselves depend on the same [[Forward Points in Currency|forward rate]] trajectories and, hence, can be calculated from the selected paths.  

The same exercise can be repeated by starting from perturbed values of volatilities and initia forward rates to obtain the relevant [[Chapter 26 - Pricing Options: Monte Carlo Simulation|Greeks]] for risk-management purposes as well.  

# 14.7 IN-ARREARS SWAPS AND CONVEXITY  

Although an overwhelming proportion of swap transactions involve the vanilla swap, in some cases parties transact the so-called LIBOR-in-arrears swap. In this section, we study this instrument because it is a good example of how Forward LIBOR volatilities enter [[Arbitrage Pricing of Derivatives|pricing]] directly through [[PSET II Fixed Income Asset Pricing 1|convexity]] adjustments.  
...
1. [[Primer on Interest Rate Swaps|Interest rate swap]]  
...
[[Primer on Interest Rate Swaps|Interest rate swap]] versus LIBOR-in-arrears swap.  
...
The simple modification of paying the $L_{t_{i+1}}$ observed at time $t_{i+1}$ rather than the previously observed $L_{t_{i}}$ makes a significant difference in [[Arbitrage Pricing of Derivatives|pricing]]. We will work with a simple case of a twoperiod (forward) swap first, and then give the generalized formulas.  

# 14.7.1 VALUATION  

The valuation of the [[Pricing Interest Rate Swaps|fixed leg]] of the in-arrears swap is the same as that of the vanilla swap, except of course the swap coupons are different. Let the $s_{t_{0}}$ be the vanilla [[Teaching Note 4 Interest Rate Derivatives|swap rate]] fixed at time $t_{0},N$ and $\delta$ be the notional amount and accrual parameters, respectively. Then the [[Pricing Interest Rate Swaps|fixed leg]] payments are easy to value:  
$$
...
where the $i s_{t_{0}}$ is the [[Teaching Note 4 Interest Rate Derivatives|swap rate]] of the in-arrears swap.  

The difference in valuations emerge in the [[Pricing Interest Rate Swaps|floating leg]]. Note that in the case of the in-arrears swap, the expected value under the $P^{t_{2}}$ forward measure of the floating rate payments would be  
$$
...
Thus the valuation of the [[Pricing Interest Rate Swaps|floating leg]] becomes  
$$
...
This can be expressed as the [[Pricing Interest Rate Swaps|floating leg]] of a vanilla swap plus an adjustment called the [[Chapter 10 - Bonds: Duration and Convexity|convexity adjustment]]:  
$$
...
Note that the second bracketed term in the [[Chapter 10 - Bonds: Duration and Convexity|convexity adjustment]] is positive. This makes the value of the floating rate payments in the in-arrears swap be greater than the value of the floating payments in the vanilla swap. The consequence of this is that the in-arrears [[Interest Rate Swaps|fixed swap rate]] denoted by $\tilde{s}_{t}$ is bigger than the vanilla fixed rate  
$$
...
A special case of this is if we look at a single period in-arrears swap. Then we get a relation between forward rates and [[Futures Not Subject to Cash-And-Carry|futures]] rates.  

A Eurodollar contract leads to an exchange of $f_{t_{0}}$ for $L_{t_{i}}$ at time $t_{i}$ . The [[Forward Points in Currency|forward contract]] leads to an exchange of $\boldsymbol{F}_{t_{0}}$ for $L_{t_{i-1}}$ at $t_{i}$ . So this is the one-period replica of the comparison we just made.  
...
Directly from Eq. (14.177) of the previous section. The right-hand side is known as the [[Chapter 10 - Bonds: Duration and Convexity|convexity adjustment]] that needs to be applied when going from [[Futures Not Subject to Cash-And-Carry|futures]] to forward rates. Note that the [[Futures Price and the Quality Option Before E|futures price]] of the contract will then be smaller than the [[Forward Contracts and Forward Prices|forward price]].32  

# 14.8 CROSS-CURRENCY SWAPS  

A [[Currency Swap Basics|cross-currency swap]] can serve different purposes. It can be used for [[Key Rates O1s Durations and Hedging|hedging]], that is to reduce the exposure to exchange [[Profit and Loss Attribution with an OAS|rate changes]]. Alternatively, one can use it to exploit [[Class Note 13 The LTCM Meltdown|arbitrage opportunities]] between different rates.  

The following reading illustrates some of the recent issues related to cross-[[Financial Instruments PSET Solutions|currency swaps]].  

# EXAMPLE  

It has been a rollercoaster couple of years for the Australian [[Table of Contents|cross-currency basis swap]] [. . .], which has ripsawed to unprecedented levels primarily as a result of large irregular capital dislocations during the [[8. Credit Modeling and Credit Derivatives|global financial crisis]]. On October 10, 2008, a shortage of US dollars, coupled with major rehedging of Japanese power reverse dual [[Forwards and Futures Notes|currency]] (PRDC) notes, caused the five-year AUD/USD [[Table of Contents|cross-currency basis swap]] to drop to $-50$ basis points—quite a move for a swap that has traditionally traded in a fairly predictable range of around six to 10 bp. Its 10-year equivalent suffered an equally volatile fate, hitting $-47$ bp on the same date. By September 2009, this situation had completely reversed. Kangaroo bond issuance had all but ground to a halt, while Australian banks—buoyed by AA credit ratings and government guarantees—were able to comfortably tap [[Characteristics of the Eurodollar Market|offshore markets]] to shore up their balance sheets. [. . .]  

These extreme levels of volatility have attracted the attention of a number of [[Basis Trade Explainer|hedge funds]]. “The hedge fund community has been active in the [[Basis Swaps|basis swap]] market for some time. By nature, it is a function of opportunity,” says Anthony Robson, head of rates for Australian and New Zealand dollars at Barclays Capital in Sydney.  

Those opportunities, he says, have occurred regularly in the past two years. “In 2008 and early 2009, the yield curve was steeply negative, which reflected the sheer illiquidity at that time, severe global funding issues and bank [[Key Rates O1s Durations and Hedging|hedging]] need to receive in the long-end around 30 years. For [[Basis Trade Explainer|hedge funds]] looking for the [[Basis Swaps|basis swap]] to widen, that presented attractive opportunities to pay. For example, they could pay a five-year [[Basis Swaps|basis swap]], starting in five years’ time and pick up 60 bp, benefiting from the negative-shaped curve and the very rapid change in levels,” he says, adding: “At the moment, and especially late last year, the [[Basis Swaps|basis swap]] is quite steeply positive, so there are certainly receivers of the forward basis spread.”  

Dealers say that since hitting its peak in late 2009, interest from hedge fund managers in the [[Basis Swaps|basis swap]] has noticeably picked up. Betting on the basis to trend back towards its historical average, they have put on a range of forward-starting basis swaps to extract value from the shape of the curve. “A fund manager could receive a five-year [[Basis Swaps|basis swap]] on a hold-to-maturity basis, yielding say, $+3I.5b p$ ,” says Ian Martin, head of global rates for Australia and New Zealand at Deutsche Bank in Sydney, who says [[Basis Trade Explainer|hedge funds]] have been particularly involved in two- to five-year maturities. “By [[Key Rates O1s Durations and Hedging|hedging]] the first year’s cash flows at $+5.5$ bp, the manager can create a forward-starting four-year swap at 39 bp. Assuming the swap tightens in line with its historical average, they would benefit from carry and capital appreciation as the trade rolls down the curve.”  

Since the fourth quarter of 2009, following a surge in kangaroo issuance, the three- to seven-year [[Basis Swaps|basis swap]] tenors have come back in substantially from their late 2009 peaks, with the five-year AUD/USD crosscurrency swap falling to $+32$ bp on February 17. Ten-year and longer tenors, however, have not fallen as  
...
against paying the five-year and 15-year) is one of the better trade opportunities in [[Basis Swaps|basis swap]] spreads at   
...
As an example of the use of a [[Currency Swap Basics|cross-currency swap]] for [[Key Rates O1s Durations and Hedging|hedging]], consider a European company that intends to buy US dollar bonds and would like to hedge the US dollar [[PSET 3 Financial Instruments|exchange rate risk]]. The European company could buy a [[Currency Swap Basics|cross-currency swap]] from a US-based bank. As a result of the swap, the European company would pay in euros and receive a (fixed) US dollar [[Preview of the Book|cash flow]]. These US dollar cash flows from the swap could be used to buy the US dollar bond. The valuation of a [[Currency Swap Basics|cross-currency swap]] is similar to that of an interest-rate swap but one difference relates to the exchange of notional. A [[Currency Swap Basics|cross-currency swap]] has two principal amounts, one for each [[Forwards and Futures Notes|currency]]. The initial principals can be exchanged or not. The nonexchange of the initial amounts is a minor issue. But eliminating the exchange of the final amounts changes the [[Arbitrage Pricing of Derivatives|pricing]] structure significantly. The exchange rate used to determine the principals is the prevailing [[The Foreign Exchange Market Annotations|spot rate]].  

With an [[Primer on Interest Rate Swaps|interest rate swap]], there is no exchange of principal at either the start or end of the transaction, as both principal amounts are the same and therefore net out. For a [[Currency Swap Basics|cross-currency swap]], it is essential that the parties agree to exchange principal amounts at maturity. The exchange of principal at the start is optional.  

Like all swaps, a [[Currency Swap Basics|cross-currency swap]] can be replicated using on-balance sheet instruments, in this case with money market deposits or FRNs denominated in different currencies. This explains the necessity for principal exchanges at maturity as all loans and deposits also require repayment at maturity.33  

The initial exchange can be replicated by the bank by entering into a spot exchange transaction at the same rate quoted in the [[Currency Swap Basics|cross-currency swap]]. Actually, all foreign exchange [[Forwards and Futures|forwards]] can be described as cross-[[Financial Instruments PSET Solutions|currency swaps]] as they are agreements to exchange two streams of cash flows in different currencies. Many banks manage long-term foreign exchange [[Forwards and Futures|forwards]] as part of the crosscurrency swap business, given the similarities.  

Like [[Financial Instruments PSET Solutions|FX forwards]], the [[Currency Swap Basics|cross-currency swap]] exposes the user to foreign exchange risk. The swap leg the party agrees to pay is a liability in one [[Forwards and Futures Notes|currency]], and the swap leg they have agreed to receive is an asset in the other [[Forwards and Futures Notes|currency]]. One of the users of cross-[[Financial Instruments PSET Solutions|currency swaps]] are debt issuers. In the Eurobond markets, issuers sell bonds in the [[Forwards and Futures Notes|currency]] with the lowest cost and swap their exposure to the desired [[Forwards and Futures Notes|currency]] using a [[Currency Swap Basics|cross-currency swap]].  

# 14.8.1 PRICING  

At the inception of the swap, the present value of one leg must be equal to the present value of the other leg at the then-prevailing [[The Foreign Exchange Market Annotations|spot rate]]. Using this simple logic, it would seem natural that cash flows of LIBOR (flat) payments in one [[Forwards and Futures Notes|currency]] could be exchanged for cash flows of LIBOR (flat) payments in another [[Forwards and Futures Notes|currency]].  

In reality this is not true, and there is a constant spread. By convention [[Currency Swap Basics|cross-currency swap]] spreads are quoted on the [[Forwards and Futures Notes|currency]] side against USD LIBOR: USD LIBOR versus Foreign [[Forwards and Futures Notes|Currency]] LIBOR plus a spread. The spread can be positive or negative. Before 2007 the EUR/USD crosscurrency swap spread was close to zero, but during the GFC it reached 120 bp. The reasons for the spread are twofold. First is the daily demand supply imbalances that are always possible. There may be more demand for paying a LIBOR in a particular [[Forwards and Futures Notes|currency]] and this will lead to a positive (basis) spread to be paid. Consider US companies, for example, that issue a loan in euros. The companies will exchange principal and coupons payments in EUR for principal and coupons payments in USD. If the [[Calculating Novel Cross-Currency Bases a|cross-currency basis]] is negative, the companies will have to pay LIBOR but will receive [[EURIBOR Forward Rate Agreements and Futures|Euribor]] minus the basis. Such corporate supply and [[Key Rates O1s Durations and Hedging|hedging]] activity can be a potential driver crosscurrency swap basis. If the corporate issuance in EUR is high and most of it is swapped into USD, there could be a positive or spread to exchange EUR payments in USD payments. Such corporate activity, however, cannot explain the huge swings in the spread during the GFC.  

The second effect that leads to positive spreads is [[Quantitative Trading Strategies Lecture Notes|credit risk]]. Counterparties may not have the same [[Quantitative Trading Strategies Lecture Notes|credit risk]] and the [[Forwards and Futures Notes|currency]] swap spread may then reflect this. For example, if one party is paying the Philippine peso equivalent of the LIBOR rate against USD LIBOR, then this party is likely to have a higher [[Quantitative Trading Strategies Lecture Notes|credit risk]]. So the party will pay a higher spread. During the GFC, an excess demand of USD by non-US banks drove the EUR/USD [[Currency Swap Basics|cross-currency swap]] down dramatically in 2008. In 2011, US Money-Market funds withdrew funding to European banks which also led to a decline in the spread. These examples illustrate that the spread reflects the relative creditworthiness of banks across different countries.34  

# 14.8.2 CONVENTIONS  

The usual convention for quoting the [[Forwards and Futures Notes|currency]] swap spread, also called the basis, is to quote it relative to the USD LIBOR.  

# 14.9 DIFFERENTIAL (QUANTO) SWAPS  

Financial Accounting Standard (FAS) 133 and the [[Week 9-Information Risks and (International) Accounting Differences, Platform Businesses|International Accounting]] Standard (IAS) 39 set the [[Week 6 Bank Business Model, Regulation, and Accounting Rules|accounting rules]] for [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] for the United States and for European companies, respectively. According to these rules, a derivative position will have to be marked-to-market and included in income statements unless it qualifies for hedge accounting.35  

A quanto (differential) swap is a special type of [[Currency Swap Basics|cross-currency swap]]. It is an agreement where one party makes payments in, say, USD LIBOR and receives payments, say, in Euro LIBOR. However, the important point is that both parties make payments in the same [[Forwards and Futures Notes|currency]]. In other words, the quanto swap value is an exposure on pure play of international [[What Really Is the Cross-Currency Basis|interest rate differentials]] and has [[Zero-coupon convertible bonds|no foreign exchange risk]].  
...
This discussion is limited to US dollar markets. The particular [[Interest Rate Quotations|interest rates]] discussed below will change if other currencies are considered, since basis swaps are directly related to the business environment in an economy. A [[Basis Swaps|basis swap]] is a floating floating [[Primer on Interest Rate Swaps|interest rate swap]] in which cash flows based on one floating reference rate are exchanged against the cash flows based on a different floating reference rate.  
...
The most common types of basis swaps are [[Fed Fund Futures|Fed Funds]] against LIBOR,36 T-bill rates against LIBOR, CP rates against LIBOR, and the prime rate against LIBOR.37 Which [[Basis Swaps|basis swap]] a client picks depends on his or her business. For example, a party that has concerns about credit squeezes can use [[Fed Fund Futures|Fed Funds]]-LIBOR basis swaps or the T-bill-LIBOR basis swaps. During a credit squeeze, a flight to safety will make the [[Basis Swaps|basis swap]] spread increase. On the other hand, the Prime-LIBOR [[Basis Swaps|basis swap]] can hedge the exposures of those players involved in credit card, auto, or consumer loans.  

# 14.10 CONCLUSIONS  

This chapter was devoted to the connections between the swap, FRA, and bond markets. Our discussion led us to the issue of constructing a satisfactory yield curve, which is the fundamental task of a financial engineer. Two main tools were introduced in the chapter. The first was the $T.$ -forward measures and the second was the related measure change technology. This permitted setting up convenient [[Arbitrage Pricing of Derivatives|arbitrage]]-free dynamics for a sequence of forward rates. These dynamics were then used as a tool for calculating the desired quantities using the formulas that connect swap rates, forward rates, and their [[Chapter 9 Arbitrage and Hedging With Options|derivatives]].  
...
You also know that the current 3-month LIBOR rate is $4\%$ . a. Calculate the discount bond prices $B(t_{0},t_{i})$ , where $t_{i}=6$ , 9, 12, 15, and 18 months. b. Calculate the yield curve for maturities 0 18 months. c. Calculate the [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]] for the same maturities. d. Are the two curves different? e. Calculate the par yield curve. f. Calculate the zero-coupon yield curve.  
...
a. The [[Bid Ask and Transaction Prices in a Specialist Market With Heterogeneously Informed Traders|bid ask]] on a [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]] that starts in 2 years with maturity in 3 years. The swap is against 12-month LIBOR.   
b. The [[Forward Contracts and Forward Prices|forward price]] of a coupon bond that will be delivered at time 2. The bond pays coupon $7\%$ and matures in 2 years.  

# EXCEL EXERCISES  

4. Write a VBA program to determine the bond price and swap rates from the set of forward rates given below and plot the yield curves along with the [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]]. Comment on their characteristics.  

<html><body><table><tr><td>Term (Month)</td><td>[[Forward Points in Currency|Forward Rate]]</td></tr><tr><td>0</td><td>4.00%</td></tr><tr><td>3</td><td>4.50%</td></tr><tr><td>6</td><td>4.70%</td></tr><tr><td>9</td><td>5.00%</td></tr><tr><td>12</td><td>5.50%</td></tr><tr><td>15</td><td>6.10%</td></tr><tr><td>18</td><td>6.40%</td></tr><tr><td>21</td><td>6.46%</td></tr><tr><td>24</td><td>7.00%</td></tr></table></body></html>  

5. Write a VBA program to determine the bond price and forward rates from the set of swap rates below with the term of 1 year as the input. Assume that the current LIBOR rate is $5.00\%$ . Plot the yield curves along with [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]] to observe their characteristic.  

<html><body><table><tr><td>Term (Years)</td><td>[[Teaching Note 4 Interest Rate Derivatives|Swap Rate]]</td></tr><tr><td>2</td><td>6.20%</td></tr><tr><td>3</td><td>6.40%</td></tr><tr><td>4</td><td>7.00%</td></tr><tr><td>5</td><td>7.50%</td></tr><tr><td>6</td><td>8.10%</td></tr><tr><td>7</td><td>8.70%</td></tr></table></body></html>  

6. Write a VBA program to determine the FRA rates and swap rates from the set of bond prices below as the input and plot the yield curves along with [[Determining the Expression for the Fair Value of the Swap Spread|swap curve]]. Comment on their characteristics.  
...
7. Write a VBA program to determine the price of an [[Primer on Interest Rate Swaps|interest rate swap]] which makes a floating payment every 6 months at the rate equal to USD LIBOR rate against a fixed rate of $5.10\%$ for the notional amount of $\$10,000$ . Use the data given in the “Input” worksheet for carrying out the calculation.  

<html><body><table><tr><td>Time (Years)</td><td>[[Forward Points in Currency|Forward Rate]]</td></tr><tr><td>0.00</td><td>5.70%</td></tr><tr><td>0.50</td><td>5.18%</td></tr><tr><td>1.00</td><td>5.83%</td></tr><tr><td>1.50</td><td>5.10%</td></tr><tr><td>2.00</td><td>5.75%</td></tr><tr><td>2.50</td><td>5.74%</td></tr><tr><td>3.00</td><td>5.28%</td></tr><tr><td>3.50</td><td>5.65%</td></tr><tr><td>4.00</td><td>5.53%</td></tr></table></body></html>  

8. ([[Currency Swap Basics|Cross-Currency Swap]]) Write a VBA program to determine the [[Currency Swap Basics|cross-currency swap]] rate based on the information below. The fixed payment is made in USD in the exchange for the floating payment (in USD) rate equal to the GBP LIBOR at every 3 months. Also include the exchange of notional amount d10,000 against payment in dollars at their corresponding exchange [[The Foreign Exchange Market Annotations|spot rate]] during initiation and maturity of the swap in the [[Financial Instruments Midterm 2022 Solutions|swap rate calculation]].  

Notional amount $(\mathbf{GBP})=\pounds10,000$ , notional amount $(\mathrm{USD})=\$20,000$  

<html><body><table><tr><td>Time (Month)</td><td>[[Forward Points in Currency|Forward Rate]] (USD)</td><td>Exchange Rate (USD/GBP)</td><td>LIBOR Rate (GBP)</td></tr><tr><td>0</td><td>4.00%</td><td>2.0000</td><td>5.70%</td></tr><tr><td>3</td><td>4.50%</td><td>2.0225</td><td>5.18%</td></tr><tr><td>6</td><td>4.70%</td><td>2.0150</td><td>5.83%</td></tr><tr><td>9</td><td>5.00%</td><td>1.9875</td><td>5.10%</td></tr><tr><td>12</td><td>5.50%</td><td>1.9750</td><td>5.75%</td></tr><tr><td>15</td><td>6.10%</td><td>1.9685</td><td>5.74%</td></tr><tr><td>18</td><td>6.30%</td><td>1.9480</td><td>5.28%</td></tr><tr><td>21</td><td>6.36%</td><td>1.9325</td><td>5.65%</td></tr><tr><td>24</td><td>6.40%</td><td>1.9300</td><td>5.53%</td></tr></table></body></html>  

# MATLAB EXERCISE  

9. a. Write a MATLAB program to determine the bond price and [[Teaching Note 4 Interest Rate Derivatives|swap rate]] from the given set of FRA rates and calculate the yield, par yield, and zero-coupon yield. The input data are to be read from the “Forward” worksheet of the “Data.xlsx” file on the chapter webpage. Now plot all the curves simultaneously and comment on their characteristics. b. Write a MATLAB program to determine the bond price and FRA rate from the given set of [[Chapter 39 - Swaptions, Forward Swaps, and MBS|forward swap]] rates and calculate the yield, par yield, and zero-coupon yield. The input data are to be read from the “Swap” worksheet of “Data.xlsx” file. Now plot all the curves simultaneously and comment on their characteristics. c. Write a MATLAB program to determine the FRA price and [[Teaching Note 4 Interest Rate Derivatives|swap rate]] from the given set of bond prices and calculate the yield, par yield, and zero-coupon yield. The input data are to be read from the “Bond” worksheet of “Data.xlsx” file. Now plot all the curves simultaneously and comment on their characteristics.  
