...
16.1 [[Squam Lake Group Introduction|Introduction]] to Correlation as an Asset Class. 546   
...
16.5.1 An Interpretation of Dynamic [[Key Rates O1s Durations and Hedging|Hedging]].. 556   
...
16.8 [[Squam Lake Group Introduction|Introduction]] to the Smile 562   
...
16.16 [[Arbitrage Pricing of Derivatives|Pricing]] with a Smile.. 583   
16.17 [[Exotic Interest Rate Options|Exotic]] Options and the Smile . 584   
16.17.1 A Hedge for a Barrier . 584  

16.17.2 Effects of the Smile. 585  

16.17.2.1 An example of technical difficulties . 586   
16.17.2.2 [[Arbitrage Pricing of Derivatives|Pricing]] exotics. 586   
...
[[Exercises|Exercises]] ... . 588   
Excel Exercise.. 589   
MATLAB [[Exercises|Exercises]].. . 590  
...
Since implied correlation has historically been above realized correlation, a short correlation (swap) position can be a good source of return. The hedge fund industry and several [[Risk Management Lessons From Long Term Capital Management|hedge fund strategies]] are exposed to correlation risk as Buraschi et al. (2014) show. Correlation is a major component of volatility and similar to volatility it is negatively correlated with equity index [[Assets|returns]]. Therefore, correlation trading can provide [[Week 8 Opportunities And Risks Of Investing Internationally|diversification benefits]]. Since correlation is the residual from index and single-[[Option Risk|stock volatility]], correlation trading can be used to express views and take positions regarding the relative moves of index versus singlestock volatility.  
...
With correlation at historic highs across many indices, traders continue to see investors rush to monetise the widely-held view that correlation will fall over the next 12 months as macro-economic drivers become less prevalent in shaping [[Lecture 4- Investor Sentiment|investor sentiment]], triggering the return of alpha strategies and increased stock dispersion. After effectively [[Appendix 17.B The Solution With Shutting Down and Restarting|shutting down]] amid heightened uncertainty in the wake of Lehman Brothers’ bankruptcy, correlation trading has been gaining traction in recent months.   
...
While macro [[Basis Trade Explainer|hedge funds]] continue to favour the dispersion trade—buying volatility swaps on separate stocks while selling volatility swaps on the index—options strategies such as dispersion straddles have increased in popularity as [[Class Note 10 Liquidity and Class Note 10 Liquidity and Liquidity Managementliquidity management|liquidity]] in more traditional products declines.  

“Funds are now building dispersion books and the key is diversification,” said Mattatia. “They are trading all [[Credit Markets Session 1|instrument types]] including straddles, variance swaps and volatility swaps, and looking across all indices.”  

(Thomson Reuters IFR 1850, ‘Correlation trading diversifies’, September 11, 2010)  

The above reading illustrates that correlation trading now extends across asset classes. Pension funds are mentioned as participants in correlation trading. They typically go long correlation swaps, which implies that they benefit when correlation unexpectedly increases. Macro [[Basis Trade Explainer|hedge funds]] on the other hand are reported to be short correlation, by selling index and buying single stock variance swaps, thus benefiting when markets remain calm and correlations remain relatively low. To understand how correlation exposure can be created it is instructive to start with relatively simple correlation [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] linked to one asset class, such as equities. Next we examine the [[Forward and Futures Contracts|replication]] and construction of dispersion traders and correlation swaps.  

The purest way to trade correlation is directly via a correlation swap. A correlation swap provides exposure to the average pairwise correlation of a predetermined basket of stocks. Average pairwise correlation $\rho_{A}$ is a simple [[Chapter 24 - Analysis of Black–Scholes|equally weighted average]] of the correlations between all pairs of distinct stocks in an index:  
$$
...
The swap strike is the level of (average pairwise) correlation that is bought or sold. It is typically scaled by a factor of 100 (that is a correlation of 0.5 is quoted as a strike of 50). Similar to the [[Variance Swaps|variance swap]] (discussed in the previous chapter), the payout of a correlation swap is the notional amount multiplied by the difference between the swap strike and the subsequent realized average pairwise correlation on the basket of underlyings:  
$$
\mathrm{Payout=notional\times(realized~average~pairwise~correlation-strike)}
$$  

Figure 16.1 shows the cash flows in a typical correlation swap [[Preview of the Book|cash flow]]. Let’s examine a correlation [[Modeling Vanilla Interest Rate Swaps Using QuantLib Python|swap example]].  

# EXAMPLE: CORRELATION SWAPS  

Suppose that on January 6, 2014, a hedge fund sells a 6-month correlation swap on an equally weighted basket of stocks consisting of the constituents of the Eurostoxx 50 index. The counter-party is assumed to be a [[Uses of Interest Rate Swaps|pension fund]]. The [[Call and Put Payoffs at Expiry|strike price]] is 40 and the notional amount is h10,000. This means that the hedge fund is short the correlation swap and the [[Uses of Interest Rate Swaps|pension fund]] is long the swap. One way to interpret the transaction is to say that, for a fee, the hedge fund sells insurance against unexpected increases of the correlation to the [[Uses of Interest Rate Swaps|pension fund]].  

After 6 months, on July 6, 2014, we calculate the realized 6-month average pairwise correlation of the stocks in our basket as 0.3. Then the buyer of the correlation swap, the [[Uses of Interest Rate Swaps|pension fund]], pays the following amount to the seller of the [[Uses of Interest Rate Swaps|pension fund]]: Notional $\times(55-42)=\in10,000(40-30)=\in100,000$ .  

In the above example, the hedge fund received a premium for bearing the risk of unexpected increases in correlations. A good proxy for the correlation strike, also known as implied correlation, is the square of the ratio between the implied index volatility and the average of the equity component implied volatilities (as [[A Real-Life Option Pricing Exercise|implied volatility]] is a good estimate of the forward-realized volatility, we could then also infer that implied correlation is a good estimate of the forward-realized correlation).  
...
For market professionals and [[Basis Trade Explainer|hedge funds]], the issue of how to fund an [[An Asset Allocation Primer|investment]] is as important as the [[An Asset Allocation Primer|investment]] itself. After all, a hedge fund would look for the “best way” to borrow funds to carry a position. The best way may sometimes carry a negative interest. In other words, the hedge fund would make money from the [[An Asset Allocation Primer|investment]] and from the funding itself.  
...
First we know from Chapter 9 that delta-hedged short option positions are convex exposures that will pay the gamma. These payouts are unknown initially. As [[Class Slides On Terrausd Runs 2|market volatility]] is observed, the hedge is dynamically adjusted, and depending on the [[Class Slides On Terrausd Runs 2|market volatility]] the hedge fund will face a cash outflow equal to gamma. To the hedge fund this is similar to paying floating money market [[Interest Rate Quotations|interest rates]].  
...
Merrill notes “one of the most overcrowded trades in the market has been to take advantage of the long term trading range,” by selling volatility and “earning carry via [[Fremont Financial Corp. (b)|mortgage-backed securities]].”  
...
Options were introduced as volatility instruments in Chapter 9. This is very much in line with the way traders think about options. We showed that when we deal with options as volatility instruments mathematically we arrived at the same formula, in this case the same partial differential equation (PDE) as the [[Black-Scholes Model|Black Scholes]] PDE. Mathematically the approach was identical to the standard textbook treatment that considers options as directional instruments.3 Yet, although the interpretation in Chapter 9 is more in line with the way traders and option markets think, in that discussion there was still a major missing component.  

It turns out that everything else being the same, an out-of-the-money put or call has a higher [[A Real-Life Option Pricing Exercise|implied volatility]] than an ATM call or put. This effect, alluded to several times up to this point, is called the volatility smile. However, in order to discuss the smile in this chapter we adopt still another interpretation of options as instruments.  

The discussion in Chapter 9 showed that the option price (after some adjustments for interest receipts and payments) is actually related to the expected gamma gains due to volatility in the underlying. The interpretation we use in this chapter will show that these expected gains will depend on the option’s strike. One cost to pay for this interesting result is the need for a different mathematical approach. The advantage is that the smile will be the natural outcome. A side advantage is that we will discuss a dynamic [[The Value of the Swap Contract after Initiation|hedging strategy]] other than the well-known delta-[[Key Rates O1s Durations and Hedging|hedging]]. In fact, we start the chapter with a discussion of options from a more “recent” point of view which uses the so-called dirac delta functions. It is perhaps the best way of bringing the smile explicitly in option [[Arbitrage Pricing of Derivatives|pricing]].  
...
Now consider the “[[FORWARD RATES AND TERM STRUCTURE|expectations]]” calculated with such an $f(x)$ . Let $C(x_{t})$ be a random value that depends on the random variable $x_{t}$ , indexed by the time $t$ . Then we can write  
$$
...
The major advantage of the dirac delta functions, interpreted as the limits of distributions, is in differentiating functions that have points that cannot be differentiated in the usual sense. There are many such points in option trading. The payoff at the strike $K$ is one example. Knock-in, knock-out barriers is another example. Dirac delta will be useful for discussing [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] at those points.  

Before we proceed, for simplicity we will assume in this section that [[Interest Rate Quotations|interest rates]] are equal to zero:  
$$
r_{t}=0
$$  
...
Note that with [[Interest Rate Quotations|interest rates]] being zero, the drift is eliminated and that the volatility is not of the [[Black-Scholes Model|Black Scholes]] form. It depends on the random variable $S_{t}$ . Let  
$$
\begin{array}{r l}{f(S_{T})}&{{}=\operatorname*{max}[S_{T}-K,0]}\\ {\ }&{{}=\left(S_{t}-K\right)^{+}}\end{array}
$$  

be the vanilla call option payoff shown in Figure 16.4. The function is not differentiable at $S_{T}=K$ , yet its first-order derivative is like a step function. More interestingly, the second-order derivative can be interpreted as a dirac delta function. These [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] are shown in Figures 16.4 and 16.5.  
...
where the first term on the right-hand side is the time value of the option at time $t_{0}$ and is known with certainty. We also know that with zero [[Interest Rate Quotations|interest rates]], the option price $C\left({{S}_{t_{0}}}\right)$ will be given by  
$$
...
There are many dynamic strategies that replicate an option’s final payoff. The best known is [[Financial Instruments|delta hedging]]. In [[Financial Instruments|delta hedging]], the financial engineer will buy or sell the delta $\mathbf{\Psi}=D_{t}\mathbf{\Psi}$ units of the underlying, borrow any necessary funds, and adjust $D_{t}$ as the underlying $S_{t}$ moves over time. As $t\rightarrow T,$ the [[Risk Neutral Pricing of Options|expiration date]], this will duplicate the option’s payoff. This is the case because, as the time value goes to zero the option price merges with $\left(S_{T}-K\right)^{+}$ .  

However, there is an alternative dynamic [[Key Rates O1s Durations and Hedging|hedging]] procedure that is similar to the approach adopted in the previous section. The dynamic [[Key Rates O1s Durations and Hedging|hedging]] technique, called stop-loss strategy, is as follows.  
...
Clearly the switches at $S_{t}=K$ cannot be done instantaneously at zero cost. The trader is moving with time $\Delta$ while the underlying [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] is moving at a faster rate $\sqrt{\Delta}$ . These adjustments are shown in Figures 16.6 and 16.7. The resulting [[Key Rates O1s Durations and Hedging|hedging]] cost is the options value.  

![](edbbeea520fbc9f7773c1c62cb23172a1267d40d480af20d25bb1ae143471e26.jpg)  

# FIGURE 16.6  

[[The Value of the Swap Contract after Initiation|Hedging strategy]] adjustment—call option.  

![](1d022481d2f83bdaba923cf694843e51708defffeac987cf48c345f48bca26a3.jpg)  

# FIGURE 16.7  

[[The Value of the Swap Contract after Initiation|Hedging strategy]] adjustment and positions.  

# 16.6 BREEDEN LITZENBERGER SIMPLIFIED  

The so-called Breeden Litzenberger Theorem is an important result that shows how one can back out risk-adjusted probabilities from liquid [[Arbitrage Pricing of Derivatives|arbitrage]]-free option prices. In this section, we discuss a trader’s approach to Breeden Litzenberger. This approach will show the theoretical relevance of some popular option strategies used in practice. Below, we use a simplified framework which could be generalized in a straightforward way. However, we will not generalize these results, but instead in the following section use the dirac delta approach to prove the Breeden Litzenberger theorem.  
...
Finally we use the LIBOR rate $L_{t}$ to discount cash flows to be received at time $T.$ . The [[Discount Factors|discount factor]] will then be given by  
$$
\frac{1}{(1+L_{t}\delta)}
$$  

Next we define a simple [[Uncertainty Information and Stochastic Processes|probability space]]. We assume that the strike prices define the four states of the world where $S_{T}$ can end up. Hence the state space is discrete and is assumed to be made of only four states $\{\omega^{1},...,\omega^{4}\}$ .5  
$$
\omega^{i}=K^{i}
$$  
...
The [[6. A Brief Introduction to Stochastic Calculus|arbitrage-free pricing]] of Chapter 11 can be applied to these vanilla options:  
$$
C_{t}^{i}=\frac{1}{(1+L_{t}\delta)}E_{t}^{\tilde{P}}\big[(S_{T}-K,0)^{+}\big]
$$  

The straightforward application of this formula using the probabilities $p^{i}$ gives the following [[Arbitrage Pricing of Derivatives|pricing]] equations, where possible payoffs are weighed by the corresponding probabilities (Figure 16.8).  
...
In other words, this is an option position that is long two wings and short the center twice. In fact this is a butterfly centered at $K_{2}$ . It turns out that the [[Exercises|arbitrage-free market]] value of this butterfly multiplied by the $(1+L_{t}\delta)\Delta K$ gives the risk-adjusted probability that the underlying $S_{t}$ will end up at state $K_{2}$ . Letting $\Delta K{\rightarrow}0$ we get  
$$
...
The idea behind the Breeden Litzenberger result has been discussed before. It rests on the fact that by using liquid and [[Arbitrage Pricing of Derivatives|arbitrage]]-free options prices we can back out the risk-adjusted probabilities associated with various states of the world in the future. The probabilities will relate to the future values of the underlying price, $S_{T}$ .  
...
Apply the twice differential operator to the definition of both sides of the [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|arbitrage-free price]] $C(S_{t},K)$ . By definition, this means  
$$
...
The question then is, how does a trader “characterize” an option using these [[Key Rates O1s Durations and Hedging|hedging]] gains? First of all, in liquid option markets the order flow determines the price and the trader does not have to go through a [[Arbitrage Pricing of Derivatives|pricing]] exercise. But still, can we use these trading gains to represent the frame of mind of an options trader?  

The discussion in the previous section provides a hint about this issue. The trader buys or sells an option with [[Call and Put Payoffs at Expiry|strike price]] $K$ . The cash needed for this transaction is either borrowed or lent. Then the trader delta hedges the option. Finally, this hedge is adjusted as the underlying price fluctuates around the initial $S_{t_{0}}$ .  
...
We can explain this statement. First, for simplicity assume [[Interest Rate Quotations|interest rates]] are equal to zero. We then let the price of the vanilla call be denoted by $C(S_{t},t)$ . Then by definition we have  
$$
...
Now, we know from the [[Black-Scholes Model|Black Scholes]] PDE that  
$$
...
Note that on the right-hand side, the integral is evaluated at the constant $S_{t_{0}}$ so we don’t need to take [[FORWARD RATES AND TERM STRUCTURE|expectations]].  
...
Markets trade many options with the same underlying, but different strike prices and different expirations. Does the difference in [[Call and Put Payoffs at Expiry|strike price]] between options that are identical in every other aspect have any important implications?  

At first, the answer to this question seems to be no. After all, vanilla options are written on an underlying, with say, price $S_{t},$ and this price will have only one volatility at any time t, regardless of the [[Call and Put Payoffs at Expiry|strike price]] $K_{i}.$ Hence, it appears that, regardless of the differences in the [[Call and Put Payoffs at Expiry|strike price]], the [[A Real-Life Option Pricing Exercise|implied volatility]] of options written on the same underlying, with the same expiration, should be the same.  

Yet, this first impression is wrong. In reality, options that are identical in every respect, except for their strike, in general, have different implied volatilities. Overall, the more out-of-the-money a call (put) option is, the higher is the corresponding [[A Real-Life Option Pricing Exercise|implied volatility]]. This well-established empirical fact is known as the volatility smile, or [[Swaption Skew|volatility skew]], and has major implications for [[Key Rates O1s Durations and Hedging|hedging]], [[Arbitrage Pricing of Derivatives|pricing]], and marking-to-market of many important instruments. In the remainder of this chapter, we discuss the volatility smile and skews using [[Interest Rate Derivatives-An Introduction to the  Pricing of Caps and Floors|caps and floors]] as vehicles for conveying the main ideas. This will indirectly give us an opportunity to discuss the engineering of this special class of convex instruments.  
...
The volatility smile has important implications for trading, [[Key Rates O1s Durations and Hedging|hedging]], and [[Arbitrage Pricing of Derivatives|pricing]] [[A Practical Guide for Actuaries and other Business Professionals.|financial instruments]]. To illustrate how far things have come in this area, we look at a position taken with the objective of benefiting from abnormal conditions regarding the volatility smile.  
...
The reading suggests that this “abnormal” skew may have attracted some [[Basis Trade Explainer|hedge funds]] who expected the abnormality to disappear in the longer run. According to one theory, these funds sold out-of-the-money volatility and bought ATM volatility. This position will make money if the skew flattens and out-of-the-money volatility decreases relative to the ATM volatility.9 As this example shows, the skew, or smile, should be considered as an integral part of [[Financial Markets and Institutions Lecture Notes|financial markets]] activity. However, as we see in this section, its existence causes several complications and difficulties in [[Mathematics of the Financial Markets|financial modeling]] and in [[Financial Mathematics Course|risk management]].  

# 16.10 A FIRST LOOK AT THE SMILE  

The volatility smile can be a confusing notion, and we need to discuss some preliminary ideas before getting into the mechanics of [[Arbitrage Pricing of Derivatives|pricing]] and market applications. It is well known that the [[Black-Scholes Model|Black Scholes]] assumptions are not very realistic. And yet, the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] is routinely used by options traders, although these traders know better than anybody else that the assumptions behind the model are problematic. One of the major [[Black-Scholes Model|Black Scholes]] assumptions, for example, is that volatility is constant during the life of an option. How can a trader still use the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] if the realized volatility is known to fluctuate significantly during the life of the option?  
...
1. First, note that the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] is simple and depends on a small number of parameters. In fact, the only major parameter that it depends on is the volatility, $\sigma.$ . A simple formula has some advantages. It is easy to understand and remember. But, more importantly, it is also easy to realize where or when it may go wrong. A simple formula permits developing ways to correct for any inaccuracies informally by making subjective adjustments during trading. The BlackScholes formula has one parameter, and it may be easier to remember how to “adjust” this parameter to cover for the imperfections of the formula.10  

2. An important aspect of the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] is that it has become a convention. In other words, it has become a standard among professionals and also in computer platforms. The formula provides a way to connect a volatility quote to a dollar value attached to this quote. This way traders use the same formula to put a dollar value on a volatility number quoted by the market. This helps in developing common platforms for [[Key Rates O1s Durations and Hedging|hedging]], risk managing, and trading volatility.  

3. Thus, once we accept that the use of the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] amounts to a convention, and that traders differ in their selection of the value of the parameter $\sigma$ , then the critical process is no longer the option price, but the volatility. This is one reason why in many markets, such as caps, [[Caps and Floors|floors]], and swaptions markets, the volatility is quoted directly.  

One way to account for the imperfections of the [[Black-Scholes Model|Black Scholes]] assumptions would be for traders to adjust the volatility parameter.  
...
The volatility smile and its generalization, the [[7. Black Scholes Model|volatility surface]], could then contain a great deal of information concerning the implied volatilities and any [[Arbitrage Pricing of Derivatives|arbitrage]] relations between them. Trading, [[Arbitrage Pricing of Derivatives|pricing]], [[Key Rates O1s Durations and Hedging|hedging]], and arbitraging of the smile thus become important.  

# 16.11 WHAT IS THE VOLATILITY SMILE?  

Consider the [[Black-Scholes Model|Black Scholes]] world with vanilla European [[Notes on Basic Options Properties|call and put]] options written on the equity price (index), $S_{t},$ that expire on the same date $T.$ Let $K_{i}$ denote the ith strike of the option series and $\sigma_{i}$ the constant [[Black-Scholes Model|Black Scholes]] instantaneous (implied) volatility coefficient for the strike $K_{i}$ . Finally, let $r$ be the constant [[Black Scholes Derivation|risk-free rate]].  

The [[Black-Scholes Model|Black Scholes]] setting makes many assumptions beyond that of constant volatility. In particular, the underlying equity does not make any [[Exercises|dividend payments]], and there are no transaction costs, tax issues, or regulatory costs. Finally, $S_{t}$ is assumed to follow the geometric [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] (SDE)  
$$
\mathrm{d}S_{t}=\mu S_{t}\mathrm{d}t+\sigma S_{t}\mathrm{d}W_{t}\quad t\in[0,\infty]
$$  

where $W_{t}$ is a [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] defined under the probability $P$ . Here, the parameter $\mu$ may also depend on $S_{t}.$ The crucial assumption is that the diffusion component is given by $\sigma S_{t}$ . This is the assumption that we are concerned with in this chapter. The [[Black-Scholes Model|Black Scholes]] setting assumes that the absolute volatility during an infinitesimally small interval $\mathrm{d}t$ is given (heuristically) by11  
$$
...
In this environment, a typical put option’s price is given by the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]]:  
$$
...
Suppose the markets quote [[A Real-Life Option Pricing Exercise|implied volatility]], $\sigma$ . To get the monetary value of an option with strike $K_{i}$ , the trader will put the current values of $S_{t},t.$ , and $r$ and the quoted value of the [[A Real-Life Option Pricing Exercise|implied volatility]] $\sigma_{i}$ at which the trade went through, in this formula. According to this interpretation, the BlackScholes formula is used to assign a dollar value to a quoted volatility. Conversely, given the correct option price $P(.)$ , the [[A Real-Life Option Pricing Exercise|implied volatility]], $\sigma_{i}$ , for the $K_{i}$ put could be extracted.  

We can now define the volatility smile within this context. Consider a series of $T_{\mathbf{\delta}}$ -expiration, liquid, and [[Arbitrage Pricing of Derivatives|arbitrage]]-free out-of-the-money put option prices indexed by the strike prices $K_{i}$ , denoted respectively by $P_{K_{i}}$ :  
$$
P_{K_{1}},...,P_{K_{n}}
$$  
...
Then, given the (bid or ask) option prices, we can use the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] backward and extract $\sigma_{i}$ that the trader used to conclude the deal on $P_{K_{i}}$ . If the assumptions of the [[Black-Scholes Model|Black Scholes]] world are correct, all the implied volatilities would turn out to be the same  
$$
...
Volatility smile: [[A Real-Life Option Pricing Exercise|implied volatility]] against [[Call and Put Payoffs at Expiry|strike price]].  

since the put options would be identical except for their [[Call and Put Payoffs at Expiry|strike price]]. Thus, in a market that conforms to the [[Black-Scholes Model|Black Scholes]] world, the traders would use the same $\sigma$ in the [[Black-Scholes Model|Black Scholes]] put formula to obtain each $P_{K_{i}},i=0,...,n.$ . Going backward, we would then recover the same constant $\sigma$ from the prices.12  
...
In other words, the more out-of-the-money the put option is, the higher the corresponding [[A Real-Life Option Pricing Exercise|implied volatility]]. As a result, we would obtain a “smiley” curve.  
...
[[Credit Markets Session 5|Volatility smiles]] observed in reality seem to have the following characteristics,  
...
3. Options on various [[Interest Rate Quotations|interest rates]] yield a more monotonous one-sided smile than the equity indices. The fact that “smile” patterns vary from market to market would suggest, on the surface, that there are different explanations involved.  

It is also natural to think that the dynamics of the smile vary depending on the sector. This point is relevant for [[Financial Mathematics Course|risk management]], running swaption, cap/floor books, and volatility trading. But, before we discuss it, we consider an example.  

# EXAMPLE  

Table 16.1 displays all the options written on the S&P100 index with a very short expiration. These data were obtained from live quotes early in the morning, so few trades had passed. However, the option [[Bid Ask and Transaction Prices in a Specialist Market With Heterogeneously Informed Traders|bid ask]] quotes were live, in the sense that reasonably sized trades could be conducted on them.  

When the data were gathered, the underlying was trading at 589.14. We use 12 out-of-themoney puts and 9 out-of-the-money calls to obtain the [[Black-Scholes Model|Black Scholes]] implied volatilities.  

The interest rate is taken to be $1.98\%$ , and the [[Chapter 16 - Black–Scholes Model|time to expiration]] was 8/365. Using these values and the bid prices for the options given in the table, the equations  
$$
...
[[A Real-Life Option Pricing Exercise|Implied volatility]] of short-dated OEX options against moneyness.  

The resulting vols were plotted against $K_{i}/S_{t}$ in Figure 16.11. We see a pronounced smile. For example, the January 400 put, which traded at about $32\%$ out-of-the-money, had a volatility of about $26\%$ , while the ATM option traded at an [[A Real-Life Option Pricing Exercise|implied volatility]] of $18.5\%$ .  
...
The way a smile is plotted varies from one market to another. The [[A Real-Life Option Pricing Exercise|implied volatility]], denoted by $\sigma_{i:}$ , always appears on the $y$ -axis. Unless stated otherwise, we extract this volatility from the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] in equity or FX markets, and from the Black formula in the case of [[Interest Rate Quotations|interest rates]]. The implied volatilities are then treated as if they were random and time varying.  

What to put on the horizontal axis is a more delicate question and eventually depends on how we define “moneyness” of an option. Sometimes the smile is plotted against moneyness measured by the ratio of the [[Call and Put Payoffs at Expiry|strike price]] to the current market price, $K_{i}/S_{t}.$ . If the smile is a function of how much the option is out-of-the-money only, then this normalization will stabilize the smile in the sense that as $S_{t}$ changes, the smile for that particular option series may be more or less invariant. But there are almost always factors other than moneyness that affect the smile, and some practitioners define moneyness differently.  

For example, sometimes the smile is plotted against $K_{i}e^{-r(T-t)}/S_{t}$ . For short-dated options, this makes little difference, since $r(T-t)$ will be a small number. For longer-dated options, the difference is more relevant. By including this [[Discount Factors|discount factor]], market practitioners hope to eliminate the effect of the changes in the remaining life of the option.  

Sometimes the horizontal axis represents the option’s delta. FX traders take the size of delta as a measure of moneyness. This practice can be challenged on the grounds that an option’s delta depends on more variables than just moneyness. It also depends, for example, on the instantaneous [[A Real-Life Option Pricing Exercise|implied volatility]]. Yet, as we will see later, there are some deltas at which volatility trading is particularly liquid in FX markets.  

The reader should note that the smile in Figure 16.9 is a plot of the [[A Real-Life Option Pricing Exercise|implied volatility]] against the strike only. Figures 16.12 and 16.13 are plots of the [[A Real-Life Option Pricing Exercise|implied volatility]] against the delta. These curves  
...
relate to a particular time $t$ and [[Risk Neutral Pricing of Options|expiration date]] $T.$ As the latter change, the smile will, in general, shift. It is quite important to know how changes in time $t$ and [[Risk Neutral Pricing of Options|expiration date]] $T$ affect the smile.  

# 16.11.3 REPLICATING THE SMILE  

The volatility smile is a plot of the [[A Real-Life Option Pricing Exercise|implied volatility]] of options that are alike in all respects except for their moneyness. The basic shape of the volatility smile has two major characteristics. The first relates to the extent of symmetry in the smile. The second is about how “pronounced” the curvature is. There are good approximations for measuring both characteristics.  

First, consider the issue of symmetry. Figure 16.12 shows three smiles for FX markets. One is symmetric, the other two are asymmetric with different [[Week 2 Fundamentals Of Forecasting|biases]]. If the smile is symmetric, the volatilities across similarly out-of-the-money puts and calls will be the same. This means that if a trader buys a call and sells a put with the same moneyness, the structure will have zero value. Such positions were called risk reversals in Chapter 11. A symmetric smile implies that a zero cost [[Chapter 30 - Other Options|risk reversal]] can be achieved by buying and selling similarly out-of-the-money options. In the case of asymmetric smiles, puts and calls with similar moneyness are sold at different implied volatilities, and this is labeled a bias. Thus, a [[Chapter 30 - Other Options|risk reversal]] is one way of measuring the bias in a volatility smile.  

The way risk reversals measure the symmetry of the volatility smile is shown in Figure 16.13. We use the delta of the option as a measure of its moneyness on the $x$ -axis. ATM options would have a delta of around 50 and would be in the “middle” of the $x$ -axis. The volatility of the 25-delta [[Chapter 30 - Other Options|risk reversal]] gives the difference between the volatilities of a 25-delta put and a 25-delta call as indicated in the graph. We can write this as  
$$
\sigma(25-d e l t a R R\mathrm{spread})=\sigma(25-d e l t a\mathrm{put})-\sigma(25-d e l t a\mathrm{call})
$$  

where $\sigma(25\$ -delta $R R$ spread), $\sigma(25$ -delta put), and $\sigma$ (25-delta call) indicate, respectively, the implied volatilities of a [[Chapter 30 - Other Options|risk reversal]], a 25-delta put, and a 25-delta call.  
...
These equalities can be used to determine out-of-the-money volatilities in the case of vanilla options. For example, if ATM, RR, and butterfly volatilities are liquid, we can use these equations to “calculate” 25-delta [[Notes on Basic Options Properties|call and put]] volatilities. However, it has to be noted that for [[Exotic Interest Rate Options|exotic]] options, adjusting the volatility parameter this way will not work. This issue will be discussed at the end of the chapter.  
...
The answers to these questions change depending on which [[Risk Neutral Pricing of Options|underlying asset]] is considered. This is because there is more than one explanation for the existence of the smile, and for different sectors, different explanations seem to prevail. Thus, before we analyze the smile dynamics and its properties any further, we need to discuss the major explanations advanced for the existence of the volatility smile.  

# 16.13 HOW TO EXPLAIN THE SMILE  

The volatility smile is an empirical phenomenon that violates the assumptions of the [[Black-Scholes Model|Black Scholes]] world. At the same time, the volatility smile is related to the implied volatilities obtained from the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]]. This may give rise to confusion. The smile suggests that the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] is not valid, while at the same time, the trader obtains the smile using the very same [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]]. Is there an internal inconsistency?  
...
This goes for options also. The [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] may be the wrong formula if we substitute one particular volatility, but may give the right answer if we use another volatility. And the latter may be different than the real-world volatility at that instant. But traders can still use a particular volatility to obtain the right option price from this “wrong” formula, just as in the earlier present value example. This particular volatility, when associated with the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]], may give the correct value for the option even though the assumptions leading to the formula are not satisfied.  

Thus, suppose the [[Arbitrage Pricing of Derivatives|arbitrage]]-free option price obtained under the “correct” assumptions is given by  
$$
C(S_{t},t,T,K,\sigma_{t}^{*},\theta_{t})
$$  

where $K$ is the [[Call and Put Payoffs at Expiry|strike price]], $T$ is the [[Risk Neutral Pricing of Options|expiration date]], and $S_{t}$ is the [[Black Scholes Derivation|underlying asset price]]. The (vector) variable $\theta_{t}$ represents all the other parameters that enter the “correct” formula and that may not be taken into account by the [[Black-Scholes Model|Black Scholes]] world. For example, the volatility may be stochastic, and some parameters that influence the volatility dynamics may indirectly enter the formula and be part of $\theta_{t}$ .13 The critical point here is the meaning that is attached to $\boldsymbol{\sigma}_{t}^{*}$ . We assume for now that it is the correct instantaneous volatility as of time $t.$ .  

The (correct) [[Arbitrage Pricing of Derivatives|pricing]] function in Eq. (16.68) may be more complex and may not even have a closed-form solution in contrast to the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]], $F(S_{t},\ t,\ \sigma)$ . Suppose traders ignore Eq. (16.68) but prefer to use the formula $F(S_{t},\ t,\ \sigma)$ , even though the latter is “wrong.” Does this mean traders will calculate the wrong price?  
...
Thus, we may be able to get the correct option price from the “unrealistic” [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] if we proceed as follows:  

1. We quote the $K_{i}$ -strike option volatilities $\sigma_{i}$ directly at every instant $t$ , under the condition that the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] be used to obtain the option value. Then, liquid and [[Arbitrage Pricing of Derivatives|arbitrage]]-free markets will supply “correct” observations of the ATM volatility $\sigma_{0}$ .14   
...
where $f(.)$ is, in general, positive and implies a smile effect. The adjustment made to the ATM volatility, $\sigma_{0}.$ , is such that when $\sigma((S/K_{i}),S)$ is used in the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]], it gives the correct value for $K_{i}$ strike option:  
$$
F{\Bigg(}S_{t},t,K_{i},\sigma_{0}+f{\Bigg(}{\frac{S}{K_{i}}},S{\Bigg)}{\Bigg)}=C(S_{t},t,K_{i},\sigma_{t}^{*},\theta_{t})
$$  

The adjustment factor $f((S/K_{i}),S)$ is determined by the trader’s experience, knowledge, and the trading environment at that instant. The relationships between [[Chapter 30 - Other Options|risk reversal]], butterfly, and ATM volatilities discussed in the previous section can also be used here.15  

The trader, thus, adjusts the volatility of the non-ATM options so that the wrong formula gives the correct answer, even though what is used in the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] may not be the “correct” instantaneous realized volatility of the $S_{t}$ process.  

The $f((S/K_{i}),S)$ is, therefore, an adjustment required by the imperfections of the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] in adequately representing the real-world environment. The upshot is that when we plot $\sigma((S/K_{i}),S)$ against $K_{i}/S$ or $K_{i}$ we get a smile, or a skew curve, depending on the time and the sector we are working with.  

For what types of situations should the volatilities be adjusted? At least three inconsistencies of the [[Black-Scholes Model|Black Scholes]] assumptions with the real world can be corrected for by adjusting the volatilities across the strike $K_{i}$ . The first is the lognormal process assumption. The second is the fact that if asset prices fall dramatically during a relatively short period of time, this could increase the “fear factor” and volatility would increase. The third involves the organizational and regulatory assumptions concerning [[Financial Markets and Institutions Lecture Notes|financial markets]]. We discuss these in more detail next.  

# 16.13.1 CASE 1: NONGEOMETRIC PRICE PROCESSES  

Suppose the underlying obeys the true [[The Vasicek Model|risk-neutral dynamics]] described by the SDE:  
$$
\mathrm{d}S_{t}=r S_{t}\mathrm{d}t+\sigma S_{t}^{\alpha}\mathrm{d}W_{t}\quad t\in[0,\infty)
$$  

With $\alpha=1,\ S_{t}$ would be lognormal. Everything else being conformable to the [[Black-Scholes Model|Black Scholes]] world, there would be no smile in the implied volatilities.  

The case of $\alpha<1$ would require an adjustment to the volatility coefficient used in the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] as the strike changes. This is true, since, unlike in the case of $\alpha=1$ , now the percentage volatility is dependent on the level of $S_{t}$ . We divide by $S_{t}$ to obtain  
$$
{\frac{\mathrm{d}S_{t}}{S_{t}}}=r\mathrm{d}t+\sigma S_{t}^{\alpha-1}\mathrm{d}W_{t}\quad t\in[0,\infty)
$$  

The percentage volatility is given by the term $\sigma S_{t}^{\alpha-1}$ . This percentage volatility will be a decreasing function of $S_{t}$ if $\alpha<1$ . As $S_{t}$ declines, the percentage volatility increases. Thus, the trader needs to use higher [[A Real-Life Option Pricing Exercise|implied volatility]] parameters in the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] for put options with lower and lower strike prices. This means that the more out-of-the-money the put option is, the higher the volatility used in the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] must be.  

This illustrates the idea that although the trader knows that the [[Black-Scholes Model|Black Scholes]] world is far from reality, the volatility is adjusted so that the original [[Black-Scholes Model|Black Scholes]] framework is preserved and that a “wrong” formula can still give the correct option value.  
...
Ignoring other complications that may exist in reality, we use the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] straightforwardly with  
$$
...
This is shown in Figure 16.15. Clearly, as the moneyness of the puts decreases, the volatility increases. Option market makers will conclude that, if in 6 months, US [[An Introduction to Equity Markets|equity markets]]  

![](a6647095e146f87e6cda22953fd174d085bfbd7dc74b871e27151ad3a6616418.jpg)  

# FIGURE 16.15  

[[A Real-Life Option Pricing Exercise|Implied volatility]] of OEX options against moneyness.  
...
Option market makers may now think that if, in 6 months, US [[An Introduction to Equity Markets|equity markets]] were to increase by $20\%$ , then the fear factor would decrease and so would volatility.  
...
Consider again the standard [[Black Scholes Derivation|geometric Brownian motion]] case:  
$$
\mathrm{d}S_{t}=r S_{t}\mathrm{~d}t+\sigma S_{t}\mathrm{~d}W_{t}\quad t\in[0,\infty)
$$  
$W_{t}$ is a [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] under the risk-neutral probability $\tilde{P}_{\l}$ . Now, keep the volatility parameterization the same, but instead, add a jump component as discussed in Lipton (2002). For example, let  
$$
...
This jump-diffusion model captures some crash phenomena. [[Hedge Fund Strategies|Stock market]] crashes, major defaults, 9/11-type events, and [[Forwards and Futures Notes|currency]] devaluations can be modeled as rare but discrete events that lead to jumps in prices.  

The way these types of jumps create a smile can be heuristically explained as follows. In a world where the [[Black-Scholes Model|Black Scholes]] assumptions hold, with a geometric $S_{t}$ process, a constant volatility parameter $\tilde{\sigma}$ , and no jumps, the volatility trade yields the [[Arbitrage Pricing of Derivatives|arbitrage]] relation:  
$$
\frac{1}{2}C_{s s}\tilde{\sigma}^{2}S^{2}+C_{t}+r C_{s}S-r C=0
$$  

With a jump term added to the geometric process as in Eq. (16.77), the corresponding [[Arbitrage Pricing of Derivatives|arbitrage]] relation becomes  
$$
\frac{1}{2}C_{s s}^{*}\sigma^{2}S^{2}+C_{t}^{*}+(r-\lambda m)C_{s}^{*}S-r C^{*}+\lambda E_{t}^{\tilde{P}}\left[C^{*}(S e^{j},t)-C^{*}(S,t)\right]=0
$$  

where $\Tilde{P}$ is the risk-neutral probability. Suppose we decide to use, as a convention, the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]], but believe that the true PDE is the one in Eq. (16.83). Then, we would select $\tilde{\sigma}$ such that the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] yields the same option value as the other PDE would yield.  

For example, out-of-the-money options will have much smaller gammas, $C_{s s}$ . If the expected jump is negative, then $\tilde{\sigma}$ will be bigger, and the more out-of-the-money the options are. As the [[Risk Neutral Pricing of Options|expiration date]] $T$ increases, $C_{s s}$ will increase and the smile will be less pronounced.  

# 16.13.3 OTHER EXPLANATIONS  

Many other effects can cause a volatility smile. One is [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]]. Consider a local volatility specification using  
$$
\mathrm{d}S_{t}=\mu S_{t}\mathrm{d}t+\sigma S_{t}^{\alpha}\mathrm{d}W_{t}\quad t\in[0,\infty)
$$  

with, say, $\alpha<1$ . In this specification, percentage volatility will be stochastic since it depends on the random variable $S_{t}$ . But often this specification does not express what is meant by models of [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]]. What is captured by [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]] is a situation where an additional [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] $\mathrm{d}B_{t}$ , possibly correlated with $\mathrm{d}W_{t}$ , affects the dynamics of percentage volatility. For example, we can write  
$$
...
It can be shown that [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]] generates a volatility smile. In fact, with [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]], we can perform an analysis similar to the PDE with a jump process (Lipton, 2002). The result will essentially be similar. However, it is important to emphasize that, everything else being the same, this model may be incomplete in the sense that there may not be enough instruments to hedge the risks associated with $\mathrm{d}W_{t}$ and $\mathrm{d}B_{t}$ completely, and form a risk-free, [[PSET 5 Solution-Financial Instruments|self-financing portfolio]]. The jump-diffusion model discussed in the previous section may entail the same problem. To the extent that the jump part and the diffusion part are affected by different processes, the model may not be complete.  

# 16.13.3.1 Structural and regulatory explanations  

Tax effects ([[Credit Markets Session 5|Merton]], 1976) and the capital requirements associated with carrying out-of-the-money options in options books may also lead to a smile in [[A Real-Life Option Pricing Exercise|implied volatility]]. We briefly touch on the second point.  

The argument involves the concept of gamma. A negative gamma position is considered to be more risky, the more out-of-the-money the option is. Essentially, negative gamma means that the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] has sold options and delta hedged them, and that he or she is paying the gamma through the rebalancing of this hedge. If the option is deep out-of-the-money, gamma would be close to zero. Yet, if the option suddenly becomes in-the-money, the gamma could spike, especially if the option is about to expire. This may cause significant losses. Out-of-the-money options, therefore, involve substantial risk and require more capital. Due to such costs, the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] may want to sell the out-of-the-money option at a higher price than warranted by the ATM volatility.  

# 16.14 THE RELEVANCE OF THE SMILE  

The volatility smile is important in financial engineering for at least three reasons.  

First, if we associate a volatility smile with all the [[PSET 6- Financial Instruments|risk factors]], and if this smile shifts randomly over time, then we may be able to trade it, take spread positions, and [[Arbitrage Pricing of Derivatives|arbitrage]] it. The smile dynamics, thus, imply new opportunities for a market professional.  

The second reason for the relevance of the volatility smile is that it may contain important information about the dynamics of the underlying realized volatility processes. With a volatility smile, [[Arbitrage Pricing of Derivatives|pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] may become much more complicated, especially if the instrument has characteristics of an [[Exotic Interest Rate Options|exotic]] option. Is volatility constant or a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]? If the latter is the case, then what type of [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] is it? Are there jumps or is a process with Wiener-type increments a sufficiently good approximation? These questions are important for [[Financial Mathematics Course|risk management]] and [[Arbitrage Pricing of Derivatives|pricing]].  
...
[[Arbitrage Pricing of Derivatives|Pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] are fairly closely related activities, at least in abstract settings. Once an asset is replicated with liquid securities, the price of the asset is the cost of the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] plus some [[Operating Margin|profit margin]]. At several points in the previous chapters, we saw that assets can be replicated using a series of options with different strike prices. This was the method applied for finding a hedge for a [[Variance Swaps|volatility swap]] in Chapter 15, for example. The [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] was made of a weighted sum of relevant options with the same characteristics except for their strikes. In Chapter 12, we saw that option portfolios could replicate statically almost any future payoff function. Again, these options were similar except for their strike prices.  

The potential use of options with different strikes makes the volatility smile a crucial parameter in forming [[Key Rates O1s Durations and Hedging|hedging]] portfolios and in [[Arbitrage Pricing of Derivatives|pricing]] complex instruments. In fact, if implied volatilities depend on the moneyness of the option, then the volatility parameters used in formulas for replicating portfolios would automatically change as the markets move and the moneyness of the replicating options changes. The critical point is that this will be true even if the underlying realized volatility remains the same. This section presents two examples of this phenomenon.  

The first involves the class of interest rate [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] known as [[Interest Rate Derivatives-An Introduction to the  Pricing of Caps and Floors|caps and floors]]. These are among the most widely traded instruments. Their [[Key Rates O1s Durations and Hedging|hedging]] and [[Arbitrage Pricing of Derivatives|pricing]] are influenced in a crucial way if there is a volatility smile. The second involves the [[Arbitrage Pricing of Derivatives|pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] of [[Exotic Interest Rate Options|exotic]] options. Here also, the methodology and market practices crucially depend on the existence of the volatility smile.  

# 16.17 EXOTIC OPTIONS AND THE SMILE  

The second major category of instruments where the existence of the volatility smile can change [[Arbitrage Pricing of Derivatives|pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] practices significantly is [[Exotic Interest Rate Options|exotic]] options. In this section, we consider a simple knock-out call that is representative of the main ideas we want to convey. Due to the contractual equation between vanilla options, knock-out calls, and knock-in calls, our discussion immediately extends to knock-in calls as well. At the end of this section, we briefly discuss digital options and how the existence of the volatility smile affects them.  
...
There are several ways of [[Key Rates O1s Durations and Hedging|hedging]] knock-out options used by practitioners. Here, we explain a hedge that (i) has nice financial engineering implications and (ii) shows clearly the important role played by the smile.  

Suppose we bought the corresponding vanilla call and sold a carefully chosen out-of-the money vanilla put with strike $K^{*}$ , $K^{*}<K.$ , with a very precise objective. We want the put and the call to be such that, as $S_{t}$ approaches the barrier $H$ , the [[An Asset Allocation Primer|portfolio]]’s value becomes zero. This [[An Asset Allocation Primer|portfolio]], which is, in fact, a type of [[Chapter 30 - Other Options|risk reversal]], approximately replicates the knock-out option. If $S_{t}$ moves away from $H_{;}$ , the put becomes more out-of-the-money, and its value will decline. Then the [[An Asset Allocation Primer|portfolio]] looks more and more like a vanilla call. This is what the knock-out option accomplishes anyway. On the other hand, as $S_{t}$ approaches $H$ , the put becomes more valuable. If it is carefully chosen, at $H$ the value of the put position can equal the value of the vanilla call and the [[An Asset Allocation Primer|portfolio]] would have zero value. This is, again, what the knock-out option accomplishes near $H.$ . As $S_{t}$ falls below $H.$ , the [[An Asset Allocation Primer|portfolio]] has to be liquidated. Thus, the [[An Asset Allocation Primer|portfolio]] of  
$$
...
then the value of $x$ units of these $K^{*}$ puts would equal the value of the $K$ call as $S_{t}$ approaches $H$ . As a result, the [[An Asset Allocation Primer|portfolio]] would replicate the knock-out call, except that once the barrier is hit, the [[An Asset Allocation Primer|portfolio]] needs to be liquidated.  

# 16.17.2 EFFECTS OF THE SMILE  

Consider first the effect of a stable volatility smile on this procedure. If the smile does not shift over time, then it is easy to incorporate the effect into the previous [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]]. Suppose the smile is downward sloping over $[K^{*},K]$ . Then, one could plug different volatility parameters in [[Black-Scholes Model|Black Scholes]] formulas for each vanilla option. The same $K^{*}$ put selected earlier would be relatively more valuable than in the case of a flat smile. This means that the knockout option could be sold at a cheaper price. If the smile was upward sloping over the range, then the reverse would be true and the knockout would be more expensive. Hence, the smile has a direct effect that needs to be taken into account in the [[Arbitrage Pricing of Derivatives|pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] of the knockout.  

There is a second effect of the smile as well. Suppose the smile is not stable during the life of the option, and that it shifts as time passes. Then the logic of [[Forward and Futures Contracts|replication]] would fall apart since a smile that shifts over time would make the relative values of the call and the put differ from the originally intended ratio as $S_{t}$ approaches $H.$ . Given that in most markets the smile is unstable over time, the [[Key Rates O1s Durations and Hedging|hedging]] technique by this [[Forward and Futures Contracts|replication]] is questionable. The [[Arbitrage Pricing of Derivatives|pricing]] of the knock-out would also be unreliable.  

# 16.17.2.1 An example of technical difficulties  

We can look at the complications introduced by the volatility smile using knock-out [[Arbitrage Pricing of Derivatives|pricing]] formulas in case all standard assumptions are satisfied. The [[Arbitrage Pricing of Derivatives|pricing]] formula for knock-outs was given in Chapter 9. In particular, the price of a down-and-out call written on a stock $S_{t},$ , satisfying all standard assumptions, and paying no dividends, was given by  
$$
C^{b}(t)=C(t)-J(t)
$$  

where $C(t)$ is the value of a vanilla call, given by the standard [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]], and where $J(t)$ is the “discount” that needs to be applied because the option may die if $S_{t}$ falls below $H$ during the life of the contract. The formula for $J(t)$ was  
$$
...
Note that just like the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]], this [[Arbitrage Pricing of Derivatives|pricing]] function contains a single volatility parameter $\sigma$ . In the plain vanilla case, this parameter could be manipulated to make the formula yield the correct answer, even when the underlying assumptions do not hold. Thus, in a smile environment with nonconstant volatilities, the trader could use one value for volatility and make the formula yield the correct answer. In fact, this was used in [[Arbitrage Pricing of Derivatives|pricing]] [[Interest Rate Derivatives-An Introduction to the  Pricing of Caps and Floors|caps and floors]] even though the volatilities of the individual forward rates that are relevant to these instruments were different.  

Unfortunately, this approach is not guaranteed to work for the down-and-out call [[Arbitrage Pricing of Derivatives|pricing]] formula given here if a volatility smile exists and if this smile is continuously (and stochastically) shifting over time. In fact, there may not be a single well-behaved volatility value to replace in Eq. (16.94) to obtain the correct price of the down-and-out call. Even when the realized and ATM implied volatilities remain the same, if the slope of the smile changes, the price of the down-andout call may change as in the previous argument. In fact, if the smile becomes less negatively sloped, the short put component of the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] will become relatively less expensive and the value of the down-and-out call may increase. Hence, in the case of [[Exotic Interest Rate Options|exotic]] options, the relationship between volatility adjustments and the correct option price may become much more complex as smile effects become significant.  

# 16.17.2.2 Pricing exotics  

Actual trading takes place in the presence of a volatility smile, and the prices of [[Exotic Interest Rate Options|exotic]] options need to be set so as to take into account the future costs and benefits of adjusting the hedge to the [[Exotic Interest Rate Options|exotic]] option. With the presence of smile, as time passes, the vega hedge of an [[Exotic Interest Rate Options|exotic]] option book needs to be adjusted for the reasons explained earlier. As this happens, depending on the net position of the option book, the trader may realize some net cash gains or losses. The present value of these “expected” cash gains and losses needs to be incorporated into the market price. At the end, the market price of the [[Exotic Interest Rate Options|exotic]] may be higher or lower than the theoretical price indicated by Eq. (16.94).  

# 16.17.3 THE CASE OF DIGITAL OPTIONS  

Chapter 11 showed that a theoretical [[Forward and Futures Contracts|replication]] of a European digital call with strike $K$ and expiration $T$ would be to buy $1/h$ units of the vanilla call with strike $K$ and to sell $1/h$ units of the vanilla call with strike $K+h$ . In this case $h$ would be the minimum tick in a [[Futures Not Subject to Cash-And-Carry|futures]] market.  
...
Activity in the dollar/yen foreign exchange markets over the past fortnight has emphasized the severe complexities associated with [[Arbitrage Pricing of Derivatives|pricing]] [[Exotic Interest Rate Options|exotic]] options. More importantly, it has provided sophisticated option houses with an opportunity to test their [[Arbitrage Pricing of Derivatives|pricing]] theories against each other and against their less-advanced competitors. . .  

However, it was not the decline in the [[The Foreign Exchange Market Annotations|spot rate]] itself that provided the interest for options dealers but the resulting risk-reversal position. Risk-reversal is an expression of the directional [[Utility Indices|preference]] in the market. If spot is expected to fall, as in the case of dollar/yen, then there will be greater demand for puts relative to calls, and so the volatility trader will pay a higher price for the puts than the calls. The upshot of this, said one commentator, is that volatility is not constant, as assumed by the standard [[Black-Scholes Model|Black Scholes]] option-[[Arbitrage Pricing of Derivatives|pricing]] model, but instead changes according to the option delta.  

One-month dollar/yen [[Chapter 30 - Other Options|risk reversal]] shot up to nearly 3 last week and has continued to hover at levels not seen since the summer of 1995. This extraordinary situation enabled sophisticated traders to find value in the [[Arbitrage Pricing of Derivatives|pricing]] of their so-called naive counterparts.  

“A lot of banks must have learned a lot about risk-reversal over the past few days,” said one trader. According to market insiders, the less advanced houses failed to adequately account for the effects of riskreversal in [[Arbitrage Pricing of Derivatives|pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] [[Exotic Interest Rate Options|exotic]] structures such as knock-out and path-dependent options.  

They also asserted that simple off-the-shelf option [[Arbitrage Pricing of Derivatives|pricing]] software was unable to cope with [[Arbitrage Pricing of Derivatives|pricing]] [[Exotic Options|exotic derivatives]] in a risk-reversal scenario. These packages did not allow the user to enter different volatilities for different deltas and so failed to capture the nuances of exotics [[Arbitrage Pricing of Derivatives|pricing]]. However, other commentators argued that this too was an over-simplification and that other “thirdorder” effects came into play when [[Key Rates O1s Durations and Hedging|hedging]] certain types of options in very high risk-reversal scenarios. They added that the third-order effects meant the barrier described might not be overpriced by the [[Black-Scholes Model|Black Scholes]] user, merely mispriced.  

(Thomson Reuters IFR Issue 1188, June 1997)  

As this reading illustrates, risk reversals are creations of the volatility smile in FX markets and are heavily traded. But, as indicated in the reading, market practitioners involved in riskreversal trading are clearly dealing with the underlying smile dynamics. The dynamics can become very complex. [[Arbitrage Pricing of Derivatives|Pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] such positions on [[Exotic Interest Rate Options|exotic]] options may become much more difficult.  

# 16.18 CONCLUSIONS  

The volatility smile is a fascinating topic in finance. Yet, it is also a complex phenomenon and more research needs to be done on its causes and on the ways to model it. This chapter offered a simple [[Squam Lake Group Introduction|introduction]]. However, it has illustrated some of the essential points associated with this topic.  

# SUGGESTED READING  

Allen and Granger (2005) provide an excellent [[Overview of Financial Markets|overview]] of techniques for trading equity correlation. The text by Brigo and Mercurio (2006) deals with the volatility smile in the interest rate sector. For equity smiles, the reader should consult the papers by Derman et al. (1994) and (1996), at a minimum. A comprehensive treatment of the volatility smile is provided in Lipton (2002). Taleb (1996) is the source that we used to discuss most market practices. There is also a flurry of papers dealing with empirical and theoretical issues involving the volatility smile. One recent source is Johnson and Lee (2003). The cited sources contain further references on the previous research.  

# EXERCISES  

1. Consider the following table displaying the [[Bid Ask and Transaction Prices in a Specialist Market With Heterogeneously Informed Traders|bid ask]] prices for all options on the OEX index passed on January 10, 2002, at 9:46. These options have February 22, 2002, expiry and at the time of data collection, the underlying was at 589.14.  
...
a. Using the out-of-the-money ask prices for the puts, calculate the [[A Real-Life Option Pricing Exercise|implied volatility]] for the relevant strikes. Plot the volatility smile against K/S.   
b. Using the out-of-the-money bid prices for the puts, calculate the [[A Real-Life Option Pricing Exercise|implied volatility]] for the relevant strikes. Plot the volatility smile against K/S. Are the [[Bid Ask and Transaction Prices in a Specialist Market With Heterogeneously Informed Traders|bid ask]] spreads for these vols constant?   
...
2. (Stop-loss hedge). Write a VBA program to show the stop-loss hedged [[An Asset Allocation Primer|portfolio]] adjustments and cash flows for 100 long calls from the dealers’ perspective with the following data:  
$$
...
Plot the graph for the volatility payoff for 30 instances for these sets of adjustment frequency [10, 20, 50, 100, 500]. Also show the performance of the method on the graph as a plot of mean return of the [[The Value of the Swap Contract after Initiation|hedging strategy]] versus frequency of adjustment.  

4. ([[Valuing European Option Using the Heston Model in QuantLib Python|Stochastic volatility]]) Write a MATLAB program to observe the [[A Real-Life Option Pricing Exercise|implied volatility]] smile for the option based on the [[Chapter 16 - Black–Scholes Model|stock price]] having [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]]. Use the following data for the calculation.  
$$
S(0)=100;K=100;T=0.5;\quad r=8\%\sigma=30\%;\sigma_{\sigma}=10\%,r_{\sigma}=10\%
$$  

Plot the graph of the estimated [[A Real-Life Option Pricing Exercise|implied volatility]] against the moneyness of the option.  

5. (Nongeometric [[Continuous-Time Stochastic Processes|Brownian motion]]) Write a MATLAB program to observe the [[A Real-Life Option Pricing Exercise|implied volatility]] smile for the option based on the [[Chapter 16 - Black–Scholes Model|stock price]] following the dynamics of nongeometric [[Continuous-Time Stochastic Processes|Brownian motion]]. Use the following data for the calculation.  
$$
S(0)=100;K=100;T=0.5;\quad r=8\mathcal{H};\sigma=30\mathcal{H};\alpha=0.8
$$  

Plot the graph of the estimated [[A Real-Life Option Pricing Exercise|implied volatility]] against the moneyness of the option. 6. ([[Chapter 16 - Black–Scholes Model|Stock price]] crashes and jumps)  

Write a MATLAB program to observe the [[A Real-Life Option Pricing Exercise|implied volatility]] smile for the option based on the [[Chapter 16 - Black–Scholes Model|stock price]] having jumps (particularly crashes). Use the following data for the calculation.  
$$
...
7. (VIX calculation) Using the S&P 500 option data on the chapter website (collected on July 4, 2013 for options expiring on July 25, 2013), calculate the VIX index value following the procedure discussed in the book. Take the [[Exercises|risk-free interest rate]] value to be $5\%$ for the purpose of calculation. Also highlight the [[Call and Put Payoffs at Expiry|strike price]] $K_{0}$ and show the contribution by strike for all the options.  