...
[^2]: The underlying stock price dynamics are described in continuous time by the stochastic differential equation%20Model.md) (SDE):15
$$
\mathrm{d}S_{t}=\mu(S_{t})S_{t}\mathrm{~d}t+\sigma S_{t}\mathrm{~d}W_{t}\quad t\in0,\infty)
$$  

where $W_{t}$ represents a [Wiener process%20Process.md) with respect to real-world probability $P$ .16  
...
These increments are the continuous time equivalents of sequences of normally distributed variables. For a discussion of stochastic differential equations and the Wiener process%20Process.md), see, for example, Øksendal (2010). Hirsa and Neftci (2013) provides the heuristics.  
...
An SDE, driven by a Wiener process%20Process.md) $W_{t},$ is written as  
$$
\mathrm{d}S_{t}=a(S_{t},t)\mathrm{d}t+b(S_{t},t)\mathrm{d}W_{t}\quad t\in0,\infty)
$$  

This equation describes the dynamics of $S_{t}$ over time. The [Wiener process%20Process.md) $W_{t}$ has increments $\Delta W_{t}$ that are normally distributed with mean zero and variance $\Delta$ , where the $\Delta$ is a small time interval. These increments are uncorrelated over time. As a result, the future increments of a Wiener process%20Process.md) are unpredictable given the information at time $t$ , the $I_{t}$ .  
...
$$
\mathrm{d}S_{t}=\mu\mathrm{d}t+\sigma\mathrm{d}W_{t}\quad t\in0,\infty)
$$  

Here, $W_{t}$ is a standard [Wiener process%20Process.md) with variance $t$ . In this SDE, the coefficients $\mu$ and $\sigma$ do not have time subscripts $t$ , as time passes, they do not change.  
...
$$
\mathrm{d}S_{t}=\mu S_{t}\mathrm{d}t+\sigma S_{t}\mathrm{d}W_{t}\quad t\in0,\infty)
$$  
...
In heuristic terms, this theorem says the following. If we are given a Wiener process%20Process.md) $W_{t}$ , then, we can multiply the probability distribution of this process by a special function $\boldsymbol{\xi}_{t}$ that depends on time $t.$ , and on the information available at time $t$ , the $I_{t}$ . This way we can obtain a new Wiener process%20Process.md) $\tilde{W}_{t}$ with probability distribution $\tilde{P}$ . The two processes will relate to each other through the relation:  
$$
