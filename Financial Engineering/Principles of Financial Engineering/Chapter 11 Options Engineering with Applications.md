...
1.1 [[Squam Lake Group Introduction|Introduction]] . 352   
11.1.1 Payoff Diagrams .. 352   
11.1.1.1 Examples of xt . 354   
1.2 Option Strategies .... . 355   
11.2.1 Synthetic [[Market Size and Participants|Long and Short Positions]]. 355   
11.2.1.1 An application .. 357   
11.2.1.2 [[Arbitrage Pricing of Derivatives|Arbitrage]] opportunity?.. . 360   
...
[[Exercises|Exercises]] . 389   
EXCEL [[Exercises|Exercises]] .. 391   
MATLAB Exercise . 391  

# 11.1 INTRODUCTION  

This chapter discusses traditional option strategies from the financial engineering perspective and provides market-based examples. It then moves on to discuss [[Exotic Interest Rate Options|exotic]] options. We are concerned with portfolios and positions that are taken with a precise gain loss profile in mind. The players consciously take risks in the hope of benefiting or protecting themselves from an expected movement in a certain risk factor. Most [[Runs On Money Market Mutual Funds|investor behavior]] is of this kind. Investors buy a stock with a higher (systematic) risk, in anticipation of higher [[Assets|returns]]. A high-yield bond carries a higher default probability, which the bond holder is willing to bear. For all the different instruments, there are one or more [[PSET 6- Financial Instruments|risk factors]] that influence the gains and losses of the position taken. The investor weighs the risks due to potentially adverse movements in these factors against the gains that will result, if these factors behave in the way the investor expected. Some of the [[Key Rates O1s Durations and Hedging|hedging]] activity can be interpreted in a similar way. This chapter deals with techniques and strategies that use options in doing this. We consider classical (vanilla) as well as [[Exotic Interest Rate Options|exotic]] options tools.  

According to an important theorem in modern finance, if options of all strikes exist, carefully selected option portfolios can replicate any desired gain loss profile that an investor or a hedger desires. We can synthetically create any asset using a (static) [[An Asset Allocation Primer|portfolio]] of carefully selected options, since financial positions are taken with a payoff in mind. Hence, we start our discussion by looking at payoff diagrams.  
...
First of all, for fairly priced exposures that have zero value of initiation, net exposures to a risk factor, $x_{T}$ , must be negative for some values of the underlying risk. Otherwise, we would be making positive gains, and there would be no risk of losing money. This would be an [[Arbitrage Pricing of Derivatives|arbitrage]] opportunity. Swap-type instruments fall into this category. If, on the other hand, the final payoffs of the contract are non-negative for all values of $x_{T}$ , the exposure has a positive value at initiation, and to take the position an upfront payment will have to be made. Option positions have this characteristic.3  

Second, exposures can be convex, concave, or linear with respect to $x_{T},$ , and this has relevance for an investor or market professional. The implication of linearity is obvious: the sensitivity of the position to movements in $x_{T}$ is constant. The relevance of [[PSET II Fixed Income Asset Pricing 1|convexity]] was discussed in Chapters 9 and 10. With [[PSET II Fixed Income Asset Pricing 1|convexity]], movements in volatility need to be priced in, and again options are an important category here.  
...
Various [[Interest Rate Quotations|interest rates]]: The best examples are LIBOR rates and swap rates. But the commercial paper (CP) rate, the [[Topics in Fiscal and Monetary Policies and Stabilization- Empirical Issues|federal funds rate]], the index of overnight [[Interest Rate Quotations|interest rates]] (an example of which is EONIA, Euro Over Night Index Average), and many others are also used as reference rates. Exchange rates, especially major exchange rates such as dollar euro, dollar yen, dollar sterling (“cable”), and dollar Swiss franc. Equity indices: Here also the examples are numerous. Besides the well-known US indices such as the Dow, NASDAQ, and the S&P500, there are European indices such as CAC40, DAX, and FTSE100, as well as various Asian indices such as the Nikkei 225 and emerging market indices. [[Futures Not Subject to Cash-And-Carry|Commodities]] are also quite amenable to such positions. [[Futures Not Subject to Cash-And-Carry|Futures]] on coffee, soybeans, and energy are other examples for $x_{T}.$ Bond price indices: One example is the EMBI $+$ prepared by [[Basis Trade Explainer|JPMorgan]] to track emerging market bonds.  
...
2. The correlation between two [[PSET 6- Financial Instruments|risk factors]] can be traded in a similar way.  
...
We divide the engineering of option strategies into two broad categories. First, we consider the classical option-related methods. These will cover strategies used by market makers as well as retail investors. They will themselves be divided into two groups, those that can be labeled directional strategies, and those that relate to views on the volatility of some underlying instrument. The second category involves [[Exotic Interest Rate Options|exotic]] options, which we consider as more efficient and sometimes cheaper alternatives to the classical option strategies. The underlying risks can be any of those mentioned in the previous section.  

# 11.2.1 SYNTHETIC LONG AND SHORT POSITIONS  

We begin with strategies that utilize options essentially as directional instruments, starting with the creation of [[Market Size and Participants|long and short positions]] on an asset. Options can be used to create these positions synthetically.  

Consider two plain vanilla options written on a [[Forward Contracts and Forward Prices|forward price]] $F_{t}$ of a certain asset. The first is a short put, and the second a long call, with prices $P(t)$ and $C(t)$ , respectively, as shown in Figure 11.3. The options have the same [[Call and Put Payoffs at Expiry|strike price]] $K$ , and the same expiration time $T.^{4}$ Assume that the [[Black-Scholes Model|Black Scholes]] conditions hold, and that both options are of European style. Importantly, the [[Risk Neutral Pricing of Options|underlying asset]] does not have any payouts during $[t,T]$ . Also, suppose the appropriate [[An Overview of the Vasicek Short Rate Model|short rate]] to discount [[Advanced Derivatives Pricing Methodology|future cash flows]] is constant at $r$ .  

Now consider the [[An Asset Allocation Primer|portfolio]]  
$$
\{1\mathrm{Long}K\mathrm{-Call},1\mathrm{Short}K\mathrm{-Put}\}
$$  

At expiration, the payoff from this [[An Asset Allocation Primer|portfolio]] will be the vertical sum of the graphs in Figure 11.3 and is as shown in Figure 11.4. This looks like the payoff function of a [[Forward Rates Agreement|long forward]] contract entered into at $K$ . If the options were at-the-money (ATM) at time $t$ , the [[An Asset Allocation Primer|portfolio]] would exactly duplicate the [[Forward Rates Agreement|long forward]] position and hence would be an exact synthetic. But there is a close connection between this [[An Asset Allocation Primer|portfolio]] and the [[Forward Points in Currency|forward contract]], even when the options are not ATM.  

At expiration time $T.$ , the value of the [[An Asset Allocation Primer|portfolio]] is  
$$
C(T)-P(T)=F_{T}-K
$$  
...
where $F_{T}$ is the time- $T$ value of the [[Forward Contracts and Forward Prices|forward price]]. This equation is valid because at $T,$ only one of the two options can be in-the-money. Either the call option has a value of $F_{T}-K$ while the other is worthless, or the put is in-the-money and the call is worthless, as shown in Figure 11.3.  

Subtract the time- ${\bf\nabla}\cdot{\bf\nabla}t$ [[Forward Contracts and Forward Prices|forward price]], $F_{t}$ , from both sides of this equation to obtain  
$$
C(T)-P(T)+(K-F_{t})=F_{T}-F_{t}
$$  

This expression says that the sum of the payoffs of the long call and the short put plus $(K-F_{t})$ units of cash should equal the time- $T$ gain or loss on a [[Forward Points in Currency|forward contract]] entered into at $F_{t},$ at time $t$ . Take the expectation of Eq. (11.4). Then the time- ${\mathbf{\nabla}}\cdot t$ value of the [[An Asset Allocation Primer|portfolio]],  
$$
...
should be zero at $t$ , since credit risks and the cash flows generated by the forward and the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] are the same. This implies that  
$$
C(t)-P(t)=e^{-r(T-t)}(F_{t}-K)
$$  

This relationship is called [[7. Black Scholes Model|put-call parity]]. It holds for European options. It can be expressed in terms of the spot price, $S_{t},$ as well. Assuming zero [[Primary vs. Secondary Commodities|storage costs]], and no convenience yield:5  
$$
F_{t}=e^{r(T-t)}S_{t}
$$  
...
[[7. Black Scholes Model|Put-call parity]] can thus be regarded as another result of the application of contractual equations, where options and cash are used to create a synthetic for the $S_{t}$ . This situation is shown in Figure 11.5.  

# 11.2.1.1 An application  

Option market makers routinely use the [[7. Black Scholes Model|put-call parity]] in exploiting windows of [[Class Note 13 The LTCM Meltdown|arbitrage opportunities]]. Using options, market makers construct synthetic [[Futures Not Subject to Cash-And-Carry|futures]] positions and then trade them  

![](f61c7d6ab4476e27deeb8b5f816e8b69b6f0aec4d1eff8fbc72a524e79d0a05a.jpg)  

# FIGURE 11.5  

[[7. Black Scholes Model|Put-call parity]] illustration and payoff of a synthetic stock position.  

against [[Mathematics of the Financial Markets|futures contracts]]. This way, small and temporary differences between the synthetic and the true contract are converted into “riskless” profits. In this section, we discuss an example.  
...
and that the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] can buy and sell ATM options that expire in 30 days. Suppose also that the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] faces a funding cost of $5\%$ . The stock never pays dividends and there are no corporate actions.  

Also, and this is the real-life part, the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] faces a transaction cost of 20 cents per traded option and a transaction cost of 5 cents per traded stock. Finally, the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] has calculated that to be able to continue operating, he or she needs a margin of 0.25 cent per position. Then, we can apply [[7. Black Scholes Model|put-call parity]] and follow the conversion strategy displayed in Figure 11.6.  
...
The position is fully hedged, as any potential gains due to movement in $S_{t}$ will cover the potential losses. This means that the only factors that matter are the transaction costs and any price differentials that may exist between the call and the put. The [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] will monitor the difference between the put and call premiums and take the [[Arbitrage Pricing of Derivatives|arbitrage]] position shown in Figure 11.6 if this difference is bigger than the total cost of the conversion.  

![](cf62839c2bebfa0fdd0033be553a87272ef1d2c2ab09ec5291c4bca03b9184d7.jpg)  

Adding together...If prices are different enough" then there will be [[Arbitrage Pricing of Derivatives|arbitrage]] opportunity.  

![](2fd870ffa25ebd2f617a9e69550088b95543d5653694d0a8d8488c3088e518ee.jpg)  

# FIGURE 11.6  

[[7. Black Scholes Model|Put-call parity]] application and conversion strategy.  

# EXAMPLE  

Suppose $S_{t}=100$ , and 90-day [[Notes on Basic Options Properties|call and put]] options trade actively. The interest cost is $5\%$ . A [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] has determined that the [[Chapter 17 - Option Strategies|call premium]], $C(t)$ , exceeds the [[Chapter 17 - Option Strategies|put premium]], $P(t)$ , by $\$2.10$ :  
$$
C(t)-P(t)=2.10
$$  
...
The [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] incurs a total cost of $\$1.95$ . It turns out that under these conditions, the net cash position will be positive:  
$$
...
If, in the example just discussed, the put-[[Chapter 17 - Option Strategies|call premium]] difference is negative, then the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] can take the opposite position, which would be called a reversal.6  

# 11.2.1.2 Arbitrage opportunity?  

An outside observer may be surprised to hear that such “[[Arbitrage Pricing of Derivatives|arbitrage]]” opportunities exist, and that they are closely monitored by market makers on the trading floor. Yet, such opportunities are available only to market makers on the “floor” and may not even constitute [[Arbitrage Pricing of Derivatives|arbitrage]] in the usual sense.  

This is because of the following: (i) Off-floor investors pay much higher transaction costs than the on-floor market makers. Total costs of taking such a position may be prohibitive for off-floor investors. (ii) Off-floor investors cannot really make a simultaneous decision to buy (sell) the underlying, and buy or sell the implied puts or calls to construct the strategy. By the time these strategies are communicated to the floor, prices could move. (iii) Even if such opportunities are found, net gains are often too small to make it worthwhile to take such positions sporadically. It is, however, worthwhile to a [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] who specializes in these activities. (iv) Finally, there is also a serious risk associated with these positions, known as the pin risk. Pin risk refers to the situation when the market price of the underlying of an option contract at the time of the contract’s expiration is close to the option’s [[Call and Put Payoffs at Expiry|strike price]]. Traders pay attention to whether an underlying such as a stock, for example, may be close to pinning, since a small movement of the price of the underlying through the strike can have a large impact on a trader’s net position in the underlying on the trading day after expiration.  

# 11.2.2 A REMARK ON THE PIN RISK  

It is worthwhile to discuss the pin risk in more detail, since similar risks arise in [[Key Rates O1s Durations and Hedging|hedging]] and trading some [[Exotic Interest Rate Options|exotic]] options as well. Suppose we put together a conversion at 100, and waited 90 days until expiration to unwind the position. The positions will expire some 90 days later during a Friday. Suppose at expiration $S_{t}$ is exactly 100. This means that the stock closes exactly at the [[Call and Put Payoffs at Expiry|strike price]]. This leads to a dilemma for the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]].  

The [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] owns a stock. If he or she does not exercise the long put and if the short call is not assigned (i.e., if he or she does not get to sell at $K$ exactly), then the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] will have an open [[Chapter 4 - Futures: Hedging and Speculation|long position]] in the stock during the weekend. Prices may move by Monday and he or she may experience significant losses.  

If, on the other hand, the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] does exercise the long put (i.e., he or she sells the stock at $\boldsymbol{K}$ and if the call is assigned (i.e., he or she needs to deliver a stock at $K_{\cdot}$ ), then the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] will have a short stock position during the weekend. These risks may not be great for an end investor who takes such positions occasionally, but they may be substantial for a professional trader who depends on these positions. There is no easy way out of this dilemma. This type of risk is known as the pin risk.  

The main cause of the pin risk is the kink in the expiration payoff at $S_{T}=K.$ . A kink indicates a sudden change in the slope—for a long call, from zero to one or vice versa. This means that even with small movements in $S_{t},$ the [[Chapter 27 - Delta Hedging|hedge ratio]] can be either zero or one, and the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] may be caught significantly off guard. If the slope of the payoff diagram changed smoothly, then the required hedge would also change smoothly. Thus, a risk similar to pin risk may arise whenever the delta of the instrument shows discrete jumps.  

# 11.2.3 RISK REVERSALS  

A more advanced version of the synthetic long and short [[Futures Not Subject to Cash-And-Carry|futures]] positions is known as risk reversals. These are liquid synthetics especially in the foreign exchange markets, where they are traded as a commodity. Risk reversals are directional positions but differ in more than one way from synthetic long short [[Futures Not Subject to Cash-And-Carry|futures]] positions discussed in the previous section.  

The idea is again to buy and sell calls and puts in order to replicate long and short [[Futures Not Subject to Cash-And-Carry|futures]] positions—but this time using options with different strike prices. Figure 11.7 shows an example. The underlying is $S_{t}$ . The strategy involves a short put struck at $K_{1}$ , and a long call with strike $K_{2}$ . Both options are out-of-the-money initially, and the $S_{t}$ satisfies  
$$
K_{1}<S_{t}<K_{2}
$$  
...
Payoff of a long call, short put, and a [[Chapter 30 - Other Options|risk reversal]] position.  

Since strikes can be chosen such that the put and call have the same premium, the [[Chapter 30 - Other Options|risk reversal]] can be constructed so as to have zero initial price.  

By adding vertically the option payoffs in the top portion of Figure 11.7, we obtain the expiration payoff shown at the bottom of the figure. If, at expiration, $S_{T}$ is between $K_{1}$ and $K_{2}$ , the strategy has zero payoff. If, at expiration, $S_{T}{<}K_{1}$ , the [[Chapter 30 - Other Options|risk reversal]] loses money, but under $K_{2}<S_{T}$ , it  

makes money. Clearly, what we have here is similar to a [[Chapter 4 - Futures: Hedging and Speculation|long position]] but the position is neutral for small movements in the underlying starting from $S_{t}$ . If taken naked, such a position would imply a bullish view on $S_{t}$ .  

We consider an example from foreign exchange (FX) markets where risk reversals are traded as [[Futures Not Subject to Cash-And-Carry|commodities]].  
...
Traders said market makers in EUR calls were buying risk reversals expecting further euro upside. The 1-month [[Chapter 30 - Other Options|risk reversal]] jumped to 0.91 in favor of euro calls Wednesday from 0.3 3 weeks ago. [[A Real-Life Option Pricing Exercise|Implied volatility]] spiked across the board. One-month volatility was $13.1\%$ Wednesday from $11.78\%$ 3 weeks ago as the euro appreciated to USD1.0215 from USD1.0181 in the spot market.  

The 25-delta risk reversals mentioned in this reading are shown in Figure 11.8. The [[Chapter 30 - Other Options|risk reversal]] is constructed using two options, a call and a put. Both options are out-of-the-money and have a “current” delta of 0.25. According to the reading, the 25-delta EUR call is more expensive than the 25-delta EUR put.  

# 11.2.3.1 Uses of risk reversals  

Risk reversals can be used as “cheap” [[Key Rates O1s Durations and Hedging|hedging]] instruments. Here is an example.  

# EXAMPLE  

A travel company in Paris last week entered a zero-cost [[Chapter 30 - Other Options|risk reversal]] to hedge US dollar exposure to the USD. The company needs to buy dollars to pay suppliers in the United States, China, Indonesia, and South America.  
...
The company entered a [[Chapter 30 - Other Options|risk reversal]] rather than buying a dollar call outright because it was cheaper. The head of treasury said the rest of its exposure is hedged using different strategies, such as buying options outright. (Based on an article in [[Chapter 9 Arbitrage and Hedging With Options|Derivatives]] Week (now part of GlobalCapital).)  

Here we have a corporation that has EUR receivables from tourists going abroad but needs to make payments to foreigners in dollars. Euros are received at time $t$ , and dollars will be paid at some future date $T,$ with $t<T$ . The [[Chapter 30 - Other Options|risk reversal]] is put together as a zero-cost structure, which means that the premium collected from selling the put (on the USD) is equal to the [[Chapter 17 - Option Strategies|call premium]] on the USD. For small movements in the exchange rate, the position is neutral, but for large movements it represents a hedge similar to a [[Futures Not Subject to Cash-And-Carry|futures]] contract.  

![](fc7f14f57cc136a376431000bdcacb63dadf033a946a2bf92e28b3dbe5cd47e7.jpg)  

# FIGURE 11.8  

Payoff of a long put, short call a [[Chapter 30 - Other Options|risk reversal]] position.  

Of course, such a position could also be taken in the [[Futures Not Subject to Cash-And-Carry|futures]] market. But one important advantage of the [[Chapter 30 - Other Options|risk reversal]] is that it is “composed” of options, and hence involves, in general, no daily mark-to-market adjustments.  

# 11.2.4 YIELD ENHANCEMENT STRATEGIES  

The class of option strategies that we have studied thus far is intended for creating synthetic short and long [[Futures Not Subject to Cash-And-Carry|futures]] positions. In this section, we consider option synthetics that are said to lead to yield enhancement for [[An Asset Allocation Primer|investment]] portfolios.  
...
The simplest case is the following. At time $t$ , an investor takes a [[Chapter 4 - Futures: Hedging and Speculation|long position]] in a stock with current price $S_{t}.$ , as shown in Figure 11.9. If the [[Chapter 16 - Black–Scholes Model|stock price]] increases, the investor gains; if the price declines, he or she loses. The investor has, however, a subjective [[Lecture 1- Probability Distributions of Returns|expected return]], $\bar{R}_{t}$ , for an interval of time $\Delta$ , that can be expressed as  
$$
...
and that expires at $T=t+\Delta$ . This option sells for $C(t)^{\mathrm{max}}$ at time $t$ . We can then recommend the following [[An Asset Allocation Primer|portfolio]] to this investor:  
$$
\mathrm{Yield\enhanced\portfolio}=\{\mathrm{Long}S_{t},\mathrm{Short}C(t)^{\mathrm{max}}\}
$$  

Assuming zero [[Interest Rate Quotations|interest rates]], at time $T=t+\Delta$ , this [[An Asset Allocation Primer|portfolio]] has the following value, $V_{t+\Delta}$ :  
$$
...
next month is shifting, say some options strategists. “The market has had no direction since May last year,” said a head of equity [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] strategy in   
...
Thus, the basic building blocks of volatility positions considered in this section are [[Composite Payoffs at Expiry|straddles and strangles]]. The following example indicates how an option position is used to take a view on volatility, rather than the price of the underlying.  

# EXAMPLE  

An Italian bank recommended the following position to a client. We will analyze what this means for the client’s [[FORWARD RATES AND TERM STRUCTURE|expectations]] [views] on the markets. First we read the episode.  
...
But the reading also gives the [[A Real-Life Option Pricing Exercise|implied volatility]] in the market. From here we can use the square root formula and calculate the [[A Real-Life Option Pricing Exercise|implied volatility]] for the period under consideration  
$$
...
A bank is recommending its clients to sell 1-month 10-delta euro/dollar strangles to take advantage of low holiday volatility. The strategists said the investors should sell $I$ -month strangles with puts struck at USD1.3510 and calls struck at USD1.3610. This will generate a premium of $0.3875\%$ of the notional size. Spot was trading at USD1.3562 when the trade was designed last week. The bank thinks this is a good time to put the trade on because [[A Real-Life Option Pricing Exercise|implied volatility]] traditionally falls over Christmas and New Years, which means spot is likely to stay in this range.  

(Based on [[Chapter 9 Arbitrage and Hedging With Options|Derivatives]] Week (now part of GlobalCapital))  
...
A [[PSET 4 Solution-Financial Instruments|short straddle]] generates premiums but has an unlimited downside. This may not be acceptable to a risk manager. Hence, the trader buys a strangle to limit these potential losses. But this type of insurance involves costs and the net cash receipts become smaller. The following shows a practical use of the short butterfly strategy.  

# EXAMPLE  

As the Australian dollar continues to strengthen on the back of surging [[Primary vs. Secondary Commodities|commodity prices]], dealers are looking to take advantage of an anticipated lull in the [[Forwards and Futures Notes|currency]]’s bull run by putting in place butterfly structures. One structure is a 3-month [[Two-Variable Regression Hedging|butterfly trade]]. The dealer sells an ATM Aussie dollar [[Notes on Basic Options Properties|call and put]] against the US dollar, while buying an Aussie call struck at $A U D0.682$ and buying puts struck at AUD0.6375. The structure can be put in place for a premium of $0.3\%$ of notional, noted one trader, adding that there is value in both the puts and the calls.  
...
Up to this point, the chapter has dealt with option strategies that used only plain vanilla calls and puts. The more complicated volatility building blocks, namely [[Composite Payoffs at Expiry|straddles and strangles]], were generated by putting together plain vanilla options with different strike prices or expiration. But the use of plain vanilla options to take a view on the direction of markets or to trade volatility may be considered by some as “outdated.” There are now more practical ways of accomplishing similar objectives.  

The general principle is this. Instead of combining plain vanilla options to create desired payoff diagrams, lower costs, and reach other objectives, a trader would directly design new option contracts that can do similar things in a “better” fashion. Of course, these new contracts imply a hedge that is, in general, made of the underlying plain vanilla options, but the new instruments themselves would sell as [[Exotic Interest Rate Options|exotic]] options.8 Before closing this chapter, we would like to introduce further option strategies that use [[Exotic Interest Rate Options|exotic]] options as building blocks. We will look at a limited number of exotics, although there are many others that we relegate to the [[Exercises|exercises]] at the end of the chapter.  

# 11.4.1 BINARY, OR DIGITAL, OPTIONS  

To understand binary options, first remember the static strangle and straddle strategies. The idea was to take a long (short) volatility position, and benefit if the underlying moved more (less) than what the [[A Real-Life Option Pricing Exercise|implied volatility]] suggested. Binary options form essential building blocks for similar [[Chapter 17 - Option Strategies|volatility strategies]], which can be implemented in a cheaper and perhaps more efficient way. Also, binary options are excellent examples of option engineering. We begin with a brief description of a European-style binary option.  
...
Expiration payoff of the binary is displayed by the step function shown in Figure 11.16. Now, make two additional assumptions. First, assume that the underlying $S_{t}$ is the price of a [[Futures Not Subject to Cash-And-Carry|futures]] contract traded at an exchange, and that the exchange has imposed a minimum tick rule such that, given $S_{t}.$ , the next instant’s price, $S_{t+\Delta}$ , can only equal  
$$
S_{t+\Delta}=S_{t}+i h
$$  
...
It is fairly easy to find a [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] for the binary option under these conditions. Suppose the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] buys one vanilla European call with strike $K$ , and, at the same time, sells one vanilla European call with strike $K+h$ on the $S_{t}$ . Figure 11.17 shows the time- $T$ payoff of this [[An Asset Allocation Primer|portfolio]]. The payoff is similar to the step function in Figure 11.16, except that the height is $h$ , and not 1. But this is easy to fix. Instead of buying and selling 1 unit of each call, the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] can buy and sell $\frac{1}{h}$ units. This implies the approximate contractual equation  
...
That is to say, at the limit the price of the binary is, in fact, the partial derivative of a vanilla call with respect to the [[Call and Put Payoffs at Expiry|strike price]] $K.$ If all [[Black-Scholes Model|Black Scholes]] assumptions hold, we can take this partial derivative analytically, and obtain9  
$$
...
$$  
$\sigma$ being the constant percentage volatility of $S_{t},$ and, $r$ being the constant risk-free [[The Foreign Exchange Market Annotations|spot rate]].  
...
This permits us to draw a graph of the binary price, $C^{\mathrm{{bin}}}(t)$ . Under the [[Black-Scholes Model|Black Scholes]] assumptions, it is clear that this price will be as indicated by the S-shaped curve in Figure 11.16.  
...
A range option is constructed using binary puts and calls with the same payoff. This option has a payoff depending on whether the $S_{t}$ remains within the range $[H^{\operatorname*{min}},H^{\operatorname*{max}}]$ or not. Thus, consider the [[An Asset Allocation Primer|portfolio]]  
$$
...
We can obtain a contractual equation for barrier options and the corresponding vanilla options. Consider two European-style barrier options with the same strike $K.$ . The underlying risk is $S_{t},$ and, for simplicity, suppose all [[Black-Scholes Model|Black Scholes]] assumptions are satisfied. The first option, a knock-out call, whose premium is denoted by $C^{O}(t)$ , has the standard payoff $i f$ the $S_{t}$ never touches, or falls below, the barrier $H$ . The premium of the second option, a knock-in call, is denoted by $C^{I}{(t)}$ . It entitles its holder to the standard payoff of a vanilla call with strike $K$ , only if $S_{t}$ does fall below the barrier $H$ . These payoffs are shown in Figure 11.22. In each case, $H$ is such that, when the option knocks in or out, this occurs in a region with zero intrinsic value. Now consider the following logic that will lead to a contractual equation.  
...
Putting these two payoff [[Exotic Interest Rate Options|ranges]] together, we obtain the contractual equation:  

![](6a44528910a80cba657e361c3f20d15408c5a8a5b021a6af41134c8453ad0acd.jpg)  

From here, we can obtain the [[Arbitrage Pricing of Derivatives|pricing]] formulas of the knock-in and knock-out barriers. In fact, determining the [[Arbitrage Pricing of Derivatives|pricing]] function of only one of these barriers is sufficient to determine the price of  

the other. In Chapter 9, we provided a [[Arbitrage Pricing of Derivatives|pricing]] formula for the knock-out barrier where the underlying satisfied the [[Black-Scholes Model|Black Scholes]] assumptions.12 The formula was given by  
$$
...
The $C(t)$ is the value of the vanilla call given by the standard [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]], and the $J(t)$ is the discount that needs to be applied because the option may disappear if $S_{t}$ falls below $H$ during $[t,T]$ .  
...
Thus, the expressions in Eqs. (11.42) (11.44) provide the necessary [[Arbitrage Pricing of Derivatives|pricing]] formulas for barrier options that knock-out and knock-in, when they are out-of-the-money under the [[Black-Scholes Model|Black Scholes]] assumptions. It is interesting to note that when $S_{t}$ touches the barrier,  
$$
S_{t}=H
$$  

the formula for $J(t)$ reduces to the standard [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]]:  
$$
...
The next example illustrates another way knock-ins can be used in [[Forwards and Futures Notes|currency]] markets. Figures 10.20 and 10.21 illustrate these cases.  

# EXAMPLE  

US dollar puts (yen calls) were well bid last week. Demand is coming from stop-loss trading on the back of [[Exotic Interest Rate Options|exotic]] knock-in structures. At the end of December, some players were seen selling 1-month dollar puts struck at JPY119 which knock-in at JPY109.30. As the yen moved toward that level early last week, those players rushed to buy cover.   
[[Basis Trade Explainer|Hedge funds]] were not the only customers looking for cover. Demand for short-term dollar puts was widely seen. “People are still short yen,” said a trader. “The [[Chapter 30 - Other Options|risk reversal]] is four points in favor of the dollar put, which is as high as I have ever seen it.”  

(Based on an article in [[Chapter 9 Arbitrage and Hedging With Options|Derivatives]] Week (now part of GlobalCapital))  

According to the example, as the dollar fell toward 110.6 yen, the [[Basis Trade Explainer|hedge funds]] who had sold knock-in options were suddenly facing the possibility that these options would come into existence, and that they would lose money.13 As a result, the funds started to cover their positions by buying out-of-the-money puts. This is a good illustration of new risks often associated with [[Exotic Interest Rate Options|exotic]] structures. The changes during infinitesimal intervals in mark-to-market values of barrier options can be discrete instead of “gradual.”  
...
Airlines face three basic costs: labor, capital, and fuel. Labor costs can be “fixed” for long periods using wage contracts. However, both [[Analysis of Fixed Income Securities|interest rate risk]] and fuel price risk are floating, and sudden spikes in these at any time can cause severe harm to an airline. The following example shows how airlines can hedge these two risks using a single barrier option.  

# EXAMPLE  

Although these are slow days in the [[Exotic Interest Rate Options|exotic]] option market, clients still want alternative ways to hedge cheaply, particularly if these hedges offer payouts linked to other exposures on their balance sheets. Barrier products are particularly popular. Corporates are trying to cheapen their projections by asking for knockout options.  

For example, an airline is typically exposed to both interest rate and fuel price risks. If [[Interest Rate Quotations|interest rates]] rose above a specified level, a conventional cap would pay out, but under a barrier structure it may not if the airline is enjoying lower fuel prices. Only if both rates and fuel prices are high is the option triggered. Consequently, the cost of this type of hedge is cheaper than separate options linked to individual exposures.  

(Thomson Reuters IFR, May 13, 1995)  

The use of such barriers may lower [[Key Rates O1s Durations and Hedging|hedging]] costs and may be quite convenient for businesses. The [[Exercises|exercises]] at the end of the chapter contain further examples of [[Exotic Interest Rate Options|exotic]] options. In the next section, we discuss some of the new risks and difficulties associated with these.  

# 11.4.3 NEW RISKS  

[[Exotic Interest Rate Options|Exotic]] options are often inexpensive and convenient, but they carry their own risks. [[Financial Mathematics Course|Risk management]] of [[Exotic Interest Rate Options|exotic]] options books is nontrivial because there are (i) discontinuities in the respective [[Chapter 26 - Pricing Options: Monte Carlo Simulation|Greeks]] due to the existence of thresholds and (ii) smile effects in the [[A Real-Life Option Pricing Exercise|implied volatility]].  

As the previous three chapters have shown, [[Financial Mathematics Course|risk management]] of option books normally uses various [[Chapter 26 - Pricing Options: Monte Carlo Simulation|Greeks]] or their modified counterparts. With threshold effects, some [[Chapter 26 - Pricing Options: Monte Carlo Simulation|Greeks]] may not exist at the threshold. This introduces discontinuities and complicates [[Financial Mathematics Course|risk management]]. We review some of these new issues next.  

1. Barrier options may exhibit jumps in some [[Chapter 26 - Pricing Options: Monte Carlo Simulation|Greeks]]. This is a new dimension in risk-managing option books. When spot is near the threshold, barrier option [[Chapter 26 - Pricing Options: Monte Carlo Simulation|Greeks]] may change discretely even for small movements in the underlying. These extreme changes in sensitivity factors make the respective delta, gamma, and vega more complicated tools to use in measuring and managing underlying risks.   
2. Barrier options are path dependent. For example, the threshold may be relevant at each time point until the option expires or until the barrier is hit. This makes [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]] [[Arbitrage Pricing of Derivatives|pricing]] and risk-managing techniques more delicate and more costly. Also, near the thresholds the spot may need further simulated trajectories and this may also be costly.   
3. Barrier option [[Key Rates O1s Durations and Hedging|hedging]] using vanilla and digital options may be more difficult and may be strongly influenced by smile effects.  

We will not discuss these [[Financial Mathematics Course|risk management]] and [[Key Rates O1s Durations and Hedging|hedging]] issues related to [[Exotic Interest Rate Options|exotic]] options in this book. However, smile effects will be dealt with in Chapter 16.  

# 11.5 QUOTING CONVENTIONS  

Quoting conventions in option markets may be very complicated. Given that market makers look at options as instruments of volatility, they often prefer quoting volatility directly, rather than a cash value for the option. These quotes can be very confusing at times. The best way to study them is to consider the case of risk reversals. [[Chapter 30 - Other Options|Risk reversal]] quotes illustrate the role played by volatility, and show explicitly the existence of a skewness in the volatility smile, an important empirical observation that will be dealt with separately in Chapter 16.  

One of the examples concerning risk reversals presented earlier contained the following statement:  

The 1-month [[Chapter 30 - Other Options|risk reversal]] jumped to 0.81 in favor of euro calls Wednesday from 0.2 2 weeks ago.  
...
The interpretation of such [[Bid Ask and Transaction Prices in a Specialist Market With Heterogeneously Informed Traders|bid ask]] spreads is not straightforward. The numbers in the quotes do not relate to dollar figures, but to volatilities. In simple terms, the number to the right of the slash is the volatility spread the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] is willing to receive for selling the [[Chapter 30 - Other Options|risk reversal]] position and the number to the left is the volatility spread he is willing to pay for the position.  

The numbers to the right are related to the sale by the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] of the 25-delta USD call and simultaneously the purchase of a 25-delta USD put, which, from a client’s point of view is the [[Chapter 30 - Other Options|risk reversal]] shown in Figure 11.24a. Note that, for the client, this situation is associated with “dollar strength.” If the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] sells this [[Chapter 30 - Other Options|risk reversal]], he will be short this position.  
...
The left side in this quote is “flat.” This means that the purchase of the 25-delta USD call, and a simultaneous sale of the 25-delta USD put, would be done at the same volatilities. A client who sells this to the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] pays or receives nothing extra and the deal has “zero cost.” In other words, the two sides would agree on a single volatility and then plug this same number into the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] to obtain the cost of the put and the cost of the call. The right-hand number in the quote shows a bias. It means that the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] is willing to sell the 25-delta USD call, and buy the 25-delta USD put, only if he can earn 0.3 volatility points net. This implies that the volatility number used in the sale of USD call will be 0.3 points higher than the volatility used for the 25-delta USD put. The [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] thinks that there is a “bias” in the market in favor of dollar strength; hence, the client who purchases this [[Chapter 30 - Other Options|risk reversal]] will incur a net cost.  
...
is more complicated to handle, although the interpretation of the 0.6 is similar to the first example. With this number, the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] is announcing that he or she needs to receive 0.6 volatility points net if a client wants to bet on the dollar strength.  

However, the left-hand element of the quote is not “flat” anymore but is a positive 0.3. This implies that the bias in the market, in favor of dollar strength is so large, and so many clients demand this [[Chapter 4 - Futures: Hedging and Speculation|long position]] that, now the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] is willing to pay net 0.3 volatility points when buying the 25-delta call and selling the 25-delta put.  

Thus, in [[Chapter 30 - Other Options|risk reversal]] quotes, the left-hand number is a volatility spread that the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] is willing to pay, and the second number is a volatility spread the [[Class Note 9 Bid and Ask Prices With Private Information|market maker]] would like to earn. In each case, to see how much the underlying options would cost, market participants have to agree on some base volatility and then, using it as a benchmark, bring in the volatility spreads.  
...
The existence of volatility smile has especially strong effects on [[Arbitrage Pricing of Derivatives|pricing]] and [[Key Rates O1s Durations and Hedging|hedging]] of [[Exotic Interest Rate Options|exotic]] options. If a volatility smile exists, the [[A Real-Life Option Pricing Exercise|implied volatility]] becomes a function of the [[Call and Put Payoffs at Expiry|strike price]] $K$ . For example, the expression that gave the binary option price in Eqs. (11.30) (11.31) has to be modified to  
$$
...
The resulting formula and the analogy to plain vanilla deltas will change. These types of modifications have to be applied to [[Key Rates O1s Durations and Hedging|hedging]] and synthetically creating barrier options as well. Major modification will also be needed for barrier options.  

# 11.6.2 EXISTENCE OF POSITION LIMITS  

At time $t$ before expiration, an option’s value depends on many variables other than the underlying $x_{t}$ . The volatility of $x_{t}$ and the [[Exercises|risk-free interest rate]] $\boldsymbol{r}_{t}$ are two random variables that affect all the positions discussed for $t<T.$ This is expressed in the [[Lecture Note 5- Black Scholes Formula|Black Scholes formula]] for the [[Chapter 17 - Option Strategies|call premium]] of $t<T$ :  
$$
...
Now, if the $r$ and $\sigma$ are stochastic, then during the $t\in[0,T)$ , the positions considered here will be subject to vega and rho risks as well. A player who is subject to limits on how much of these risks he or she can take, may have to unwind the position before $T.$ . This is especially true for positions that have [[Residual Risk of Options Gamma Vega and Volatil|vega risk]]. The existence of limits will change the setup of the problem since, until now, sensitivities with respect to the $r$ and $\sigma$ parameters, did not enter the decision to take and maintain the positions discussed.  

# 11.7 CONCLUSIONS  

In this chapter, we discussed how to synthetically create payoff diagrams for positions that take a view on the direction of markets and on the direction of volatility. These were static positions. We specifically concentrated on the payoff diagrams that were functions of a single risk factor and that were to be replicated by plain vanilla [[Futures Not Subject to Cash-And-Carry|futures]] and options positions. The second part of the chapter discussed the engineering of similar positions using simple exotics.  

# SUGGESTED READING  

There are several excellent books that deal with classic option strategies. Hull (2014) is a very good start. The reader may also consult Natenberg (2014) for option basics. See also the textbooks [[Credit Risk Models and the Valuation of Credit  Default Swap Contracts|Jarrow and Turnbull]] (1999) and Kolb (1999). Taleb (1996) is a good source on exotics from a market perspective. For a technical approach, consider the chapter on exotics in Musiela and Ruthkowski (2007). James (2003) is a good source on the technicalities of option trading and option [[Arbitrage Pricing of Derivatives|pricing]] formulas. It also provides a good discussion of exotics.  

# EXERCISES  

1. Construct a payoff and [[PSET 4 Solution-Financial Instruments|profit diagram]] for the purchase of a 105-strike call and sale of a 95- strike call. Verify that you obtain exactly the same [[PSET 4 Solution-Financial Instruments|profit diagram]] for the purchase of a 105- strike put and a sale of 95-strike put. Explain the difference in the initial cost of these positions. Assume the following parameters: $S(0)=100\$ ; $T=1$ ; $r=8\%$ ; $\sigma=30\%$  

2. Assume that a trader believes that during the vacation periods actual realized volatility is lower than the [[A Real-Life Option Pricing Exercise|implied volatility]]. To exploit this opportunity a trader takes a short position in a strangle to cover the cost of the long straddle position. If the actual volatility is $30\%$ less than the [[A Real-Life Option Pricing Exercise|implied volatility]], sketch out his volatility strategy. Assume the following parameters:  
$$
S(0)=1600;T=3\ \mathrm{month};r=5\%;\sigma=53\%
$$  

3. Consider a bear spread. An investor takes a short position in a [[Futures Not Subject to Cash-And-Carry|futures]] denoted by $x_{t}$ . But he or she thinks that $x_{t}$ will not fall below a level $x^{\mathrm{min}}$ .  
...
Last Monday, 6- and 12-month euro/Danish krone volatility was at $I.55\%/I.95\%$ , up from $0.6\%/0.9\%$ for the whole year until April 10, 2000, owing to growing bias among Danes against joining the EMU. On the week of April 10, volatility spiked as a couple of banks bought 6-month and 9-month, at-the-money vol. (Based on [[Chapter 9 Arbitrage and Hedging With Options|Derivatives]] Week (now part of GlobalCapital), April 24, 2000.)  

a. Plot the zero-cost risk-reversal strategy on a diagram. Show the DKK7.44 and DKK7.52 put and call explicitly.   
b. Note that the [[The Foreign Exchange Market Annotations|spot rate]] is at DKK7.45. But, this is not the midpoint between the two strikes. How can this strategy have zero cost then?   
...
5. The following questions deal with range binaries. These are another example of [[Exotic Interest Rate Options|exotic]] options. Read the following carefully and then answer the questions at the end.  
...
This way, the buyer will have a short Vega position on high [[A Real-Life Option Pricing Exercise|implied volatility]] levels. (Based on an article in [[Chapter 9 Arbitrage and Hedging With Options|Derivatives]] Week (now part of GlobalCapital).)  
...
Fluctuating U.S. dollar/yen volatility is prompting option traders managing their books to capture high volatilities through range binary structures while [[Key Rates O1s Durations and Hedging|hedging]] with butterfly trades. Popular trades include one-year double no touch options with barriers of JPY126 and JPY102. Should the [[Forwards and Futures Notes|currency]] pair stay within that range, traders could benefit from a USD1 million payout on premiums of $15-20\%$ .  

On the back of those trades, there was buying of butterfly structures to hedge short vol positions. Traders were seen buying out-of-the-money dollar put/yen calls struck at JPY102 and an out-of-the-money dollar call/yen put struck at JPY126. (Based on an article in [[Chapter 9 Arbitrage and Hedging With Options|Derivatives]] Week (now part of GlobalCapital).)  
...
7. The next question deals with a different type range option, called a range accrual option. Range accrual options can be used to take a view on volatility directly. When a trader is short volatility, the trader expects the actual volatility to be less than the [[A Real-Life Option Pricing Exercise|implied volatility]]. Yet, within the bounds of classical volatility analysis, if this view is expressed using a vanilla option, it may require dynamic [[Key Rates O1s Durations and Hedging|hedging]], otherwise expensive straddles must be bought. Small shops may not be able to allocate the necessary resources for such dynamic [[Key Rates O1s Durations and Hedging|hedging]] activities.  
...
The Ontario Teachers’ Pension Plan Board, with CAD72 billion (USD48.42 billion) under management, is looking at ramping up its use of equity [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] as it tests programs for range accrual options and options overwriting on equity portfolios.  

The equity [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] group is looking to step up its use now because it has recently been awarded additional staff as a result of notching up solid [[Assets|returns]], said a [[An Asset Allocation Primer|portfolio]] manager for Canadian equity [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]. . .. With a staff of four, two more than previously, the group has time to explore more sophisticated [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] strategies, a trader explained. Ontario Teachers’ is one of the biggest and most sophisticated end users in Canada and is seen as an industry leader among pension funds, according to market officials.  

A [[Chapter 4 - Futures: Hedging and Speculation|long position]] in a range accrual option on a single stock would entail setting a range for the value of the stock. For every day during the life of the option that the stock trades within the range, Ontario Teachers would receive a payout. It is, hence, similar to a short vol position, but the range accrual options do not require dynamic [[Key Rates O1s Durations and Hedging|hedging]], and losses are capped at the initial premium outlay. (Based on an article in [[Chapter 9 Arbitrage and Hedging With Options|Derivatives]] Week (now part of GlobalCapital).)  
...
c. In what sense does the range accrual option accomplish what dynamic [[Key Rates O1s Durations and Hedging|hedging]] strategies accomplish?   
d. How would you synthetically create a range accrual option for other “vanilla” exotics?  

# EXCEL EXERCISES  

8. Write a VBA program to show the construction of a bull and bear spread, first using calls only and then using puts only. Also plot both the call spread and put spread in both bull and bear phases. Explain your observation for the difference in the initial cost of [[Composite Payoffs at Expiry|call and put spreads]].  
