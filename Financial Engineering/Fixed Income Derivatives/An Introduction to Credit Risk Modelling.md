...
This property defines the random time  $\uptau$   as a stopping time. Equation 2.1 says that we can observe the event at the time it occurs. But it does not require that the event comes as a surprise. The value of the stopping time may be known a long time before time  $t=\tau$  . The maximum and minimum of a set of stopping times is again a stopping time, and the sum of the two stopping times. In order to represent stopping time with a stochastic process%20Process.md), we define its indicator process that jumps from zero to one at the stopping time:
$$
