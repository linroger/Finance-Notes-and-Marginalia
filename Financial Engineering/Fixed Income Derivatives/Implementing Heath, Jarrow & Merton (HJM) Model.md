...
The [[Implementing Heath, Jarrow & Merton (HJM) Model|Heath-Jarrow-Morton]] framework refers to a class of models that are derived by directly  modelling the dynamics of instantaneous forward-rates. The central insight of this framework  is to recognize that there is an explicit relationship between the [[Black-Scholes Model|drift and volatility]] parameters  of the forward-rate dynamics in a no-[[Arbitrage Pricing of Derivatives|arbitrage]] world.

The familiar short-rate models can be derived in the [[Implementing Heath, Jarrow & Merton (HJM) Model|HJM framework]] but in general, however,  HJM models are non-Markovian. As a result, it is not possible to use the PDE-based  computational approach for [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|pricing derivatives]]. Instead, discrete-time HJM models and MonteCarlo methods are often used in practice.

Instead of modelling the short-end of the curve and then building the forward curve for the  longer end of the curve, HJM employ a model for the whole [[Forward-Bucketo1s|forward rate curve]].

Let  $F(t;T)$  denote the [[Forward-Bucketo1s|forward rate curve]] for time t. Therefore the price of a zero coupon bond  at time  $t$    and maturing at time  $\mathrm{T}$   and which pay   $\S1$  , can be written as:
$$
...
We can use the [[Forward Points in Currency|forward rate]] to derive the [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] for the [[The Foreign Exchange Market Annotations|spot rate]].
$$
\boldsymbol{r}(t)=\boldsymbol{F}(t;t)
$$

Assume today is   $\mathbf{t}^{*}$   and the whole forward curve is known today,   $F(t;T)$  , the [[The Foreign Exchange Market Annotations|spot rate]] for  anytime t in the future can then be written as:
$$
\begin{array}{r}{r(t)=F(t;T)=F(t^{*},t)+\int_{t^{*}}^{t}d F(s;t).}\end{array}
$$

From (1.4) for the [[Forward Points in Currency|forward rate]] process for F, we have
$$
...
The [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] for r is found after differentiating (1.6) with respect to time  t.$$
...
The last term is highly path-dependent as both the random and the volatility component depend  on historical observations of these parameters. Because of the dependency of historical values,  this makes the above model a non Markov for derivative [[Arbitrage Pricing of Derivatives|pricing]], one need to be in [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|risk neutral]]  world. To move to the [[Binomial Option Pricing Model|risk-neutral world]], we start with a hedged [[An Asset Allocation Primer|portfolio]].

Suppose we can hedge a bond with another bond with maturity T. The hedged [[An Asset Allocation Primer|portfolio]] is then  :
$$
\Pi=Z(t;T_{1})-\Delta Z(t;T_{2})
$$

The change of the [[An Asset Allocation Primer|portfolio]] is given by
...
The [[An Asset Allocation Primer|portfolio]] is hedged and is therefore risk-free. Let the return equal to the [[Black Scholes Derivation|risk-free rate]] r(t)  and rearranging we find that
$$
...
The SDE for the risk-neutral [[Forward-Bucketo1s|forward rate curve]] is written as:
$$
...
This is a single [[Pricing Factors in a One-Period Framework|factor model]] which can be problematic if one is [[Arbitrage Pricing of Derivatives|pricing]] derivative with various  maturities. For example, spread option which pays the difference between rates at two different  maturities. The above HJM model can be generalized for as   $\mathbf{N}$  -dimensional stochastic  differential equation
$$
...
Principal Component Analysis (PCA) is a way to analyse the yield curve. It makes use of  historical time series data and implied co variances to find factors that explain the variance in  the [[The Vasicek Model|term structure]]. Each additional factor is found so that they cumulatively maximize the  contribution to the variance. Based on these factors, volatility functions are obtained to explain  the underlying volatility of an [[An Overview of the Vasicek Short Rate Model|interest rate model]]. For example, when implementing the HJM  model there are two main elements used in the model construction process-one is the definition  of volatility function for the model, the other involves a practical way of obtaining prices from  the model.
...
The largest eigenvalue at the maturity point of 1.5 has a value of 0.002647853 and a variance  of  $67\%$  . This means that   $67\%$   of total variations are explained by the first principal component  which has the highest eigenvalue.   $84.9\%$   of the total variation is explained by the sum of the  first two [[Principal Component Analysis|principal components]] and finally,   $93\%$   of the total variations are explained by the all  three [[Principal Component Analysis|principal components]]. The figure below shows the term-structure of ei gen vectors  corresponding to the largest eigenvalues ([[Principal Component Analysis|principal components]]).
...
To compute first the drift, we take the largest eigenvalues given in table 1.1. We take the square  root of these and multiply these result with an array of data corresponding to the three principal  components. The result is a volatility matrix corresponding to all three [[Principal Component Analysis|principal components]]  for all maturities (see the volatility matrix tab).
...
Where  $\mathrm{F(t,t)}$   is the current rate,  $\mathbf{M}(\mathbf{t})$   is the drift, vols are the vol at ili ties for the first, second and  third factors. The  $\mathbf{W}$   is the [[Continuous-Time Stochastic Processes|Brownian motion]] generated via the excel function Rand.  We then get the simulated entire HJM forward curve which we can use to price bonds or  [[Chapter 9 Arbitrage and Hedging With Options|derivatives]] for any maturity points.
...
From our previous calculation we do have the risk-neutral drift for all maturity points. We do  also have all the volatility factors for each maturity points. In addition, we do have also the  initial yield curve data. The next step is to integrate the [[Forward-Bucketo1s|forward rate curve]] for specific maturity  point and repeat these  $\mathbf{N}$   times to come up with final ZCB price using the formula
$$
\exp(-\sum r(t)d t)
$$
...
Since the underlying forward rates under HJM follow some [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]],  we need to use unique discounting factors for each simulation. For example, for  each simulation, the pay-off made at expiry T should be discounted by
$$
...
We then take the above payoff and scale these by a unique [[Discount Factors|discount factor]] for each simulation.  In the attached spreadsheet, you will be able to see the discounted cap and floor prices for each  simulation
...
Paul Wilmott,  P.Wilmott on [[Quantitative Trading Strategies Lecture Notes|Quantitative Finance]]”,  $2^{n d}$   Edition, Volume 3, Wiley.     CQF  Lecture 2010/2011 – Heath, Jarrow and [[Credit Markets Session 5|Merton Model]]     CQF Lecture 2010/ 2011 - Advanced Volatility Modelling in [[Financial Mathematics Course|Complete Markets]]     P. Wilmott, A. Oztukel, “