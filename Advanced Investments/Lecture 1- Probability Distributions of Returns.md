...
We often summarize the probability distribution in terms of moments. The first (central) moment of $R_{t}$ is the [[Lecture 1- Probability Distributions of Returns|expected return]],  $\mathbb{E}\left[R_{t}\right]$; the second (central) moment of the [[An Asset Allocation Primer|portfolio]] return is the variance,  $\operatorname{var}\left(R_{t}\right)$; the third (central) moment is skewness,  etc. Some simple probability distributions can be summarized by a few moments. For example,  if we know the mean and variance of a normally distributed random variable,  we have completely described the distribution. If the distribution of single period (e.g.,  day,  month,  or year) [[Assets|returns]] $R_{t}$ is a normal distribution with mean $\mu$ and variance $\sigma^{2}$,  we write
$$
\begin{equation*}
...
Figure 1.1 plots a histogram of monthly [[Assets|returns]] and overlaid on it a curve that shows a normal [[Lecture 1- [[Exercises|Probability Distributions]] of [[Assets|Returns]]|probability distribution]] function with mean and variance set equal to the empirical historical mean and variance of the return series. As we can see,  broadly speaking,  the fit is not bad. The bell curve shape of the normal distribution also appears in the histogram of the observed historical [[Assets|returns]]. Most monthly [[Assets|returns]] are within a few percent of zero,  monthly [[Assets|returns]] outside the ${} \pm 15\\%$ range are very rare.
...
Figure 1.1: [[Lecture 1- Probability Distributions of Returns|Empirical distribution]] of monthly U.S. [[Hedge Fund Strategies|stock market]] [[Assets|returns]] 1927-2022 with normal distribution fit

That said,  if we look more closely,  this figure already shows some empirical features of real-world stock [[Assets|returns]] that a normal distribution cannot capture properly and that we will have to investigate a bit more in the coming weeks. While monthly [[Assets|returns]] outside the ${} \pm 15 \\%$ range are very rare,  they do happen occasionally. Yet according to the normal distribution,  the probability of this happening should be extremely close to zero.

Another question that we will defer for a couple of weeks concerns changes in the probability distribution over time. For example,  is the distribution of future stock [[Assets|returns]] the same in an economic boom period and in times when we are in a deep recession? Probably not. This is then a question of how the [[Lecture 3- Dynamics of Returns|conditional distribution of returns]] looks like. For example, following a big market crash (i.e., conditional on very low recent [[Assets|returns]]) does the probability distribution of [[Assets|returns]] look the same as after a boom period in the [[Hedge Fund Strategies|stock market]] (i.e., conditional on very high recent [[Assets|returns]])?

But one thing at a a time. For now,  we are going to stick to the assumption that [[Assets|returns]] are independently and identically distributed (IID) over time. This means that the [[Lecture 3- Dynamics of Returns|conditional distribution of returns]] is not changing. Under this assumption, no matter what happened in the past, looking forward, the probability distribution of future [[Assets|returns]] is always the same.

The IID assumption is a useful approximation to get us started, but this is not to say that this is entirely the right perspective. We will later spend a considerable amount of time precisely on the empirical evidence that [[Assets|returns]] are not IID. But for now, to discuss basic properties of [[Lecture 7-Risk and Return of Bonds|risk and return]], it makes sense conceptually to leave the issue of dynamically changing probability distributions for the next step of our analysis.

Since we are dealing with IID [[Assets|returns]],  we reduce notational clutter by dropping the time-subscripts and just write $\mathbb{E}[R],        \operatorname{var}(R)$,  and so on.

## 1.2 What is Risk?

The purpose of using probability distributions in [[Lecture 2- Asset Allocation with Multiple Risky Assets|asset allocation]] analyses is to assess [[An Asset Allocation Primer|portfolio]] choices affect the [[Lecture 7-Risk and Return of Bonds|risk and return]] of the portfolio. If we pick different assets, or different combinations of assets, this changes the [[Lecture 7-Risk and Return of Bonds|risk and return]] properties of our [[An Asset Allocation Primer|portfolio]] and probability distributions help us analyze this.

What we mean by return is clear. And when we talk about return in a forward-looking way, as the prospective future return on an [[An Asset Allocation Primer|investment]], we mean the expected return, $\mathbb{E}[R]$.

But what exactly is risk? How can we measure it? Broadly speaking, it has to do with the uncertainty that we face about future [[Assets|returns]]. If the assets we invest in are not riskless, then realized [[Assets|returns]] are going to deviate from the expected return and so $R-\mathbb{E}[R]$ will be nonzero (positive or negative).

Suppose we evaluate a risky asset [[An Asset Allocation Primer|investment]] over a horizon of one period. A [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] earns a certain return $R_{f}$ per period. We have several risky assets to choose from which all offer the same [[Lecture 1- Probability Distributions of Returns|expected return]] $\mathbb{E}[R]>R_{f}$,  but different return variances. For now,  we'll think about how to choose one of these assets. (We'll think about combining assets into a [[An Asset Allocation Primer|portfolio]] later.) Given that they have the same [[Lecture 1- Probability Distributions of Returns|expected return]],  we would like to pick the one with the lowest risk. But what is the right measure of risk?
...
i.e.,  the deviations $R-\mathbb{E}[R]$ are squared and then averaged (by taking the expectation). The square root of variance,  $\sigma=\sqrt{\operatorname{var}(R)}$,  is the standard deviation. We often refer to the standard deviation of [[Assets|returns]] as volatility.

Given a time-series of [[Assets|returns]] from $t=1$ to $t=T$,  we can estimate variance as
$$
\begin{equation*}
...
Figure 1.2: Return distributions with different variances and same [[Lecture 1- Probability Distributions of Returns|expected return]]

Figure 1.2 shows three [[Lecture 1- [[Exercises|Probability Distributions]] of [[Assets|Returns]]|probability distribution]]s. All with the same expected value ${} \mathbb{E}[R]=0.5 \\%$ (which is about a typical average monthly return on a broad [[Hedge Fund Strategies|stock market]] index),  and three different variances. Using variance to measure risk,  asset A is the preferred asset,  as it has the lowest variance.

There are cases where we know that variance is a perfect measure of risk. This is when the distribution of [[Assets|returns]] is fully described by mean and variance. This is the case for the normal distribution.

For the [[Lecture 1- [[Exercises|Probability Distributions]] of [[Assets|Returns]]|probability distribution]]s shown in this figure,  most people would probably agree with this choice. If all three assets offers the same return on average,  it makes sense to pick asset A whose realized [[Assets|returns]] tend to be much more closely concentrated around the [[Lecture 1- Probability Distributions of Returns|expected return]] and hence less uncertain. (These distributions may look like normal distributions,  but they aren't. They are log-normal distributions and have a

slightly skewed to the right. We'll look in more detail at log-normal distributions later in this course).

To many people,  including some [[An Asset Allocation Primer|investment]] practitioners,  using variance to measure risk seems odd,  as variance considers upside and downside deviations symmetrically. Recall the definition of variance above: positive deviations of $R$ from $\mathbb{E}[R]$ contribute to variance in the same way as negative deviations. But shouldn't investors be concerned about the downside,  not the upside? Isn't upside a good thing so we shouldn't include positive deviations in a [[Leverage as a Measure of Risk|risk measure]]?
...
The first fallacy is to think that when we compare assets in terms of variance,  we are completely ignoring the fact that having an upside of potentially high [[Assets|returns]] is beneficial. We are already accounting for the upside in the [[Lecture 1- Probability Distributions of Returns|expected return]] $\mathbb{E}[R]$. For example,  imagine taking the [[Lecture 1- [[Exercises|Probability Distributions]] of [[Assets|Returns]]|probability distribution]] of asset (B) in Figure 1.2 and pulling the right tail of the distribution further to the right,  so that high [[Assets|returns]] become more likely,  while leaving the left half of the distribution unchanged. This would increase $\operatorname{var}(R)$,  which is the property of variance as a [[Leverage as a Measure of Risk|risk measure]] that some people find objectionable,  but note this that this would also,  at the same time,  raise $\mathbb{E}[R]$. Since we are considering $\mathbb{E}[R]$ and $\operatorname{var}(R)$ jointly in assessing [[Lecture 7-Risk and Return of Bonds|risk and return]],  we are not ignoring the benefits of greater upside.

A more subtle issue is whether $\mathbb{E}[R]$ and $\operatorname{var}(R)$ jointly properly capture the information that an investor needs to make a decision. Is the impact of right-tail observations on $\operatorname{var}(R)$ perhaps misleading and would it perhaps be better to consider $\mathbb{E}[R]$ in conjunction with a [[Leverage as a Measure of Risk|risk measure]] focused on downside risk?

Let's have a look at a common downside [[Leverage as a Measure of Risk|risk measure]] that is sometimes proposed as a better alternative to variance: the semi-variance
$$
\begin{equation*}
...
Figure 1.3: Return distributions with same variances and same [[Lecture 1- Probability Distributions of Returns|expected return]]
...
Note that both assets have the same [[Lecture 1- Probability Distributions of Returns|expected return]] $\mathbb{E}\left[R_{A}\right]=E\left[R_{B}\right]=25 \%$. But the risks are different. Asset $A$ 's payoffs are symmetric around the expected payoff of $1$. In contrast,        $B$ 's payoff has positive skewness. There is a chance of a positive payoff of $2.00$ that is far above the expected payoff.
...
Let's now consider a simple [[Lecture 5- Dynamic [[An Asset Allocation Primer|Portfolio]] Choice|[[An Asset Allocation Primer|portfolio]] choice]] problem using [[Lecture 2- Asset Allocation with Multiple Risky Assets|CRRA preferences]]. Suppose the investor wants to choose among two assets: A [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] that offers return $R_{f}$ and a risky asset that offers IID [[Assets|returns]] $R$. If the investor starts with $W_{0}$ at the beginning of a period and invests a proportion $\omega$ into the risky asset,  then the return on their wealth [[An Asset Allocation Primer|portfolio]] is$$
...
If the wealth [[An Asset Allocation Primer|portfolio]] earns the overall return $R_{w}$,  then the investor's wealth at the end of the period is $W=W_{0}\left(1+R_{w}\right)$. Therefore,  the investors' objective is to$$
...
$$
Let's consider a simple example to understand the meaning of this first-order condition and also solve for the optimal [[An Asset Allocation Primer|portfolio]] weight. Suppose $R_{f}=0$ and $W_{0}=1$ so that $W=1+\omega R$. Further,  assume that the risky asset return can only take two values,
$R_{L}=-0.10$ or $R_{H}=0.20$,  with equal probability. In this simple case,  we'll be able to figure out the optimal [[An Asset Allocation Primer|portfolio]] weight of risky assets.

But first let's use this simple special case to understand the meaning of the first-order condition. Consider what happens to wealth if we take one dollar from [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] and put it into risky asset: $W_{L}$ changes by -0.10 ,  and $W_{H}$ changes by 0.20 . So the difference in wealth between the good state,  when the risky asset return is high,  and the bad state,  when the risky asset return is low,  becomes bigger.
...
The reduction of utility in the bad state is the marginal cost,  in utility terms,  of increasing the risky asset exposure of the [[An Asset Allocation Primer|portfolio]]. The increase in the utility in the good state is the marginal benefit. As in any economic optimization problem,  the optimum choice is where marginal costs and benefits are equal,  because this means the utility from the [[An Asset Allocation Primer|portfolio]] cannot be improved in any way. (In contrast,  if,  we are at a point where,  say,  the marginal cost exceed the marginal benefit,  we can improve the [[An Asset Allocation Primer|portfolio]] by reducing the risky asset share because the reduction of cost in this case outweighs the reduction of benefit.)
...
We would now like to solve the first-order condition in 1.19 for $\omega$ with [[Lecture 2- Asset Allocation with Multiple Risky Assets|CRRA preferences]] in the general case of more realistic [[Lecture 1- [[Exercises|Probability Distributions]] of [[Assets|Returns]]|probability distribution]]s of [[Assets|returns]] that allow [[Assets|returns]] to take infinitely many values,  not just two. But this turns out to be an intractable problem. There is no mathematical solution that delivers a simple formula.

For this reason,  we will now use an approximation approach. As it turns out,  this approximation will lead us to a solution that trades off mean and variance of [[Assets|returns]],  and hence provides foundation for the [[Exercises|mean-variance]] approach to [[Lecture 5- Dynamic [[An Asset Allocation Primer|Portfolio]] Choice|[[An Asset Allocation Primer|portfolio]] choice]] that you encountered in your introductory investments course and that is pervasive in industry practice. We first scale the first-order condition 1.19 to$$
...
Figure 1.11: Actual marginal utility and [[Convexity|first-order approximation]] (power utility with $\gamma=5,        W_{0}=1 ; \omega=0.5$ ). Risky asset return on $x$-axis shown in excess of $\mathbb{E}[R]$
...
$$
This is the [[Exercises|mean-variance]] optimal [[An Asset Allocation Primer|portfolio]] solution for the single risky asset case.

In this solution,  variance of [[Assets|returns]] measures risk. This reinforces the conclusion from our earlier discussion of risk measures that using a symmetric measure of risk that equally weights positive and negative (squared) deviations from the [[Lecture 1- Probability Distributions of Returns|expected return]] is not in contradiction to being averse to risk.
...
of marginal utility may substantially overstate the risks. This may be relevant especially for extremely risky assets,  or at long horizons. (In contrast,  if the distribution of [[Assets|returns]] is symmetric,  such as with a normal distribution,  then the errors on the upside and downside would offset to zero on average.)
...
The longer the horizon over which we measure [[Assets|returns]],  the bigger the deviations of ending wealth from the wealth level around which we are approximating marginal utility. Moreover,  at long horizons [[Assets|returns]] basically have to be skewed because they can't be less than $-100 \%$,  while the upside is unlimited. So approximation errors are likely to grow with the return measurement horizon. But we can avoid large approximating errors due to long horizons by using means and variances measured over short horizons such as monthly rather than multi-year periods (and,  correspondingly,  rebalancing the [[An Asset Allocation Primer|portfolio]] more frequently back to the optimal [[An Asset Allocation Primer|portfolio]] share).

## 1.5 Calibrating Risk Aversion

Risk aversion determines which distribution of [[An Asset Allocation Primer|portfolio]] [[Assets|returns]] we want. But what is a plausible value for $\gamma$ ? How much does a typical investor really care about risk?

One way of getting some rough idea about likely plausible magnitudes for risk aversion is to look at the [[An Asset Allocation Primer|portfolio]] of the "average" investor (with "average" we really mean wealth-weighted average,  but to reduce clutter,  we'll just say "average"). How does the average investor's [[An Asset Allocation Primer|portfolio]] look like? Let's do a rough back-of-the-envelope calculation where we treat all bonds,  government and corporate,  as [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] and let's treat stocks as the risky asset class. We ignore other assets. Then we compute the average investor's equity share as the total market value of equity securities divided by the sum of outstanding equity and [[Analysis of Fixed Income Securities|debt securities]] held by U.S. domestic investors. The Financial Accounts of the United States tells us that this share is $57 \%$ at the end of 2022. Figure 1.12 shows the post-WWII history of this equity share.

So to explain the average investor's [[Lecture 2- [[Lecture 2- Asset Allocation with Multiple Risky Assets|Asset Allocation]] with [[Lecture 2- Asset Allocation with Multiple Risky Assets|Multiple Risky Assets]]|[[Lecture 2- Asset Allocation with Multiple Risky Assets|asset allocation]]]] at the end of 2022 with our formula (1.26),  we need it to yield$$
...
Figure 1.12: Equity share as a fraction of total [[Class Note 1- Borrower-Lender conflicts and implied agency problems|debt and equity]] securities held by U.S. domestic investors

This tells us that [[CCAPM with Alternative Preferences|relative risk aversion]] in the range of around 2 to 3 is a plausible value for a typical investor.

## 1.6 Market Equilibrium

When we try to figure out the best choices for our own [[An Asset Allocation Primer|portfolio]],  it's also useful to keep an eye on what this implies for what the rest of the market,  i.e.,  other investors must be doing. In other words,  it's useful to consider how our own choices fit into the bigger scheme of things in terms of market equilibrium.

Consider the data we just discussed that the average investor in the U.S. seems to have an equity share of roughly $60 \%$. Now suppose according to our beliefs about expected [[Assets|returns]] and risk,  and considering our own risk aversion,  we come to the conclusion that we want to hold $80 \%$ percent in equities. But for us to hold more in equities than the average investor,  someone else must be willing to hold less than the average investor.

Along the same lines,  if we decide to increase our equity share (for example,  because

our beliefs about future [[Lecture 7-Risk and Return of Bonds|risk and return]] have changed),  then for us to be able to do so,  someone else must be willing to decrease it. In the short-run at least,  the supply of bonds and stocks is fixed. So for every buyer,  there must be a seller. Otherwise the market would not clear. Supply would not equal demand.

This equilibrium thinking leads to a number of important insights:

Price can change without (much) buying or selling: For example,  what happens if the average investor wants to get more exposure to equities and hold a [[An Asset Allocation Primer|portfolio]] with a higher equity share? Again,  in the short-run at least,  the supply of bonds and stocks is fixed. So investors' desire for a higher equity share cannot be satisfied by giving them a greater number of shares. Instead,  the price must adjust. If investors want to hold more shares of stocks than the total supply available to the market,  stock prices will go up (and possibly bond prices go down). This has two effects. First,  a rise in the price with [[FORWARD RATES AND TERM STRUCTURE|expectations]] of future prices and [[Week 2 Fundamentals Of Forecasting|fundamentals]] unchanged lowers the [[Lecture 1- Probability Distributions of Returns|expected return]] of equities. This makes equities less attractive and lowers investors' desired equity share. At the same time,  the rise in the price raises the aggregate equity share at market prices. Both of these effects contribute to getting investors' desired equity share to equal the aggregate equity share at market prices. If the price rise is big enough,  desired [[An Asset Allocation Primer|portfolio]] weights and market [[An Asset Allocation Primer|portfolio]] become equal,  supply equals demand and the market clears and is in equilibrium.

If we aim to hold a risky asset share that differs from the market's aggregate equity share at current prices,  we are implicitly expressing the view that we are different from the average investor,  either in terms of beliefs about [[Lecture 7-Risk and Return of Bonds|risk and return]] $\left(\mathbb{E}\left[R-R_{f}\right]\right.$ and $\left.\operatorname{var}(R)\right)$ or risk aversion $(\gamma)$. For example,  if the market's aggregate equity share is $60 \%$,  but we hold an equity share of $80 \%$,  we are implicitly taking a stand that we are less risk averse than the typical investor or more optimistic about equity [[Lecture 7-Risk and Return of Bonds|risk and return]] than the average investor.

Similarly,  if we change our share differently from how the market's aggregate equity share changes,  we are implicitly expressing the view that our beliefs about [[Lecture 7-Risk and Return of Bonds|risk and return]],  or our risk aversion,  have changed differently from the average investor's.

The idea that cash flows "into the [[Hedge Fund Strategies|stock market]]" from the "sidelines" or "out of the [[Hedge Fund Strategies|stock market]]" to the "sidelines" doesn't make any sense (except perhaps somewhat in the long-run when the supply of securities may respond to demand,  e.g.,  via initial and seasoned equity offerings by companies). For every buyer who goes (more) into the [[Hedge Fund Strategies|stock market]],  there is a seller that (partly) exits the market. Flows of cash can be going to or from some investors,  say equity mutual funds,  but
...
The optimal [[An Asset Allocation Primer|portfolio]] formula we developed needs inputs: Expected [[Assets|returns]] and variances. In practice,  we don't know these numbers and we need to estimate them from data. A lot of what we do in this course is looking at data that can be informative about these unknown moments.

Other problems,  such as whether the [[Exercises|mean-variance]] approximation of the [[An Asset Allocation Primer|portfolio]] problem is sufficiently accurate,  often pale relative to the problem of how to get reliable inputs to the [[Lecture 5- Dynamic Portfolio Choice|optimal portfolio formulas]].

We can estimate expected [[Assets|returns]] $\mu=\mathbb{E}\left[R_{t}\right]$ with the sample mean$$
...
$$
To see what this does,  write the return on the asset as the [[Lecture 1- Probability Distributions of Returns|expected return]] $\mu=\mathbb{E}\left[R_{t}\right]$ plus random noise:$$
...
$$
Here $\mu$ is the "signal" that we would like to tease out from [[Assets|returns]]; $\varepsilon_{t}$ is noise that obscures this signal. This noise is the reason why we can't observe expected [[Assets|returns]] directly in the sample. What we can observe is a mix of the true [[Lecture 1- Probability Distributions of Returns|expected return]] and noise. If the variance of $\varepsilon_{t}$ is big,  this will make $\hat{\mu}$ a more imprecise estimate of $\mu$. Specifically,  in a given sample,  our estimate$$
...
$$
has the true [[Lecture 1- Probability Distributions of Returns|expected return]] $\mu$ obscured by the noise $\frac{1}{T} \sum_{t=1}^{T} \varepsilon_{t}$. Since we don't know the true $\mu$ and the $\varepsilon_{t}$,  we cannot tell the two apart. The only thing we can do is to try to quantify how big the noise distortion in our estimate $\hat{\mu}$ is likely to be. Statistics tells us that if the $\varepsilon_{t}$ each periods are IID (which means,  among other things,  that knowing
$R_{t-1}$ or earlier [[Assets|returns]] we can't predict $R_{t}$ ) and they have variance $\sigma^{2}$,  then the standard error of the mean that quantifies our uncertainty about $\mu$ can be calculated as ${ }^{6}$$$
...
$$
The assumption of IID noise is actually not a bad approximation. As we will discuss in the next lecture,  [[Assets|returns]] in various asset classes generally have little autocorrelation,  i.e.,  [[Assets|returns]] in earlier periods don't help to predict $R_{t}$. This is one requirement for the independence in "IID." Also note that this assumption of serial independence is about lack of dependence across different time periods. Within time periods,  [[Assets|returns]] in different asset classes can (and typically will be) correlated with each other. We are not ruling out such correlation between asset class [[Assets|returns]].
...
$$
and we have no other knowledge about the location of $\mu$ than what we saw in the data set of [[Assets|returns]] from $t=1$ to $T$,  then$$
...
Figure 1.13: Uncertainty about [[Lecture 1- Probability Distributions of Returns|expected return]] with $\hat{\mu}=0.05$ and s.e. $(\hat{\mu})=0.02$

Figure 1.13 illustrates the uncertainty we have about the [[Lecture 1- Probability Distributions of Returns|expected return]] after seeing estimates $\hat{\mu}=0.05$ and s.e. $(\hat{\mu})=0.02$.
...
Of course,  once we use such a long data set to estimate expected [[Assets|returns]],  we might also wonder whether data from so long ago is still informative about the [[Lecture 1- Probability Distributions of Returns|expected return]] that can be earned on the [[Hedge Fund Strategies|stock market]] going forward. Perhaps the true $\mu$ has changed over time. After all,  the economy,  they types of firms on the [[Hedge Fund Strategies|stock market]],  who participates in [[Hedge Fund Strategies|stock market]] investing,  etc. is all quite different from a century ago. This is a fundamental tradeoff we face: If we use more recent data for estimation,  we can be more confident that the estimates are actually still relevant if $\mu$ slowly changes over time. If we use data that extends further back in time,  we get more statistically more precise estimates under the assumption that $\mu$ is constant over time.

The imprecision of [[Lecture 1- Probability Distributions of Returns|expected return]] estimates is a major issue that makes [[Lecture 5- Dynamic [[An Asset Allocation Primer|Portfolio]] Choice|[[An Asset Allocation Primer|portfolio]] choice]] difficult in practice.

An important practical issue is also how to compare [[Assets|returns]] (and risk measures estimated from these [[Assets|returns]]) at different frequencies. With IID [[Assets|returns]],  the following relationships hold approximately

Mean of annual [[Assets|returns]] $\approx 12 \times$ mean of monthly [[Assets|returns]]

Variance of annual [[Assets|returns]] $\approx 12 \times$ variance of monthly [[Assets|returns]]

Standard deviation of annual [[Assets|returns]] $\approx \sqrt{12} \times$ standard deviation of monthly [[Assets|returns]]
...
When [[Assets|returns]] are not IID,  things are more complicated. But we'll get to this later.

Table 1.1: Annualized [[Lecture 7-Risk and Return of Bonds|risk and return]] estimates for various asset classes 1980-2021
...
Table 1.1 shows annualized estimates of mean [[Assets|returns]] in excess of the average T-bill return and risk measures for various asset classes over the period 1980-2022. Stocks:

U.S. broad [[Hedge Fund Strategies|stock market]] index; I-Stocks: International stocks; Value: U.S. value stocks; Small: U.S. Small-cap stocks; T-Bonds: U.S. Treasury bonds; I-Bonds: International bonds; C-Bonds: U.S. [[Class Notes 2 – Corporate Bond Contracts|Corporate Bonds]]; REITS: U.S. Real estate [[An Asset Allocation Primer|investment]] trusts.

The underlying data is monthly [[Assets|returns]] and I annualized the estimates using the approach we just discussed. The roughly four decades covered by the sample have been ones in which risky assets generally performed well. For this reason we see that most asset classes have estimated expected excess [[Assets|returns]],  $\hat{\mu}-R_{f}$,  far more than two standard errors from zero. But still,  there is substantial remaining uncertainty about the true expected [[Assets|returns]] of these asset classes.

The table also shows the a popular measure of the ratio of reward to risk: the Sharpe ratio,  named after the economics Nobel laureate and inventor of the [[2. Forwards, Swaps, Futures, and Options|Capital Asset Pricing model]] Bill Sharpe:$$
...
$$
If there are two assets with different $S R$,  and the investor can pick only one of them,  not a combination of the two,  then any risk averse investor (for whom variance is the correct [[Leverage as a Measure of Risk|risk measure]]) would go for the one with the higher $S R$.

The reason is that if one,  say asset $A$,  has a higher $S R$ than the other,  asset $B$,  then we could always use [[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]] or a combination with an [[An Asset Allocation Primer|investment]] in the [[2. Forwards, Swaps, Futures, and Options|risk-free asset]] to make our [[An Asset Allocation Primer|investment]] in $A$ deliver the same variance,  but a higher [[Theoretical Factors|expected excess return]] than an [[An Asset Allocation Primer|investment]] in $B$. To be concrete,  suppose asset $A$ has $E\left[R_{A}\right]-R_{f}=0.06$ and $\sigma_{A}=0.2$ and hence $S R_{A}=0.3$ while asset $B$ has $E\left[R_{B}\right]-R_{f}=0.02$ and $\sigma_{A}=0.1$ and hence $S R_{A}=0.2$. If we put $\omega=0.5$ into asset $A$ and the rest in the [[2. Forwards, Swaps, Futures, and Options|risk-free asset]],  then this [[An Asset Allocation Primer|portfolio]] will have [[Theoretical Factors|expected excess return]] of $0.5 \times 0.06=0.03$ and return standard deviation $0.5 \times 0.2=0.1$. Hence,  it will have the same standard deviation as asset $B$,  but a higher [[Theoretical Factors|expected excess return]],  which makes it clearly better: same risk,  but higher [[Lecture 1- Probability Distributions of Returns|expected return]].
