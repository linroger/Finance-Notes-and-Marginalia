...
Here is another way to think about the performance deterioration of a leveraged strategy performance due to [mean reversion](The Ornstein-Uhlenbeck (OU) Process.md). Recall from our discussion in the last lecture that rebalancing to a fixed risky asset weight was a contrarian strategy. If you start with $\omega=0.5$ and the value of stocks doubles,  the [portfolio](An Asset Allocation Primer.md) weight of stocks goes to $\omega=0.66$ and so to rebalance back to $\omega=0.5$ you need to sell stocks. However,  that discussion back then focused on a strategy without [leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) and hence $\omega<1$. When a strategy is leveraged,  and hence $\omega>1$,  it is actually the opposite: Rebalancing is now a positive feedback strategy,  not a contrarian one. For example,  if you start with $\omega=2$ and the value of stocks doubles,  then the value of your equity position triples,  hence [leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) falls to $\omega=4 / 3 \approx 1.33$,  so to rebalance back to $\omega=2$ you need to buy stocks! And following a positive-feedback strategy when [returns](Assets.md) are mean-reverting is a money loser.

The few paths in Figure 6.5 on which the 3 x levered strategy earned huge positive [returns](Assets.md) are also ones on which the index went up many times in a row,  i.e.,  there was a lot of return continuation on these paths. Hence,  a positive feedback strategy did well on these paths. In contrast,  on paths on which the market largely moved sideways overall,  we have that price movements up are largely offset by price movements down in other periods which hurts the performance of a positive-feedback strategy.

Overall,  this analysis shows how a simple leveraged strategy with regular rebalancing to a constant [[[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) Ratio|[leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) ratio]] can result in surprisingly complex and not-easy-tounderstand [risk and return](Lecture 7-Risk and Return of Bonds.md) properties. It is not the long-term wealth destroyer that it is sometimes labeled as,  but it does generate a highly skewed distribution of long-term [returns](Assets.md) that may indeed be unattractive to some investors unless they have sufficiently low risk aversion. Moreover,  a strategy of rebalancing to a constant high [[[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) Ratio|[leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) ratio]] can be a destroyer of wealth when the asset we are investing in has strongly mean-reverting [returns](Assets.md).

### 6.3 Option-like payoffs from dynamic rebalancing

We can get further insights into the risk-return profile of leveraged strategies by examining in more detail how long-term [returns](Assets.md) of leveraged strategies relate to the long-term [returns](Assets.md) on the underlying index. Figure 6.6 shows scatter plots of several leveraged strategies' simulated 3 -year [returns](Assets.md) against the 3-year market index [returns](Assets.md).

The blue circles show the 3 -year [returns](Assets.md) of a strategy where we initially pick a [[[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) Ratio|[leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md) ratio]] of 3.0 and then refrain from rebalancing during the 3 -year period (with 750 trading days). As one would expect,  the long-term return of this strategy,  $R_{t: t+750}$,  is linear in the long-term return on the index,  $R_{m,       t: t+750}$. More precisely,
$$
...
The [long position](Chapter 4 - Futures: Hedging and Speculation.md) in ATM call options generates the [[PSET II Fixed Income Asset Pricing 1|convexity]] in the relationship between the combined position's [returns](Assets.md) and the index [returns](Assets.md). The short position in the OTM call options cancels some of the [[PSET II Fixed Income Asset Pricing 1|convexity]]. Without this short position in OTM calls,  the [[PSET II Fixed Income Asset Pricing 1|convexity]] would be too strong in the region of high index [returns](Assets.md) on the right-hand side of the plot.
...
U.S. Treasury bonds seem to fit this requirement. They did well in all three crisis episodes. In the next lecture,  we will look more closely at the relationship between stock and bond markets to understand why bond prices behave this way,  and whether this good performance in crises is actually a reliable relationship over longer periods than the past 25 years considered here. For now,  I'll just note that as a [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] hedge we might want an asset that has a more skewed payoff,  with payoffs that are very high in crisis,  but otherwise not necessarily strongly negatively correlated with stock markets in "normal times." Treasury bonds don't fit this requirement.

Buying credit protection against high yield (a.k.a. "junk") credit (HY CDS) is more like this. High-yield credit spreads tend to spike strongly during crisis times. Exchange rates are a mixed bag. The Swiss Franc (CHF) is perhaps the most reliable "safe haven" [[Forwards and Futures Notes|currency]] that tends to appreciate in times of crises when investors seek safety,  but the magnitudes of appreciation seem rather small,  so not a great [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] hedge.

Among [[Futures Not Subject to Cash-And-Carry|commodities]],  gold is a popular among some investors as a [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] hedge. For gold,  too,  the price rise during crisis episodes seems rather small. Not clear that it's a great [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] hedge.

Finally,  the last block of rows includes one product that we will shortly examine more closely: VIX [[Futures Not Subject to Cash-And-Carry|futures]]. The [returns](Assets.md) of a VIX [[Futures Not Subject to Cash-And-Carry|futures]] [investment](An Asset Allocation Primer.md) look consistent with [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] insurance properties. VIX [[Futures Not Subject to Cash-And-Carry|futures]] produced extremely high [returns](Assets.md) during the three crisis episodes (insurance pays out) and strongly negative average [returns](Assets.md) overall (insurance premium to be paid). Adding VIX [[Futures Not Subject to Cash-And-Carry|futures]] to a [portfolio](An Asset Allocation Primer.md) can have somewhat similar effects as adding put protection. We will explore in more detail why this is so.

But first a few final remarks on [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]].

Our discussion focused on [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]] for the purpose of avoiding [margin calls](Class Note 13 The LTCM Meltdown.md) in leveraged strategies. But [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]] can be applied more broadly. For example,  an unlevered investor may want to put a floor on how much their [portfolio](An Asset Allocation Primer.md) can decline in value. Put options,  or other forms of [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]] can be used to accomplish this.

But it's important to keep in mind that that the insurance against [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] events does not come for free. In fact,  it could be quite expensive. A substantial part of the equity risk premium earned by stocks over risk-free assets is likely a compensation for the risk of extreme declines of stock prices in times of economic crises. Insuring against these extreme declines may also remove much of the risk premium that the [portfolio](An Asset Allocation Primer.md) earns. This may be fine for a risk averse investor,  or an investor who wants to take more risk via [leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md),  but must ensure that there is zero risk of [margin calls](Class Note 13 The LTCM Meltdown.md). But it may be counterproductive for a relatively risk-tolerant investor who is investing to earn substantial risk premia.

There is also an organizational challenge of how to sustain a [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]] program in the long-term if such a program is deemed useful. There seems to be a tendency of investors setting up [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]] programs after an experiencing a severe crash. But then,  as years pass by without a crisis event,  and the [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]] program only produces costs and no payouts,  the program is abandoned (before,  eventually,  another crisis hits). ${ }^{4}$ This is misguided. Whether the benefits of [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.6 [[Key Rates O1s Durations and Hedging|Hedging]] tail risks|tail risk]] [[Key Rates O1s Durations and Hedging|hedging]] justify the costs
...
For example,  consider the VIX [[Futures Not Subject to Cash-And-Carry|futures]] prices from August 25,  2022 shown in Figure 6.12. The price for [[Futures Not Subject to Cash-And-Carry|futures]] expiring on October $10 / 19 / 2022$ was $25.50$. And so the payoff on October 19,  2022 of a [long position](Chapter 4 - Futures: Hedging and Speculation.md) entered on August 22,  2022 would be the level of the VIX on October 19 minus $25.50$ ( $\times 1000$ since each [[Futures Not Subject to Cash-And-Carry|futures]] contract is for a notional value of 1000 times the index). If there were no [[Lecture 6-[Leverage](Lecture 6-Leverage, Tail Risk, Volatility Products.md), Tail Risk, Volatility Products#6.1.3 [[Case Study Mf Globals Repo-To-Maturity Trades|Margin requirements]]|[[[Case Study Mf Globals Repo-To-Maturity Trades|margin requirements]]](Case Study Mf Globals Repo-To-Maturity Trades.md)]],  no money would change hands at all at the initiation of the contract on August 25. But
...
We can look at some exchange-traded products as an easy way to track the [returns](Assets.md) from a strategy that rolls over long positions in near-term VIX [[Futures Not Subject to Cash-And-Carry|futures]]. Figure 6.13 shows the holdings of one popular product with the ticker VXX on August 22,  2022. ${ }^{6}$
...
Adding VXX to a stock [portfolio](An Asset Allocation Primer.md) should therefore substantially reduce the downside risk of the [portfolio](An Asset Allocation Primer.md). How would this have played out in the period that my VXX return data sample spans,  February 2009 to December 2021? Table 6.2 shows the mean [returns](Assets.md),  standard deviations,  and Sharpe ratio (all annualized) from a strategy that invests $100 \%$ in the CRSP value-weighted [[Hedging Strategies with Forwards|stock index]] and one that rebalances every day to $90 \%$ in the [[Hedging Strategies with Forwards|stock index]] and 10% in VXX. Not surprisingly,  the combined strategy has substantially lower standard deviation of [returns](Assets.md). The mean [returns](Assets.md) are lower,  too,  consistent with the fact that investing in VXX means paying an insurance premium. In fact,  the mean [returns](Assets.md) are sufficiently low so that the Sharpe ratio of the combined strategy of 0.75 is somewhat lower than the Sharpe ratio of the [[Hedging Strategies with Forwards|stock index]] of 0.87 . (Even so,  a sufficiently risk averse investor might prefer the combined strategy).

The 1st percentiles of the daily return distribution (second-to-last row,  in percent) show that the tail-risk [[Key Rates O1s Durations and Hedging|hedging]] benefit is modest. The 1st percentile of the mixed strategy is pulled toward zero a little more strongly than the 99th percentile on the upside. But it's not a huge difference. So if one is really interested in a targeted tail-risk hedge,  directly going for the tail-risk hedge with out-of-the money options rather than adding VXX (or VIX [[Futures Not Subject to Cash-And-Carry|futures]]) to the [portfolio](An Asset Allocation Primer.md) may be a more reliable strategy.
