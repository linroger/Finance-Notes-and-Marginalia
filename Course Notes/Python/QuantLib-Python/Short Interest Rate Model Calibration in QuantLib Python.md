...
Provides examples of short [[An Overview of the Vasicek Short Rate Model|interest rate model]] [[Credit Markets Session 4|calibration]] to swaption volatilities in [[Valuing Callable Bonds Using QuantLib Python|QuantLib Python]]
...
I have talked about [[On the Convergence of Hull White Monte Carlo Simulations|Hull-White model]] in my earlier blog posts. The focus of those posts was to see how to use the model classes. The model parameters were assumed to be given. However in practice,  the model parameters need to calibrated from market data. Typically instruments such as swaptions,  caps or [[Caps and Floors|floors]] and their market prices / volatilities are taken as inputs. Then the model parameters are fit in such a way that the [[Credit Markets Session 4|model prices]] these options close enough. The goodness of fit depends,  apart from the choice of the numerical methods,  on the type of model itself. This is because models such as Hull-White 1 factor cannot fit some of the humped volatility term structures observed in the market. Never the less,  Hull-White is usually a good starting point to understand [[Credit Markets Session 4|calibration]] process.

Here we will discuss [[On the Convergence of Hull White Monte Carlo Simulations|Hull-White model]] in detail. Then we will also show how the same procedure can be applied to calibrate other [[An Overview of the Vasicek Short Rate Model|short rate]] models.
...
[[On the Convergence of Hull White Monte Carlo Simulations|Hull-White model]] was one of the first practical exogenous models that attempted to fit to the market interest rate term structures. The model is described as:
$$
d r_{t} = \left(\right. \theta \left(\right. t \left.\right) - a r_{t} \left.\right) d t + \sigma d W_{t}
$$

where $a$ is the [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]] constant,  $\sigma$ is the volatility parameter. The parameter $\theta \left(\right. t \left.\right)$ is chosen in order to fit the input [[6. A Brief Introduction to Stochastic Calculus|term structure of interest rates]].
...
Here we use the `JamshidianSwaptionEngine` to value the swaptions as part of [[Credit Markets Session 4|calibration]]. The `JamshidianSwaptionEngine` requires one-factor affine models as input. For other interest rate models,  we need a [[Arbitrage Pricing of Derivatives|pricing]] engine that is more suited to those models.
...
Some times we need to calibrate with one parameter held fixed. This can be done in the [[Introduction to QuantLib Python|QuantLib]] libraries. However,  this ability is not exposed in the SWIG wrappers as of version 1.6. I have created a [github issue](https://github.com/lballabio/[[Introduction to QuantLib Python|quantlib]]/issues/336) and provided a patch to address this issue. You will need this patch to execute the following cells. Below,  the model is calibrated with a fixed reversion value of 5%.
...
The [[Short Interest Rate Model Calibration in QuantLib Python|Black Karasinski model]] is described as:
$$
...
In order to calibrate,  we use the `TreeSwaptionEngine` which will work with all [[An Overview of the Vasicek Short Rate Model|short rate]] models. The [[Credit Markets Session 4|calibration]] is shown below.
...
As a final example,  let us look at a [[Credit Markets Session 4|calibration]] example of the 2-factor G2++ model.
$$$
...
Once again,  we use the `TreeSwaptionEngine` to value the swaptions in the [[Credit Markets Session 4|calibration]] step.
