...
Provides an [[Squam Lake Group Introduction|introduction]] to constructing [[Modeling Volatility Smile and Heston Model Calibration Using QuantLib Python|implied volatility surface]] consistent with the smile observed in the market and calibrating [[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]] using [[Valuing Callable Bonds Using QuantLib Python|QuantLib Python]].
...
European options on an equity underlying such as an index (S&P 500) or a stock (AMZN) trade for different combinations of strikes and maturities. It turns out that the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]] [[A Real-Life Option Pricing Exercise|implied volatility]] for these options with different maturities and strikes is not the same. The fact that the [[Swaption Skew|implied volatility varies]] with strike is often referred in the market as having a _smile_.
...
Following is a sample matrix of volatility quote by exipiry and strike. The volatilities are log-normal volatilities and can be interpolated to construct the [[Modeling Volatility Smile and Heston Model Calibration Using QuantLib Python|implied volatility surface]].
...
[[Modeling Volatility Smile and Heston Model Calibration Using QuantLib Python|Implied Volatility Surface]]
--------------------------
...
Now the Black [[7. Black Scholes Model|volatility surface]] can be constructed using the ```BlackVarianceSurface ``` method.
```python
...
The whole [[7. Black Scholes Model|volatility surface]] can also be visualised as shown below.
```python
...
One can also construct a local [[7. Black Scholes Model|volatility surface]] (_a la_ Dupire) using the ```LocalVolSurface```. There are some issues with this as shown below.
```python
...
The correct [[Arbitrage Pricing of Derivatives|pricing]] of local [[7. Black Scholes Model|volatility surface]] requires an [[Arbitrage Pricing of Derivatives|arbitrage]] free [[Modeling Volatility Smile and Heston Model Calibration Using QuantLib Python|implied volatility surface]]. If the input [[Modeling Volatility Smile and Heston Model Calibration Using QuantLib Python|implied volatility surface]] is not [[Arbitrage Pricing of Derivatives|arbitrage]] free,  this can lead to negative [[A Primer on Probability Theory and Stochastic  Modelling|transition probabilities]] and/or negative local volatilities and can give rise to mispricing. Refer to Fengler's arbtirage free smoothing [1] which [[Introduction to QuantLib Python|QuantLib]] currently lacks.

When you use an arbitrary smoothing,  you will notice that the local [[7. Black Scholes Model|volatility surface]] leads to undesired negative volatilities.
```python
...
[[Modeling Volatility Smile and Heston Model Calibration Using QuantLib Python|Heston Model Calibration]]
------------------------

[[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]] is defined by the following stochastic differential equations.$$\begin{eqnarray} dS(t,    S) = \mu S dt + \sqrt{v} S dW_1 \\ dv(t,    S) = \kappa (\theta - v) dt + \sigma \sqrt{v} dW_2 \\ dW_1 dW_2 = \rho dt \end{eqnarray}$$

Here the asset is modeled as a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] that depends on volatility $v$ which is a mean reverting [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] with a constant volatility of volatility $\\sigma$. The two stochastic processes have a correlation $\rho$.

Let us look at how we can calibrate the [[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]] to some market quotes. As an example,  let's say we are interested in trading options with 1 year maturity. So we will calibrate the [[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]] to fit to [[Modeling Volatility Smile and Heston Model Calibration Using QuantLib Python|market volatility quotes]] with one year maturity. Before we do that,  we need to construct the [[Arbitrage Pricing of Derivatives|pricing]] engine that the [[Credit Markets Session 4|calibration]] routines would need. In order to do that,  we start by constructing the [[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]] with some dummy starting parameters as shown below.
```python
# dummy parameters
...
Now that we have the [[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]] and a [[Arbitrage Pricing of Derivatives|pricing]] engine,  let us pick the quotes with all strikes and 1 year maturity in order to calibrate the [[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]]. We build the [[Valuing European Option Using the Heston Model in QuantLib Python|Heston model]] helper which will be fed into the [[Credit Markets Session 4|calibration]] routines.
```python
heston_helpers = []
black_var_surface.setInterpolation("bicubic")
...
Let us look at the quality of [[Credit Markets Session 4|calibration]] by [[Arbitrage Pricing of Derivatives|pricing]] the options used in the [[Credit Markets Session 4|calibration]] using the model and lets get an estimate of the relative error.
```python
...
\[1\] Mathias R. Fengler,  _Arbitrage Free Smoothing of [[A Real-Life Option Pricing Exercise|Implied Volatility]] Surface_,  [https://core.ac.uk/download/files/153/6978470.pdf](https://core.ac.uk/download/files/153/6978470.pdf)
