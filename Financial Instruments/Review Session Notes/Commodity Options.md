...
c. [[Call and Put Payoffs at Expiry|Strike price]] on call option for oil:$\$P > 0$.
...
6. Payoff from option: Given a call option on oil with a [[Call and Put Payoffs at Expiry|strike price]]$P$, if the oil price per barrel rises to$(P + \Delta)$, then each option yields a profit of$\$1,000 \cdot \Delta$since each option trades on one lot of 1,000 oil barrels. What is the profit from the call option if the oil price per barrel falls to$(P - \Delta)$?
...
2. Zero-cost: Let$c(P) > 0$denote option price as a function of the [[Call and Put Payoffs at Expiry|strike price]]. Given a target [[Call and Put Payoffs at Expiry|strike price]]$P_c > 0$for the call option, the optimal choice of the [[Call and Put Payoffs at Expiry|strike price]]$P_p > 0$is that which yields a zero net payoff.
$$c(P_p) - c(P_c) = 0$$

#### 1.3.2. DETERMINING ZERO COST COLLAR STRATEGY (PART II)

1. Suppose instead one chooses a [[Call and Put Payoffs at Expiry|strike price]] for the put option. To satisfy the zero-cost at inception, one must determine the number of put options that satisfies zero-cost.
...
3. Given a [[Call and Put Payoffs at Expiry|strike price]]$P_c$for the call option and a [[Call and Put Payoffs at Expiry|strike price]]$P_p$for the put option, determine the number of put options$N_p$.$$N_c \cdot P_c = N_p \cdot P_p$$