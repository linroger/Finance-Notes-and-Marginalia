...
Keywords: [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return,  [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]],  ran dom walk,  martingale,  Markov property [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]],  geometric Brownian. Motion,  Ito calculus
...
Now consider dividing up each period into 772 sub-periods each of length $\Delta t$ This is illustrated in Figure 1. Then if the compounding is done $7t$ times per period,  the [[1. DeterministicCashFlows|compound interest rate]] 7 satisfies
$$(1+r(T))=(1+\frac{r}{n})^{nT}.$$
...
This gives a simple method for calculating the [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return 7 from the formula $(1+r(T))=e^{rT}$ .Since $(1+r(T))=B_{T}/B_{0}$ simply take logs of both sides gives (since $\ln e^{r^{\prime}I^{\prime}}=rT$
$$r=\frac{1}{T}\ln\left(\frac{B_T}{B_0}\right)=\frac{1}{T}(\ln S_B-\ln B_0).$$

This is known as the [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return

### The Continuously Compounded Rate of Return

The [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return has the property that longer period rates of return canbe computed simply by adding shorter continuously [[Asset Price Dynamics|compounded rates of return]]. This is a very convenient feature which makes using the continuously [[Asset Price Dynamics|compounded rates of return]] especially simple. To see this let $7t$ denote the [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] ofreturn from period $t$ to $t+1$ ,   that is$$r_t=\ln\left(\frac{B_{t+1}}{B_t}\right)$$
...
Thus the [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return over two periods is simply the sum of the two periodby periodreturns. In generalfor any value of $T$ we can write
...
Which has a [[Verification of Central Limit Theorem|standard normal distribution]] with mean of zero and standard deviation of one. Such avariable is called a standard normalvariable. To make this transformation,    we subtract the mean and divide by the standard deviation (square root of the variance). Thus
$$\frac{\ln S_T-\ln S_0-\nu T}{\sigma\sqrt{T}}$$

is a standard normal variable. We let $N(x)$ denote the cumulative probability that the standard normal variable is less than or equal to $JL$ .A [[Verification of Central Limit Theorem|standard normal distribution]] is drawn in Figure 5. It can be seen that $N(0)=0.5$ as the normal distribution is symmetric and half the distribution is to the left of the mean value of zero. It also follows from symmetry that if $x>0$ then $1-N(x)=N(-x)$ .We will use this property later when we look at the [[Credit Markets Session 5|Black-Scholes formula]].
...
The value $V$ is the [[Arithmetic and Geometric Rates of Return|continuously compounded rate]] of return. It is given by
...
where $U.$ and $b$ are constants. This formula for the change in the value of ${:}L$ consists of two components,    a deterministic component adt and a stochastic component $b$ $dz ( t)$ .The deterministic component is $dx= a$ $dt$ or $\frac {dx}{dt}$ = $a$ which shows that $x$ = $x_{0}+ at$ so that $d.$ is simply the trend term for $JL$ Thus the increase in the value of ${:}\boldsymbol{L}$ over one time period is $d$ .The stochastic component $b$ $dz ( t)$ adds noise or variability to the path for $2 L$ .The amount of variability added is $b$ times the [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]]. Since the [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] has a standard deviation of one the generalizedprocess has a standard deviation of $b$
...
Where $\omega t$ is are [[Verification of Central Limit Theorem|independent random variables]] with mean $U$ and standard deviation of 0 .The continuous time version of this equation is therefore
$$d\:\ln S (t)=\nu\:dt+\sigma\:dz$$
...
Hence using [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's lemma]]$$dS (t)=(\nu S (t)+0+\frac{1}{2}\sigma^{2}S (t)) dt+\sigma S (t)\:dz$$ or
...
As wehave seen before the forwardprice just depends on the current price of the underlying,    the interest rate and the [[Chapter 16 - Black–Scholes Model|time to expiration]]. With [[Interest Rate Quotations|continuous compounding]] we can write the [[Forward Contracts and Forward Prices|forward price]] equation as
$$F (S (t),    t)=S (t) e^{r (T-t)}.$$

This shows the [[Forward Contracts and Forward Prices|forward price]] is a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] which depends on the price of the [[Risk Neutral Pricing of Options|underlying asset]] which itself is a stocastic process. Since we have that the [[Forward Contracts and Forward Prices|forward price]] is a function of a [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]] we can use [[Determining the Stochastic Process for a Forward Contract from Ito’s Lemma|Ito's lemma]]. Upon differentiation we have
