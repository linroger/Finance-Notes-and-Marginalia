...
The objective is to find the value of the option or derivative at the initial node of the tree. Let's consider an example with $u=0.75$,  $d$ = -0.25,  $S=100$,  $X=100$,  and $r=0.25$ and extend it to two periods. The ending values for the [[Risk Neutral Pricing of Options|underlying asset]] are 306.25,  131.25,  and 56.25. To value the call option at the initial node,  we first value the call at the final nodes and then work backward. The value of the call at the final nodes is simply 206.25,  31.25,  and 0 ,  that is,  the stock value minus the [[Call and Put Payoffs at Expiry|strike price]]. The value at the intermediate node then can be computed using the methods of [[Financial Instruments|delta hedging]],  synthetic options,  or risk-neutral valuation used in the section on the [[Exercises|one-period model]]. The method of risk-neutral valuation is simple because the risk-neutral probability is the same for each period as it depends only on $u$,  $d$,  and 7 that are unchanging. Taking this risk-neutral valuation method,  the value following the first up move is
...
Thus,  given the [[Call and Put Payoffs at Expiry|strike price]] $\rho=(R-D)/(U-D)$,  the [[Hedging Strategies with Forwards|time to maturity]] $7 t$,  and the [[Exercises|risk-free interest rate]] Thus,  are the call value. Let $JL$ denote the time to be taken for the terminal value $7 t$,  and the [[Exercises|risk-free interest rate]] $ 7$,  it is possible to be greater than the call value.

Then,  the minimum number of up branches that need to be taken for the terminal value $S_{0}U^{x}D^{n-x}$ to be greater than the [[Call and Put Payoffs at Expiry|strike price]] $X$. Then,  the above formula can be rewritten as
...
This is really exactly the same as in the [[Exercises|one-period model]]. To see,    suppose for simplicity that the call has a positive value in the up state so $x=1$ and $n=1$. Then we have seen that the value of the call in the [[Exercises|one-period model]] is$$c_0=\frac{1}{R}\rho (US_0-X)=S_0\rho\frac{U}{R}-\frac{X}{R}\rho=sS_0-\rho\frac{X}{R}.$$
...
This approximation will be better the smaller the [[Interest Rate Quotations|interest rates]] are and the smaller the variance is. In the example $\mu=0.2$ and $\sigma=0.1$ so $\frac{1}{2}\sigma^{2}=0.005$ and $\mu-\frac{1}{2}\sigma^{2}=0.1950$ which is close to the actual geometric mean of 0.1958.
...
As there will be roughly an equal number of up [[Assets|returns]] and down [[Assets|returns]],    the geometric return that produces the same outcome after $Tl$ periods is $(1+g)^{n}$. Thus ,   $$(1+\nu)^n=((1+\mu+\sigma)(1+\mu-\sigma))^\frac{n}{2}$$
