...
This introductory section positions the topic of “Portfolio Choice | portfolio choice]]”within the wider context of the course and sorts out a few preliminary issues.
...
Probability Distributions: To determine how risky a portfolio or asset is,  we will need to know the Probability Distributions of Returns | probability distribution]]s of its payoffs.
...
The cumulative Probability Distributions of Returns | probability distribution]] $F (x)$ is the area below the probability density function $f (t)$ below the value of $JL$
...
Domar and Musgrave? Proposed a measure of risk that measures the expected value of returns less than the returns on a risk-free investment. Letting 7 denote the return on a risk-free asset,  the Domar-Musgrave risk index is given by$$\rho_{DM}=-\int_{x_{min}}^{r}(x-r) f (x)\:dx$$
...
Risk Aversion IPreference for Certainty]: We say that an investor has “preference for certainty” and is risk averse if a risk-free investment is preferred over a risky investment of equal expected payoffs
...
That corresponds to the relative risk premium. The von Neumann Morgen stern utility functions that exhibit constant relative risk aversion are of the power form$$u (w)=\frac{w^{(1-r)}}{(1-r)}$$

With $u (w)=\log_{e}(w)$ when $r=1$ .Again it is easily checked that $R (w)=r$

## 5 Stochastic Dominance

First-order stochastic dominance: To examine stochastic dominance of two investments we look at the cumulative Probability Distributions of Returns | probability distribution]] functions
...
In the example $E$ is not independent of $B$ because the distribution function for $t$ depends upon $B$ .However $B$ and $t$ are uncorrelated. This can be easily checked as$$B\cdot\epsilon=\begin{pmatrix}6\\-3\\3\\-3\end{pmatrix}$$
