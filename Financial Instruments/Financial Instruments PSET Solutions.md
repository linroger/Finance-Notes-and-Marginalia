...
```python
    ...
    # [[7. Black Scholes Model|Put-call parity]]: call - put = S - K * exp (-rT)  =>  exp (-rT) = (S - (call - put)) / K
    Exp_minus_rT = (S_oil - (call_105 - put_105)) / 105.0
    R_implied = - (1/T_option) * np.Log (exp_minus_rT)
    Print (" (3. D) Using [[7. Black Scholes Model|put-call parity]] with strike $105: ")
    Print ("    S = ${}, Call = ${:. 3 f}, Put = ${:. 3 f}, T = {} years".Format (S_oil, call_105, put_105, T_option))
    Print ("    Implied [[PSET 7 Solutions-Financial Instruments|continuously compounded interest]] rate: {:. 3%}".Format (r_implied))
...
```python
    Print ("1. (1) Leeson’s strategy – selling one call and one put with the same strike and maturity – is called a [[PSET 4 Solution-Financial Instruments|SHORT STRADDLE]].\n")```
...
```python
    Print ("1. (2) A [[PSET 4 Solution-Financial Instruments|short straddle]] is profitable if at maturity the underlying remains near the [[Call and Put Payoffs at Expiry|strike price]], "
          "so that both the [[Notes on Basic Options Properties|call and put]] expire worthless (or with minimal intrinsic value). "
...
```python
    ...
    Plot_payoff_plotly (S_range, short_call_profit,
                       Title="[[PSET 4 Solution-Financial Instruments|Profit Diagram]] at Maturity for 1 Short Call Option",
                       Xlabel="Nikkei 225 Index at Maturity",
                       Ylabel="Profit per Contract (JPY)")
    Plot_payoff_plotly (S_range, short_put_profit,
                       Title="[[PSET 4 Solution-Financial Instruments|Profit Diagram]] at Maturity for 1 Short Put Option",
                       Xlabel="Nikkei 225 Index at Maturity",
                       Ylabel="Profit per Contract (JPY)")
    Plot_payoff_plotly (S_range, short_straddle_profit,
                       Title="[[PSET 4 Solution-Financial Instruments|Profit Diagram]] at Maturity for 1 [[PSET 4 Solution-Financial Instruments|Short Straddle]] (Call+Put)",
                       Xlabel="Nikkei 225 Index at Maturity",
                       Ylabel="Profit per Contract (JPY)",
...
```python
    ...
    # Plot the overall [[PSET 4 Solution-Financial Instruments|profit diagram]]:
    Plot_payoff_plotly (S_range, overall_profit,
                       Title="[[PSET 4 Solution-Financial Instruments|Profit Diagram]] at Maturity for Entire [[PSET 4 Solution-Financial Instruments|Short Straddle]] Position (35,500 Contracts)",
                       Xlabel="Nikkei 225 Index at Maturity",
...
```python
    Print ("1. (5) An increase in [[Class Slides On Terrausd Runs 2|market volatility]] increases the likelihood that the underlying will move far away "
          "from the strike. For a [[PSET 4 Solution-Financial Instruments|short straddle]], this means higher risk (more potential loss). Thus, as volatility increases, "
          "the value of the [[PSET 4 Solution-Financial Instruments|short straddle]] position decreases (i.e. its losses become larger).\n")```
...
Def problem 1_leeson_nikkei_options ():
    ...
    # (2) When is a [[PSET 4 Solution-Financial Instruments|short straddle]] profitable?
    Print ("1. (2) A [[PSET 4 Solution-Financial Instruments|short straddle]] is profitable if at maturity the underlying remains near the [[Call and Put Payoffs at Expiry|strike price]], "
          "so that both the [[Notes on Basic Options Properties|call and put]] expire worthless (or with minimal intrinsic value). "
          "The risk is that if the underlying moves significantly away from the strike (in either direction), "
          "the losses can be very large (and, in theory, unlimited on the call side).\n")
    ...
    # For a short call, profit = premium received – max (S - K, 0)*multiplier.
    # For a short put, profit = premium received – max (K - S, 0)*multiplier.
    # For the [[PSET 4 Solution-Financial Instruments|short straddle]], profit = total premium received – multiplier * |S - K|.
    # The break-even points occur when multiplier * |S - K| = total_premium.
    BE = total_premium / multiplier  # break–even deviation in index points
    Print ("1. (3) For one option contract, the break–even deviation is {:. 2 f} index points.".Format (BE))
    Print ("That is, the strategy breaks even if the Nikkei is at K ± {:. 2 f} at maturity.\n".Format (BE))
    ...
    # (5) Effect of an increase in [[Hedge Fund Strategies|stock market]] volatility
    Print ("1. (5) An increase in [[Class Slides On Terrausd Runs 2|market volatility]] increases the likelihood that the underlying will move far away "
          "from the strike. For a [[PSET 4 Solution-Financial Instruments|short straddle]], this means higher risk (more potential loss). Thus, as volatility increases, "
          "the value of the [[PSET 4 Solution-Financial Instruments|short straddle]] position decreases (i.e. its losses become larger).\n")
...
Def problem 3_multiperiod_binomial ():
    Print ("==== Problem 3: [[PSET 5 Financial Instruments|Multiperiod Binomial Tree]] ====\n")
    # Setup:
    S 0 = 100
    U = 1.1
    D = 1 / u
    ...
    # Build the two-period [[Rate and Price Trees|binomial tree]] for stock prices:
    # At time 0: S 0.
    ...
    # where [[Binomial Trees and Option Pricing MBA|risk neutral probability]] p is given by:
    P = (R - d) / (u - d)
    ...
    Print ("3. (a) Two–period [[Rate and Price Trees|binomial tree]] call option [[Arbitrage Pricing of Derivatives|pricing]]: ")
    ...
    # [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|Replicating portfolio]]:
    ...
    Print ("\nReplicating [[An Asset Allocation Primer|portfolio]] (dynamic [[Key Rates O1s Durations and Hedging|hedging]]): ")
    ...
    For dS in deltas:
        ...
        V_RP.Append (C 0_new)
    Df_RP = pd.DataFrame ({"ΔS": deltas, "[[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|Replicating Portfolio]] Value": V_RP})
    Print ("\n 3. (b) [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|Replicating portfolio]] value for changes in S 0: ")
    Display (df_RP)
    ...
    # (c) Self-financing? By construction, the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] is self-financing.
    Print ("3. (c) The [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] constructed via backward induction is self-financing; "
          "no additional capital is injected after time 0.\n")

    # (d) Profit/loss on the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]]:
    # At expiration, the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] exactly replicates the option payoff. Thus the P/L is zero.
    Print ("3. (d) At expiration, the [[Pricing Forwards, Futures, Bonds, Swaps, Swaptions, Caps and Floors under No-Arbitrage and Risk-Neutral Pricing|replicating portfolio]] replicates the option payoff exactly, so the profit/loss is zero.\n")

    # (e) Price if the stock pays a 5% dividend yield in period 1.
    Y = 0.05
    # In the up state at t=1, before dividend the [[Chapter 16 - Black–Scholes Model|stock price]] is S 0*u; after dividend the ex-dividend price is S 0*u*(1-y).
    S 0_u_div = S 0 * u * (1 - y)
    S 0_d_div = S 0 * d * (1 - y)
    ...
    D = 5
    # In period 1, the [[Chapter 16 - Black–Scholes Model|stock price]] in the up state will drop by $5: S 0*u - D, and in the down state: S 0*d - D.
    ...
    Print ("    The fixed-dollar dividend reduces the [[Chapter 16 - Black–Scholes Model|stock price]] by a constant amount, "
...
Print ("Extracted [[Black Scholes Derivation|Risk-Free Rate]] (Closest to Expiration): ")
Print (df_risk_free)
...
Df_options = pd.DataFrame ({
    ...
    "Maturity": ["2024-06-21"] * 5,  # Same [[Risk Neutral Pricing of Options|expiration date]] (June 21, 2024)
})
...
R = 5.07 / 100  # [[Black Scholes Derivation|Risk-free rate]] (annualized)
T = (21/365) * 3  # [[Chapter 16 - Black–Scholes Model|Time to expiration]] in years (3 months)
...
Def black_scholes_call (S, K, T, r, sigma, q=0):
    """Compute [[Mathematical Modeling of Derivative Pricing|Black-Scholes]] European call option price."""
...
Def implied_volatility_call (C_market, S, K, T, r, q=0):
    """Compute [[A Real-Life Option Pricing Exercise|implied volatility]] for a call option using numerical methods."""
...
Def black_scholes_call (S, K, T, r, sigma, q):
    """
    Computes the [[Mathematical Modeling of Derivative Pricing|Black-Scholes]] price for a European Call Option.

    Parameters:
    S (float): Current [[Chapter 16 - Black–Scholes Model|stock price]]
    K (float): [[Call and Put Payoffs at Expiry|Strike price]]
    T (float): [[Hedging Strategies with Forwards|Time to maturity]] (in years)
    R (float): Continuously compounded [[Black Scholes Derivation|risk-free rate]]
    Sigma (float): Volatility (standard deviation of log-[[Assets|returns]])
...
R = 0.0539  # 3-month [[Black Scholes Derivation|risk-free rate]] (converted to [[Interest Rate Quotations|continuous compounding]])
Q = 0.0150  # Dividend yield ([[Interest Rate Quotations|continuous compounding]])
...
# -*- coding: utf-8 -*-
"""
Revised [[PSET 6- Financial Instruments|Implied Volatility Calculation]] Using [[7. Black Scholes Model|Put-Call Parity]]
"""
...
Def synthetic_call_price (P, S, K, T, r, q):
    """
    Computes the synthetic call price using [[7. Black Scholes Model|put-call parity]]:
...
Def implied_volatility_put (P_market, S, K, T, r, q):
    """
    Computes the [[A Real-Life Option Pricing Exercise|implied volatility]] for a European put option using [[7. Black Scholes Model|put-call parity]].
    ...
    K (float): [[Call and Put Payoffs at Expiry|Strike price]]
    T (float): [[Hedging Strategies with Forwards|Time to maturity]] (in years)
    R (float): [[Black Scholes Derivation|Risk-free rate]] (continuously compounded)
    Q (float): Dividend yield (continuously compounded)

    [[Assets|Returns]]:
    Float: [[A Real-Life Option Pricing Exercise|Implied volatility]]
    """
...
```python
    Print ("**(a) [[Arbitrage Pricing of Derivatives|Pricing]] with r = 2% (no dividends)")
    R 1 = 0.02
...
**(b) Early Exercise?**
```python
    Print ("\n**(b) [[Bond Futures Options|Early Exercise]]?")
...
```python
    ...
    # For the call, even with higher [[Interest Rate Quotations|interest rates]], [[Bond Futures Options|early exercise]] is usually not optimal (no dividends).
    # For the put, a higher interest rate increases the [[Discount Factors|discount factor]], so [[Bond Futures Options|early exercise]] may be more attractive.
...
```python
    ...
    # One common approach in a [[Rate and Price Trees|binomial tree]] is to use an adjusted risk–neutral probability:
    # p = (exp ((r - q)*dt) - d)/(u - d).
    ...
    Print ("    so the call value decreases and [[Bond Futures Options|early exercise]] of the call might become optimal.")
    Print ("  - For the put, dividends generally increase its value (since dividends lower the [[Chapter 16 - Black–Scholes Model|stock price]]),")
    Print ("    so the put value increases and [[Bond Futures Options|early exercise]] may occur earlier than in the no-dividend case.\n")```
...
Def build_tree (S 0, u, d, n):
    """
    Build a recombining [[Rate and Price Trees|binomial tree]] for stock prices.
    [[Assets|Returns]] a 2 D list (levels 0 to n) with stock prices.
    """
    Tree = []
    For i in range (n+1):
        Level = []
...
Def american_option_tree (S 0, K, u, d, r, n, option_type='call', dividend_yield=0.0):
    """
    Price an American option (call or put) using a [[Rate and Price Trees|binomial tree]].
    - S 0: initial [[Chapter 16 - Black–Scholes Model|stock price]]
    - K: strike
    - U: up factor
    - D: down factor
    - R: continuously compounded [[Black Scholes Derivation|risk-free rate]]
    - N: number of periods (each of length Δt=1 year)
    - Option_type: 'call' or 'put'
    - Dividend_yield: [[Hedging Strategies with Forwards|continuous dividend]] yield (if >0, adjust [[Chapter 16 - Black–Scholes Model|stock price]] evolution)
    [[Assets|Returns]]: option value at time 0 and a tree (2 D list) of option values.
    """
    ...
    # Build [[PSET 5 Solution-Financial Instruments|stock price tree]]
    Stock_tree = []
    For i in range (n+1):
        Level = []
        For j in range (i+1):
            ...
            # However, one common approach is to discount the [[Chapter 16 - Black–Scholes Model|stock price]] later.
            Price = S 0 * (u**(i - j)) * (d**j)
            Level.Append (price)
...
Def part 1_american_options ():
    Print ("=== Part 1: American Options in a 3–Period [[Binomial Option Pricing|Binomial Model]] ===\n")
    S 0 = 100
    K = 100
    N = 3
    U = 1.1
    ...
    Print ("**(c) [[Arbitrage Pricing of Derivatives|Pricing]] with r = 5% (no dividends)")
    R 2 = 0.05
    Value_call_5, stock_tree_5, opt_tree_call_5, p 5 = american_option_tree (S 0, K, u, d, r 2, n, option_type='call')
    Value_put_5, _, opt_tree_put_5, _ = american_option_tree (S 0, K, u, d, r 2, n, option_type='put')
    Print ("American ATM Call value (r=5%): {:. 4 f}".Format (value_call_5))
...
# ---------------------------
S 0 = 1329.51              # Initial index level on [[Arbitrage Pricing of Derivatives|pricing]] date
Principal = 10.0          # Stated principal per PLUS note ($)
[[Lecture 6-Leverage, Tail Risk, Volatility Products|Leverage]] = 3.0            # [[Lecture 6-Leverage, Tail Risk, Volatility Products|Leverage]] factor (300%)
...
Cap_level = S 0 * (1 + max_pct_gain/[[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]])
# For our numbers:
...
Def plus_payoff (S, S 0=S 0, principal=principal, [[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]]=[[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]], cap_level=cap_level, max_payment=max_payment):
    """Return the terminal payoff of a PLUS note given final index level S."""
    If S <= S 0:
        Return principal * (S / S 0)
    Elif S <= cap_level:
        Return principal + principal * [[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]] * ((S - S 0) / S 0)
    Else:
        Return max_payment```
...
Def plot_southwest_hedging ():
    """Visualize Southwest Airlines' option [[Key Rates O1s Durations and Hedging|hedging]] strategies using Plotly."""

    # Define parameters
    Total_fuel_Q 1 = 377750000  # gallons
    Options_needed_1 a = total_fuel_Q 1 / 42000  # number of options required
    K_call = 105  # call strike in $/barrel
    K_call_gal = K_call / 42  # per gallon [[Call and Put Payoffs at Expiry|strike price]]
...
```python
    ...
    Print ("  This indicates that after the bailout announcement, market–implied [[Default Risk and Credit Derivatives 183|default risk]] decreased.\n")```
...
```python
    ...
    Print ("  (Here, a lower default probability implies a lower [[Cds-Equivalent Bond Spread|credit spread]].)\n")```
...
```python
    ...
    # Under the [[Credit Markets Session 5|Merton model]], equity is E = Call (V, D) and bondholders receive the remainder.
...
T_m = 1          # [[Hedging Strategies with Forwards|time to maturity]] (1 year)
R_kmv = 0.025    # [[Black Scholes Derivation|risk-free rate]] (continuously compounded) for KMV
...
Def merton_equations (x, E, sigma_E, D, r, T):
    ...
    [[Assets|Returns]] the residuals for the [[Credit Markets Session 5|Merton]] equations.
...
def part2_kmv():
    ...
    # Deposits = 780.343, [[When Safe Proved Risky Commercial Paper During the Financial Crisis of 2007 2009|Short-term debt]] = 352.274.
    # Therefore, effective asset value X (book) = 2050.469 - (780.343 + 352.274) ≈ 917.852.
    # Default point D = [[When Safe Proved Risky Commercial Paper During the Financial Crisis of 2007 2009|Short-term debt]] + 0.5 * (Long-term debt)
    ...
    print("  [[When Safe Proved Risky Commercial Paper During the Financial Crisis of 2007 2009|Short-term debt]]      = 352.274")
    ...
    # the market is [[Arbitrage Pricing of Derivatives|pricing]] Citigroup’s equity as a deeply out–of–the–money call on X.
    ...
    # (c) [[Cds-Equivalent Bond Spread|Credit spread]]: A rough approximation (in continuous time) is:
    # Spread ≈ -ln(1-PD). Compute for the two cases.
    ...
    print("  A lower PD implies a lower [[Cds-Equivalent Bond Spread|credit spread]].\n")
...
def problem4_black_scholes():
    print("==== Problem 4: Black and Scholes (and [[Credit Markets Session 5|Merton]]) Formula ====\n")
    # Given:
    S = 42
    sigma = 0.20
    ...
    T = 0.5       # 6 months
    K = 40
    # (1) [[Credit Markets Session 5|Black-Scholes formula]]:
    ...
    print("4.(1) [[Mathematical Modeling of Derivative Pricing|Black-Scholes]] prices:")
    def problem4_black_scholes():
        print("==== Problem 4: Black and Scholes (and [[Credit Markets Session 5|Merton]]) Formula ====\n")
    ...
    # Compute [[Mathematical Modeling of Derivative Pricing|Black-Scholes]] d1 and d2:
    d1 = (np.log(S/K) + (r_cont + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)
    ...
    fig.update_layout(title="Option Prices vs [[Chapter 16 - Black–Scholes Model|Stock Price]]",
                      xaxis_title="[[Chapter 16 - Black–Scholes Model|Stock Price]] (USD)",
    ...
    sensitivity = pd.DataFrame({
        "Input": ["[[Chapter 16 - Black–Scholes Model|Stock Price]]", "[[Call and Put Payoffs at Expiry|Strike Price]]", "Volatility", "Maturity", "Risk free rate"],
    ...
    print("4.(3) Sensitivity of [[Mathematical Modeling of Derivative Pricing|Black-Scholes]] option prices to inputs:")
    display(sensitivity)
    print("Intuition: For example, an increase in [[Chapter 16 - Black–Scholes Model|stock price]] increases the call value and put value (but put value increases because the intrinsic value of a deep–in–the–money put may change), etc.\n")

    # (4) [[Key Rates O1s Durations and Hedging|Hedging]] an at-the-money short put.
    ...
    print("4.(4) [[Key Rates O1s Durations and Hedging|Hedging]] an at–the–money short put:")
    ...
    # (5) Build a [[Rate and Price Trees|binomial tree]] and show convergence.
    steps_list = [2, 5, 10, 25, 50, 125, 250]
    prices = []
    deltas = []
    for n in steps_list:
        dt = T / n
        # [[Binomial Trees and Option Pricing MBA|Risk neutral probability]]:
    ...
    print("4.(5) Convergence of [[Rate and Price Trees|binomial tree]] to [[Mathematical Modeling of Derivative Pricing|Black-Scholes]] as number of steps increases:")
    ...
    # Show the [[Black-Scholes Model and Extensions|Black-Scholes equation]] in LaTeX:
    display(Latex(r"""
    \text{[[Mathematical Modeling of Derivative Pricing|Black-Scholes]] Call Option Price: } C = S\;N(d_1) - K e^{-rT} N(d_2)
    """))
    display(Latex(r"""
