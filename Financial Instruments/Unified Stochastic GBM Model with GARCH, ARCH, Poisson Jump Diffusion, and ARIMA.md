...
3. **Action:** **Write down the continuous-time SDE for the [[A Preview of Alternative Formulations|asset price]]** under a _time-varying_ drift $\mu_t$ and _time-varying_ volatility $\sigma_t$.
...
• Alternatively, we can keep it purely discrete, updating $\mu_{k+1}$ from $\mu_k$ based on squared past [[Assets|returns]].
...
- **Specific Model:** We aim to derive a general-form mathematical model of an [[A Preview of Alternative Formulations|asset price]] $S (t)$ following a [[Black Scholes Derivation|Geometric Brownian Motion]] (GBM) in continuous time. The model incorporates:
  ...
  - **Jump Diffusions:** Modeled as a [[A Poisson Model of Single Issuer Default|Poisson process]] with an intensity implying a 1% jump probability per unit time, with jump sizes skewed to the downside and exhibiting a fat right tail.
  - **Volatility-Dependent Behavior:** The model exhibits lognormal properties when volatility $\sigma (t) < 0.25$ and mean-reverting properties during high volatility periods, such as [[Unified Stochastic GBM Model with GARCH, ARCH, Poisson Jump Diffusion, and ARIMA|volatility clustering]].

- **Objective:** This model addresses the [[Arbitrage Pricing of Derivatives|pricing]] and dynamic behavior of financial assets under realistic market conditions, capturing stylized facts such as [[Unified Stochastic GBM Model with GARCH, ARCH, Poisson Jump Diffusion, and ARIMA|volatility clustering]], [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]], asymmetric jump risks, and [[Lecture 3- Dynamics of Returns|time-varying expected returns]]. It provides a framework for options [[Arbitrage Pricing of Derivatives|pricing]] and [[Financial Mathematics Course|risk management]] by integrating advanced stochastic processes.
...
- **Equation Setup:** Below, we outline the foundational equations and concepts that will underpin the derivation, each accompanied by definitions and financial principles.
  ...
  - **Standard [[Black Scholes Derivation|Geometric Brownian Motion]] (GBM):**
    $$ dS(t) = \mu S(t) \, dt + \sigma S(t) \, dW(t) $$
    - **Variables:** $S (t)$ is the [[A Preview of Alternative Formulations|asset price]] at time $t$, $\mu$ is the constant drift ([[Lecture 1- Probability Distributions of Returns|expected return]]), $\sigma$ is the constant volatility, and $W (t)$ is a standard [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] ([[Continuous-Time Stochastic Processes|Brownian motion]]) with $dW (t) \sim N (0, dt)$.
    - **Principle:** Represents the continuous-time evolution of an [[A Preview of Alternative Formulations|asset price]] under constant [[Black-Scholes Model|drift and volatility]], leading to lognormal price distributions.
  ...
  - **GARCH (1,1) Model (Discrete Time):**
    ...
    - **Principle:** Models [[Unified Stochastic GBM Model with GARCH, ARCH, Poisson Jump Diffusion, and ARIMA|volatility clustering]] by making variance dependent on past shocks ($\epsilon_{t-1}^2$) and past variances.
  ...
  - **[[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck Process]] (Continuous Time):**
    $$ dX(t) = a (b - X(t)) \, dt + c \, dW(t) $$
    - **Variables:** $X (t)$ is the process, $a > 0$ is the speed of [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]], $b$ is the long-run mean, $c > 0$ is the volatility of the process, and $W (t)$ is a [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]].
    - **Principle:** Models mean-reverting behavior, suitable for processes like volatility or dynamic means.

  - **Poisson Jump Process:**
    $$ dN(t) $$
    - **Variables:** $N (t)$ is a [[A Poisson Model of Single Issuer Default|Poisson process]] with intensity $\lambda$, where $P (N (t + dt) - N (t) = 1) = \lambda dt$ and $P (N (t + dt) - N (t) > 1) = o (dt)$.
...
- **Action:** Formulate the [[Implementing Heath, Jarrow & Merton (HJM) Model|stochastic differential equation]] (SDE) for $S (t)$ by extending the standard GBM to include stochastic drift, [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]], and jump diffusions.
...
- **Mathematical Work:**
  ...
  - Replace constant $\mu$ with time-varying $\mu (t)$ and constant $\sigma$ with $\sigma (t)$:
    ...
    - Here, $W_s (t)$ denotes the [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] specific to the [[A Preview of Alternative Formulations|asset price]] to distinguish it from others introduced later.
  - Incorporate jump diffusions via a [[A Poisson Model of Single Issuer Default|Poisson process]]:
    $$ dS(t) = \mu(t) S(t) \, dt + \sigma(t) S(t) \, dW_s(t) + S(t-) (e^{J(t)} - 1) \, dN(t) $$
    - $N (t)$ is a [[A Poisson Model of Single Issuer Default|Poisson process]] with intensity $\lambda$.
...
- **Reasoning:** The standard GBM assumes constant parameters, which oversimplifies real markets. Introducing $\mu (t)$ and $\sigma (t)$ as stochastic processes allows the model to capture [[Lecture 3- Dynamics of Returns|time-varying expected returns]] and [[Unified Stochastic GBM Model with GARCH, ARCH, Poisson Jump Diffusion, and ARIMA|volatility clustering]]. The jump term, driven by a [[A Poisson Model of Single Issuer Default|Poisson process]], models sudden price shocks (e.g., market crashes or booms), a critical feature for assets exhibiting fat-tailed return distributions. The multiplicative form $S (t-) (e^{J (t)} - 1)$ ensures jumps scale with the current price level, consistent with percentage-based movements in [[Financial Markets and Institutions Lecture Notes|financial markets]].
...
- **Mathematical Work:**
  ...
  - For continuous time, approximate GARCH as a mean-reverting [[The Ornstein-Uhlenbeck (OU) Process|stochastic process]]. Define $V (t) = \sigma^2 (t)$ (variance):
    $$ dV(t) = \kappa (\theta(t) - V(t)) \, dt + \xi V(t) \, dW_v(t) $$
    - $\kappa > 0$: Speed of [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]].
    - $\theta (t)$: Time-varying long-run variance (to be modeled next).
    - $\xi > 0$: Volatility of variance.
    - $W_v (t)$: [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] independent of $W_s (t)$ and $N (t)$.

- **Reasoning:** GARCH models volatility persistence via lagged shocks and variances. In continuous time, Nelson (1990) showed that GARCH (1,1) converges to a [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]] model as the time step shrinks, often resembling a mean-reverting process like the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck]] or CIR process. Here, $dV (t)$ mean-reverts to $\theta (t)$, capturing [[Unified Stochastic GBM Model with GARCH, ARCH, Poisson Jump Diffusion, and ARIMA|volatility clustering]] (high $V (t)$ persists before reverting). The term $\xi V (t) \, dW_v (t)$ introduces stochasticity, mimicking GARCH’s dependence on past shocks. Using $V (t)$ rather than $\sqrt{V (t)}$ in the diffusion term simplifies the model while preserving positivity if $V (0) > 0$ and parameters are appropriately chosen, though we assume $V (t) > 0$ for simplicity (a common approximation in [[Mathematical Modeling of Derivative Pricing|stochastic volatility modeling]]).
...
- **Mathematical Work:**
  ...
  - In continuous time, this resembles an [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]]. Define $\theta (t)$:
    $$ d\theta(t) = a (b - \theta(t)) \, dt + c \, dW_\theta(t) $$
    - $a > 0$: Speed of [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]] (related to $\phi$).
    - $b > 0$: Long-run mean of $\theta (t)$.
    - $c > 0$: Volatility of $\theta (t)$.
    - $W_\theta (t)$: [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] independent of $W_s (t)$, $W_v (t)$, and $N (t)$.

- **Reasoning:** ARIMA (1,1,0) implies a first-differenced AR (1) process, which in continuous time corresponds to a mean-reverting process like the [[Equilibrium Interest Rate Models|Ornstein-Uhlenbeck process]], where $a = -\ln (1 - \phi)$ for small time steps. This dynamic $\theta (t)$ allows the volatility’s target level to evolve, reflecting changing market regimes (e.g., shifts in macroeconomic conditions). The stochastic term $c \, dW_\theta (t)$ introduces randomness, ensuring $\theta (t)$ isn’t static, aligning with the requirement for a “dynamic mean that changes over time” in the GARCH structure.
...
- **Mathematical Work:**
  ...
  - For the drift $\mu (t)$, hypothesize it mean-reverts to a level influenced by $V (t) = \sigma^2 (t)$:
    ...
    - $\beta > 0$: Speed of [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]].
    - $\omega$: Baseline drift component.
    - $\gamma \geq 0$: Sensitivity of drift to variance.
    - $\eta > 0$: Volatility of drift.
    - $W_\mu (t)$: [[The Ornstein-Uhlenbeck (OU) Process|Wiener process]] independent of others.

- **Reasoning:** ARCH models variance as a function of past squared [[Assets|returns]]. Here, we adapt this idea to the drift, positing that $\mu (t)$ adjusts toward a level proportional to current volatility ($\gamma V (t)$), reflecting empirical evidence that expected [[Assets|returns]] correlate with risk (e.g., risk premium effects). The mean-reversion term ensures stability, while $\eta \, dW_\mu (t)$ adds stochasticity. This is a continuous-time proxy for ARCH’s dependence on past shocks, where $V (t)$ serves as a proxy for recent volatility magnitude, akin to $\epsilon_{t-1}^2$ in discrete time.
...
- **Mathematical Work:**
  - [[A Preview of Alternative Formulations|Asset price]]:
    $$ dS(t) = \mu(t) S(t) \, dt + \sigma(t) S(t) \, dW_s(t) + S(t-) (e^{J(t)} - 1) \, dN(t) $$
  - Variance:
  ...
  - Processes:
    ...
    - $N (t)$ is a [[A Poisson Model of Single Issuer Default|Poisson process]] with $\lambda = 0.01$, independent of Wiener processes.
    - $J (t) \sim F_J$, i.i.d., with negative skew and fat right tail.

- **Reasoning:** This system unifies GBM’s continuous dynamics with GARCH-inspired [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]] (via $\sigma^2 (t)$ and $\theta (t)$), ARCH-inspired drift (via $\mu (t)$), and Poisson jumps. Independence among stochastic drivers ensures distinct sources of randomness, reflecting separate market influences (price shocks, volatility fluctuations, drift adjustments, jumps). The model captures all required features: dynamic volatility mean, volatility-dependent drift, and asymmetric jumps.
...
- **Mathematical Work:**
  ...
  - For the diffusion part ($dS (t) = \mu (t) S (t) \, dt + \sigma (t) S (t) \, dW_s (t)$):
    - Compute [[Chapter 9 Arbitrage and Hedging With Options|derivatives]]:
...
- **Reasoning:** Deriving $\ln S (t)$ provides insight into return dynamics, essential for [[Arbitrage Pricing of Derivatives|pricing]] and [[Week 3 Financial Risk Analysis|risk analysis]]. Ito’s lemma adjusts for the curvature of the log function, subtracting $\frac{1}{2} \sigma^2 (t)$ due to stochasticity. The jump term adds $J (t)$ at random times, preserving the model’s discontinuous nature. This SDE confirms the model’s structure and serves as a “mathematical solution” by expressing the process explicitly, though [[Chapter 40 - Pricing Fixed Income Options: Black’s Model and MCS|closed-form solutions]] for $S (t)$ are infeasible due to stochasticity.
...
- **Volatility Threshold Behavior:**
  ...
  - During high volatility (e.g., $\sigma (t) > 0.25$), $\kappa (\theta (t) - \sigma^2 (t))$ drives $\sigma^2 (t)$ back toward $\theta (t)$, exhibiting [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]], especially during clustering when $V (t)$ spikes.

- **Assumptions:**
  - $\sigma^2 (t) > 0$ always (ensured by positive initial conditions and parameter constraints).
  - Independence of stochastic processes simplifies correlation structure, though real markets may exhibit [[Lecture 6-Leverage, Tail Risk, Volatility Products|leverage]] effects (negative correlation between $W_s (t)$ and $W_v (t)$).

- **Limitations:** No closed-form solution exists due to coupled SDEs and jumps; numerical methods (e.g., [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Monte Carlo]]) are required for [[Arbitrage Pricing of Derivatives|pricing]].

---

### 4. Conclusion

We have derived a comprehensive continuous-time GBM model with [[Valuing European Option Using the Heston Model in QuantLib Python|stochastic volatility]] via a GARCH (1,1)-like process with an ARIMA (1,1,0)-driven mean, an ARCH (1)-inspired stochastic drift, and Poisson jump diffusions. The system of SDEs captures [[Unified Stochastic GBM Model with GARCH, ARCH, Poisson Jump Diffusion, and ARIMA|volatility clustering]], [[The Ornstein-Uhlenbeck (OU) Process|mean reversion]], dynamic drift, and asymmetric jumps, meeting all specified requirements. The log-[[Chapter 47 - Asset Price Dynamics|price dynamics]] provide a mathematical solution, laying the groundwork for applications in options [[Arbitrage Pricing of Derivatives|pricing]] and [[Financial Mathematics Course|risk management]], though further [[Credit Markets Session 4|calibration]] and numerical analysis are necessary for practical implementation.

--- 

This derivation exemplifies the rigor and depth expected at the University of Chicago, offering a pedagogical yet advanced exploration of [[Mathematics of the Financial Markets|financial modeling]].