...
where $Z(i + 1)$ is the value of a zero coupon bond maturing the next period and q∗ is the [[Binomial Trees and Option Pricing MBA|risk neutral probability]]. Given the [[PSET 7 Solutions-Financial Instruments|continuously compounded interest]] rate $r$ we can easily find that $Z(i + 1)$ as equal to $Z(i + 1) = e^{−r·dt}$. In our case $r = 0.02$ and $dt = 1$,  so $Z(i + 1) = 0.98$. We also know that the [[Binomial Trees and Option Pricing MBA|risk neutral probability]] $q^∗$ is given by
...
Before proceeding back to period$i = 1$,  we compare the values to waiting ("wait" in the tree) with the values from [[Bond Futures Options|early exercise]]. At each node the value from [[Bond Futures Options|early exercise]] will be simply equal to$$C_{i,       j}^{E E}=m a x(S_{i,       j}-K)$$where EE stands for "[[Bond Futures Options|Early Exercise]]." Figure (3) compares both values. As you can see,  the value to waiting is always greater than the value to [[Bond Futures Options|early exercise]],  so we the option values at period$i = 2$will be equal to the values from waiting. We then keep going backward until we reach period$i = 0$. Figure 4 displays the result. We apply the same procedure for the American put. Figure (5) shows the period$i = 2$comparison between values to waiting and values to [[Bond Futures Options|early exercise]].
 ![500](97212d4591459bf8b7bb76c6a6024a87.png)

In this case we note that$P_{33}^{wait}=15.38<P_{3.3}^{EE}=17.36$,  therefore the option value in node (3,  3) will be equal to$P_{3,       3}=P_{3,       3}^{EE}=17.36.$Going back until period$i=0$we obtain the tree shown in Figure (6). [[Bond Futures Options|Early exercise]] nodes are underlined.
...
While Figure (8) reports the tree for the American put option. Again,  [[Bond Futures Options|early exercise]] nodes are underlined. In this case [[Bond Futures Options|early exercise]] of the put is optimal twice,  namely at nodes (2,  2) and (3,  3).

The reason is that when [[Chapter 16 - Black–Scholes Model|stock price]] drops,  the benefit that we get from investing the [[Call and Put Payoffs at Expiry|strike price]] at the risk free rate are greater when the risk free rate is greater,  and this increases the benefits from [[Bond Futures Options|early exercise]]. Note that at the same time,  as interest rate goes up the value of the put goes down. More in detail the value of the European put goes down by much more than the American put and this is due to the fact that the value from [[Bond Futures Options|early exercise]] increases.

 ![500](af4fa979544f686ce9c72cd984392f15.png)
(d) When we have [[Exercises|dividend payments]] the computation is a bit trickier.
With a dividend we must worry about the drop [[Chapter 16 - Black–Scholes Model|stock price]] that occur as a consequence of a payment to shareholders. In other words,  dividends are the amounts of price drops. There are two ways to deal with this: change the [[Chapter 16 - Black–Scholes Model|stock price]] outcomes and fix the [[Financial Instruments|risk-neutral probabilities]] or change the [[Chapter 16 - Black–Scholes Model|stock price]] outcomes and fix the [[Financial Instruments|risk-neutral probabilities]]. The spreadsheet "HW7 Solution Question 1" provides details on both approaches. Here I outline how to do the calculation with a change in the [[Chapter 16 - Black–Scholes Model|stock price]] outcome.
...
- (i) Start from current [[Chapter 16 - Black–Scholes Model|stock price]]
...
- We can start by computing the number of shares using the [[Price-to-Sales Ratios in Stock Valuation|market capitalization]] and the [[Chapter 16 - Black–Scholes Model|stock price]] on Feb 13 2009. We get (in billions)$$N={\frac{M C a p}{S}}={\frac{19.02}{3.49}}=5.450$$
...
What are the inputs that we need to apply the Black and Scholes (and [[Credit Markets Session 5|Merton]]) formula?

We need the value of the underlying (VA),  the [[Call and Put Payoffs at Expiry|strike price]] (K),  the risk free rate (r),  the annualized volatility of the [[Assets|returns]] of the underlying (σA) and the maturity of the option
...
And the maturity? Let's assume$T=7.$Below is a table showing how the results would have changed for different values of$T$) What about the risk free rate? We get it from the Excel file (I assume that it is annually compounded. Note that we have two [[Interest Rate Quotations|interest rates]],  one for each date,  that are$$r_1^{10/10}= 3.27\% \text{ and } r_1^{10/14}= 3.48\%)$$Same for the volatility of equity,  we have$$\sigma_E^{10/10}= 1.7551 \text{ and } \sigma_E^{10/14}= 1.8947$$We are now ready to use the solver Here are the steps (for Oct$10$only. You can do the same for Oct 14 and check the results):
 ...
 - [[Call and Put Payoffs at Expiry|Strike price]]: Cell B6 equal to 791.79
...
- (a)After the bailout,        on Oct 14,        the probability of default increases counterintuitively. Indeed we would expect the bailout to insure markets that the institution will be safer. This is an issue with the [[Credit Markets Session 5|Merton model]]: For given $σ_A$,        there is a very tight relation between equity value and volatility of equity. The bailout announcement made equity increase from 75 to 100 billion. According to the relationship assumed in the [[Credit Markets Session 5|Merton model]],        the volatility of equity should have declined substantially (as $V_E$ is inversely related to $σ_E$). Since the realized volatility of equity did not decrease (as $σ^{10/14}_E >σ^{10/10}_E$),        volatility of assets has to increase dramatically in order to match both the new equity value and the value of volatility. The higher volatility of assets increases the probability of default.
...
- (c) The [[Cds-Equivalent Bond Spread|credit spread]] can be computed as$$c s=-{\frac{1}{T}}\cdot l n\left[1-e^{r\cdot T}\cdot B S P\left({\frac{V_{0}}{F}},       r,       T,       \sigma\right)\right]$$
where BSP stands for "Black and Scholes Put". This comes from the fact that under the [[Credit Markets Session 5|Merton model]] debt is a combination of a a bond and a short position in a put option. We can make the same assumptions made for computing the value of equity,        namely that the short term is paid immediately and that the [[Call and Put Payoffs at Expiry|strike price]] of the option ($F$ in the formula) is equal to the long term liabilities. Of course this implies that the value of the underlying $V_0$ is the value of the Residual Assets $V_{RA}^0$ computed above. Finally $σ$ is the volatility of assets $σ_A$.
