...
## a. Compute the fair value of the CDS premium and default legs.```python
...
## b. Compute the CDS PV and the CDS Par Spread.```python
...
Plot the VZ yields (Y-axis) by TTM (X-axis).```python
# Corp_symbology_vz
...
What do you think about the quality of the model fit?```python
# Plot the model vs market prices (Y-axis) by maturity (X-axis).
...
![png](CreditMarketSolutions_289_1.png)```python
# Plot the model vs market yields (Y-axis) by maturity (X-axis).
...
```
    Text (0.5,            0,            'Bond Maturity')
![png](CreditMarketSolutions_290_1.png)```python
# Plot the edges in yield space (Y-axis) by maturity (X-axis).
