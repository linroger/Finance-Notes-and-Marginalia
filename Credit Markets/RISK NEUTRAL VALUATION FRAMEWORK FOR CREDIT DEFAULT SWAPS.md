...
This set of lecture notes provides a thorough explanation of the [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|risk neutral]] valuation framework for [[Arbitrage Pricing of Derivatives|pricing]] and analyzing Credit Default Swaps (CDS). The key components covered include:

1. [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Risk neutral]] valuation principles
1. Simple case of [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|deterministic interest rates]]
1. CDS premium/[[Pricing Interest Rate Swaps|fixed leg]] cash flows
1. [[The Default Correlation of the Reference Issuer|CDS default]]/loss leg cash flows
1. Valuation of CDS using [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|hazard rates]]
1. Properties of [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|hazard rates]]
1. Numerical quadrature for CDS valuation
1. [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|CDS duration]],  ParSpread,  and valuation formulas

## RISK NEUTRAL VALUATION PRINCIPLES

The [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|risk neutral]] valuation framework is based on the following principles:
...
1. [[Teaching Note 7-Exotic Options And Derivative Pricing By Monte Carlo Simulation|Risk neutral]] valuation uses a "market implied" probability measure $\mathbb{P}$ for determining prices of securities.
1. A bank savings account security $B$ is used as a numeraire for discounting [[Advanced Derivatives Pricing Methodology|future cash flows]]: $$B(t) = e^{\int_0^t r_s ds},         \quad t \geq 0$$
...
For the simple case of [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|deterministic interest rates]]:

1. The "time value of money" at time $t$ for time $s$ is captured in the calibrated [[Credit Markets Session 3|discount factor curve]]: $$DF(t,         s) = B(t) \cdot \mathbb{E}\left[B(s)^{-1}|\mathcal{F}_t\right] = e^{-\int_t^s r_u du},         \quad 0 \leq t \leq s \quad (3)$$
...
1. The same formula holds when security cash flows and [[Interest Rate Quotations|interest rates]] are independent.

## CDS PREMIUM/FIXED LEG CASH FLOWS

For CDS premium/[[Pricing Interest Rate Swaps|fixed leg]] cash flows:
...
1. Market implied issuer survival & [[Credit Markets Session 3|default probabilities]]: $$SP(t,         s):= \mathbb{P}(\tau > s | \tau > t),         \quad 0 \leq t \leq s,         \quad (7)$$ $$DP(t,         s):= \mathbb{P}(\tau \leq s | \tau > t) = 1 - SP(t,         s). \quad (8)$$

## CDS DEFAULT/LOSS LEG CASH FLOWS

For [[The Default Correlation of the Reference Issuer|CDS default]]/loss leg cash flows:
...
1. $R$ can be thought of as the expected (average) [[Credit Markets Session 3|recovery rate]] for a given issuer and seniority rank.
...
The present value of the [[The Default Correlation of the Reference Issuer|CDS default]]/loss leg is:$$PV_{CDS_DL}(t) = \mathbb{E}\left[\int_t^T DF(t,        s) \cdot dDL(s) | \tau > t\right] \tag{16}$$
...
The intuitive interpretation is that the default leg present value at the (random) issuer default time is:$$ PV_{CDS_DL}(t) = \int_t^T (1 - R) \cdot DF(t,        s) \cdot dDP(t,        s) \tag{21} $$
...
1. Multiplying with the [[Discount Factors|discount factor]] for that interval: $DF(t,        s)$.
...
1. [[Advanced Usage of QuantLib analytics library|Hazard rate curves]] $h(t,        \cdot)$ defined implicitly from [[A Poisson Model of Single Issuer Default|Survival Probability]] curves:
...
[[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|Hazard rates]] $h(t,        s)$ quantify [[A Deterministic Credit Migration Model|conditional probabilities]] of default "around" specific times $s$ in the future:$$\mathbb{P}(s < \tau \leq s + \varepsilon | \tau > s,        \tau > t) \tag{26}$$
...
1. [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|CDS Duration]] defined as the "Unit" 1bp Premium Leg PV: $$Duration(t):= \sum_{i=1}^n \Delta T_i \cdot DF(t,        T_i) \cdot SP(t,        T_i) \tag{35}$$
1. CDS Premium Leg valuation via [[Key Rates O1s Durations and Hedging|Duration]]: $$PV_{CDS_PL}(t) = c \cdot Duration(t) \tag{36}$$
1. ParSpread defined from [[The Default Correlation of the Reference Issuer|CDS Default]] Leg and [[Key Rates O1s Durations and Hedging|Duration]]:$$PV_{CDS_DL}(t) = ParSpread(t) \cdot Duration(t) \tag{37}$$
...
1. CDS ParSpread measures the [[Quantitative Trading Strategies Lecture Notes|credit risk]] of the CDS contract:
...
1. Deterministic [[Discount Factors|Discount Factor]]:
...
1. Simplified Present Value ([[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|Deterministic Interest Rates]]):
...
1. [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|Hazard Rates]]:
	$$SP(t,        s) = \exp\left[-\int_t^s h(t,        u) du\right],        \quad 0 \leq t \leq s. \tag{24}$$
...
1. Present Value of Default Leg (using [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|Hazard Rates]]):
...
1. CDS Valuation:
	$$PV_{CDS}(t) = PV_{CDS_PL}(t) - PV_{CDS_DL}(t) \tag{33}$$
	The value of a CDS contract $PV_{CDS}(t)$ is determined by the difference between the present value of the premium leg $PV_{CDS_PL}(t)$ and the present value of the default leg $PV_{CDS_DL}(t)$. The premium leg represents the payments made by the [[Credit Default Swaps|protection buyer]] to the [[Credit Default Swaps|protection seller]],  while the default leg represents the potential payment made by the [[Credit Default Swaps|protection seller]] to the [[Credit Default Swaps|protection buyer]] in the event of the issuer's default.

1. [[RISK NEUTRAL VALUATION FRAMEWORK FOR CREDIT DEFAULT SWAPS|CDS Duration]]:
