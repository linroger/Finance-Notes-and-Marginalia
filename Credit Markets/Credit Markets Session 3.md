...
- We consider the simple case of deterministic interest rates. The "time value of money" $\$a_t$$ time t for time s is captured in the calibrated discount factor curve:
...
- ... multiplied with the discount factors $DF(t, T_i)$ $\$a_t$$ cash-flow times $T_i$ (time value of money)
- ... and multiplied with probability $SP(t, T_i)$ of issuer being "alive" $\$a_t$$ cash-flow time $T_i$ (credit risk "adjuster")
...
Present value of loss payment $\$a_t$$ (random) issuer default time:
...
- Forward interest rates $f(t, s), s \geq t$ represent information about interest rates $\$a_t$$ future time $s$, as seen from time $t$.
...
- Hazard rates $h(t, s)$ represent infinitesimal probabilities of default $\$a_t$$ future time $s$, using market information as of time $t$. Similar concept to forward interest rates.
...
### CDS trades "$\$a_t$$ par" when ParSpread matches the coupon:
...
- Risky semi-annual coupons $c$ $\$a_t$$ times $\{T_i\}^n_{i=1}$ and principal payment $$a_t$$ maturity $T = T_n$
$\$PV_{Bond\_FL}(t) = \sum_{i=1}^{n} c \cdot \\Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i) + DF(t, T_n) \cdot SP(t, T_n) \tag{47}$$

### Default Leg Valuation:

- Bond pays recovery value $R_i$ $\$a_t$$ default time $\tau \in [T_{i-1}, T_i]$, if issuer defaults before maturity $T$
$$\begin{aligned}
...
Bond trading $\$a_t$$ 100% "par" price in hazard rate model:
$\$PV=1 \iff c=r+(1-R) \cdot h \tag{59}$$

Bond trading $\$a_t$$ 100% "par" price in yield/spread model:
