...
Note: TTM = trailing twelve months = last twelve months
TLFI (Truck load freight index) is an index that measures the supply and demand for truck load services
Class 8 truck = Heavy truck
...
- This Investment Behavior Exacerbates Demand-Driven Price Cycles
	...
	- Large expansions $\$a_t$$ the peak translate into excess capacity in the downturn
	- Expensive to shut down an operation/facility once started
	- Thus, supply-side effects make the industry even more cyclical
- Why do cyclical companies tend to invest $\$a_t$$ the "worst" time?
...
- For the other moat sources, we look $\$a_t$$ how successful competitors have been $$a_t$$ replicating a product, process, or technology … The less successful rivals have been, the wider the moat
...
## MOAT EXAMPLE: "VERBUNDSTRATEGIE" AT BASF
...
- What Is A "Verbund"?
	...
	- "Verbund" groups chemical production plants together and vertically integrates production
		- By-products of one plant can be used as raw material $\$a_t$$ another
...
- Cost advantage:
	...
	- Unique in how they (vertically) link numerous plants together $\$a_t$$ the same scale
## KEY VALUE DRIVERS
...
- Example: Alcoa is world largest alumina producer outside of China
	- Production costs $\$a_t$$ various Alcoa alumina refineries relative to global cost curve
...
$\$\text{Operating Income} = -6,395 + 911.32 \times \text{(Average Oil Price)}$$
...
$\$\text{Value of operating assets} = \frac{\text{Operating Income} (1 + g)(1 - \text{tax rate})(1 - \frac{g}{\text{ROC}})}{\text{(Cost of capital - g)}}$$

$\$= \frac{34.61(1.02)(1 - 0.35)(1 - \frac{0.02}{0.21})}{(0.0944 - 0.02)} = \$279.05 \text{ billion}$$

- Subtracting net debt to arrive $\$a_t$$ equity value:
...
## CYCLICAL VALUATIONS
- Looking $\$a_t$$ price and EPS path illustrates why multiples expand and contract
...
- For example: Use scenarios to develop $\$a_t$$ least two cycles:
...
- Forecast future EPS $\$a_t$$ peak of cycle
...
- ROIC $\$a_t$$ the end of the forecast horizon should be sustainable given expected competition, industry conditions, etc.
...
$\$\text{Continuing Value}_t = \frac{NOPAT_{t+1}}{WACC - g} \times \left(1 - \frac{g}{RONIC}\right) \quad \text{Allows for adjustment of long-run profits}$$
...
[^1]: $\$TV = \frac{FCF_{t+1}}{(WACC-g)} = FCF_{T} \times \frac{(1+g)}{(WACC-g)}$$
