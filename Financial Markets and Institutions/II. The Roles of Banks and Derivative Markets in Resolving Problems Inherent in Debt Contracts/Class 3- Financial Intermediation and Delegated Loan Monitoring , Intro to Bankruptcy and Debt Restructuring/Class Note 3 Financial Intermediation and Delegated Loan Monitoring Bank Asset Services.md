...
- I want to show how [[Financial Intermediation as Delegated Monitoring|financial intermediaries]] can be set up to create [[Class Note 3 Financial Intermediation and Delegated Loan Monitoring Bank Asset Services|synthetic large investors]].
...
- When monitoring cost is prohibitive,  the [[Lecture 3 Slides|optimal contract]] is publicly-traded debt with face value **1.3125** (see class note 2).
  - Delegating monitoring to one agent avoids the duplication,  but causes [[10. Other Topics in Quantitative Finance|incentive problems]] for the agent delegated the monitoring task.
...
- Suppose that the banker monitors a single loan (runs a one-loan bank) on behalf of the small lenders,  and does not diversify across loans.
  - When the borrower's project [[Assets|returns]] **1**,  the banker can monitor and collect the **1** without actually liquidating.
  - However,  the bank itself would need to be liquidated in this case,  because the face value of the bank's debt must exceed **1**.
  - If the bank's debt contract with the small depositors has a face value of **1** or less,  the small depositors never receive more than **1**,  which delivers less than the **1.05** expected repayment they need to receive the required **5% [[Lecture 1- Probability Distributions of Returns|expected return]]**.
...
- Suppose the banker monitored not one loan,  but a [[Chapter 5 - Index Futures|diversified portfolio]] of loans.
  - A very simple way to show the value of diversification is to examine the two-loan bank.
- In particular,  suppose the banker monitors loans of two borrowers,  whose [[Assets|returns]] are independently distributed but are otherwise just like that of the single borrower (each loan has a **0.8** probability of returning **1.4** and **0.2** probability of returning **1**).
...
1. In the text I ignore the **$100** of capital that the banker can contribute,  to simplify the explanation.
   ...
   - The expected repayment to give a **5% [[Lecture 1- Probability Distributions of Returns|expected return]]** is then **(1.05)(0.9999) = 1.04995**.
   - This is equivalent to the case where the banker has none of his own capital but outside investors require a **4.995% [[Lecture 1- Probability Distributions of Returns|expected return]]**.
...
- Assume that liquidating the bank yields nothing to depositors or to the banker,  similar to the liquidation of borrowing firms.
  ...
  - If the bank must be liquidated when it collects face value of **F** from one borrower and **1** from the other,  it will be liquidated whenever at least one loan defaults,  and there will be no possible savings in costs of [[Southland Corp. (c)|financial distress]].
...
- If there is never default on debt (because **L ≥ 1 + r**),  there is no cost of [[Southland Corp. (c)|financial distress]].
...
- Publicly-traded debt's cost of [[Southland Corp. (c)|financial distress]] is then given by **S = L(1 - P)**,  because **L** is the loss from inefficient liquidation when **V = L**,  and the probability of this is **1 - P**.
- When there is a cost to [[Southland Corp. (c)|financial distress]],  there is value to monitoring to avoid it.
  - The saving due to monitoring is **S**,  because a lender who monitors does not need to liquidate when **V = L**.
  - Monitoring and restructuring the debt outside [[Course Notes/HBR Notes/A Strategic Perspective on Bankruptcy|bankruptcy]] increases the borrower's payment when the project [[Assets|returns]] **L** from **0** to **L**.
  - Monitoring can make both borrower and lender weakly better off if its cost is less than the cost of [[Southland Corp. (c)|financial distress]],  **S = (1 - P)L**.
...
- As before,  [[Financial Intermediation as Delegated Monitoring|financial intermediation]] is a way to delegate monitoring by manufacturing synthetic high-wealth investors.
  - [[Financial Intermediation as Delegated Monitoring|Delegated monitoring]] by one banker takes place without any monitoring of the banker,  because monitoring the banker costs **mK**.
...
- As we saw above,  when an unmonitored borrower's project [[Assets|returns]] **L**,  a lender receives a payment net of liquidation costs of **0**.
...
- [[Financial Intermediation as Delegated Monitoring|Delegated monitoring]] [[HKS The Banking Industry|banking]] is feasible only if the borrower,  lenders,  and the banker can be made at least as well off as with publicly traded debt,  and when the banker is provided with the proper incentives.
- A bank that monitored a single loan with face value **F** would issue deposits to **m** lenders to raise the amount **1**.
- The total face value of [[Class Note 4 Class Slide 4-Restructuring Debt Outside BankruptcyRestructuring Debt Outside Bankruptcy|bank debt]] (per loan) is **B**,  and there is one loan so total face value is **B**.
...
- The bank will provide an [[Lecture 1- Probability Distributions of Returns|expected return]] of exactly **r** to each depositor if the face value per loan is given by:
$$
...
- is the lowest such that an [[Lecture 1- Probability Distributions of Returns|expected return]] of **1 + r** is received without monitoring. 
...
- For any face value of [[Class Note 4 Class Slide 4-Restructuring Debt Outside BankruptcyRestructuring Debt Outside Bankruptcy|bank debt]] where it is preferred by borrowers to non-monitored debt,       the banker will receive at least as much profit from monitoring all of its loans as monitoring none. 
...
- The [[Lecture 1- Probability Distributions of Returns|expected return]] received by the bank is **1 + r + K**,       just enough to cover its cost of capital,       **1 + r**,       plus its monitoring cost,       **K**.
