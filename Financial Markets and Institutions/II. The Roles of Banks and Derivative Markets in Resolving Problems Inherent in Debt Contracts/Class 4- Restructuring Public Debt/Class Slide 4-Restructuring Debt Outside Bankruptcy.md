...
- This third option (besides bankruptcy or extending maturity with face value) is limited by negative pledge clause in the public debt.
- The negative pledge covenant prohibits issuing future debt that is senior to it.
- The bank has a senior claim of B=0.3, then the public has a claim on P=1.6. Any new claims that the borrower issues (to the bank or to other lenders) must be junior to the public.

The Example

- The future ("date 2") value of the firm is 2, 1, or 0, with probability q_2, q_1, and q_0 respectively. The current going concern value of the firm is then: $2q_2+q_1$ - Let $q_2=0.2$, $q_1=0.6$ and $q_0=0.2$
- The going concern value, VGO, is the expected value of all cash flow if the project is not liquidated: VGO = 2(0.2) + 0.6(1) = 1.0.
...
- How can we do this? See note 6 on coercive Bankruptcy Exchange Offers.
...
- If a large holder owned all of the bonds, they would reduce the face value to $P'=1$, which would allow the bank to do better by extending the loan, increasing the payment to the bank, and avoiding liquidation.
- Liquidation yields $L=0.4$, the bank is senior with a claim of 0.3, and the bondholders get only 0.1!
- A large holder is "pivotal" and takes account that the effect of reducing the debt to $P'=1$ is to induce the bank not to force liquidation.
...
- Although the Federal Trust Indenture Act prohibits such concessions by voting, it does not prohibit Bankruptcy Exchange Offers.

There can be a "holdout" problem with such Bankruptcy Exchange Offers
...
Suppose bonds are paid by absolute priority rule. (one class gets <100% means zero to more junior)

- Old bond has face value $P=1.6$ (from class note 4)
- New bond has face value $P'=1.0$ in total and is senior to old.

The Priority Stack

- New with face $P'=1$ be senior to the original P=1.6 bonds.
- This means that if you exchange for the new bond, you will be paid prior to those who hold out.
- This added priority is not relevant when the firm is worth 0 (all bonds are worthless) or when the firm is worth 2 (both the bonds with face P=1.6 and those with $P'=1$ are paid in full).
- The added priority is useful when the firm is worth 1.
...
<table>
	<tbody>
		<tr>
			...
			<th>Per unit holding of old junior bonds</th>
		</tr>
		<tr>
			<td>0.2</td>
			<td>2</td>
...
Suppose a fraction f=0.01 of other bonds are exchanged to the new bond
...
<table>
	<tbody>
		<tr>
			...
			<th>Total value, if liq. is avoided</th>
			<th>B=0.3 To Bank</th>
			<th>Per unit, to senior of new face f=0.01</th>
		...
		<tr>
			...
			<td>0.3</td>
			<td>$\min\{0.7/f, 1\}=1$</td>
			<td>$\max\{0, (0.7-f)/(1-f)\}=(0.69/0.99)=0.697$</td>
		</tr>
		<tr>
			<td>0.2</td>
			<td>0</td>
		...
		<tr>
			<td>Expected</td>
			<td>Value of the bonds</td>
			<td></td>
			<td>0.2(1) + 0.6(1) = 0.8</td>
			<td>0.2(1.6) + 0.6(0.697) = 0.7382</td>
		</tr>
	</tbody>
</table>
...
- Coercion can be used for:
  - "good" (exchanges offers in the collective interest of bondholders) or
  - "evil" (Bankruptcy Exchange Offers not in the collective interest of bondholders).
...
If you expect the consent vote to succeed, small non-pivotal will tender if and only if Bn +p ≥ B'.

- This might be coercive, if the old bond drops in value (Bn +p < B). You would tender here, even though you would not if you owned the total bond issue.
- That is if $$B_n + p < B \text{ while } B_n + p > B'$$
...
- B=0.001 (1% of the 0.1 payoff from liquidation with B=0.3 of bank senior)
- Bn > B': The punishment was used for "good," not evil.
