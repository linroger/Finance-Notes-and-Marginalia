...
- FORWARD RATES AND TERM STRUCTURE and financial analysis
- FORWARD RATES AND TERM STRUCTURE in modern finance
- Applications of FORWARD RATES AND TERM STRUCTURE
- 'Case study: FORWARD RATES AND TERM STRUCTURE'
...
- bootstrap-method
- current-yield
title: Forward Rates and Term Struct
---
--
...
PSET III Fixed Income Asset Pricing
...
- The Risk and Return of Bonds#7.4 Expectations hypothesis | expectations hypothesis]] is the hypothesis that long-term yields just reflect
...
- The **Risk and Return of Bonds#7.4 Expectations hypothesis | expectations hypothesis]]** substitutes the perfect foresight with forecasts:$$The\ long\ term\ yield=\ forecasted\ average\ path\ of\ future\ rates$$$$y_{t}(n+1)\ =\ E_{t}\left[\frac{1}{n+1}\sum\limits_{i=0}^{n}y_{t+i}(1)\right]\tag{1}$$
...
- The forecasts are for the three-month T-bill rate for the current quarter,  denote in the figure by$r(t)$,  and for the next three quarters,  denoted by$r(t + 3m)$,  $r(t + 6m)$,  and$r(t + 9m)$,  respectively.
 !500 − r(t) together with the 1-year "expectation spread".
 !500.
...
- The expectation hypothesis then implies that for every i the forward rate is equal to the expected future short-term rate:$$f_{t}(\tau_{i},        \tau_{i}+\Delta t)=E_{t}[y_{t+\tau_{i}}(\Delta t)].$$
- !500 of Bonds#7.4 Expectations hypothesis | expectations hypothesis]] (and the time variation in sprepnguggests the existence of an additional term in Equation (1):$$y_{t}(n+1)\ =\ E_{t}\left[\frac{1}{n+1}\sum\limits_{i=0}^{n}y_{t+i}(1)\right]+RP_{t}\tag{5}$$
- where$RP_{t}$reflects a risk premium from holding the bond from$t$to$t+1$.
- The time variation in$RP_t$invalidates the Risk and Return of Bonds#7.4 Expectations hypothesis | expectations hypothesis]].
...
- Let$y_t(n + 1$) = yield at t of a bond with *time to maturity* (n + 1) years from t$$Z_{t}(n+1)=e^{-y_{t}(n+1)(n+1)}\tag{6}$$
...
- Using the properties of the normal distribution we then obtain$$E_{t}[Z_{t+1}(n)]=e^{-\overline{{{y}}}\times n+\frac{(n)^{2}}{2}V}$$
- Using (6) and (7) we then obtain$$y_{t}\left(n+1\right)=\frac{1}{n+1}\times y_{t}\left(1\right)+\frac{n}{n+1}\times E_{t}\left[y_{t+1}\left(n\right)\right]\qquad\text{(Expected future yield)}$$$$+\frac{RP}{n+1}\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\text{(Risk premium)}$$$$-\frac{n^{2}}{2(n+1)}V_{t}\qquad\qquad\qquad\qquad\qquad\qquad\text{(Convexity)}\tag{8}$$
...
- A steep term structure (on the RHS) signals the market expects an increase in the yield$y_t (n + 1)$between$t$and$t + 1$(on the LHS)$$E_{t}\left[y_{t+1}\left(n\right)\right]>y_{t}\left(n+1\right)$$
...
- The Risk and Return of Bonds#7.4 Expectations hypothesis | expectations hypothesis]] has$α = 0$and$β = 1.$
...
-  This result implies that the slope of the term structure predicts$$L R P_{t}=R P_{t}-\frac{n^{2}}{2}V_{t},        \tag{13}$$
...
- LER ${}_{t}$* is the ex-post realized empirical counterpart of *LIRP*:$$LRP_{t}=E_{t}[LER_{t}]$$
...
-  The change in the CPI measures the realized inflation/War%20Economies%20and%20Hyperinflation.md) during the period.
## SIMPLE EXAMPLE
...
-  Let now be t2 and let our income be the same. Because of inflation/War%20Economies%20and%20Hyperinflation.md),         now  $CPI(t_2)$  =
...
-  Changes in CPI are unknown =⇒ nominal securities have inflation/War%20Economies%20and%20Hyperinflation.md) risk
...
-  An investor in TIPS is still subject to a small inflation/War%20Economies%20and%20Hyperinflation.md) risk during the two months
...
-  Relevant to inflation/War%20Economies%20and%20Hyperinflation.md) linked securities are bonds that are denominated in units of the consumption basket that underlies the CPI index calculation.
...
discount factor,         such as the Nelson Siegel model$$r_{real}(0,        T)\ =\ \theta_{0}+(\theta_{1}+\theta_{2})\,        \frac{1-e^{-\frac{T}{\kappa_{1}}}}{\frac{T}{\kappa_{1}}}-\theta_{2}e^{-\frac{T}{\kappa_{1}}}+\theta_{3}\left(\frac{1-e^{-\frac{T}{\kappa_{2}}}}{\frac{T}{\kappa_{2}}}-e^{-\frac{T}{\kappa_{2}}}\right)\tag{24}$$
...
-  We now show that nominal rate = real rate + expected inflation/War%20Economies%20and%20Hyperinflation.md) + inflation/War%20Economies%20and%20Hyperinflation.md) risk premium− convexity
...
-  Let π be the annualized inflation/War%20Economies%20and%20Hyperinflation.md) rate between 0 and T$$\pi=\frac{1}{T}\ln\left(\frac{\mathsf{C P I}(T)}{\mathsf{C P I}(0)}\right)$$
...
-  To obtain the *dollar* value,         we multiply this by the price of consumption good$$\mathsf{D o l l a r \ pre se n t\;v a lu e\;o f\;\;}C(T)=\mathsf{C P I}(0)\left(Z^{r e a l}(0,        T)\times{\frac{\$1}{\mathsf{C P I}(T)}}\right)$$
...
-  Assume now inflation/War%20Economies%20and%20Hyperinflation.md) rate is not known and assume it is log-normal$$\log\left(\frac{\mathsf{CPI}(T)}{\mathsf{CPI}(0)}\right)=x\sim\mathcal{N}(\pi\ T,        \sigma\pi^{2}\ T)$$
...
Dollar Present value of expected $C(T)=e^{-(r_{real}(0,        T)+\kappa)T}\times E[e^{-x}]$$$=e^{-(r_{real}(0,        T)+\kappa)T}\times e^{-\pi T+\frac{1}{2}\sigma^{2}T}$$
- This is dollar present value of \$1 at $T$. Therefore it equals$ Z(0,        T)=e^{-r(0,        T)T}$.$$r(0,        T)=r_{real}(0,        T)+\pi+\kappa-\frac{1}{2}\sigma\pi^{2}$$
...
1. Save the 100 dollars,         and buy $\frac{\$100}{Z(0,        T)}$ zero coupon bonds with maturity $T$. At that time,         the investor can buy $\frac{1}{Z(0,        T) CPI(T)}$ of the consumption good,         for an additional amount of utility$$\frac{\$100}{Z\left(0,        T\right)}\frac{1}{\mathsf{CPI}(T)}U^{\prime}\left(C(T),        T\right)$$
## EQUILIBRIUM
...
-  Assume that inflation/War%20Economies%20and%20Hyperinflation.md) and real GDP growth have the joiny log-normal distribution$$\left.\begin{pmatrix}\log\left(\frac{\mathsf{CPl}(T)}{\mathsf{CPl}(0)}\right)\\\log\left(\frac{Y(T)}{Y(0)}\right)\end{pmatrix}=\begin{pmatrix}x\\y\end{pmatrix}\thicksim\mathcal{N}\left(\begin{pmatrix}\overline{\pi}\\\overline{g}T\end{pmatrix}\right.,        \begin{pmatrix}\sigma\pi[^2]T&,        &\sigma\pi\sigma_g\rho_{g,        \pi}T\\\sigma\pi\sigma_g\rho_{g,        \pi}T&,        &\sigma_g[^2]T\end{pmatrix}\right)$$
## EQUILIBRIUM PRICES 
...
- Thus,         the nominal rate is:
  - $$r(0,         T) = \left( \rho + \gamma g - \frac{\gamma[^2]}{2} \sigma_g[^2] \right) + \left( \pi - \frac{1}{2} \pi[^2] \right) - \gamma \pi \sigma_g \rho_{g,        \pi}$$
    ...
    - **Expected Inflation/War%20Economies%20and%20Hyperinflation.md) & Convexity**: $\pi - \frac{1}{2} \pi[^2]$
...
+  Intuitively:
	+ If $ρ_{g,        π} <0 \rho_{g\pi}$ =⇒ high inflation/War%20Economies%20and%20Hyperinflation.md) occurs when real GDP is low
...
-  In the meantime,         real GDP volatility σg and inflation/War%20Economies%20and%20Hyperinflation.md) volatility σπ declined.
...
- Late 1970s/ early 1980s: *Fear of stagflation* (high inflation/War%20Economies%20and%20Hyperinflation.md) and low growth)
=⇒ high inflation/War%20Economies%20and%20Hyperinflation.md) shocks are bad news for the economy =⇒ both nominal bonds and stock drop
- Mid 2000s: *Fear of deflation* (low inflation/War%20Economies%20and%20Hyperinflation.md) and low growth)
=⇒ high inflation/War%20Economies%20and%20Hyperinflation.md) shocks are good news for the economy =⇒ nominal bonds drop but stock rise
...
-  They can be generalized by assuming that expected inflation/War%20Economies%20and%20Hyperinflation.md) πt and expected GDP growth gt are time varying.
...
- Inflation/War%20Economies%20and%20Hyperinflation.md) and real GDP growth themselves are$$\begin{array}{r c l}{{}}&{{d\log(\mathsf{CPI}_{t})\;=\;\pi_{t}d t+\sigma_{\pi}\;d W_{\pi}}}\\ {{}}&{{}}&{{d\log(Y_{t})\;=\;\bar{g}_{t}d t+\sigma_{g}\;d W_{g}}}\end{array}$$
...
- Many risk premia are accounted for in bond pricing:
	  1. **Inflation/War%20Economies%20and%20Hyperinflation.md) risk premium**:$\gamma \sigma_{\pi} \rho_{\pi g} \sigma_g$. If$\rho_{\pi g} < 0 \Rightarrow Z(t,         T) \uparrow \Rightarrow r(t,         T) \uparrow$
	  1. **Expected inflation/War%20Economies%20and%20Hyperinflation.md) risk premium**:$\gamma \sigma_g \rho_{\pi g} \sigma_{\pi}$. If$\rho_{g,         \pi} < 0 \Rightarrow r(t,         T) \uparrow$
...
- As we take the limit$T - t \rightarrow 0$,  we obtain the formula for the "overnight" rate:
  ...
  - **Expected Inflation/War%20Economies%20and%20Hyperinflation.md) & Convexity**:$\pi_t - \frac{1}{2} \sigma_{\pi}[^2]$
...
- From previous results and using Ito's formula,  the real rate follows a Vasicek model$$d r_{r e a l,        t}=\beta_{g}\left(\bar{r}_{r e a l}-r_{r e a l,        t}\right)d t+\sigma_{r e a l}d W_{\bar{g}}$$
...
- The real zero-coupon bond pricing formula is then$$Z_{r e a l}(t,        T)=e^{\overline{{{A}}}(t,        T)-C(t,        T)r_{r e a l,        t}}$$
...
- The risk neutral process of the real rate is$$d r_{r e a l,        t}=\beta_{g}\left(\overline{{{r}}}_{r e a l}^{*}-r_{r e a l,        t}\right)d t+\sigma_{r e a l}d W_{g}^{*}$$
...
1. Expectation of future inflation/War%20Economies%20and%20Hyperinflation.md) 2. Expectation of future real growth 3. A whole set of risk premia
- Inflation/War%20Economies%20and%20Hyperinflation.md) risk premium
- Expected inflation/War%20Economies%20and%20Hyperinflation.md) risk premium - Expected growth risk premium
- Multi-factor models tend to fit well the term structure of interest rates
- The above setting with expected inflation/War%20Economies%20and%20Hyperinflation.md) and expected GDP growth can be generalized to any factors$ϕ_{1,        t},         ϕ_{2,        t}….$
