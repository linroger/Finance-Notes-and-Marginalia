...
[PSET VI [[Lecture Notes Bonds,  Preferred Stock,  and Structured Products|Fixed Income]]](PSET%20VI%20Fixed%20Income.md)
...
- Three main government sponsored agencies exist:
	...
	- FHLMC (Federal Home Loan Mortgage Corporation,  or Freddie Mac): Similarly to Fannie Mae,  it provides a link between [[Teaching Note 6 Mortgage Backed Securities|mortgage lenders]] and capital markets. It buys from S&Ls,  mortgage bankers and commercial banks,  and sell mortgage pass-through securities.
...
- Pass through MBSs have their own interest rate rPT,  lower than the average mortgage rate in the pool as agencies keep a fee for arranging the [[Teaching Note 6 Mortgage Backed Securities|MBS market]] (and provide [[Credit Default Swaps|default insurance]])
...
- Consider a 30-year [[Fixed-Rate Mortgage Loans|fixed-rate mortgage]],  with mortgage rate of $r_{m}$.
...
- where$$A=\frac{1}{1+\frac{r_{12}}{12}}\tag{2}$$
...
- The value of the mortgage debt changes over time as mortgage rates change:$$\text{Value of mortgage debt}  = P(t)=\sum\limits_{i=1}^n{\frac{C}{{\left({1+r_{12}^m(t, T_i)/12}\right)^i}}}$$
	- where $n$ is the number of payments left at time $t$ and $r_{m}^{12}(t,  T_i)$ are appropriate spot rates,  related to the current [[The Vasicek Model|term structure]] of mortgage rates.
...
- As in any bond,  $P(t)$ increases in value whenever [[Interest Rate Quotations|interest rates]] decline.
	- $\Rightarrow$ when [[Interest Rate Quotations|interest rates]] decline,  $P(t)$ may increase to a higher level than $L_{t}$
...
- Other reasons include the following:
	...
	1. Burnout effect: Mortgage pools heavily refinanced in the past tend to be insensitive to [[Interest Rate Quotations|interest rates]].
		- If a [[Mortgage Pools|mortgage pool]] has been heavily refinanced in the past,  most homeowners who could refinance already did.
...
- This measure makes the amount of prepayment depend only on the age of the [[Mortgage Pools|mortgage pool]].
...
## PSA AND MBS PRICING
- A PSA prepayment speed is simply a quoting convention to allow traders to quote RMBS (Residential [[Fremont Financial Corp. (b)|Mortgage-Backed Securities]]).
- Given a PSA speed,  it defines the path for the monthly prepayment probability $p_{t}$.
- Given the path $\{p_{t}\}$,  the [[Mortgage Pricing Spreads and Duration|MBS cash flows]] follow. Let $L_{t}$ be the outstanding principal,  then:$$\begin{align*}
...
\end{align*}$$
- Given the dynamics of $L_{t}$ implicit in PSA,  we can compute the path through [[Advanced Derivatives Pricing Methodology|future cash flows]].
...
- However,  if prepayment occurs,  then Mortgage Backed Securities will move towards principal.
	- $\Rightarrow$ [[Mortgage Pricing Spreads and Duration|negative convexity]]
- Indeed,  MBS is equal to a [[An Asset Allocation Primer|portfolio]] with long bond and short call option.
- While long bond has [[Convexity|positive convexity]],  the short call has a strong [[Mortgage Pricing Spreads and Duration|negative convexity]].
- [[Mortgage Pricing Spreads and Duration|Negative convexity]] implies a negative loss on average:$$P^{MBS}(t+1, T)\thickapprox P^{MBS}(t, T)+\frac{dP^{MBS}(t, T)}{dr}(r(t+1)-r(t))+\frac{1}{2}\frac{d^{2}P^{MBS}(t, T)}{dr^{2}}(r(t+1)-r(t))^{2}+..$$
...
![500](Z.%20Clippings/Teaching%20Note%206%20Mortgage%20Backed%20Securities-20240513044731231.png)
 - MBS securities pay a higher coupon than similar [[US Markets|Treasury securities]],  even if [[Quantitative Trading Strategies Lecture Notes|credit risk]] is the same 
...
- This calculation yields D = 4.39 and C = 36.1590.
	- Where is the [[Mortgage Pricing Spreads and Duration|negative convexity]]?
...
- The PSA-adjusted Effective [[Key Rates O1s Durations and Hedging|Duration]] corrects for this.
...
- A third methodology is to use past data to inform about the sensitivity of MBS to changes to [[Interest Rate Quotations|interest rates]].
	- This methodology can be particularly useful as we can check the relation between MBS prices and the relevant interest rate for [[Key Rates O1s Durations and Hedging|hedging]].
	- Given the [[Class Note 10 Liquidity and Class Note 10 Liquidity and Liquidity Managementliquidity management|liquidity]] of the [[Assessing the LIFFE Swapnote|OTC swap]] market,  interest rate swaps are typically the preferred derivative instrument to hedge against changes in [[Interest Rate Quotations|interest rates]].
- Next figure shows the relation between GNMA prices,  and the 5-year [[Teaching Note 4 Interest Rate Derivatives|swap rate]].
- Using these data,  and denoting $c_{t}(5)$ the 5-year [[Teaching Note 4 Interest Rate Derivatives|swap rate]] at $t$,  we can estimate a (simple) polynomial regression:$$P_{t}=\alpha+\sum_{i=1}^{4}\beta_{i}\times c_{t}(5)^{i}+\epsilon_{t}$$
...
- Suppose that interest [[Rate and Price Trees|rate tree]] is the following:
...
- The lower coupon paid by the issuer of the PT security to investors compared to the average mortgage coupon rate (WAC) is due in part as a compensation for holding [[Default Risk and Credit Derivatives 183|default risk]] and in part as a service fee.
...
- First,  we fit a tree to the current [[6. A Brief Introduction to Stochastic Calculus|term structure of interest rates]]:
...
- The Pass Through security displays [[Mortgage Pricing Spreads and Duration|negative convexity]]
...
- The idea is to redistribute the cash flows from a pool of mortgages in relation to their sensitivity to [[Risk Factors and Hedging Agency MBS|prepayment risk]].
	- $\Rightarrow$ Varying levels of protection against [[Risk Factors and Hedging Agency MBS|prepayment risk]],  suited for different types of investors.
- Characteristics:
	- High credit rating
...
- Each Tranche appeals to a certain types of investors:
	- Tranche A/B: Short-medium [[Key Rates O1s Durations and Hedging|duration]]. Investors = money managers,  commercial banks;_
	- Tranche C: Longer [[Key Rates O1s Durations and Hedging|duration]]. Investors = insurance companies,  pension funds;_
	- Tranche Z: Longest [[Key Rates O1s Durations and Hedging|duration]]. Investors = insurance companies,  pension funds._
...
- What do we simulate?
	1. [[Interest Rate Quotations|Interest rates]].
	2. Housing prices.
...
- We simulate all the processes under the [[Financial Instruments|risk-neutral probabilities]].
...
- From the MBS markets,  a number of mortgage "[[Chapter 9 Arbitrage and Hedging With Options|derivatives]]" can be constructed to fit the needs of investors.  
- Strips: Obtained by dividing the cash flows from an underlying pool of mortgages through specific allocation of [[Notes on Currency Swaps|interest and principal]].
...
- The price movements of IO are highly sensitive to [[How Countries Go Broke-Chapter 12 to Chapter 14|interest rate changes]].
    - If $r_{t} \downarrow \Rightarrow$ prepayment $\uparrow \Rightarrow$ no more interest payments $\Rightarrow$ IO Strip loses value.
- Notice that most [[Valuing Callable Bonds Using QuantLib Python|Fixed Income instruments]] tend to appreciate when $r_{t}$ declines.
...
- Consider a [[Mortgage Pools|mortgage pool]] with 10 years to maturity and average mortgage rate of $r^{m}=6.5\%$.
- Suppose prepayments depend only on past [[Interest Rate Quotations|interest rates]] and assume the following simple function for the probability that a homeowner would refinance its mortgage at time $t$:
...
- In thislatter case,  we are assuming a quicker responsiveness of prepayments to changes in [[Interest Rate Quotations|interest rates]],  which increase the prepayment rate. This implies that the $MBS_{Prepay}$ would decline.
...
- In fact,  by increasing the [[PSET 7- Kohler|discount rate]] by 10 basis points and re-simulating the CFs,  one obtains a price equal to the [[Accrued Interest|quoted price]].
- A positive OAS shows that the market price of the security is too low,  calling for a strategy to buy it and contemporaneously hedge it using other traded securities with similar (non-linear) [[Key Rates O1s Durations and Hedging|duration]],  such as portfolios of [[Interest Rate Derivatives-An Introduction to the  Pricing of Caps and Floors|caps and floors]].