...
- Teaching Note 1-Introduction To Fixed Income Securities (1) and financial analysis
- Teaching Note 1-Introduction To Fixed Income Securities (1) in modern finance
- Applications of Teaching Note 1-Introduction To Fixed Income Securities (1)
- 'Case study: Teaching Note 1-Introduction To Fixed Income Securities (1)'
...
- bootstrap-method
- current-yield
title: Teaching Note 1-Introduction To Fixed Income Securit
---
--
...
1. Treasury Inflation/War%20Economies%20and%20Hyperinflation.md)-Protected Securities (TIPS): Floating coupon bonds indexed to inflation/War%20Economies%20and%20Hyperinflation.md)
...
## MARKETABLE U.S. TREASURY SECURITIES (MILLION OF DOLLARS) DECEMBER 31,  2021
 ...
 | Notes | 12,        992,        160 | 
 | Bonds | 3,        474,        153 | 
 | Inflation/War%20Economies%20and%20Hyperinflation.md)-Protected Securities | 1,        727,        968 | 
...
- In the limit as $n\rightarrow\infty$ we obtain the usual continuous compounding formula$$Z(T)=\frac{1}{\left(1+\frac{r}{n}\right)^{n\times T}}\longrightarrow Z(T)=e^{-rT}$$
## DISCOUNTS AND RATES
...
- We denote the discount factor at t for a dollar to be received at time T by$$Z(t,        T)=e^{-r(t,        T)(T-t)}$$
...
Z (*t,  T*i) for all maturities Ti's,  a simple no arbitrage argument implies$$P(t,        T_{n})=c/2\times Z(t,        T_{1})+c/2\times Z(t,        T_{2})+…+(100+c/2)\,        Z\,        (t,        T_{n})$$ $$=\sum\limits_{i=1}^{n}\frac{c/2}{\left(1+r_{2}\left(t,        T_{i}\right)/2\right)^{2\times\left(T_{i}-t\right)}}+\frac{100}{\left(1+r_{2}\left(t,        T_{n}\right)/2\right)^{2\times\left(T_{n}-t\right)}}$$
...
- Recall that given the spot curve r 2 (*t,  T*i),  we also had$$P\left(t,        T_{n}\right)=\sum\limits_{i=1}^{n}{\frac{c/2}{\left(1+r_{2}\left(t,        T_{i}\right)/2\right)^{2\times\left(T_{i}-t\right)}}}+{\frac{100}{\left(1+r_{2}\left(t,        T_{n}\right)/2\right)^{2\times\left(T_{n}-t\right)}}}$$
...
- The term T is decided at time t: Most Class Slides On Repurchase Agreements | repurchase agreements]] are for a very short term (overnight). Some longer term agreements reach 30 days or more.
...
- Hence,  the return on capital to the trader is$\text{Return}=\dfrac{P_{T}-P_{t}-\text{repo rate}\times\frac{n}{360}\times(P_{t}-\text{haircut})}{\text{haircut}}$
...
- By no arbitrage (and if credit risk is negligible) we must have$$\$10\ \textsf{m}\times\left(1+{\frac{r_{2}\left(0,        1\right)}{2}}\right)^{2}=\$10\ \textsf{m}\times\left(1+{\frac{r_{2}\left(0,        0.5\right)}{2}}\right)\times\left(1+{\frac{f_{2}\left(0,        0.5,        1\right)}{2}}\right)$$
...
[^1]: 2. Invest in a 6-months zero coupon bond and enter (today) into a forward contract to invest the proceeds at $T_1=0.5$ until $T_2=1$ for a rate $f_2\left(0,        0.5,        1\right)$ . At maturity we have$$\frac{\$10\ m}{Z(0,        T_{1})}\times\left(1+\frac{f_{n}\left(0,        T_{1},        T_{2}\right)}{n}\right)^{n(T_{2}-T_{1})}$$
...
- If we take the limit $T_{2}\longrightarrow T_{1}$ (that is,  the horizon of the forward contract shrinks to zero) we obtain$$f(0,        T_{1},        T_{1})=r(0,        T_{1})+T_{1}\times\frac{dr(0,        T_{1})}{dT_{1}}\tag{4}$$
...
- Recall the pricing equation for coupon bonds with maturity $T_{i}$ is equal to$$P^{i}(t,         T_{i})=\frac{c^{i}}{2}\sum\limits_{j=1}^{i}Z (t,         T_{j})+1\times Z (t,         T_{i})\tag{5}$$
