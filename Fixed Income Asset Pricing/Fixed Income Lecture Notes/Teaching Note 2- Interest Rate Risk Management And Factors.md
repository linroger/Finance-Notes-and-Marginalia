...
- Traditionally,  the duration is defined with respect to changes in yield-to-maturity. - However,  it is convenient to introduce it using the continuously compounded yield$r(t,            T)$,  as formulas are easier and the interpretation is also easier.
...
- Hence,  we define the duration of a security with price P by$$D_{P}=-\frac{1}{P}\frac{d\ P}{d\ r}$$
...
- Let Π be the value of the portfolio$$\Pi=N_{1}P_{1}+N_{2}P_{2}$$
...
- We know a bond is equivalent to a portfolio of zero-coupon bonds$$P(0,           T)=\sum_{i=1}^{n}c(T_{i})Z(0,           T_{i})$$
...
- We defined the duration as$$D=-\frac{1}{P}\frac{dP}{dr}$$
...
- The Macaulay duration is defined as$$D^{M c}=-\frac{(1+y/2)}{P}\frac{d P}{d y}$$
...
## MODIFIED DURATION
- The modified duration is instead defined as$$M D=-\frac{1}{P}\frac{d P}{d y}$$
...
- The reason are the transaction costs,  such as Bid and Ask Prices With Private Information | bid-ask spreads]] in the Treasury market and in the Repo market.
...
- Then,  the trader can hedge the interest rate risk by setting$$n_{2}\times P V B P_{2}=n_{30}\times P V B P_{30}$$
...
- The trader lost money,  even if the spread did increase.
- The reason,  of course,  are the transaction costs,  such as Bid and Ask Prices With Private Information | bid-ask spreads]] in the Treasury market and in the Repo market.
- The trader lost money,  even if the spread did increase. - The reason,  of course,  are the transaction costs,  such as Bid and Ask Prices With Private Information | bid-ask spreads]] in the
Treasury market and in the Repo market.
## INTEREST RATE RISK MANAGEMENT: CONVEXITY
- Convexity measures the curvature of a security with respect to interest rates$$C=\frac{1}{P}\frac{d^{2}\ P}{d\ r^{2}}$$
...
- Let $k_1$ and $k_2$ be the positions in the bonds. The value of the hedged portfolio is$$V=P_{c}+k_{1}\ P_{1}+k_{2}\ P_{2}\Longrightarrow d\ V=d\ P_{c}+k_{1}\ d\ P_{1}+k_{2}\ d\ P_{2}$$
...
D = 7.491
- Let's duration hedges with the zero with T =Feb 15,            2005 (duration DS = 0.87):$$k_{S}=-{\frac{D\times P}{D_{S}\times P_{S}}}={\frac{7.491\times101.5}{0.87\times99.0019}}=-8.83$$
...
## AN ISSUE WITH DURATION HEDGING
- In sharp contrast,            the new yield curve implies the value of the hedged portfolio is$$V_{4/15/2004}=\$98.20,           \text{ a decline of 3.30%}$$$$
