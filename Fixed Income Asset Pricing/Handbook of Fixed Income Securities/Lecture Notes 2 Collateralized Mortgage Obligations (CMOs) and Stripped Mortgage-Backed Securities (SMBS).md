...
- Lecture Notes 2 Collateralized Mortgage Obligations (CMOs) and Stripped Mortgage-Backed
  Securities (SMBS) and financial analysis
- Lecture Notes 2 Collateralized Mortgage Obligations (CMOs) and Stripped Mortgage-Backed
  Securities (SMBS) in modern finance
- Applications of Lecture Notes 2 Collateralized Mortgage Obligations (CMOs) and Stripped
  Mortgage-Backed Securities (SMBS)
- 'Case study: Lecture Notes 2 Collateralized Mortgage Obligations (CMOs) and Stripped
  Mortgage-Backed Securities (SMBS)'
...
- **Definition and Purpose**:
  ...
  - Stripped Mortgage-Backed Securities.md) (SMBS) are derivatives of mortgage-backed securities.md) (MBS) that separate interest and principal cash flows into distinct securities,  such as Interest-Only (IO) and Principal-Only (PO) tranches.
- **Historical Context**:
...
- **External Credit Enhancements**:
  - **Pool Insurance**:
	- Covers losses due to fraud,  special hazards,  or bankruptcy cramdowns.
...
- During the crisis,  correlations spiked,  leading to fat-Leverage, Tail Risk, Volatility Products#6.6 Hedging tail risks | tail risks]].
...
- **Definition**:
  - Hybrid between Corporate Bonds and mortgage-backed securities.md).
...
- **Legislative Covered Bonds**:
  - Backed by specific laws granting bankruptcy protection to collateral assets.
  - Example: German Pfandbrief Act ensures covered assets are excluded from insolvency proceedings.
- **Structured Covered Bonds**:
  - Use SPEs to achieve bankruptcy protection in jurisdictions without specific legislation.
...
- Housing market.md) dynamics,  including geographic and tier segmentation,  significantly impact RMBS performance.
...
### 5.3 CMBS
- Valuation of underlying properties and loan structures are essential for credit Financial Risk Analysis | risk analysis]].
...
- **Risk Retention (Dodd-Frank Act and Its Impact | Dodd-Frank]] Act)**:
...
- **Subordination**:
  - Junior tranches provide credit enhancement.md) for senior tranches.
...
- **Monthly Payment Rate (MPR)**:
  - Indicates the percentage of principal repaid each month,  with higher MPRs providing implied credit enhancement.md).
...
- **Definition and Purpose**:
  ...
  - They provide issuers with direct access to capital markets while isolating assets from bankruptcy risk.

### 5.1 Securitization Process
- **Special Purpose Vehicle (SPV)**:
  - Assets are sold to an SPV to achieve bankruptcy remoteness.
  - The SPV issues securities backed by the cash flows from the assets.
- **Credit Enhancement.md)**:
  - Includes excess spread,  subordination,  and reserve accounts to absorb potential losses.

### 5.2 Key Asset Classes
- **Auto Loans and Leases**:
  - Backed by prime and subprime auto loans,  with varying levels of credit enhancement.md) based on expected losses.
...
- **Credit Card ABS**:
  ...
  - Excess spread and subordination are critical for credit enhancement.md) and investor protection.
- **Non-Mortgage ABS**:
  - Securitization provides issuers with access to capital markets while isolating assets from bankruptcy risk.
  - Credit enhancement.md) and asset quality are key determinants of ABS performance.
...
- **Key Features**:
  - ABS transactions are structured to isolate the underlying assets from the originator's bankruptcy risk,  enabling the securities to achieve higher credit ratings than the originator.
  - Credit enhancement.md) mechanisms are employed to protect investors from losses and ensure timely payments.

## 2. Credit Enhancement in ABS Transactions
- **Overview**:
  - Credit enhancement.md) refers to techniques used to reduce the risk of loss to investors in ABS transactions.
  - It can be categorized into **internal sources** and **external sources**.

### 2.1 External Credit Enhancement
- **Definition**:
  - External credit enhancement.md) involves third-party guarantees or insurance to protect investors.
- **Types**:
  - **Bond Insurance**:
	- Guarantees timely payment of principal and interest.
...
- **Decline in Usage**:
  - The financial crisis exposed vulnerabilities in external credit enhancement.md),  leading to a shift toward internal mechanisms.

### 2.2 Internal Credit Enhancement
- **Definition**:
  - Internal credit enhancement.md) relies on structural features within the ABS deal to absorb losses.
- **Types**:
  - **Excess Spread**:
...
- **Key Metrics**:
  ...
  - **Defaults and Net Losses**:
	- Used to determine credit enhancement.md) levels.
...
- **Owner Trust Structure**:
  - Allows for time tranching.md) of senior classes and credit tranching.md) of subordinated debt.
...
- **Characteristics**:
  - Higher credit enhancement.md) levels due to increased default risk.
  - Example:
	- Prime auto loan ABS may require 35% credit enhancement.md) for a AAA rating.
	- Subprime auto loan ABS may require up to 65% credit enhancement.md).
...
- **Categories**:
  ...
  - **Private Student Loans**:
	...
	- Higher risk due to direct-to-consumer disbursement and lack of bankruptcy discharge.
- **Key Risks**:
  - **Credit Risk**:
...
This concludes the lecture notes on ABS and credit enhancement.md) mechanisms. The next session will focus on collateralized loan obligations (Close) and their structural features.
...
- **Key Features of Interest Rate Models**:
  ...
  - **Mean Reversion%20Process.md)**:
	- Some models incorporate mean reversion%20Process.md),  where rates tend toward a long-term target.
...
- **Process**:
  ...
  - Incorporates mean reversion%20Process.md),  where $\phi$ controls the speed of reversion.
- **Key Features**:
  - Rates are lognormally distributed and mean-reverting.
  - Extension of the KWF model with mean reversion%20Process.md).
...
- **Process**:
  ...
  - Mean reversion%20Process.md) is endogenous,  driven by the slope of the volatility curve.
- **Key Features**:
  - Rates are lognormally distributed.
  - Mean reversion%20Process.md) depends on the volatility structure.
...
- **Examples**:
  ...
  - **BDT Lattice**:
	- Mean reversion%20Process.md) is driven by the volatility structure.
	- Decreasing volatility increases mean reversion%20Process.md),  reducing the spread of rates.
...
- **Advantages**:
- Lower Leverage, Tail Risk, Volatility Products#6.6 Hedging tail risks | tail risk]] compared to macro trading.
...
- **Interpretation**:
  - High absolute Z-scores indicate strong signals for mean reversion%20Process.md).

### 5.2 Mean Reversion
- **Assumption**:
  - Prices or yields will revert to their historical mean over time.
- **Limitations**:
  - Structural changes or market shocks can violate mean reversion%20Process.md).
...
- **Interest Rate Models**:
  - Understanding model assumptions (e.g.,  distribution,  mean reversion%20Process.md)) is critical for effective application.
- **Relative Value Trading**:
  - Combines theoretical models and market data to identify mispriced securities.
- **Risk Management**:
- Proper controls and awareness of model limitations are essential to avoid catastrophic losses (e.g.,  LTCM Meltdown | LTCM]]).
...
- Example: Basis Swap Markets#London Interbank Offered Rate (LIBOR) | LIBOR]] Market Model (LMM) for realistic swap rate dynamics.
...
This concludes the lecture notes on the valuation of bonds with embedded options and mortgage-backed securities.md). The next session will focus on advanced prepayment modeling techniques and their applications in structured finance.
...
- **Definition and Importance**:
  - Mortgage-Backed Securities.md) (MBS) are fixed-income securities backed by pools of mortgage loans. Their valuation is complex due to embedded prepayment options and path-dependent cash flows.
  - MBS valuation requires advanced modeling techniques to account for interest rate dynamics,  prepayment behavior,  and housing market.md) fundamentals.
...
- **Introduction to Globalization | Globalization]]**:
...
### 2.7 Event Risk
- **Definition**:
- Event risk refers to major disruptions,  such as Capital Structure Module Note | Note On LBO Capital Structure | leveraged buyouts]],  natural disasters,  or accounting fraud.
- **Examples**:
...
- Financial buyers may prioritize short-term gains,  increasing credit risk in Capital Structure Module Note | Note On LBO Capital Structure | leveraged buyouts]].
...
This concludes the lecture notes on industry and Introduction to Financial Analysis | financial analysis]]  for credit evaluation. The next session will focus on advanced techniques for modeling credit risk and assessing default probabilities.
...
- **Economic Sensitivity**:
  - The performance of certain loan types is closely tied to economic conditions.md).
  - Example:
